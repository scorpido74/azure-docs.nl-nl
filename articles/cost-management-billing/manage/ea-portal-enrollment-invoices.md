...
### <a name="to-update-the-marketplace-purchase-order-number"></a>Het inkoopordernummer voor Marketplace bijwerken:
...
**Directe inschrijvingen met een begin datum vóór 1 mei 2018**: Enter prise Azure (EA)-klanten worden ingesteld als een jaarlijkse facturerings cyclus voor Azure-Services (met uitzonde ring van Marketplace-Services). Uw factureringsperiode is gebaseerd op de inschrijvingsdatum. De inschrijvingsdatum is de datum waarop uw overeenkomst van kracht werd. Om de eerste overschrijdingsfactuur van Azure-services wilt ontvangen, moet u 150% van uw financiële toezeggingsdrempel ('MC') overschrijden.  Zodra het totale serviceverbruik meer dan 150% van uw MC-drempel overschrijdt, wordt automatisch overgegaan op een driemaandelijkse factureringsperiode op basis van de inschrijvingsdatum.  Als u de 150% van uw MC-drempel niet overschrijdt, blijft de inschrijving staan op een jaarlijkse factureringsperiode en wordt overschrijdingsfactuur aan het einde van het toezeggingsjaar ontvangen.

**Directe inschrijvingen met een begin datum na 1 mei 2018**: het Azure-verbruik en de kosten van de klant worden afzonderlijk in rekening gebracht op basis van een maandelijkse facturerings cyclus.  Kosten die niet worden gedekt door de financiële toezegging van Azure, worden gezien als door een overschrijdingsbetaling.  

**Indirecte inschrijvingen met een inschrijving die vóór 1 mei 2018 is gestart**: klanten met een indirecte Azure-onderneming (EA) worden ingesteld als een driemaandelijkse facturerings cyclus.  De kanaalpartner (CP) stuurt de klant rechtstreeks een factuur.  

**Indirecte inschrijvingen met een begin datum na 1 mei 2018**: elke indirecte overeenkomst met een begin datum die groter is dan of gelijk is aan 1 mei 2018 wordt maandelijks gefactureerd.  

### <a name="increasing-commitment"></a>Toenemende toezegging

Toezegging kan op elk gewenst moment worden verhoogd en wordt gefactureerd voor het aantal resterende maanden voor de toezeggingsperiode van dat jaar. Als u zich bijvoorbeeld aanmeldt voor een abonnement van één jaar en u de toezegging hebt vergroot in zes maanden, wordt u gefactureerd voor de resterende zes maanden van die termijn. Uw toezeggingshoeveelheden worden vervolgens bijgewerkt voor de laatste zes maanden van uw toezeggingstermijn voor het bepalen van eventuele overschrijdingskosten.
...
1. Op deze regel ziet u de tegoeden die zijn toegepast op uw inschrijving en een korte uitleg bijvoorbeeld: Service Level Agreement tegoed.
...
Ondernemingsbeheerders kunnen de prijslijst weergeven die is gekoppeld aan hun enrollment voor Azure-services.
...
Klanten die de CSV-rapporten van de Azure EA Portal in euro's bekijken, kunnen opmaakproblemen ondervinden met betrekking tot komma's en punten.

Wat u kunt zien, is bijvoorbeeld:

| **ServiceResource** | **ResourceQtyConsumed** | **ResourceRate** | **ExtendedCost** |
| --- | --- | --- | --- |
| Uur | 24 | 0,0535960591133005 | 12.863.054.187.192.100.000.000 |

U ziet het volgende:

| ServiceResource | ResourceQtyConsumed | ResourceRate | ExtendedCost |
| --- | --- | --- | --- |
| Uur | 24 | 0,0535960591133005 | 1,2863054187192120000000 |

### <a name="root-cause"></a>Hoofdoorzaak

Excel importeert alle velden als ' algemene ' tekst en veronderstelt dat een getal wordt gescheiden in de wiskundige standaard: ' 1.000,00 '.  Een Europese valuta waarin een komma (,) wordt gebruikt voor het scheidingsteken voor duizendtallen en een punt voor het decimaal scheidingsteken (.) – '1,000.00' wordt onjuist weergegeven. Dit hangt af van uw landinstellingen.
...
- **Op basis van gebruik:** Producten die worden gemeten in transactionele eenheden.  Voor virtuele machines bijvoorbeeld worden kosten per uur in rekening gebracht en voor zoekopdrachten in de Bing API worden kosten in rekening gebracht op basis van het aantal zoekopdrachten.
- **Maandelijks bedrag:** Maandelijks gefactureerd op basis van gebruik/toegang.
...
De nieuwe meting is van kracht tussen 27 januari en 28 januari 2016. In het CSV-bestand voor uw implementatie tijdens deze overgangs periode ziet u twee regel items: één voor gebruik dat is verzonden op de a1-meter en een andere voor het gebruik dat is verzonden op de nieuwe specifieke meter die overeenkomt met de grootte van uw implementatie.
...
We hebben alle Azure- en Marketplace-kosten gecombineerd tot één factuur die overeenkomt met de factuurperiode van de inschrijving. (Dit is niet van toepassing op klanten in Australië, Japan of Singapore.)
...
**Voer de volgende stappen uit om de prijslijst te downloaden:**
...
- **Gebruik gebaseerd:** Producten worden gemeten in transactionele eenheden.  Voor virtuele machines worden bijvoorbeeld per uur kosten in rekening gebracht en voor zoekopdrachten in de Bing-API worden kosten in rekening gebracht op basis van het aantal zoekopdrachten.
- **Op basis van niet-gebruik:** Eenmalige kosten of terugkerend maandelijks tarief dat onafhankelijk is van het gebruik.
...
- **Algemeen gebruik**: de uitgebreide Power BI Pro voor Microsoft Azure Enterprise de aanbieding van de proef versie van het inhouds pakket (het ' aanbod ') is beschikbaar voor bestaande gekwalificeerde gebruikers gedurende de loop tijd van de aanbieding, zodat ze toegang hebben tot bepaalde inzichten die betrekking hebben op hun Microsoft Azure gebruik door gebruik te maken van een specifiek Power bi-inhouds pakket.
- **Geschiktheid**: gebruikers onder een Enterprise Agreement (EA) kunnen deel nemen aan de aanbieding als ze een functie hebben die betrekking heeft op de Microsoft Azure facturering, service, service en/of kosten beheer van hun organisatie.
- **Uitsluitingen**:
...
Als u de functie voor het weergeven van de kosten voor uw accounteigenaren en de afdelingsbeheerder wilt inschakelen, volgt u de bovenstaande stappen onder **Directe inschrijving**.
...
**Totaal aantal ONbewerkte eenheden:** Verbruikte hoeveelheid in het gedetailleerde gebruiks rapport.
**MOCP-resource per eenheid:** Met het upstream-gebruiks systeem worden de gebruiks gegevens voor elke service in verschillende eenheden verzonden. Uw **Verbruik per eenheid:** EA-eenheid. Uw **Prijs:** Eenheids prijs van de Azure EA-Portal.
**Totale kosten:** Uitgebreide kosten van het gedetailleerde gebruiks rapport of het toezeggings gebruik en overschrijding van de Azure EA-Portal.
