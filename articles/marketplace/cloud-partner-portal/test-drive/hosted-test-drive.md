...
Selecteer \'ja\' voor \'een test station inschakelen\' optie.

Geef de volgende velden op in het gedeelte \'Details\'.
...
In deze stap genereert u de \'-Azure AD-app-id\' en \'Azure AD-app waarden voor sleutel\' die hieronder worden beschreven.

Geef de volgende velden op in de sectie \'-technische configuratie\':

- **Type van het test station**: Kies \'micro soft gehost (voor beeld Dynamics 365 voor Customer engagement). Dit geeft aan dat micro soft de service zal hosten en onderhouden die de inrichting van de gebruiker van het test station en het ongedaan maken van de inrichting uitvoert.
...
- **Azure AD-Tenant-id**: Geef de id op van de Azure-Tenant voor uw Dynamics 365-exemplaar. Als u deze waarde wilt ophalen, meldt u zich aan bij Azure Portal en navigeert u naar \'Azure Active Directory\' -selecteert u eigenschappen in menu Blade \> de map-ID te kopiëren.\> Voorbeeld waarde: 72f988bf-86f1-41af-91ab-2d7cd0111234
...
- **URL van Web API-exemplaar**: Geef de Web-API-URL op voor uw Dynamics 365-exemplaar. U kunt deze waarde ophalen door u aan te melden bij uw micro soft Dynamics 365-exemplaar en te navigeren naar setting-\> Customization-\> Developer Resources-\>-exemplaar Web-API (kopieer deze URL). Voorbeeld waarde: https: \//Testdrive. CRM. Dynamics. com/API/data/v 9.0 
