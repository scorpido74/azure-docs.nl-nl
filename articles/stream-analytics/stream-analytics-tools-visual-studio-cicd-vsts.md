...
* Maak een [Azure DevOps-organisatie.](https://visualstudio.microsoft.com/team-services/)
...
3. Controleer uw e-mail en selecteer uw organisatie in de vervolgkeuzelijst **Azure DevOps Services Domain**. Voer de naam van de opslagplaats in en selecteer **Publish repository**.
   ...
   ![De knop Publish repository onder Push Git repo](./media/stream-analytics-tools-visual-studio-cicd-vsts/publish-repository-devops.png)
    ...
    Als u de opslagplaats pusht, wordt er een nieuw project voor uw organisatie gemaakt met dezelfde naam als de lokale opslagplaats. Als u de repo in een bestaand project wilt maken, klikt u op **Geavanceerd** naast **de naam Repository**en selecteert u een project. U kunt uw code in de browser weergeven door **See it on the web** te selecteren.
...
4. Builds worden ook getriggerd na pushen of inchecken. Als u de voortgang van de build wilt controleren, schakelt u over naar het tabblad **Builds.**  Zodra u controleert of de build succesvol is uitgevoerd, moet u een releasepijplijn definiëren die uw toepassing naar een cluster implementeert. Klik met de rechtermuisknop op het beletselteken naast uw build-pijplijn en selecteer **Edit**.
