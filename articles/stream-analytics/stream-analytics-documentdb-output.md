...
> Andere Azure Cosmos DB-API's zijn nog niet ondersteund. Als u Stream Analytics naar Azure Cosmos DB accounts die zijn gemaakt met andere Api's, zijn de gegevens mogelijk niet op de juiste manier opgeslagen. 
...
Afhankelijk van welke mate van lezen van consistentie op basis van de behoeften van uw scenario lezen en schrijven latentie, dat kunt u een consistentieniveau kiezen voor uw databaseaccount. U kunt de door Voer verbeteren door het schalen van de aanvraag eenheden (RUs) op de container. 
...
De opslag grootte voor documenten die deel uitmaken van dezelfde partitie sleutel is beperkt tot 10 GB. Een ideale partitie sleutel is een code die regel matig wordt weer gegeven als filter in uw query's en voldoende kardinaliteit heeft om ervoor te zorgen dat uw oplossing schaalbaar is.
...
Hier volgt een voor beeld van een query:
