...
1. Wanneer u het _apparaat_ in de Terminal hebt aangesloten, is een van de uitvoer berichten het volgende bericht om de online status aan te geven. De eigenschap `state`, die wordt gebruikt om aan te geven of het apparaat online is, is _waar_:
...
1. Schuif in de Terminal uitvoer van de _service_ naar het onderdeel `environmentalSensor`. U ziet dat de eigenschap `state` is gerapporteerd als _waar_:
    ```JSON
    "environmentalSensor" : {
