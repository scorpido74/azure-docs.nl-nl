...
1. In het zoekvak voert u `when a http request`in en selecteert u **wanneer er een HTTP-aanvraag wordt ontvangen** om te gebruiken als de trigger.
...
1. Onder **Kies een actie**, typt u `as2 decode`in het zoekvak en selecteert u **AS2 decode (v2)** .

   ![Zoek en selecteer ' AS2 decode (v2) '](./media/logic-apps-enterprise-integration-b2b/add-as2-decode-action.png)

1. Voer bij het **bericht voor het decoderen** van de eigenschap de invoer in die door de AS2-actie moet worden gedecodeerd. Dit is de `body` inhoud die wordt ontvangen door de HTTP-aanvraag trigger. U kunt deze inhoud op verschillende manieren opgeven als invoer, hetzij uit de lijst met dynamische inhoud hetzij als een expressie:
...
1. Voer voor de eigenschap **bericht koppen** eventuele headers in die vereist zijn voor de AS2-actie, die worden beschreven door de `headers` inhoud die wordt ontvangen door de HTTP-aanvraag trigger.
   ...
   Als u een expressie wilt invoeren die naar de `headers` uitvoer van de trigger verwijst, klikt u in het vak **bericht headers** . Nadat de lijst met dynamische inhoud wordt weer gegeven, selecteert u **expressie**. Voer in de expressie-editor hier de expressie in en selecteer **OK**:
...
1. Geef nu de antwoorden op om te retour neren of de **AS2-Decoderings** actie slaagt of niet.
   ...
   1. Selecteer **een actie toevoegen** **voor het geval** dat de **AS2-decoderings** actie slaagt. Voer in het zoekvak onder **Kies een actie**`response`in en selecteer vervolgens **antwoord**.
   ...
   1. Als de **AS2-Decoderings** actie mislukt, selecteert u in de shape **indien onwaar** **een actie toevoegen**. Voer in het zoekvak onder **Kies een actie**`response`in en selecteer vervolgens **antwoord**. Stel de **reactie** actie in om de gewenste status en fout te retour neren.
...
1. Voer in het zoekvak onder **Kies een actie**`x12 decode`in en selecteer **X12-bericht decoderen**.
