...
Voor scenario's waarin uw Logic apps en integratie accounts toegang nodig hebben tot een [virtueel Azure-netwerk](../virtual-network/virtual-networks-overview.md), maakt u een [ISE ( *Integration service Environment* )](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md). Een ISE is een speciale omgeving waarin speciale opslag en andere bronnen worden gebruikt die gescheiden worden gehouden van de ' wereld wijde ' multi tenant-Logic Apps service. Deze schei ding vermindert ook de invloed die andere Azure-tenants mogelijk hebben op de prestaties van uw apps. Een ISE biedt u ook uw eigen vaste IP-adressen. Deze IP-adressen zijn gescheiden van de statische IP-adressen die worden gedeeld door de Logic apps in de open bare multi tenant-service.
...
* Een [virtueel Azure-netwerk](../virtual-network/virtual-networks-overview.md). Het virtuele netwerk moet vier *lege* subnetten hebben die niet worden overgedragen aan een service voor het maken en implementeren van resources in uw ISE. Elk subnet ondersteunt een ander Logic Apps onderdeel dat wordt gebruikt in uw ISE. U kunt de subnetten vooraf maken, maar u kunt wachten totdat u de ISE maakt waar u subnetten tegelijk kunt maken. Meer informatie over de vereisten voor het [subnet](#create-subnet).
  ...
  > * 169.254.169.254/32
  > 
  > De namen van subnetten moeten beginnen met een alfabetisch teken of een liggend streepje en mogen niet de volgende tekens gebruiken: `<` , `>` , `%` , `&` , `\\` , `?` , `/` . Als u uw ISE via een Azure Resource Manager sjabloon wilt implementeren, moet u eerst een leeg subnet delegeren `Microsoft.Logic/integrationServiceEnvironment` . U hoeft deze delegatie niet uit te voeren wanneer u via de Azure Portal implementeert.
...
U moet ook uitgaande regels voor [app service Environment toevoegen (ASE)](../app-service/environment/intro.md):
...
1. Geef deze gegevens voor uw omgeving op en selecteer vervolgens **bekijken + maken**, bijvoorbeeld:
   ...
   |----------|----------|-------|-------------|
   | **Abonnement** | Yes | <*Azure-abonnement-naam*> | Het Azure-abonnement dat u wilt gebruiken voor uw omgeving |
   | **Resourcegroep** | Yes | <*Azure-resource-group-name*> | Een nieuwe of bestaande Azure-resource groep waar u uw omgeving wilt maken |
   ...
   | **Locatie** | Yes | <*Azure-Data Center-regio*> | De Azure Data Center-regio waar u uw omgeving kunt implementeren |
   ...
   | **Subnetten** | Yes | <*subnet-Resource-lijst*> | Een ISE vereist vier *lege* subnetten voor het maken en implementeren van resources in uw omgeving. [Volg de stappen onder deze tabel](#create-subnet)om elk subnet te maken. |
   |||||
