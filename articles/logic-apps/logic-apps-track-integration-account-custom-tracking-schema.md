...
|----------|----------|------|-------------|
| sourceType | Ja | Tekenreeks | Type van de uitvoerings bron met de volgende toegestane waarden: `Microsoft.Logic/workflows`, `custom` |
| source | Ja | Teken reeks of JToken | Als het bron type `Microsoft.Logic/workflows`is, moeten de bron gegevens dit schema volgen. Als het bron type `custom`is, is het schema een JToken. |
...
| repeatItemScopeName | Ja | Tekenreeks | Item naam herhalen als de actie zich in een `foreach`of `until` lus bevindt |
| repeatItemIndex | Ja | Geheel getal | Geeft aan dat de actie zich in een `foreach`-of `until`-lus bevindt en het herhaalde item index nummer is. |
...
| eventTime | Ja | Datum/tijd | Tijd van de gebeurtenis in UTC-notatie: *jjjj-mm-ddTuu: mm: SS. 00000Z* |
| recordType | Ja | Tekenreeks | Type track record met deze toegestane waarde: `custom` |
| teken | Ja | JToken | Aangepast record type met alleen de JToken-indeling |
|||||
