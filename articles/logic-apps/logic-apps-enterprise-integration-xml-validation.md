---
title: XML valideren voor B2B Enter prise Integration
...
1. Als u een lege logische app hebt, voert `HTTP request` u in de ontwerp functie voor logische apps in het zoekvak in als uw filter en selecteert u de trigger **Wanneer een HTTP-aanvraag is ontvangen** . Als dat niet het geval is, gaat u verder met de volgende stap.
...
1. Selecteer onder **Kies een actie de**optie **ingebouwd**. Voer `xml validation` in het zoekvak in als uw filter. Selecteer **XML-validatie**in de lijst acties.
