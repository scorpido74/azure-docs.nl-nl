...
2. Gebruik de klassieke versie van Azure Machine Learning Studio om onze resultaten te bevestigen, te optimaliseren, te verhogen en te schalen. 

Net als bij veel bedrijven gaat onze organisatie door een maandelijks omzet prognose proces. Ons kleine team van bedrijfs analisten is gewerkt met het gebruik van de klassieke versie van Azure Machine Learning Studio om het proces te ondersteunen en de nauw keurigheid van de prognose te verbeteren. Het team heeft een aantal maanden besteed aan het verzamelen van gegevens uit meerdere bronnen en het uitvoeren van de gegevens kenmerken via statistische analyses, waarmee belang rijke kenmerken van de verkoop prognoses van services worden geïdentificeerd. De volgende stap was het starten van het prototypen van statistische regressie modellen op de gegevens in Excel. Binnen een paar weken moesten we een Excel-regressie model maken dat het huidige veld heeft uitgevoerd en de prognoses voor het verwerken van de Financiën heeft uitgewerkt. Dit is het voor Spellings resultaat van de basis lijn. 

We hebben vervolgens de volgende stap ondernomen om onze predictive analytics te verplaatsen naar de klassieke versie van Studio om erachter te komen hoe de klassieke versie van Studio kan worden verbeterd op voorspel bare prestaties.

## <a name="achieving-predictive-performance-parity"></a>Vertraagde prestatie pariteit
Onze eerste prioriteit is om pariteit te halen tussen de klassieke versie van Studio-en Excel regressie modellen. Gezien dezelfde gegevens, en dezelfde splitsing voor trainings-en test gegevens, wilden we anticiperen op voorspel bare prestatie pariteit tussen Excel en de klassieke versie van Studio. In eerste instantie mislukt. Het Excel-model van een uitvoerbaar Studio model (klassiek). De fout is veroorzaakt door het ontbreken van een goed idee van de instelling basis programma in de klassieke versie van Studio. Na een synchronisatie met de klassieke versie van Studio-product team hebben we een beter inzicht in de basis instelling die is vereist voor onze gegevens sets, en is er pariteit tussen de twee modellen behaald. 
...
We hebben het *gemiddelde absolute% error* berekend en gebruiken als de prestatie meting voor het model. Het duurde drie maanden om aan een werkend model te komen met Excel. We hebben veel van het leer proces in de klassieke versie van Studio experiment gezet, wat uiteindelijk nuttig was voor de vereisten.

### <a name="create-comparable-experiment-in-studio-classic"></a>Vergelijkbaar experiment maken in Studio (klassiek)
We hebben deze stappen gevolgd om ons experiment te maken in de klassieke versie van Studio: 

1. De gegevensset is geüpload als een CSV-bestand naar de klassieke versie van Studio (zeer klein bestand)
...
* Wanneer u de [lineaire regressie][linear-regression] module in de klassieke versie van Studio gebruikt, zijn er twee methoden aanwezig:
...
Toen we de aanbevelingen hebben toegepast, hebben we dezelfde basislijn prestaties behaald in de klassieke versie van Studio als met Excel: 
...
We wilden ons model ook optimaliseren door gebruik te maken van de opties en algoritmen die beschikbaar zijn in de klassieke versie van Studio.
...
We hebben een heleboel geleerd van het uitvoeren van Excel-regressie en de klassieke versie van Studio experimenten parallel. Het basis model maken in Excel en vergelijken met modellen met behulp van Machine Learning [lineaire regressie][linear-regression] heeft ons geholpen bij het leren van Studio (klassiek) en we hebben mogelijkheden gedetecteerd om de gegevens selectie en de prestaties van het model te verbeteren. 

We hebben ook geadviseerd om op [filter gebaseerde functie selectie][filter-based-feature-selection] te gebruiken om toekomstige Voorspellings projecten te versnellen. Door functie selectie toe te passen op uw gegevens, kunt u een verbeterd model maken in de klassieke versie van Studio met betere algehele prestaties. 

De mogelijkheid om de voorspellende analytische prognose van de klassieke versie van Studio over te brengen naar Excel System root, maakt een aanzienlijke toename van de mogelijkheid om resultaten te bieden aan een breed publiek van de gebruikers. 
