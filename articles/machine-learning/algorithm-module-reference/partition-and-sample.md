...
1. Voeg de **partitie-en voorbeeld** module toe aan uw experiment in de interface en verbind de gegevensset.  

2. **Partitie of voorbeeld modus**: Stel deze optie in op **kop**.

3. **Aantal te selecteren rijen**: Typ het aantal rijen dat u wilt retour neren.
...
1. Voeg de **partitie-en voorbeeld** module toe aan uw experiment en verbind de gegevensset.
...
3. **Sampling frequentie**: Typ een waarde tussen 0 en 1. met deze waarde wordt het percentage rijen van de bron-gegevensset opgegeven dat moet worden opgenomen in de uitvoer gegevensset.
    ...
    Als u bijvoorbeeld slechts de helft van de oorspronkelijke gegevensset wilt, typt `0.5` u om aan te geven dat de sampling frequentie 50% moet zijn.
...
4. **Wille keurig zaad voor steek proeven**: Typ desgewenst een geheel getal dat moet worden gebruikt als een Seed-waarde.
...
5. **Stratified splitsen voor steek proef**: Selecteer deze optie als het belang rijk is dat de rijen in de gegevensset gelijkmatig moeten worden verdeeld over een bepaalde sleutel kolom voordat ze worden gesampled.
...
3. **Vervanging gebruiken in partities**: Selecteer deze optie als u wilt dat de geplaatste rij wordt weer gegeven in de pool met rijen voor een mogelijke hergebruik. Als gevolg hiervan kan dezelfde rij worden toegewezen aan verschillende vouwen.
...
5. **Wille keurige Seed**: Typ desgewenst een geheel getal dat moet worden gebruikt als de Seed-waarde. Deze optie is belang rijk als u wilt dat de rijen op dezelfde manier worden gedeeld als elke keer. Anders is de standaard waarde 0 betekent dat een wille keurige begin Seed wordt gebruikt.

6. **Geef de partitioner-methode**op: Geef aan hoe gegevens moeten worden verdeeld over elke partitie, met behulp van de volgende opties:
...
7. **Stratified-splitsing**: Selecteer deze optie als u wilt dat de rijen worden stratified wanneer ze worden gesplitst, en kies vervolgens de _kolom Strata_.
...
1. Voeg de **partitie en de voorbeeld** module toe aan het experiment.
...
4. **Geef op met welke vouw moet worden steek proef**: Selecteer een te gebruiken partitie door de index ervan te typen. Partitie-indexen zijn op 1 gebaseerd. Als u de gegevensset bijvoorbeeld in drie delen hebt verdeeld, hebben de partities de indices 1, 2 en 3.
    ...
    Als u een ongeldige index waarde typt, wordt een fout in de ontwerp fase gegenereerd: "Fout 0018: De gegevensset bevat ongeldige gegevens. "
