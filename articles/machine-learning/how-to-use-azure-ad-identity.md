...
In deze procedure leert u hoe u een Azure Active Directorys-id (AAD) toewijst aan uw geïmplementeerde machine learning model in de Azure Kubernetes-service. Met het [pod Identity](https://github.com/Azure/aad-pod-identity) project van Aad kunnen toepassingen veilig toegang krijgen tot Cloud bronnen met Aad met behulp van een [beheerde identiteit](https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/overview) en Kubernetes primitieven. Zo kunt u uw webservices veilig toegang geven tot uw Azure-resources zonder dat u referenties hoeft in te sluiten of tokens rechtstreeks in uw `score.py`-script hoeft te beheren. In dit artikel worden de stappen beschreven voor het maken en installeren van een Azure-identiteit in uw Azure Kubernetes service-cluster en het toewijzen van de identiteit aan uw geïmplementeerde webservice.
...
- Toegang tot uw AKS-cluster met behulp van de opdracht `kubectl`. Zie [verbinding maken met het cluster](https://docs.microsoft.com/azure/aks/kubernetes-walkthrough#connect-to-the-cluster) voor meer informatie.
...
1. Gebruik de volgende opdracht om te bepalen of het AKS-cluster RBAC is ingeschakeld:
    ...
    Met deze opdracht wordt de waarde `true` geretourneerd als RBAC is ingeschakeld. Deze waarde bepaalt de opdracht die in de volgende stap moet worden gebruikt.
...
Bewerk de implementatie om het label van de Azure Identity Selector toe te voegen. Ga naar de volgende sectie onder `/spec/template/metadata/labels`. Als het goed is, ziet u waarden zoals `isazuremlapp: “true”`. Voeg het label Aad-pod-Identity toe, zoals hieronder wordt weer gegeven.
...
Als u uw Azure Identity Lees toegang hebt gegeven tot een geheim in een **Key Vault**, heeft uw `score.py` toegang tot het bestand met de volgende code.
...
Als u uw Azure Identity Lees toegang hebt tot gegevens in een **opslag-BLOB**, heeft uw `score.py` hiervoor de volgende code gebruikt.
