...
Naast het door geven van de `inputs` gegevensset via de para meter in de Estimator, kunt u de `script_params` gegevensset ook door geven via en het gegevenspad (koppel punt) ophalen in uw trainings script via argumenten. Op deze manier kunt u uw trainings script onafhankelijk van de azureml-SDK blijven gebruiken. Met andere woorden, u kunt hetzelfde trainings script gebruiken voor lokale fout opsporing en externe training op elk cloud platform.
