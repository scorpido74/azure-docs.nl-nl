---
title: Virtuele netwerken verbinden met de peering van het virtuele netwerk-Power shell | Microsoft Docs
...
Maak een VM met [New-AzVM](/powershell/module/az.compute/new-azvm). In het volgende voor beeld wordt een VM gemaakt met de naam *myVm1* in het virtuele netwerk *myVirtualNetwork1* . Met de optie `-AsJob` wordt de virtuele machine op de achtergrond gemaakt, zodat u verder kunt gaan met de volgende stap. Wanneer u hierom wordt gevraagd, voert u de gebruikers naam en het wacht woord in waarmee u zich wilt aanmelden bij de virtuele machine.
