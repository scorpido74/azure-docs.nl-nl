...
- Het argument `-PublicIpAddress` is wegge laten of is vervangen door `-PrivateIpAddress`. Houd er rekening mee dat het privé adres zich binnen het bereik van de IP-ruimte van het subnet bevindt waarin de interne load balancer worden geïmplementeerd. Als een statisch `-PrivateIpAddress` wordt wegge laten, wordt het volgende beschik bare IPv6-adres geselecteerd in het subnet waarin de interne Load Balancer is geïmplementeerd.
- Het dubbele stack subnet waarin de interne load balancer worden geïmplementeerd, is opgegeven met het argument `-Subnet` of `-SubnetId`.
