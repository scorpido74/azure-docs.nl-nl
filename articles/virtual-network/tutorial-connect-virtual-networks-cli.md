...
Maak een peering van *myVirtualNetwork1* naar *myVirtualNetwork2* met [AZ Network vnet peering Create](/cli/azure/network/vnet/peering). Als de para meter `--allow-vnet-access` niet is opgegeven, wordt er een peering tot stand gebracht, maar kan er geen communicatie door worden uitgevoerd.
