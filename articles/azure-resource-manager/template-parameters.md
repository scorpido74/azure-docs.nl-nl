---
title: Para meters in Azure Resource Manager sjablonen
...
In de sjabloon verwijst u naar de waarde voor de para meter met behulp van de [para meters](resource-group-template-functions-deployment.md#parameters) -functie. In het volgende voor beeld wordt de waarde van de para meter gebruikt om SKU in te stellen voor het opslag account.
...
U kunt de functie [Reference](resource-group-template-functions-resource.md#reference) of een van de [lijst](resource-group-template-functions-resource.md#list) functies niet gebruiken in de para meters van de sectie. Deze functies halen de runtime status van een resource en kunnen niet worden uitgevoerd voor implementatie wanneer para meters worden omgezet.
...
* Zie [inzicht krijgen in de structuur en de syntaxis van Azure Resource Manager-sjablonen](resource-group-authoring-templates.md)voor meer informatie over de beschik bare eigenschappen voor para meters.
* Zie [Resource Manager-parameter bestand maken](resource-manager-parameter-files.md)voor meer informatie over het door geven van parameter waarden als een bestand.
