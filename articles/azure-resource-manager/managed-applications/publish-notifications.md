...
1. Stel een openbaar HTTPS-eind punt in waarmee de binnenkomende POST-aanvragen `200 OK`worden geregistreerd en geretourneerd.
...
> [!NOTE]
> Op dit moment kunt u slechts één eind punt in `notificationEndpoints` de in de eigenschappen van de toepassings definitie opgeven.
...
Type | ProvisioningState | Trigger voor melding
---|---|---
...
1. Geef een query parameter op boven op de webhook-URI, zoals: https\://your-endpoint.com? sig = GUID. Controleer bij elke melding of de query parameter `sig` de verwachte waarde `Guid`heeft.
