...
---
# <a name="quickstart-create-and-deploy-arm-templates-by-using-the-azure-portal"></a>Snelstart: ARM-sjablonen maken en implementeren met behulp van de Azure-portal

Meer informatie over het genereren van een ARM-sjabloon (Azure Resource Manager) met behulp van de Azure-portal en het proces van het bewerken en implementeren van de sjabloon vanuit de portal. ARM-sjablonen zijn JSON-bestanden die de resources definiëren die u voor uw oplossing moet implementeren. Zie overzicht van de implementatie van sjablonen voor het implementeren en beheren van de concepten die zijn gekoppeld aan het implementeren en beheren van uw [Azure-oplossingen.](overview.md)

![Snelstartportaldiagram resourcemanager-sjabloon](./media/quickstart-create-templates-use-the-portal/azure-resource-manager-export-deploy-template-portal.png)
...
Het maken van een ARM-sjabloon vanaf nul is geen gemakkelijke taak, vooral als u nieuw bent in Azure-implementatie en u niet bekend bent met de JSON-indeling. Met behulp van de Azure-portal kunt u een resource configureren, bijvoorbeeld een Azure Storage-account. Voordat u de resource implementeert, u uw configuratie exporteren naar een sjabloon. U kunt de sjabloon opslaan voor later gebruik.

Veel ervaren sjabloonontwikkelaars gebruiken deze methode om sjablonen te genereren wanneer ze Azure-resources proberen te implementeren die ze niet kennen. Zie [Resourcegroepen exporteren naar sjablonen voor](../management/manage-resource-groups-portal.md#export-resource-groups-to-templates)meer informatie over het exporteren van sjablonen met behulp van de portal. De andere manier om een werksjabloon te vinden, is van [Azure Quickstart-sjablonen.](https://azure.microsoft.com/resources/templates/)
...
1. Selecteer in het menu azure portal de optie **Een resource maken**.
    ...
    ![Selecteer Een resource maken in het menu van de Azure-portal](./media/quickstart-create-templates-use-the-portal/azure-resource-manager-template-tutorial-create-a-resource.png)

1. Selecteer > **Opslagopslagaccount**. **Storage**
...
1. Voer de volgende informatie in:
    ...
    |**Naam**|geef uw opslagaccount een unieke naam. De naam van het opslagaccount moet uniek zijn voor alle Azure's en alleen kleine letters en cijfers bevatten. De naam moet tussen de 3 en 24 tekens zijn. Als u een foutmelding krijgt met de tekst 'De naam van het opslagaccount 'mystorage1016' is al genomen", probeer ** &lt;dan uw naam>opslag&lt;de datum van vandaag in MMDD->**, bijvoorbeeld **johndolestorage1016**. Zie [Naamgevingsregels en beperkingen voor](/azure/architecture/best-practices/resource-naming)meer informatie.|
...
1. Selecteer **Controleren + maken** onderaan in het scherm. Selecteer **Niet maken** in de volgende stap.
1. Selecteer **Een sjabloon voor automatisering downloaden** onderaan in het scherm. In de portal wordt de gegenereerde sjabloon weergegeven:
    ...
    In het hoofdvenster wordt de sjabloon getoond. Het is een JSON-bestand met zes elementen van het hoogste niveau: `schema`, `contentVersion`, `parameters`, `variables`, `resources` en `output`. Zie [De structuur en syntaxis van ARM-sjablonen begrijpen voor](./template-syntax.md) meer informatie
    ...
    In de sjabloon is er één Azure-resource gedefinieerd. Het type `Microsoft.Storage/storageAccounts`is . Neem een kijkje van hoe de resource is gedefinieerd, en de definitiestructuur.
1. Selecteer **Downloaden** vanaf de bovenkant van het scherm.
1. Open het gedownloade zip-bestand en sla **template.json** op uw computer op. In het volgende gedeelte gebruikt u een implementatiehulpprogramma voor sjablonen om de sjabloon te bewerken.
1. Selecteer het tabblad **Parameter** om de waarden te bekijken die u voor de parameters hebt opgegeven. Schrijf deze waarden op. U hebt ze in het volgende gedeelte (bij het implementeren van de sjabloon) weer nodig.
    ...
    Met behulp van zowel het sjabloonbestand als het parametersbestand u in deze zelfstudie een Azure-opslagaccount maken.

## <a name="edit-and-deploy-the-template"></a>De sjabloon bewerken en implementeren

U kunt de Azure-portal gebruiken om eenvoudige wijzigingen door te voeren in sjablonen. In deze snelstart gebruikt u het portalhulpprogramma *Sjabloonimplementatie*. In deze zelfstudie wordt *Sjabloonimplementatie* gebruikt, zodat u de hele zelfstudie kunt doen met behulp van één interface: de Azure-portal. Als u een complexere sjabloon wilt bewerken, u [overwegen visual studiocode](quickstart-create-templates-use-visual-studio-code.md)te gebruiken, die uitgebreidere bewerkingsfunctionaliteiten biedt.

> [!IMPORTANT]
> Sjabloonimplementatie biedt een interface voor het testen van eenvoudige sjablonen. Het wordt afgeraden om deze functie in productie te gebruiken. Sla in plaats daarvan uw sjablonen op in een Azure-opslagaccount of een broncodeopslagplaats zoals GitHub.
...
1. Breng de volgende drie wijzigingen aan in de sjabloon:
   ...
   - Verwijder de parameter **storageAccountName** zoals weergegeven in de vorige schermafbeelding.
   - Voeg één variabele genaamd **storageAccountName** toe, zoals in de vorige schermafbeelding wordt weergegeven:
...
1. Voer de volgende waarden in:
    ...
    |Name|Waarde|
    |----|----|
    |**Resourcegroep**|Selecteer de naam van de resourcegroep die u in de laatste sectie hebt gemaakt. |
...
In deze zelfstudie hebt u geleerd hoe u een sjabloon genereert in Azure Portal en hoe u de sjabloon via de portal implementeert. De gebruikte sjabloon in deze snelstart is een eenvoudige sjabloon met één Azure-resource. Als de sjabloon complex is, kunt u beter Visual Studio Code of Visual Studio gebruiken om de sjabloon te ontwikkelen. Zie onze nieuwe beginnerszelfstudiereeks voor meer informatie over sjabloonontwikkeling:
