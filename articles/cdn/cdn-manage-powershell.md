---
title: Azure CDN beheren met Power shell | Microsoft Docs
description: Meer informatie over het gebruik van de Azure PowerShell-cmdlets voor het beheren van Azure CDN.
services: cdn
documentationcenter: ''
author: mdgattuso
manager: danielgi
...
Als u Power shell wilt gebruiken om uw Azure CDN profielen en eind punten te beheren, moet u de module Azure PowerShell hebben geïnstalleerd.  Zie [Azure PowerShell installeren en configureren](/powershell/azure/overview)voor meer informatie over het installeren van Azure PowerShell en het maken van verbinding met Azure met behulp van de `Connect-AzAccount`-cmdlet.
...
U kunt hulp krijgen bij een van deze cmdlets met behulp van de cmdlet `Get-Help`.  `Get-Help` biedt gebruik en syntaxis en optioneel voor beelden.
...
## <a name="listing-existing-azure-cdn-profiles"></a>Bestaande Azure CDN profielen weer geven
De cmdlet `Get-AzCdnProfile` zonder para meters haalt al uw bestaande CDN-profielen op.

```powershell
Get-AzCdnProfile
```

Deze uitvoer kan worden gepiped naar cmdlets voor inventarisatie.
...
U kunt ook één profiel retour neren door de profiel naam en de resource groep op te geven.
...
> [!TIP]
> Het is mogelijk om meerdere CDN-profielen met dezelfde naam te hebben, zolang ze zich in verschillende resource groepen bevinden.  Als u de para meter `ResourceGroupName` weglaat, worden alle profielen met een overeenkomende naam geretourneerd.
> 
> 

## <a name="listing-existing-cdn-endpoints"></a>Bestaande CDN-eind punten weer geven
`Get-AzCdnEndpoint` kunt een afzonderlijk eind punt of alle eind punten voor een profiel ophalen.  
...
## <a name="creating-cdn-profiles-and-endpoints"></a>CDN-profielen en-eind punten maken
`New-AzCdnProfile` en `New-AzCdnEndpoint` worden gebruikt voor het maken van CDN-profielen en-eind punten. De volgende Sku's worden ondersteund:
- Standard_Verizon
- Premium_Verizon
- Custom_Verizon
- Standard_Akamai
...
## <a name="checking-endpoint-name-availability"></a>Beschik baarheid van eindpunt naam controleren
`Get-AzCdnEndpointNameAvailability` retourneert een object dat aangeeft of er een eindpunt naam beschikbaar is.
...
`New-AzCdnCustomDomain` voegt een aangepaste domein naam toe aan een bestaand eind punt.

> [!IMPORTANT]
> U moet de CNAME met uw DNS-provider instellen, zoals wordt beschreven in het toewijzen van een [aangepast domein aan Content Delivery Network (CDN)-eind punt](cdn-map-content-to-custom-domain.md).  U kunt de toewijzing testen voordat u het eind punt wijzigt met behulp van `Test-AzCdnCustomDomain`.
> 
> 
...
## <a name="modifying-an-endpoint"></a>Een eind punt wijzigen
`Set-AzCdnEndpoint` wijzigt een bestaand eind punt.
...
`Unpublish-AzCdnEndpointContent` maakt in cache opgeslagen assets, terwijl `Publish-AzCdnEndpointContent` vooraf geladen assets op ondersteunde eind punten.
...
## <a name="startingstopping-cdn-endpoints"></a>CDN-eind punten starten/stoppen
`Start-AzCdnEndpoint` en `Stop-AzCdnEndpoint` kunnen worden gebruikt om afzonderlijke eind punten of groepen eind punten te starten en te stoppen.
...
`Remove-AzCdnProfile` en `Remove-AzCdnEndpoint` kunnen worden gebruikt om profielen en eind punten te verwijderen.
...
Zie [overzicht van CDN](cdn-overview.md)voor meer informatie over CDN-functies.
