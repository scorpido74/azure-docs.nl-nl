...
2. Open **site Recovery-infra structuur** > **VMware en fysieke machines** > **configuratie Servers** (onder voor VMware & fysieke machines).
3. Selecteer de configuratie server om in te zoomen op de server details. Klik vervolgens op **+ proces server**.
4. In **proces server toevoegen** >  **kiezen waar u uw proces server wilt implementeren**, selecteert u **een scale-out proces server on-premises implementeren**.
