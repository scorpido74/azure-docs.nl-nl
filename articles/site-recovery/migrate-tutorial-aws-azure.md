...
ms.locfileid: "86133180"
---
# <a name="migrate-amazon-web-services-aws-vms-to-azure"></a>AWS-VM’s (Amazon Web Services) migreren naar Azure

In deze zelfstudie ziet u hoe u AWS-VM’s (Amazon Web Services) naar Azure-VM’s (virtuele machines) migreert met behulp van Azure Site Recovery. Wanneer u AWS EC2-instanties migreert naar Azure, worden de VM’s behandeld als fysieke on-premises computers. In deze zelfstudie leert u het volgende:


> [!TIP]
> U moet nu de Azure Migrate-service gebruiken, in plaats van de Azure Site Recovery-service, om AWS-VM’s te migreren naar Azure. [Meer informatie](../migrate/tutorial-migrate-physical-virtual-machines.md).
...
1. Selecteer in de [Azure-portal](https://portal.azure.com) achtereenvolgens **Een resource maken** > **Netwerken** >
   **Virtueel netwerk**.
3. Voer bij **Naam** in **myMigrationNetwork**.
4. Laat de standaardwaarde staan die is ingesteld voor **Adresruimte** (moet een waarde invoeren).
...
8. Laat onder **Subnet** de standaardwaarden staan bij **Naam** en **IP-bereik** (moet een waarde invoeren).
9. Voeg instructies toe voor DDoS-beveiligingsinstellingen.
10. Laat de optie **Service-eindpunten** uitgeschakeld.
11. Voeg instructies toe voor firewallinstellingen.
...
| Voert u een migratie uit? | Selecteer **Ja**, en schakel vervolgens het selectievakje in naast **Ik begrijp het, maar ik wil graag doorgaan met Azure Site Recovery.**
...
2. Configureer de proxy op de EC2-instantie-VM die u als de configuratieserver gebruikt, zodat deze toegang heeft tot de [service-URL's](./vmware-physical-azure-support-matrix.md).
