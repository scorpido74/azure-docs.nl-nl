...
In deze zelfstudie wordt beschreven hoe u Azure Data Box Heavy gebruikt voor het migreren van uw on-premises VHD's naar beheerde schijven in Azure. De VHD's van on-premises VM's worden als pagina-blobs gekopieerd naar Data Box Heavy en als beheerde schijven geüpload in Azure. Deze beheerde schijven kunnen vervolgens worden gekoppeld aan virtuele Azure-machines.
...
Op basis van de opgegeven resourcegroepen, maakt Data Box Heavy één share voor elke gekoppelde resourcegroep per knooppunt. Als bijvoorbeeld `mydbmdrg1` en `mydbmdrg2` zijn gemaakt bij het plaatsen van de order, worden de volgende shares gemaakt:
...
3. Gebruik na het installeren van de NFS-client de volgende opdracht om de NFS-share op uw apparaat te koppelen:
    ...
    Het volgende voorbeeld toont hoe u via NFS verbinding maakt met een Data Box- of Data Box Heavy-share. Het IP-adres van het Data Box- of Data Box Heavy-apparaat is `169.254.250.200`, de share `mydbmdrg1_MDisk` wordt gekoppeld op de Ubuntu-VM, het koppelpunt is `/home/databoxubuntuhost/databox`.
...
- Lees over groottelimieten voor beheerde schijven in [Azure-objectgroottelimieten](data-box-heavy-limits.md#azure-object-size-limits).
