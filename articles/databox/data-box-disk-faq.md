...
---
# <a name="azure-data-box-disk-frequently-asked-questions"></a>Azure-gegevensvakschijf: veelgestelde vragen
...
A.  Ga naar de [beschikbaarheid van](data-box-disk-overview.md#region-availability)de regio om te zien waar de schijven van de gegevensbox beschikbaar zijn.  
...
A. Data Box Disk wordt ondersteund voor alle regio's binnen de VS, Canada, Australië, West-Europa en Noord-Europa, Korea en Japan. Alleen de Azure-regio's met een openbare cloud worden ondersteund. Azure Government of andere onafhankelijke clouds worden niet ondersteund.

### <a name="q-will-my-data-box-disk-cross-country-borders-during-shipping"></a>V. Gaat mijn Data Box Disk tijdens verzending de landsgrenzen over?
A. Data Box Disk wordt verzonden vanuit hetzelfde land als hun bestemming en zal geen internationale grenzen overschrijden. De enige uitzondering is voor bestellingen in de Europese Unie (EU), waar schijven van en naar elk EU-land kunnen worden verzonden.
...
### <a name="can-i-pick-up-my-data-box-disk-order-myself-can-i-return-the-disks-via-a-carrier-that-i-choose"></a>Kan ik mijn Data Box Disk-bestelling zelf ophalen? Kan ik de schijven retourneren via een provider die ik kies?
A. Ja. Microsoft biedt ook zelfbeheerde verzending in de Amerikaanse gov regio alleen. Wanneer u de databoxschijforder plaatst, u de optie zelfbeheerde verzending kiezen. Ga als volgt te werk om de volgorde van de gegevensboxschijf op te halen:
    
1. Nadat u de bestelling hebt geplaatst, wordt de bestelling verwerkt en worden de schijven voorbereid. U ontvangt via een e-mail een melding dat uw bestelling klaar is om af te halen. 
...
Zodra de gegevenskopie en -validatie is voltooid, voert u de volgende stappen uit om uw schijf terug te sturen:
...
3. Ga naar het **overzichtsblad** voor uw bestelling in Azure-portal. U ziet een melding met een code.
4. Gebruik die code en e-mail het [Azure Data Box Operations-team](mailto:adbops@microsoft.com) en geef ze de code. Ze zouden u informatie over waar en wanneer af te zetten van de schijven.
...
A. Er zijn drie hulpprogramma's beschikbaar voor Data Box Disk:
 - **Tool Voor het ontgrendelen van gegevensboxen**: gebruik dit hulpprogramma om de versleutelde schijven te ontgrendelen die vanuit Microsoft worden verzonden. Wanneer u schijven ontgrendelt met het hulpprogramma moet u een wachtwoordsleutel opgeven. Deze vindt u in de gegevens van de Data Box Disk-bestelling in de Azure-portal. 
 - **Tool Gegevensvakschijfvalidatie:** gebruik dit hulpmiddel om de namen van grootte, indeling en blob te valideren volgens de Azure-naamgevingsconventies. Hiermee worden ook controlesommen voor de gekopieerde gegevens gemaakt. Deze worden vervolgens gebruikt voor het verifiëren van de gegevens die naar Azure worden geüpload.
 - **Tool Gegevensboxschijfgesplitst:** gebruik dit hulpprogramma wanneer u meerdere schijven gebruikt en een grote gegevensset hebt die moet worden gesplitst en gekopieerd over alle schijven. Dit hulpprogramma is momenteel beschikbaar voor Windows. Deze tool wordt niet ondersteund met beheerde schijven. Deze tool valideert ook als het kopieert de gegevens, vandaar dat u de validatie stap overslaan bij het gebruik van deze tool.
...
A. Azure Files worden ondersteund met Data Box Disk, maar werken niet goed met Azure File Sync. Metagegevens worden niet bewaard als de bestandsgegevens worden gebruikt met Azure File Sync.
...
### <a name="q-i-copied-data-to-manageddisk-folder-i-dont-see-any-managed-disks-with-the-resource-group-specified-for-managed-disks-was-my-data-uploaded-to-azure-and-how-can-i-locate-it"></a>V. Ik heb gegevens gekopieerd naar de map ManagedDisk. Ik zie geen beheerde schijven met de resourcegroep opgegeven voor beheerde schijven. Zijn mijn gegevens geüpload naar Azure en hoe kan ik deze vinden?
A. Ja. Uw gegevens zijn geüpload naar Azure, maar als u geen beheerde schijven met de opgegeven brongroepen ziet, is dit waarschijnlijk omdat de gegevens niet geldig waren. Als de paginablobs, blokkeerblobs, Azure Files en beheerde schijven niet geldig waren, gaan deze naar de volgende mappen:
 - Paginablobs gaan naar een blokblobcontainer te beginnen met *databoxdisk-invalid-pb-*.
 - Azure Files zou gaan naar een blok blob container te beginnen met *databoxdisk-invalid-af-*.
 - Beheerde schijven gaan naar een blokblobcontainer te beginnen met *databoxdisk-invalid-md-*.

## <a name="next-steps"></a>Volgende stappen

- Controleer de [systeemvereisten voor gegevensboxschijf](data-box-disk-system-requirements.md).
