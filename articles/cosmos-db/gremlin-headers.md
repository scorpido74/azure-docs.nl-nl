...
In dit artikel worden kopteksten behandeld die Cosmos DB Gremlin-server terugkeert naar de aanroeper bij het uitvoeren van de aanvraag. Deze headers zijn handig voor het oplossen van problemen met de prestaties van aanvragen, het bouwen van een toepassing die systeem eigen integreert met Cosmos DB-service en het vereenvoudigen van klant ondersteuning.
...
| Status | Uitleg |
| --- | --- |
| **401** | Fout bericht `"Unauthorized: Invalid credentials provided"` wordt geretourneerd wanneer het verificatie wachtwoord niet overeenkomt met Cosmos DB account sleutel. Navigeer naar uw Cosmos DB Gremlin-account in de Azure Portal en controleer of de sleutel juist is.|
...
| **408** | `"Server timeout"` geeft aan dat het door lopen langer dan **30 seconden** duurde en door de server is geannuleerd. Optimaliseer uw trans acties om snel uit te voeren door hoek punten of randen te filteren op elke hop van traversal om het zoek bereik te beperken.|
| **409** | `"Conflicting request to resource has been attempted. Retry to avoid conflicts."` dit gebeurt meestal wanneer het hoek punt of een rand met een id al in de grafiek voor komt.| 
| **412** | De status code wordt aangevuld met het fout bericht `"PreconditionFailedException": One of the specified pre-condition is not met`. Deze fout wordt weer gegeven bij een optimistische gelijktijdigheids schending van het lezen van een rand of hoek punt en het terugschrijven naar het archief na wijziging. Veelvoorkomende situaties wanneer deze fout zich voordoet, is het wijzigen van eigenschappen, bijvoorbeeld `g.V('identifier').property('name','value')`. De Gremlin-engine zou het hoek punt lezen, wijzigen en terugschrijven. Als er een andere Pass-bewerking gelijktijdig wordt uitgevoerd om hetzelfde hoek punt of een rand te schrijven, wordt deze fout weer gegeven. De toepassing moet opnieuw naar de server worden verzonden.| 
| **429** | De aanvraag is beperkt en moet opnieuw worden uitgevoerd na een waarde in **x-MS-retry-na-MS**| 
| **500** | Fout bericht met `"NotFoundException: Entity with the specified id does not exist in the system."` geeft aan dat een Data Base en/of verzameling opnieuw is gemaakt met dezelfde naam. De fout verdwijnt binnen vijf minuten, aangezien wijzigingen caches in verschillende Cosmos DB-onderdelen niet doorgeven en ongeldig maken. Als u dit probleem wilt voorkomen, gebruikt u telkens een unieke database en collectie.| 
