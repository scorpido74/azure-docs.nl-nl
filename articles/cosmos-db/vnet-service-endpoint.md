...
De autorisatie valideert de machtiging voor de VNet-resource actie, zelfs als de gebruiker niet de VNET-Acl's opgeeft met behulp van Azure CLI. Op dit moment ondersteunt het besturings vlak van het Azure Cosmos-account de voltooiings status van het Azure Cosmos-account. Een van de para meters voor de besturings vlak `virtualNetworkRules`-aanroepen is. Als deze para meter niet wordt opgegeven, wordt met de Azure CLI een Get Data Base- `virtualNetworkRules` aanroep opgehaald en wordt deze waarde in de update aanroep gebruikt.
