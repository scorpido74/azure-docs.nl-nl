...
3. Plak de **tekenreeks primaire verbinding** van de portal in de waarde StorageConnectionString. Plak de tekenreeks tussen de aanhalingstekens.
...
1. Klik in Visual Studio met de rechtermuisknop op het **CosmosTableSamples-project** in **Solution Explorer** en klik vervolgens op **NuGet-pakketten beheren.** 

   ![NuGet-pakketten beheren](media/create-table-dotnet/azure-cosmosdb-manage-nuget.png)

2. Typ Microsoft.Azure.Cosmos.Table typ in het vak NuGet **Bladeren.** Daarmee wordt de clientbibliotheek Cosmos DB-tabel API gevonden. Houd er rekening mee dat deze bibliotheek momenteel beschikbaar is voor .NET Framework en .NET Standard. 
   
   ![Tabblad NuGet bladeren](media/create-table-dotnet/azure-cosmosdb-nuget-browse.png)

3. Klik **op Installeren** om de **Microsoft.Azure.Cosmos.Table-bibliotheek** te installeren. Hiermee worden het pakket voor de Azure Cosmos DB-tabel API en alle daarvoor vereiste onderdelen geïnstalleerd.

4. Wanneer u de hele app uitvoert, worden voorbeeldgegevens ingevoegd in de tabelentiteit en aan het einde verwijderd, zodat u geen gegevens ziet die worden ingevoegd als u het hele voorbeeld uitvoert. U echter enkele breekpunten invoegen om de gegevens te bekijken. Open BasicSamples.cs bestand en klik met de rechtermuisknop op regel 52, selecteer **Breekpunt**en selecteer **Breekpunt invoegen**. Voeg nog een onderbrekingspunt in op regel 55.

   ![Een onderbrekingspunt toevoegen](media/create-table-dotnet/azure-cosmosdb-breakpoint.png) 

5. Druk op F5 om de toepassing uit te voeren. In het consolevenster wordt de naam van de nieuwe tabeldatabase weergegeven (in dit geval demoa13b1) in Azure Cosmos DB. 
...
6. Sluit het tabblad **Entiteiten** in Gegevensverkenner.
