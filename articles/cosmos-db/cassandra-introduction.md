...
Met de Cassandra API u communiceren met gegevens die zijn opgeslagen in Azure Cosmos DB met behulp van de Cassandra Query Language (CQL), Cassandra-gebaseerde hulpprogramma's (zoals cqlsh) en Cassandra-clientstuurprogramma's die u al kent.
...
**Open source standaard**: Ondanks het feit dat het een volledig beheerde service is, ondersteunt Cassandra API nog steeds een groot oppervlak van het oorspronkelijke [Apache Cassandra-draadprotocol,](cassandra-support.md)zodat u toepassingen bouwen op een veelgebruikte en cloudagnostische opensourcestandaard.
...
**Elasticiteit van doorvoer en opslag**: Azure Cosmos DB biedt elasticiteit van gegarandeerde doorvoer naar alle regio’s en kan de geboden doorvoer opschalen met Azure Portal, PowerShell of de CLI. U de opslag en doorvoer voor uw tabellen [elastisch schalen](manage-scale-cassandra.md) als dat nodig is met voorspelbare prestaties.
...
**Event Sourcing**: Cassandra API biedt toegang tot een persistent change log, de [Change Feed](cassandra-change-feed.md), die event sourcing direct vanuit de database kan vergemakkelijken. In Apache Cassandra, het enige equivalent is verandering data capture (CDC), dat is slechts een mechanisme om specifieke tabellen vlag voor archivering en afwijzing van schrijft aan die tabellen zodra een configureerbare grootte-op-schijf voor de CDC log is bereikt (deze mogelijkheden zijn overbodig in Cosmos DB als de relevante aspecten automatisch worden geregeld).
