...
In deze zelf studie volgen we de stappen om aan de slag te gaan met behulp van het [Azure Cosmos DB Graph](https://aka.ms/graph-bulkexecutor-sample) -voor beeld voor bulk gebruik dat wordt gehost op github. Deze toepassing bestaat uit een .NET-oplossing die willekeurig hoekpunt- en randobjecten maakt en ze vervolgens bulksgewijs invoegt voor het opgegeven grafendatabaseaccount. Voer de onderstaande opdracht `git clone` uit om de toepassing op te halen:
