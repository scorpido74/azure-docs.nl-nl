...
In dit artikel wordt beschreven hoe de kosten die nodig zijn om gegevens van Azure Cosmos DB te lezen en te schrijven, worden berekend. Lees bewerkingen zijn onder andere [punt lezen en query's](sql-query-getting-started.md). Schrijf bewerkingen zijn onder andere invoegen, vervangen, verwijderen en upsert van items.  
...
U hoeft niet na te denken over afwijkende aanvraag eenheden tussen lees-en schrijf bewerkingen. Het Unified Currency-model van aanvraag eenheden maakt efficiency verbeteringen voor zowel lees-als schrijf capaciteit. In de volgende tabel ziet u de kosten van punt Lees-en schrijf bewerkingen in termen van RU/s voor items die 1 KB en 100 KB groot zijn.

|**Item grootte**  |**Kosten van één punt lezen** |**Kosten per schrijf bewerking**|
|---------|---------|---------|
|1 kB |1 RU |5 RUs |
|100 kB |Tien aanvraageenheden |50 RUs |

Als er een punt wordt gelezen voor een item met een grootte van 1 KB, wordt één RU. Een item schrijven dat 1 KB kosten vijf RUs heeft. De lees-en schrijf kosten zijn van toepassing wanneer u het standaard [consistentie niveau](consistency-levels.md)van de sessie gebruikt.  Aandachtspunten voor RUs zijn: item grootte, aantal eigenschappen, consistentie van gegevens, geïndexeerde eigenschappen, indexering en query patronen.

De kosten voor het [lezen van punten](sql-query-getting-started.md) zijn aanzienlijk minder dan de query's. Punt Lees bewerkingen, in tegens telling tot query's, hoeven niet te worden gebruikt om de query-engine te gebruiken voor toegang tot gegevens. De query van RU-kosten is afhankelijk van de complexiteit van de query en het aantal items dat de query-engine nodig heeft om te laden.
