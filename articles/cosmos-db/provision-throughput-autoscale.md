...
ms.locfileid: "82196489"
---
# <a name="create-azure-cosmos-containers-and-databases-in-autoscale-mode"></a>Azure Cosmos-containers en-data bases maken in de modus voor automatisch schalen

Met Azure Cosmos DB kunt u de door Voer voor uw containers inrichten in de modus hand matig of automatisch schalen. In dit artikel worden de voor delen en het gebruik van de modus voor automatisch schalen beschreven.
...
Naast het hand matig inrichten van de door Voer kunt u nu Azure Cosmos-containers configureren in de modus voor automatisch schalen. In containers en data bases die zijn geconfigureerd in de modus automatisch schalen, wordt **de ingerichte door Voer automatisch aangepast op basis van de behoeften van uw toepassing, zonder dat dit van invloed is op de beschik baarheid, latentie, door Voer of de prestaties van de werk belasting wereld wijd.**

Bij het configureren van containers en data bases in de modus voor automatisch schalen, moet `Tmax` u de maximale door Voer opgeven die niet moet worden overschreden. Containers kunnen vervolgens hun door Voer zodanig schalen `0.1*Tmax < T < Tmax`dat. Met andere woorden, containers en data bases worden direct geschaald op basis van de behoeften van de werk belasting, van slechts 10% van de maximale doorvoer waarde die u hebt geconfigureerd tot de geconfigureerde waarde voor maximale door voer. Nadat u automatisch schalen hebt geconfigureerd, kunt u de instelling voor`Tmax`de maximale door Voer () op een Data Base of container op elk gewenst moment wijzigen. Met de optie voor automatisch schalen is de minimale door Voer van 400 RU/s per container of Data Base niet meer van toepassing.

Voor de opgegeven maximale door Voer voor de container of de data base, kan het systeem binnen de berekende opslag limiet werken. Als de opslag limiet wordt overschreden, wordt de maximale door Voer automatisch aangepast naar een hogere waarde. Wanneer u door Voer op database niveau gebruikt met de modus voor automatisch schalen, wordt het aantal containers dat in een `0.001*TMax`data base is toegestaan, als volgt berekend:. Als u bijvoorbeeld 20.000 automatisch schalen van RU/s inricht, kan de data base 20 containers bevatten.

## <a name="benefits-of-autoscale-mode"></a><a id="autoscale-benefits"></a>Voor delen van de modus voor automatisch schalen

Azure Cosmos-containers die zijn geconfigureerd in de modus voor automatisch schalen, hebben de volgende voor delen:

* **Eenvoudig:** Containers in de modus automatisch schalen verwijderen de complexiteit om ingerichte door Voer (RUs) en capaciteit hand matig te beheren voor verschillende containers.

* **Schaalbaar:** Containers in de modus voor automatisch schalen kunnen de ingerichte doorvoer capaciteit naadloos zo nodig schalen. Er zijn geen onderbrekingen voor client verbindingen, toepassingen en ze hebben geen invloed op bestaande Sla's.

* **Rendabel:** Wanneer u containers gebruikt die zijn geconfigureerd in de modus voor automatisch schalen, betaalt u alleen voor de resources die de werk belasting per uur nodig heeft.

* **Maxi maal beschikbaar:** Containers in de modus voor automatisch schalen gebruiken dezelfde wereld wijd gedistribueerde, fout tolerante, Maxi maal beschik bare back-end om gegevens duurzaamheid en hoge Beschik baarheid te garanderen.

## <a name="use-cases-of-autoscale-mode"></a><a id="autoscale-usecases"></a>De modus voor automatisch schalen gebruiken

De use cases voor Azure Cosmos-containers die zijn geconfigureerd in de modus voor automatisch schalen, zijn onder andere:
...
* **Ontwikkelings-en test databases:** Als u ontwikkel aars hebt die containers gebruiken tijdens werk uren, maar ze niet op nacht of in het weekend hoeven te hebben. Als containers zijn geconfigureerd in de modus voor automatisch schalen, worden ze omlaag geschaald wanneer ze niet worden gebruikt.

* **Geplande werk belastingen/query's voor productie:** Wanneer u een reeks geplande aanvragen/bewerkingen/query's op één container hebt, en als er niet-actieve Peri Oden zijn waarin u een absolute lage door voer wilt uitvoeren, kunt u dit nu eenvoudig doen. Wanneer een geplande query/aanvraag wordt verzonden naar een container die in de modus voor automatisch schalen is geconfigureerd, wordt deze automatisch omhoog geschaald en wordt de bewerking uitgevoerd.

Oplossingen voor de vorige problemen vereisen niet alleen een enorme hoeveelheid tijd in de implementatie, maar ze veroorzaken ook complexiteit in configuratie of uw code en vereisen vaak hand matige tussen komst om ze te verhelpen. In de modus voor automatisch schalen worden de bovenstaande scenario's uit het vak ingeschakeld, zodat u zich geen zorgen meer hoeft te maken over deze problemen.

## <a name="comparison--containers-configured-in-manual-mode-vs-autoscale-mode"></a>Vergelijking: containers die zijn geconfigureerd in de hand matige modus versus modus voor automatisch schalen

|  | Containers geconfigureerd in hand matige modus  | Containers die zijn geconfigureerd in de modus voor automatisch schalen |
|---------|---------|---------|
...
| **Frequentie waarmee aanvragen/bewerkingen worden beperkt (429)**  | Dit kan gebeuren als het verbruik de ingerichte capaciteit overschrijdt. | Treedt niet op als de verbruikte door Voer is binnen de maximale door Voer die u kiest in de modus voor automatisch schalen.   |
...
## <a name="create-a-database-or-a-container-with-autoscale-mode"></a><a id="create-db-container-autoscale"></a>Een Data Base of container maken met de modus voor automatisch schalen
...
U kunt een gedeelde doorvoer database maken met de modus voor automatisch schalen door de optie **doorvoer database inrichten** in te scha kelen.
...
In de volgende tabel ziet u het maximum aantal in-en opslag limieten voor verschillende opties in de modus voor automatisch schalen:
