...
U kunt de Azure Cosmos-emulator uitvoeren op de Windows docker-container. Raadpleeg de [docker-hub](https://hub.docker.com/r/microsoft/azure-cosmosdb-emulator/) voor de docker-opdracht `Dockerfile` en [github](https://github.com/Azure/azure-cosmos-db-emulator-docker) voor meer informatie.
...
Net als bij de Azure Cosmos DB ondersteunt de Azure Cosmos-emulator alleen beveiligde communicatie via TLS.
...
![TLS/SSL-certificaat van lokale emulator Azure Cosmos DB](./media/local-emulator/database-local-emulator-ssl_certificate.png)
...
Wanneer u verbinding maakt met de emulator vanuit Python en node. js Sdk's, wordt TLS-verificatie uitgeschakeld.
...
|**Optie** | **Beschrijving** | **Cmd**| **Argumenten**|
|---|---|---|---|
...
| Sleutel | Lees de autorisatie sleutel van het opgegeven bestand. De/GenKeyFile-optie gebruiken om een keyfile te genereren | Micro soft. Azure. Cosmos. emulator. exe/KeyFile\<= file_name\> | \<file_name\>: pad naar het bestand |
...
|FailOnSslCertificateNameMismatch | Standaard genereert de emulator het zelfondertekend TLS/SSL-certificaat opnieuw als het SAN van het certificaat de domein naam van de emulator-host, het lokale IPv4-adres, ' localhost ' en ' 127.0.0.1 ' niet bevat. Als deze optie is ingeschakeld, mislukt de emulator bij het opstarten. Vervolgens moet u de/GenCert-optie gebruiken om een nieuw zelfondertekend TLS/SSL-certificaat te maken en te installeren. | Micro soft. Azure. Cosmos. emulator. exe/FailOnSslCertificateNameMismatch  | |
...
| DefaultPartitionCount| Hiermee geeft u het standaard aantal partities voor een gepartitioneerde container op. | Micro soft. Azure. Cosmos. emulator. exe/DefaultPartitionCount\<= DefaultPartitionCount\> | \<de\> standaard waarde voor defaultpartitioncount is 25.|
...
Gebruik nu het eind punt en de hoofd sleutel vanuit het antwoord in uw client en importeer het TLS/SSL-certificaat in uw host. Ga als volgt te werk om het TLS/SSL-certificaat te importeren vanaf een opdracht prompt van de beheerder:
...
De interactieve shell wordt gesloten zodra de emulator is gestart, wordt de container van de emulator afgesloten.
...
In uw toepassing moet u de URI die wordt gebruikt als eind punt wijzigen om het IPv4-adres `ipconfig.exe` te gebruiken `localhost`dat wordt geretourneerd door in plaats van.
...
1. Kopieer het CRT-bestand naar de map die aangepaste certificaten bevat in uw Linux-distributie. Doorgaans op Debian-distributies bevindt zich `/usr/local/share/ca-certificates/`op.

   ```bash
   cp YourCTR.crt /usr/local/share/ca-certificates/
   ```

1. Werk de CA-certificaten bij, zodat de `/etc/ssl/certs/` map wordt bijgewerkt.
...
1. Open de lijst met certificaten en Identificeer er een met de naam `localhost`.

1. Open het snelmenu voor dat specifieke item, selecteer *item ophalen* en onder *vertrouwen* > *Wanneer u deze certificaat optie gebruikt* , selecteer *altijd vertrouwen*. 
...
Nadat u deze stappen hebt uitgevoerd, vertrouwt uw omgeving het certificaat dat door de emulator wordt gebruikt bij het verbinden met `/AllowNetworkAccess`het IP-adres.
...
In deze zelfstudie hebt u geleerd hoe u de lokale emulator kunt gebruiken voor gratis lokale ontwikkelingsdoeleinden. U kunt nu door gaan met de volgende zelf studie en meer informatie over het exporteren van TLS/SSL-certificaten van emulator.
