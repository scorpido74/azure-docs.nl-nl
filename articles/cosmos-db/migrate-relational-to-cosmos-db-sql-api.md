...
Voor de Sink van de SqlJsonToBlobText-Kopieer activiteit kiezen we uit tekst gescheiden en wijst u deze toe aan een specifieke map in Azure Blob Storage met een dynamisch gegenereerde unieke bestands naam (bijvoorbeeld '@concat(pipeline (). RunId, '. json ').
Omdat het tekst bestand niet echt "gescheiden" is en we niet wilt dat het wordt geparseerd in afzonderlijke kolommen met komma's en u de dubbele aanhalings tekens (") wilt behouden, stellen we ' kolom scheidings teken ' in op een tab (' \t '), of een ander teken dat niet in het gegevens-en aanhalings teken voor komt  naar "geen aanhalings teken".
