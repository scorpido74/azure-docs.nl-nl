...
De door Voer die is ingericht voor een Data Base kan worden gedeeld door de containers in die data base. Voor elke nieuwe container in een gedeelde door Voer op database niveau is 100 RU/s vereist. Wanneer u containers met gedeelde data base-aanbieding inricht:

* Elke 25 containers worden gegroepeerd in een partitieset en de data base-door Voer (D) wordt gedeeld tussen de containers in de partitieset. Als er Maxi maal 25 containers in de data base en op elk moment in de tijd zijn, als u slechts één container gebruikt, kan die container een maximum van ' door Voer ' gebruiken.

* Voor elke nieuwe container die na 25 containers wordt gemaakt, wordt er een nieuwe partitieset gemaakt en wordt de data base door Voer gesplitst tussen de nieuwe partitie sets die zijn gemaakt (D/2 voor twee partitie sets, D/3 voor drie partitie sets...). Als u slechts één container uit de data base gebruikt, kan er op elk moment een maximum van (D/2, D/3, D/4 worden gebruikt... door Voer) respectievelijk. Gezien de gereduceerde door Voer, is het raadzaam dat u niet meer dan 25 containers in één data base maakt.

**Voorbeeld**

* Als u een Data Base met de naam ' MyDB ' maakt met een ingerichte door Voer van 10.000 RU/s.

* Als u 25 containers hebt ingericht onder ' MyDB ', worden alle containers gegroepeerd in een partitieset. Als u slechts één container uit de data base gebruikt, kan er op elk moment een maximum van 10.000 RU/s (D) worden gebruikt.

* Wanneer u een 26-container inricht, wordt er een nieuwe partitieset gemaakt en wordt de door Voer gelijk verdeeld tussen beide partitie sets. Als u slechts één container uit de data base gebruikt, kan deze op elk gewenst moment een maximum van 5K RU/s (D/2) gebruiken. Omdat er twee partitie sets zijn, wordt de doorvoer factor voor delen in D/2 gesplitst.

   In de volgende afbeelding ziet u het vorige voor beeld grafisch:

   ![Deelbaarheid factor in door Voer op database niveau](./media/set-throughput/database-level-throughput-shareability-factor.png)
...
U kunt de twee modellen combi neren. De door Voer voor de data base en de container inrichten is toegestaan. In het volgende voor beeld ziet u hoe u de door Voer inricht in een Azure Cosmos-data base en een container:
...
|---------|---------|---------|
|Minimum RUs |400 (na de eerste vier containers is voor elke extra container mini maal 100 RUs per seconde vereist.) |400|
|Mini maal RUs per container|100|400|
|Maximum RUs|Onbeperkt, op de data base.|Onbeperkt, op de container.|
...
|Maximale door Voer per logische partitie van een container|10K RUs|10K RUs|
|Maximale opslag (data + index) per logische partitie van een container|10 GB|10 GB|
