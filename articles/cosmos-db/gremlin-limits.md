...
*Diepte van operator* | **400** |  Totaal aantal unieke stappen in een doorkruising. ```g.V().out()``` Heeft bijvoorbeeld een operator aantal van 2: v () en out (), ```g.V('label').repeat(out()).times(100)``` heeft de operator diepte van 3: v (), Repeat () en out () omdat ```.times(100)``` dit een para meter voor ```.repeat()``` de operator is.
