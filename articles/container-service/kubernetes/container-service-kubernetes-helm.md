...
Als u typt `kubectl get svc` om alle services weer te geven die op het cluster worden uitgevoerd, ziet u dat er een IP-adres is toegewezen aan de ingangs controller. (Terwijl de toewijzing wordt uitgevoerd, ziet u `<pending>` . Het duurt enkele minuten om de bewerking uit te voeren.) 
...
> [!TIP]
> Als de implementatie mislukt, voert u uit `helm repo update` en probeert u het opnieuw.
>
...
Als u verbinding wilt maken met de client, typt u de volgende opdracht, waarbij u vervangt `v1-mariadb` door de naam van uw implementatie:
