...
U kunt ook [Azure Cloud shell](https://docs.microsoft.com/azure/cloud-shell/overview)gebruiken. Dit is de `az` Azure CLI en de `kubectl` hulpprogram ma's die al voor u zijn geïnstalleerd.
...
Dit is het [yaml-bestand van de daemonset](https://github.com/Microsoft/OMS-docker/tree/master/Kubernetes). Sla het op in een bestand met `oms-daemonset.yaml` de naam en vervang de plaatsings instellingen voor `WSID` en door de `KEY` werk ruimte-id en-sleutel in het bestand.

Nadat u uw werk ruimte-ID en-sleutel aan de configuratie van de Daemonset hebt toegevoegd, kunt u de Log Analytics agent op uw cluster installeren met het `kubectl` opdracht regel programma:
