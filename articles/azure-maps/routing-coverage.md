---
title: Routerings dekking | Microsoft Azure kaarten
description: Meer informatie over het dekkings niveau dat Azure Maps biedt in verschillende regio's voor route ring, route ring met verkeer en vracht routering.
author: anastasia-ms
ms.author: v-stharr
ms.date: 04/04/2019
ms.topic: conceptual
ms.service: azure-maps
services: azure-maps
manager: ''
ms.openlocfilehash: 05c3cfc390fed905f4a1b09c221095b56867d540
ms.sourcegitcommit: 4064234b1b4be79c411ef677569f29ae73e78731
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 10/28/2020
ms.locfileid: "92896171"
---
# <a name="azure-maps-routing-coverage"></a>Routerings dekking Azure Maps

In dit artikel vindt u informatie over de dekking van Azure Maps route ring. Bij een zoek opdracht retourneert Azure Maps een optimale route van locatie A naar locatie B. U krijgt de beschikking over nauw keurige reis tijden, actuele updates van reis gegevens en route-instructies. U kunt ook aanvullende zoek parameters toevoegen, zoals het huidige verkeer, het type Voer tuig en voor waarden om te voor komen. De optimalisatie van de route is afhankelijk van de regio. Dat komt omdat Azure Maps verschillende niveaus van informatie en nauw keurigheid voor verschillende regio's heeft. De volgende tabel geeft een lijst van de regio's en wat voor soort informatie u kunt aanvragen.
 
Bekijk de dekking voor [**geocodering**](geocoding-coverage.md).       
Bekijk de dekking van [**verkeer**](traffic-coverage.md).  
Bekijk de dekking voor [**weer gave**](render-coverage.md).
<br>  
**Legenda**

| Symbool             | Betekenis                                |
|--------------------|----------------------------------------|
| ✓                  | Het land/de regio is voorzien van gedetailleerde gegevens.   |
| Ø                  | Het land of de regio die is meegeleverd met vereenvoudigde gegevens. |
| Land ontbreekt | Er zijn geen land-of regio gegevens gegeven.          |

<br>  

De volgende tabel bevat informatie over de dekking van Azure Maps route ring.
<br>  


## <a name="africa"></a>Afrika

| Land/regio                 | Routering         | Route ring met verkeer | Vracht routering |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Algerije                        |        ✓       |                       |             |
| Angola                         |        ✓       |                       |             |
| Benin                          |        ✓       |                       |             |
| Botswana                       |        ✓       |                       |             |
| Burkina Faso                   |        ✓       |                       |             |
| Burundi                        |        ✓       |                       |             |
| Kameroen                       |        ✓       |                       |             |
| Cabo Verde                     |        ✓       |                       |             |
| Congo-Brazzaville Kinshasa    |        ✓       |                       |             |
| Congo-Kinshasa                |        ✓       |                       |             |
| Ivoor kust                  |        ✓       |                       |             |
| Egypte                          |        ✓       |         ✓            |             |
| Gabon                          |        ✓       |                       |             |
| Gambia                         |        ✓       |                       |             |
| Ghana                          |        ✓       |                       |             |
| Kenia                          |        ✓       |         ✓            |             |
| Lesotho                        |        Ø        |         ✓            |             |
| Malawi                         |        ✓       |                       |              |
| Mali                           |        ✓       |                       |             |
| Mauritanië                     |        ✓       |                       |             |
| Mauritius, Mayotte en Réunion |        ✓      |                       |             |
| Marokko                        |        ✓       |                       |             |
| Mozambique                     |        ✓       |         ✓            |             |
| Namibië                        |        ✓       |                       |             |
| Niger                          |        ✓       |                       |             |
| Nigeria                        |        ✓       |         ✓            |             |
| Rwanda                         |        ✓       |                       |             |
| Senegal                        |        ✓       |                       |             |
| Seychellen                     |        ✓       |                       |             |
| Zuid-Afrika                   |        ✓       ||         ✓            |             |
| Swaziland                      |        ✓       |                       |             |
| Tanzania                       |        ✓       |                       |             |
| Togo                           |        ✓       |                       |             |
| Tunesië                        |        ✓       |                       |             |
| Oeganda                         |        ✓       |                       |             |
| Zambia                         |        ✓       |                       |             |
| Zimbabwe                       |        ✓       |                       |             |


## <a name="americas"></a>Noord- en Zuid-Amerika

| Land/regio                 | Routering         | Route ring met verkeer | Vracht routering |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Argentinië                      |        ✓       |         ✓            |     ✓       |
| Antigua en Barbuda            |        ✓       |                      |             |
| Bahama's                        |        ✓       |                      |             |
| Barbados                       |        ✓       |                      |             |
| Belize                         |        ✓       |                      |             |
| Brazilië                         |        ✓       |         ✓            |     ✓       | 
| Canada                         |        ✓       |         ✓            |     ✓      |
| Chili                          |        ✓       |         ✓            |     ✓      |
| Colombia                       |        ✓       |         ✓            |             |
| Costa Rica                     |        ✓       |                      |             |
| Cuba                           |        ✓       |                      |             |
| Dominica                       |        ✓       |                      |             |
| Dominicaanse Republiek             |        ✓       |                      |             |
| Ecuador                        |        ✓       |                      |             |
| El Salvador                    |        ✓       |                      |             |
| Frans-Guyana                  |        ✓       |                      |             |
| Grenada                        |        ✓       |                      |             |
| Guatemala                      |        ✓       |                      |             |
| Guyana                         |        ✓       |                      |             |
| Haïti                          |        ✓       |                      |             |
| Honduras                       |        ✓       |                      |             |
| Jamaica                        |        ✓       |                      |             |
| Mexico                         |        ✓       |         ✓            |     ✓      | 
| Nicaragua                      |        ✓       |                      |             |
| Panama                         |        ✓       |                      |             |
| Paraguay                       |        ✓       |                      |             |
| Peru                           |        ✓       |         ✓            |             |
| St. Kitts en Nevis            |        ✓       |                      |             |
| Saint Lucia                      |        ✓       |                      |             |
| St. Vincent & Grenadines       |        ✓       |                      |             |
| Suriname                       |        ✓       |                      |             |
| Trinidad & Tobago              |        ✓       |                      |             |
| Verenigde Staten                  |        ✓       |         ✓            |     ✓      | 
| + Amerikaans-Samoa                |        ✓       |                      |             |
| + Noordelijke Marianen      |        ✓       |                      |             |
| + Puerto Rico                   |        ✓       |                      |             |
| + Amerikaanse Maagden eilanden           |        ✓       |                      |             |
| Uruguay                        |        ✓       |         ✓            |     ✓      | 
| Venezuela                      |        ✓       |                      |             |


## <a name="asia-pacific"></a>Azië en Stille Oceaan

| Land/regio                 | Routering         | Route ring met verkeer | Vracht routering |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Australië                      |        ✓       |         ✓            |     ✓       |
| + Christmas eiland              |        ✓       |                      |             |
| Brunei                         |        ✓       |         ✓            |             |
| Cambodja                       |        ✓       |                      |             |
| Fiji                           |        ✓       |                      |             |
| Guam                           |        ✓       |                      |             |
| Hongkong SAR                  |        ✓       |         ✓            |             |
| India                          |        ✓       |         ✓            |             |
| Indonesië                      |        ✓       |         ✓            |             |
| Kazachstan                |        ✓       |                     |             |
| Korea                          |        Ø       |                      |             |
| Laos                           |        ✓       |                      |             |
| Macau SAR                      |        ✓       |         ✓            |             |
| Maleisië                       |        ✓       |         ✓            |             |
| Myanmar                        |        ✓       |                      |             |
| Nieuw-Zeeland                    |        ✓       |         ✓            |     ✓       | 
| Filipijnen                    |        ✓       |         ✓            |             |
| Singapore                      |        ✓       |         ✓            |             |
| Taiwan                         |        ✓       |         ✓            |             |
| Thailand                       |        ✓       |         ✓            |             |
| Vietnam                        |        ✓       |         ✓            |             |


## <a name="europe"></a>Europa

| Land/regio                 | Routering         | Route ring met verkeer | Vracht routering |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Albanië                        |        ✓       |                      |             |
| Andorra                        |        ✓       |         ✓            |             |
| Oostenrijk                        |        ✓       |         ✓            |     ✓       | 
| Belarus                        |        ✓       |                      |             |
| België                        |        ✓       |         ✓            |     ✓      | 
| Bolivia                        |        ✓       |                      |             |
| Bosnia-Herzegovina             |        ✓       |                      |             |
| Bulgarije                       |        ✓       |         ✓            |     ✓      | 
| Kroatië                        |        ✓       |         ✓            |     ✓      |
| Cyprus                         |        ✓       |                      |             |
| Tsjechische Republiek                 |        ✓       |         ✓            |     ✓      |
| Denemarken                        |        ✓       |         ✓            |     ✓      |
| + Faeröer                 |        ✓       |                      |             |
| Estland                        |        ✓       |         ✓            |     ✓      |
| Finland                        |        ✓       |         ✓            |     ✓      |
| Frankrijk                         |        ✓       |         ✓            |     ✓      | 
| + Guadeloupe                    |        ✓       |                      |             |
| + Martinique                    |        ✓       |                      |             |
| + St.-Barthélemy                |        ✓       |                      |             |
| + St. Martin                    |        ✓       |                      |             |
| + St. Pierre & Miquelon         |        ✓       |                      |             |
| Georgië                        |        ✓       |                      |             |
| Duitsland                        |        ✓       |         ✓            |     ✓      | 
| Griekenland                         |        ✓       |         ✓            |     ✓      |  
| Guernsey                       |        ✓       |                      |             |
| Hongarije                        |        ✓       |         ✓            |     ✓      | 
| IJsland                        |        ✓       |         ✓            |             |
| Ierland                        |        ✓       |         ✓            |     ✓      | 
| Italië                          |        ✓       |         ✓            |     ✓      | 
| Jersey                         |        ✓       |                      |             |
| Letland                         |        ✓       |         ✓            |     ✓      | 
| Liechtenstein                  |        ✓       |         ✓            |             |
| Litouwen                      |        ✓       |         ✓            |     ✓      | 
| Luxemburg                     |        ✓       |         ✓            |     ✓      | 
| Noord-Macedonië                |        ✓       |                      |             |
| Malta                          |        ✓       |         ✓            |             |
| Moldavië                        |        ✓       |                      |             |
| Monaco                         |        ✓       |         ✓            |             |
| Montenegro                     |        ✓       |                      |     ✓      | 
| Nederland                    |        ✓       |         ✓            |     ✓      | 
| + Aruba                         |        ✓       |                      |             | 
| + Caribisch gebied Nederland         |        ✓       |                      |             |
| + Curaçao                       |        ✓       |                      |             |
| + Sint-Maarten                  |        ✓       |                      |             |
| Noorwegen                         |        ✓       |         ✓            |     ✓      | 
| Polen                         |        ✓       |         ✓            |     ✓      | 
| Portugal                       |        ✓       |         ✓            |     ✓      |  
| Roemenië                        |        ✓       |         ✓            |     ✓      | 
| Russische Federatie             |        ✓       |         ✓            |     ✓      | 
| San Marino                     |        ✓       |         ✓            |             |
| Servië                         |        ✓       |                      |     ✓      | 
| Slowakije                       |        ✓       |         ✓            |     ✓      | 
| Slovenië                       |        ✓       |         ✓            |     ✓      |
| Spanje                          |        ✓       |         ✓            |     ✓      | 
| Zweden                         |        ✓       |         ✓            |     ✓      | 
| Zwitserland                    |        ✓       |         ✓            |     ✓      | 
| Turkije                         |        ✓       |         ✓            |     ✓      | 
| Oekraïne                        |        ✓       |         ✓            |             | 
| Verenigd Koninkrijk                 |        ✓       |         ✓            |     ✓      | 
| + Anguilla                      |        ✓       |                      |             |
| + Bermuda                       |        ✓       |                      |             |
| + Britse Maagden eilanden        |        ✓       |                      |             |
| + Caymaneilanden                |        ✓       |                      |             |
| + Gibraltar                     |        ✓       |         ✓            |             | 
| + Montserrat                    |        ✓       |                      |             |
| + Turks-en Caicos-eilanden      |        ✓       |                      |             |
| Vaticaanstad                   |        ✓       |         ✓            |             |
| Isle of Man                    |        ✓       |                      |             |


## <a name="middle-east"></a>Midden-Oosten

| Land/regio                 | Routering         | Route ring met verkeer | Vracht routering |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Bahrein                        |        ✓       |         ✓            |             |
| Irak                           |        ✓       |                      |             |
| Israël                         |        ✓       |         ✓            |             |
| Jordanië                         |        ✓       |                      |             |
| Koeweit                         |        ✓       |         ✓            |             |
| Libanon                        |        ✓       |                      |             |
| Oman                           |        ✓       |         ✓            |             |
| Qatar                          |        ✓       |         ✓            |             |
| Saoedi-Arabië                   |        ✓       |         ✓            |             |
| Verenigde Arabische Emiraten           |        ✓       |         ✓            |             |
| Jemen                          |        ✓       |                      |             |


## <a name="next-steps"></a>Volgende stappen

Zie de pagina [route ring](/rest/api/maps/route) -verwijzing voor meer informatie over Azure Maps route ring.