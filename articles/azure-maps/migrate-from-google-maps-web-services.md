...
| Uitlijnen op weg            | Zie [routes en instructies berekenen](#calculate-routes-and-directions) .            |
| Snelheids limieten            | Zie [Reverse Geocode a coördinaten](#reverse-geocode-a-coordinate) Section (Engelstalig).                  |
| Statische toewijzing              | [Renderen](https://docs.microsoft.com/rest/api/maps/render/getmapimage)                       |
...
- Kaart-Url's
- Dichtstbijzijnde wegen: dit kan worden behaald met de Web-SDK, zoals [hier](https://azuremapscodesamples.azurewebsites.net/index.html?sample=Basic%20snap%20to%20road%20logic
)wordt weer gegeven, maar is momenteel niet beschikbaar als een service.
...
| `components`                | `streetNumber`<br/>`streetName`<br/>`crossStreet`<br/>`postalCode`<br/>`municipality`-plaats/stad<br/>`municipalitySubdivision`– omgeving, sub/Super City<br/>`countrySubdivision`-staat of provincie<br/>`countrySecondarySubdivision`-graafschap<br/>`countryTertiarySubdivision`-district<br/>`countryCode`-land code van twee letters |
...
> [!TIP]
> De vrije vorm-adres geocodering en fuzzy Search-Api's kunnen worden gebruikt in de modus automatisch volt `&typeahead=true` ooien door toe te voegen aan de aanvraag-URL. Hiermee wordt de server op de hoogte gebracht van de invoer tekst waarschijnlijk gedeeltelijk en wordt de zoek opdracht in de voorspellende modus uitgevoerd.
...
> [!TIP]
> De POI Search, POI categorie Search en fuzzy Search-Api's kunnen worden gebruikt in de modus voor automatisch aanvullen door toe te voegen `&typeahead=true` aan de aanvraag-URL. Hiermee wordt de server ervan op de hoogte dat de invoer tekst waarschijnlijk gedeeltelijk is. De API voert de zoek opdracht uit in voorspellende modus.
...
| `radius`                    | `radius`                            |
| `rankby`                    | *N.v.t.*                               |
| `type`                      | `categorySet –`Zie de documentatie voor [ondersteunde Zoek categorieën](supported-search-categories.md) .   |
...
> [!TIP]
> De Azure Maps route-API retourneert standaard alleen een samen vatting. Hiermee worden de afstand en tijden en de coördinaten voor het traject geretourneerd. Gebruik de `instructionsType` para meter om instructies voor inschakeling op te halen. En gebruik de `routeRepresentation` para meter om de samen vatting en het traject uit te filteren.
...
Bekijk de [Aanbevolen procedures voor de route ring van](how-to-use-best-practices-for-routing.md) documentatie.
...
| `markers`                   | `pins`                             |
| `path`                      | `path`                             |
| `region`                    | *N.v.t.* : dit is een functie die betrekking heeft op Geocode ring. Gebruik de `countrySet` para meter bij het gebruik van de Azure Maps geocoderings-API.  |
| `scale`                     | *N.v.t.*                              |
| `size`                      | `width`en `height` -kan Maxi maal 8192x8192 groot zijn. |
| `style`                     | *N.v.t.*                              |
| `visible`                   | *N.v.t.*                              |
| `zoom`                      | `zoom`                             |
...
Voeg markeringen toe met behulp van de `markers` para meter in de URL. De `markers` para meter bevat een stijl en een lijst met locaties die moeten worden weer gegeven op de kaart met die stijl zoals hieronder wordt weer gegeven:

```
&markers=markerStyles|markerLocation1|markerLocation2|...
```

Als u extra stijlen wilt toevoegen, `markers` gebruikt u de para meters voor de URL met een andere stijl en set locaties.

Geef de markerings locaties met de notatie ' breedte graad, lengte graad ' op.

Voeg markerings stijlen toe `optionName:value` met de indeling, met meerdere stijlen, gescheiden\|door sluis tekens () zoals deze "\|optionName1: waarde1 optionName2: waarde2". Houd er rekening mee dat de optie namen en waarden worden gescheiden door een dubbele punt (:). Gebruik de volgende namen van stijl opties voor stijl markeringen in Google Maps:
...
- `size`-De grootte van de markering. Kan `tiny`, `mid`of `small`.

Gebruik de volgende stijl opties namen voor aangepaste pictogrammen in Google Maps:

- `anchor`: Hiermee geeft u op hoe de pictogram afbeelding moet worden uitgelijnd op de coördinaat. Dit kan een pixel waarde (x, y) of een van de volgende waarden zijn. `top`, `bottom`, `left`, `right`, `center` `bottomleft` `bottomright`,, `topright`, of. `topleft`
...
Voeg markeringen toe aan een statische kaart installatie kopie door de `pins` para meter op te geven in de URL. Net als bij Google Maps geeft u een stijl en een lijst met locaties op in de para meter. De `pins` para meter kan meerdere keren worden opgegeven om markeringen met verschillende stijlen te ondersteunen.

```
&pins=iconType|pinStyles||pinLocation1|pinLocation2|...
```

Als u aanvullende stijlen wilt gebruiken, `pins` voegt u extra para meters toe aan de URL met een andere stijl en set locaties.
...
`iconType` Hiermee geeft u het type pincode op dat moet worden gemaakt. Deze kan de volgende waarden hebben:

- `default`: Het standaard speld pictogram.
- `none`-Er wordt geen pictogram weer gegeven, alleen labels worden gerenderd.
- `custom`: Hiermee geeft u een aangepast pictogram moet worden gebruikt. Een URL die verwijst naar de pictogram afbeelding kan worden toegevoegd aan het einde van `pins` de para meter na de locatie gegevens van de pincode.
- `{udid}`– Een unieke gegevens-ID (UDID) voor een pictogram dat is opgeslagen in het platform voor Azure Maps gegevens opslag.

Voeg PIN-stijlen toe `optionNameValue` met de indeling. Scheid meerdere stijlen met de sluis tekens\|(). Bijvoorbeeld: `iconType|optionName1Value1|optionName2Value2`. De optie namen en waarden worden niet gescheiden. Gebruik de volgende stijl namen voor stijl Markeringen:
...
- `co`: De kleur van de pincode. Geef een 24-bits hex-kleur `000000` op `FFFFFF`: tot.
- `la`: Hiermee geeft u het anker van het label. Geef X-en y-pixel waarden op in de notatie x y.
- `lc`: De kleur van het label. Geef een 24-bits hex-kleur `000000` op `FFFFFF`: tot.
...
![Meerdere markeringen Azure Maps](media/migrate-google-maps-web-services/azure-maps-multiple-markers.png)</center>

### <a name="path-url-parameter-format-comparison"></a>Vergelijking van de pad-URL-para meter

**Voor: Google Maps**

Voeg regels en veelhoek toe aan een statische kaart afbeelding met `path` behulp van de para meter in de URL. De `path` para meter heeft een stijl en een lijst met locaties die op de kaart moeten worden weer gegeven, zoals hieronder wordt weer gegeven:
...
Er zijn pad locaties opgegeven met `latitude1,longitude1|latitude2,longitude2|…` de indeling. Paden kunnen worden gecodeerd of adressen bevatten voor punten.

Voeg stijlen voor paden toe `optionName:value` met de indeling en Scheid meerdere stijlen van de\|sluis tekens (). En u kunt optie namen en-waarden scheiden met een dubbele punt (:). Zo: `optionName1:value1|optionName2:value2`. De volgende stijl optie namen kunnen worden gebruikt voor het opmaken van paden in Google Maps:

- `color`: De kleur van het pad of het overzicht van de veelhoek. Dit kan een 24-bits hex-kleur`0xrrggbb`(), een 32-bits hex-`0xrrggbbbaa`kleur () of een van de volgende waarden zijn: zwart, bruin, groen, paars, geel, blauw, grijs, oranje, rood, wit.
- `fillColor`: De kleur voor het vullen van het pad naar de locatie (veelhoek). Dit kan een 24-bits hex-kleur`0xrrggbb`(), een 32-bits hex-`0xrrggbbbaa`kleur () of een van de volgende waarden zijn: zwart, bruin, groen, paars, geel, blauw, grijs, oranje, rood, wit.
...
Voeg regels en veelhoeken toe aan een statische kaart installatie kopie door `path` de para meter op te geven in de URL. Specificeer net als Google Maps een stijl en een lijst met locaties in deze para meter. Geef de `path` para meter meerdere keren op om meerdere cirkels, lijnen en veelhoeken met verschillende stijlen weer te geven.
...
Voeg stijlen voor paden toe `optionNameValue` met de indeling. Scheid meerdere stijlen op sluis tekens\|() als volgt `optionName1Value1|optionName2Value2`. De optie namen en waarden worden niet gescheiden. Gebruik de volgende stijl opties voor stijl paden in Azure Maps:
...
- `ra`: Hiermee geeft u de RADIUS van een cirkel in meters.
...
| `region`                       | *N.v.t.* : deze functie is geocodering. Gebruik de `countrySet` para meter bij het gebruik van de Azure Maps geocoderings-API. |
| `traffic_model`                | *N.v.t.* : kan alleen opgeven of verkeers gegevens moeten worden gebruikt met de `traffic` para meter. |
...
- [Aanbevolen procedures voor route ring](how-to-use-best-practices-for-routing.md)
