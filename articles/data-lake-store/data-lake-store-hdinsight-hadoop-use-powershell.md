...
Voor ondersteunde cluster typen kan Data Lake Storage Gen1 worden gebruikt als een standaard opslag of extra opslag account. Als Data Lake Storage Gen1 als extra opslag wordt gebruikt, is het standaard opslag account voor de clusters nog steeds Azure Storage blobs (WASB) en de aan het cluster gerelateerde bestanden (zoals Logboeken, enzovoort) worden nog steeds naar de standaard opslag geschreven, terwijl de gegevens die u wilt het proces kan worden opgeslagen in een Data Lake Storage Gen1-account. Het gebruik van Data Lake Storage Gen1 als een extra opslag account heeft geen invloed op de prestaties of de mogelijkheid om te lezen/schrijven naar de opslag van het cluster.
...
1. Open op het bureau blad een nieuw Azure PowerShell venster en voer het volgende code fragment in. Wanneer u wordt gevraagd om u aan te melden, moet u zich aanmelden als een van de abonnements beheerder/eigenaar:
   ...
   > [!NOTE]
   > Als er een fout bericht wordt weer gegeven die vergelijkbaar is met `Register-AzResourceProvider : InvalidResourceNamespace: The resource namespace 'Microsoft.DataLakeStore' is invalid` bij het registreren van de Data Lake Storage Gen1 resource provider, is het mogelijk dat uw abonnement niet white list is voor Data Lake Storage Gen1. Zorg ervoor dat u uw Azure-abonnement voor Data Lake Storage Gen1 inschakelt door deze [instructies](data-lake-store-get-started-portal.md)te volgen.
   >
   >
...
1. Ga in het Power shell-venster naar de locatie waar u Windows SDK hebt geïnstalleerd (meestal `C:\Program Files (x86)\Windows Kits\10\bin\x86` en gebruik het hulp programma [makecert][makecert] om een zelfondertekend certificaat en een persoonlijke sleutel te maken. Gebruik de volgende opdrachten.
...
U kunt ook de `hdfs dfs -put` opdracht gebruiken om bestanden naar Data Lake Storage Gen1 te uploaden en vervolgens `hdfs dfs -ls` te gebruiken om te controleren of de bestanden zijn geüpload.
