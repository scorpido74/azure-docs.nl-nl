---
title: Aan de slag met Azure Blob-opslag met behulp van Visual Studio (ASP.NET)
...
1. **Selecteer in**het context menu de optie  > -**weer gave**.
...
1. Als u de blobs in een BLOB-container wilt weer geven, gebruikt u de methode `CloudBlobContainer.ListBlobs`. De methode `CloudBlobContainer.ListBlobs` retourneert een `IListBlobItem`-object dat kan worden geconverteerd naar een object van het `CloudBlockBlob`, `CloudPageBlob` of `CloudBlobDirectory`. In het volgende code fragment worden alle blobs in een BLOB-container opgesomd. Elke BLOB wordt op basis van het type naar het juiste object gecast. De naam (of URI in het geval van een **CloudBlobDirectory**) wordt toegevoegd aan een lijst.
...
1. Als u een BLOB wilt downloaden, gebruikt u de methode `CloudBlockBlob.DownloadToStream`. Met de volgende code wordt de inhoud van een BLOB overgedragen naar een Stream-object. Dit object wordt vervolgens persistent gemaakt naar een lokaal bestand. (Wijzig *&lt;local-bestands naam >* naar de volledig gekwalificeerde bestands naam waarmee de BLOB moet worden gedownload.) 
    ...
    Hieronder ziet u de voltooide methode `DownloadBlob` (met een volledig gekwalificeerde pad voor het lokale bestand dat wordt gemaakt):
