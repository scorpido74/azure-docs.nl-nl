...
* Data base &lt;db_name @ no__t-1 op server &lt;Azure_instance @ no__t-3 is momenteel niet beschikbaar. Probeer de verbinding later opnieuw. Als het probleem zich blijft voordoen, neemt u contact op met de klant ondersteuning en geeft u de sessie tracering-ID van &lt;session_id @ no__t-1
* Data base &lt;db_name @ no__t-1 op server &lt;Azure_instance @ no__t-3 is momenteel niet beschikbaar. Probeer de verbinding later opnieuw. Als het probleem zich blijft voordoen, neemt u contact op met de klant ondersteuning en geeft u de sessie tracering-ID van &lt;session_id @ no__t-1. (Microsoft SQL Server, Error: 40613)
...
| 40501 |20 |De service is momenteel bezet. Voer de aanvraag na 10 seconden opnieuw uit. Incident-ID:% ls. Code:% d. Zie voor meer informatie: <br/>&bull; &nbsp;[resource limieten voor database server](sql-database-resource-limits-database-server.md)<br/>&bull; &nbsp;[op DTU gebaseerde limieten voor afzonderlijke data bases](sql-database-service-tiers-dtu.md)<br/>&bull; &nbsp;[op DTU gebaseerde limieten voor elastische Pools](sql-database-dtu-resource-limits-elastic-pools.md)<br/>&bull; &nbsp;[limieten op basis van vCore voor afzonderlijke data bases](sql-database-vcore-resource-limits-single-databases.md)<br/>&bull; &nbsp;[limieten op basis van vCore voor elastische Pools](sql-database-vcore-resource-limits-elastic-pools.md)<br/>&bull; &nbsp;[limieten voor het beheer van beheerde exemplaren](sql-database-managed-instance-resource-limits.md).|
...
| [18470](https://docs.microsoft.com/sql/relational-databases/errors-events/database-engine-events-and-errors#errors-18000-to-18999) |14 |Aanmelding mislukt voor gebruiker '%.&#x2a;ls'. Gemotiveerd Het account is uitgeschakeld.%. &#x2a;ls |
...
| 40553 |16 |De sessie is beëindigd vanwege het overmatige geheugen gebruik. Wijzig uw query om minder rijen te verwerken.<br/><br/> Als u het aantal `ORDER BY`-en `GROUP BY`-bewerkingen in uw Transact-SQL-code vermindert, vermindert u de geheugen vereisten van uw query. |
...
| 40615 |16 |Kan server ' {0} ' die door de aanmelding is aangevraagd, niet openen. De client met het IP-adres {1} is niet gemachtigd om toegang te krijgen tot de server.<br /><br />Als u toegang wilt inschakelen, gebruikt u de SQL Database portal of voert u SP @ no__t-0set @ no__t-1firewall @ no__t-2rule uit in de hoofd database om een firewall regel te maken voor dit IP-adres of adres bereik. Het kan tot vijf minuten duren voordat deze wijziging van kracht wordt. |
...
| 40627 |20 |Bewerking op server ' {0} ' en data base ' {1} ' wordt uitgevoerd. Wacht een paar minuten voordat u het opnieuw probeert. |
...
| 40852 |16 |Kan de data base%. \*LS niet openen op de server%. \*LS die door de aanmelding is aangevraagd. Toegang tot de data base is alleen toegestaan met behulp van een connection string met ingeschakelde beveiliging. Als u toegang wilt krijgen tot deze data base, wijzigt u de verbindings reeksen zodat deze ' Secure ' bevat in de server-FQDN-' server naam '. data base. Windows. net moet worden gewijzigd in ' server naam '. data base. `secure`.windows.net. |
