...
Zodra **Replication State** is `2`, maakt u verbinding met elke Data Base of subset van data bases `<fog-name>.secondary.database.windows.net` met behulp van het secundaire eind punt en voert u een query uit op de data bases om connectiviteit, juiste beveiligings configuratie en gegevens replicatie te garanderen. 
