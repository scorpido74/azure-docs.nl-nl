...
In een niet-grootschalige-Data Base worden de gecombineerde Lees-en schrijf-IOPS voor gegevens bestanden, ten opzichte van de limiet van de [bron](/azure/sql-database/sql-database-resource-limits-database-server#resource-governance) beheer gegevens IOPS, gerapporteerd in de weer gaven [sys. dm_db_resource_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database) en [sys. resource_stats](/sql/relational-databases/system-catalog-views/sys-resource-stats-azure-sql-database) in de kolom `avg_data_io_percent`. Dezelfde waarde wordt als een gegevens-IO- _percentage_gerapporteerd in de portal. 
