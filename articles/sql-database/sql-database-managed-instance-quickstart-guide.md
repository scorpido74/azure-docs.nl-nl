...
- Een beheerd exemplaar wordt gemaakt in een eigen VNet zonder openbaar eindpunt. Voor toegang tot client toepassingen kunt u **een virtuele machine maken in hetzelfde VNet (ander subnet)** of **een punt-naar-site-VPN-verbinding met het vnet maken vanaf uw client computer** met behulp van een van de volgende Quick starts:
...
> [!TIP]
> Zie `BACKUP` [SQL Server back-up naar URL](https://docs.microsoft.com/sql/relational-databases/backup-restore/sql-server-backup-to-url)als u de Transact-SQL-opdracht wilt gebruiken om een back-up te maken van uw data base in Azure Blob-opslag.
