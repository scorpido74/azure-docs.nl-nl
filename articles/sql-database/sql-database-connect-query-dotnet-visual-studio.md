---
title: Gebruik Visual Studio met .NET en C# voor het uitvoeren van query's
...
In deze Quick start ziet u hoe u de [.NET Framework](https://www.microsoft.com/net/) -en C#-code in Visual Studio kunt gebruiken om een query uit te voeren op een Azure SQL database met Transact-SQL-instructies.
...
1. Vervang in de code-editor de inhoud **Program.cs** door de volgende code. Vervang uw waarden voor `<server>`, `<username>`, `<password>`en `<database>`.
...
1. Als u de app wilt uitvoeren, selecteert u **fout** > **opsporing starten**of selecteert u **Start** op de werk balk of drukt u op **F5**.
