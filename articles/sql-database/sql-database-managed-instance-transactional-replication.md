...
| &nbsp; | &nbsp; | &nbsp; |
  ...
  >[!NOTE]
  > - Het configureren van een replicatie met een oudere versie kan leiden tot een fout nummer MSSQL_REPL20084 (het proces kan geen verbinding maken met de abonnee.) en MSSQ_REPL40532 \<(kan de server naam niet openen> aangevraagd door de aanmelding. De aanmelding is mislukt.)
...
- Poort 1433 (TCP uitgaand) moet worden geopend als de uitgever/distributeur zich op een beheerd exemplaar bevindt en de abonnee niet. Mogelijk moet u ook de uitgaande beveiligings regel voor het beheerde exemplaar NSG voor `allow_linkedserver_outbound` de poort 1433- **doel servicetag** wijzigen van `virtualnetwork` naar `internet`. 
...
In deze configuratie publiceert één beheerd exemplaar wijzigingen in de Distributor die zijn geplaatst op een ander beheerd exemplaar. Dit kan een groot aantal beheerde instanties van de bron zijn en wijzigingen distribueren naar een of meer doelen voor een beheerd exemplaar, één data base, gepoolde data base of SQL Server.
