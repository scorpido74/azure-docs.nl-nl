...
1. Maak een Azure SQL Database Server in de doel regio als u daar nog geen geschikte server op hebt.  Deze server moet eigendom zijn van hetzelfde abonnement als de oorspronkelijke server (bron).
...
| :---- | :--------- |
| In het deel venster Back-ups beheren voor een logische server worden geen grootschalige-data bases weer gegeven. deze worden in de weer gave gefilterd  | Grootschalige heeft een afzonderlijke methode voor het beheren van back-ups, en omdat de Bewaar periode voor lange termijn retentie en het tijdstip voor het bewaren van back-ups niet van toepassing zijn/zijn ongeldig. Daarom worden grootschalige-data bases niet weer gegeven in het deel venster back-up beheren. |
