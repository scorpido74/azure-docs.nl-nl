...
- De naam van het beheerde exemplaar van de uitgever moet `sql-mi-publisher` zijn: (samen met een paar tekens voor wille keurige) en de naam van het `vnet-sql-mi-publisher`virtuele netwerk.
- De naam van het beheerde exemplaar van de distributie server `sql-mi-distributor` moet zijn: (samen met een paar tekens voor wille keurige) en moet zich _in hetzelfde virtuele netwerk bevindt als het beheerde exemplaar van de uitgever_.
...
1. Zoek `private dns zone` op Azure Marketplace. 
...
1. Geef een wille keurige naam op voor uw `repldns.com`DNS-zone, zoals. 
...
1. Herhaal deze stappen om een koppeling toe te voegen voor het virtuele netwerk van de abonnee, `Sub-link`met een naam zoals. 
...
Kopieer de toegangs sleutel voor opslag connection string in de volgende indeling:`DefaultEndpointsProtocol=https;AccountName=<Storage-Account-Name>;AccountKey=****;EndpointSuffix=core.windows.net`   
...
Maak een nieuwe Data Base op de Publisher MI. Hiervoor volgt u de volgende stappen:

1. Start SQL Server Management Studio (SSMS) op uw SQL Server. 
1. Maak verbinding met `sql-mi-publisher` het beheerde exemplaar. 
...
Zodra de verbinding tot stand is gebracht en u een voorbeeld database hebt, kunt u distributie `sql-mi-distributor` configureren voor uw beheerde exemplaar. Hiervoor volgt u de volgende stappen:

1. Start SQL Server Management Studio (SSMS) op uw SQL Server. 
1. Maak verbinding met `sql-mi-distributor` het beheerde exemplaar. 
1. Open een **Nieuw query** venster en voer de volgende Transact-SQL-code uit om distributie te configureren op het beheerde exemplaar van de distributie server: 
   ...
   > [!NOTE]
   > Zorg ervoor dat u alleen backslashes (`\`) voor de @working_directory para meter gebruikt. Het gebruik van een slash`/`() kan een fout veroorzaken wanneer er verbinding wordt gemaakt met de bestands share. 
...
Zodra de distributie is geconfigureerd, kunt u de publicatie nu maken. Hiervoor volgt u de volgende stappen: 
...
1. Op de pagina **publicatiedata base** selecteert u de `ReplTutorial` data base die u eerder hebt gemaakt. Selecteer **Next**. 
...
1. Geef op de pagina **de wizard volt ooien** uw publicatie `ReplTest` een naam en selecteer **volgende** om uw publicatie te maken. 
...
Als de publicatie eenmaal is gemaakt, kunt u het abonnement maken. Hiervoor volgt u de volgende stappen: 
...
2019-11-19 02:21:05.07 verkregen Azure Storage verbindings reeks voor replstorage 2019-11-19 02:21:05.07 verbinding maken met Azure Files Storage\\' replstorage. file. core. Windows. net\replshare ' 2019-11-19 02:21:31.21 kan geen verbinding maken met Azure Storage ' ' met een besturingssysteem fout: 53.
...
Het gebruik van een slash in plaats van een back slash in het bestandspad voor de bestands share kan deze fout veroorzaken. Dit kan een van `\\replstorage.file.core.windows.net\replshare` de volgende oorzaken hebben: een besturings systeem 55 fout:`'\\replstorage.file.core.windows.net/replshare'`
