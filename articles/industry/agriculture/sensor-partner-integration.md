...
Met dit onderdeel kunnen partners Sens oren ontwikkelen die zijn geïntegreerd met FarmBeats om de API te gebruiken en gegevens en telemetrie van klant apparaten te verzenden naar FarmBeats Datahub. De gegevens worden gevisualiseerd met behulp van de FarmBeats-Accelerator. Gegevens kunnen worden gebruikt voor gegevens fusie en voor het bouwen van machine learning/kunst matige Intelligence-modellen.

## <a name="link-a-farmbeats-account"></a>Een FarmBeats-account koppelen

Nadat u apparaten of Sens oren hebt gekocht en geïmplementeerd, hebt u toegang tot de apparaatgegevens en telemetrie op de SaaS-Portal (Software as a Service) van uw apparaat. Met apparaat-partners kunt u uw account koppelen aan uw FarmBeats-exemplaar in Azure. De volgende referenties moeten worden ingevuld door u of uw systeem integrator:
...
U hebt de mogelijkheid om een bestaande FarmBeats-integratie te ontkoppelen. Het ontkoppelen van FarmBeats mag geen apparaat-of sensor-meta gegevens verwijderen die zijn gemaakt in uw data hub. Het ontkoppelen gaat als volgt:
...
U kunt de FarmBeats-integratie-instellingen bewerken als het client geheim of connection string verandert. In dit geval kunt u alleen de volgende velden bewerken:
...
U kunt de tijds tempel weer geven van de laatste telemetriegegevens die zijn verzonden, die wordt gevonden onder **telemetrie verzonden**. Dit is het tijdstip waarop de laatste telemetrie is verzonden naar FarmBeats.
...
Als u geen apparaatgegevens of telemetrie kunt ontvangen in het opgegeven FarmBeats-exemplaar, moet de partner ondersteuning en een mechanisme voor het oplossen van problemen bieden.
...
Als er een fout optreedt in de meta gegevens van het apparaat of de sensor of de gegevens stroom voor gegevens integratie of telemetrie in het apparaat-partner systeem, ontvangt u een melding. Een mechanisme om eventuele fouten op te lossen moet ook worden ontworpen en geïmplementeerd.
...
Zie [rest API](references-for-farmbeats.md#rest-api)voor meer informatie over de rest API.
