...
| response_mode |Aanbevolen |Hiermee geeft u de methode op die wordt gebruikt om het resulterende token terug naar uw app te verzenden. Gebruik `fragment`voor impliciete stroom. U kunt twee andere modi opgeven `query` en `form_post`, maar niet werken in de impliciete stroom. |
...
ID-tokens en toegangs tokens verlopen beide na een korte periode. Uw app moet worden voor bereid om deze tokens regel matig te vernieuwen.  Als u een van beide typen tokens wilt vernieuwen, voert u dezelfde verborgen iframe-aanvraag uit die we in een `prompt=none` eerder voor beeld hebben gebruikt, met behulp van de para meter voor het beheren van Azure AD-stappen.  Als u een nieuwe `id_token` waarde wilt ontvangen, moet u `response_type=id_token` de `scope=openid` `nonce` para meter en en opgeven.
