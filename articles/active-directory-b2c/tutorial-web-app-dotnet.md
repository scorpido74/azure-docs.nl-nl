---
title: 'Zelf studie: verificatie inschakelen in een webtoepassing'
...
1. Selecteer onder **Web**de koppeling **URI toevoegen** , Voer `https://localhost:44316`in en selecteer vervolgens **Opslaan**.
...
1. Open in het project **TaskWebApp** het bestand **Web.config**.
    1. Werk de waarde van `ida:Tenant` en `ida:AadInstance` door de naam van de Azure AD B2C-Tenant die u hebt gemaakt. Vervang `fabrikamb2c` bijvoorbeeld door `contoso`.
    1. Vervang de waarde van `ida:ClientId` door de toepassings-id die u hebt vastgelegd.
    1. Vervang de waarde voor `ida:ClientSecret` door de sleutel die u hebt geregistreerd. Als het client geheim eventuele vooraf gedefinieerde XML-entiteiten bevat, bijvoorbeeld kleiner dan`<`(), groter dan`>`(), en`&`-teken (), of`"`dubbele aanhalings tekens (), moet u deze letter typen door XML-code ring door het client geheim te coderen voordat u het toevoegt aan uw web. config.
...
1. Gebruik een geldig e-mailadres en voer de verificatie uit met de verificatiecode. Stel een wachtwoord in. Geef waarden voor de aangevraagde kenmerken op.
    ...
    ![De registratie pagina die wordt weer gegeven als onderdeel van de werk stroom voor aanmelden/registreren](./media/tutorial-web-app-dotnet/sign-up-workflow.PNG)
...
De functie **taken lijst** werkt echter pas nadat u de volgende zelf studie in de reeks hebt voltooid, [zelf studie: gebruik Azure AD B2C om een ASP.net-Web-API te beveiligen](tutorial-web-api-dotnet.md).
...
> [Zelf studie: Azure Active Directory B2C gebruiken om een ASP.NET-Web-API te beveiligen >](tutorial-web-api-dotnet.md)
