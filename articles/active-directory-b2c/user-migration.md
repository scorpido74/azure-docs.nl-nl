...
Registreer eerst een toepassing die u kunt gebruiken voor beheer taken zoals gebruikers migratie.
...
1. Maak verbinding met uw online service. U doet dit door de `Connect-AzureAD`-cmdlet uit te voeren op de Windows Power shell-opdracht prompt en uw referenties op te geven.
...
![Onderdeel van UsersData. JSON-bestand met JSON-blokken van twee gebruikers](./media/user-migration/pre-migration-data-file.png)
...
- Als u **gebruikers met een wille keurig wacht woord wilt migreren**, gebruikt u de opdracht `UserMigration.exe 2`. Met deze bewerking wordt ook een Azure Table-entiteit gemaakt. Later configureert u het beleid om de REST API-service aan te roepen. De service gebruikt een Azure-tabel om het migratie proces bij te houden en te beheren.

![Opdracht prompt venster met de uitvoer van de opdracht UserMigration. exe](./media/user-migration/pre-migration-demo.png)
...
- Als u een gebruiker wilt ophalen op basis van het e-mail adres voor aanmelden, gebruikt u de voorbeeld toepassing:
   ...
   1. Open het bestand UserProfile. json in een JSON-editor om de informatie van de gebruiker weer te geven.
      ...
      ![UserProfile. JSON-bestand geopend in Visual Studio code-editor](./media/user-migration/pre-migration-get-by-email2.png)
...
Voer de volgende stappen uit om de koppeling naar het beleid voor het opnieuw instellen van het wacht woord op te halen. Bij deze procedure wordt ervan uitgegaan dat u eerder een [aangepast beleid](custom-policy-get-started.md)voor het opnieuw instellen van een wacht woord hebt gemaakt.
...
1. Kopieer de URL die wordt weer gegeven in het tekstvak **nu uitvoeren eind punt** en verzend deze naar uw gebruikers.
    ...
    ![De pagina beleid voor het opnieuw instellen van wacht woorden met eind punt nu uitvoeren gemarkeerd](./media/user-migration/pre-migration-policy-uri.png)
...
Nadat u het technische profiel voor de REST-API hebt gedefinieerd, moet u het bestaande `SelfAsserted-LocalAccountSignin-Email` technische profiel configureren om het technische profiel van REST API verder aan te roepen door het te overschrijven in uw *TrustFrameworkExtensions. XML-* bestand:
...
Wijzig vervolgens de `Id` van het technische profiel van `LocalAccountSignIn` in `LocalAccountUserMigration`.
...
1. Voer de referenties van een van de gemigreerde gebruikers in en selecteer **Aanmelden**. Het REST API moet het volgende fout bericht genereren:
    ...
    ![Aanmeldings pagina voor aanmelding met het fout bericht voor het wijzigen van het wacht woord](./media/user-migration/pre-migration-error-message.png)
...
1. Selecteer **Opslaan**
    ...
    ![Configuratie pagina diagnose Logboeken in Azure Portal](./media/user-migration/pre-migration-diagnostic-logs.png)
