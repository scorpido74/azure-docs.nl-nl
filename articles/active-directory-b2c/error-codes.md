...
| Foutcode | Bericht |
| ---------- | ------- |
| `AADB2C90002` | De CORS-resource{0}heeft een 404 geretourneerd die niet is gevonden. |
...
| `AADB2C90011` | De client-id{0}die in de aanvraag is opgegeven, komt niet overeen met de client-id{1}geregistreerd in het beleid. |
| `AADB2C90012` | De scope{0}die in de aanvraag is gegeven, wordt niet ondersteund. |
| `AADB2C90013` | Het aangevraagde antwoord type '{0}' dat in de aanvraag is opgegeven, wordt niet ondersteund. |
| `AADB2C90014` | De aangevraagde reactie modus{0}die in de aanvraag is opgegeven, wordt niet ondersteund. |
| `AADB2C90016` | Het aangevraagde client bevestigings type{0}komt niet overeen met het verwachte type{1}. |
...
| `AADB2C90025` | Het profiel{0}in het beleid{1}in de Tenant{2}bevat niet de vereiste cryptografie sleutel{3}. |
| `AADB2C90027` | De basis referenties die zijn opgegeven voor de{0}, zijn ongeldig. Controleer of de referenties juist zijn en of de resource toegang heeft gekregen. |
| `AADB2C90028` | Het client certificaat dat is opgegeven voor de{0}, is ongeldig. Controleer of het certificaat juist is, een persoonlijke sleutel bevat en of toegang is verleend door de resource. |
| `AADB2C90031` | In het beleid '{0}' is geen standaard gebruikers traject opgegeven. Zorg ervoor dat het beleid of de bovenliggende items een standaard gebruikers traject opgeven als onderdeel van een Relying Party sectie. |
...
| `AADB2C90037` | Tijdens het verwerken van de aanvraag is een testfout opgetreden. Neem contact op met de beheerder van de site die u wilt openen. |
| `AADB2C90039` | De aanvraag bevat een client bevestiging, maar in het meegeleverde beleid{0}in de Tenant{1}ontbreekt een client_secret in RelyingPartyPolicy. |
| `AADB2C90040` | Gebruikers traject '{0}' bevat geen stap voor het verzenden van claims. |
...
| `AADB2C90080` | De verleende toekenning is verlopen. Voer opnieuw een verificatie uit en probeer het opnieuw. Huidige tijd: {0}, verleende uitgifte tijd: {1}, verlenen sliding window verloop tijd: {2}. |
...
| `AADB2C90108` | In de Orchestration-stap{0}is geen CpimIssuerTechnicalProfileReferenceId opgegeven wanneer er een wordt verwacht. |
...
| `AADB2C90120` | De maximale leeftijds parameter{0}die in de aanvraag is opgegeven, is ongeldig. De maximale leeftijd moet een geheel getal tussen{1}en{2}omvatten. |
...
| `AADB2C90146` | De scope '{0}' die in de aanvraag is opgegeven, bevat meer dan één resource voor een toegangs token, wat niet wordt ondersteund. |
| `AADB2C90149` | Het laden van het script{0}is mislukt. |
...
| `AADB2C90165` | Het SAML-bericht met de id '{0}' is niet gevonden in de status. |
...
| `AADB2C90188` | Het SAML-technische profiel '{0}' geeft een PartnerEntity-URL van '{1}' op, maar het ophalen van de meta gegevens mislukt met de reden{2}. |
| `AADB2C90194` | De claim{0}die is opgegeven voor de Bearer-token, komt niet voor in de beschik bare claims. Beschik bare claims{1}. |
...
| `AADB2C90242` | Het SAML-technische profiel '{0}' geeft PartnerEntity CDATA op dat niet kan worden geladen voor de reden '{1}'. |
...
| `AADB2C90276` | De aanvraag is niet consistent met de besturings instelling{0}:{1}in de technicalProfile{2}voor het beleid{3}Tenant{4}. |
...
| `AADB2C90279` | De opgegeven client-id{0}komt niet overeen met de client-id die de toekenning heeft verleend. |
...
| `AADB2C90288` | UserJourney met id{0}waarnaar wordt verwezen in TechnicalProfile{1}voor het vernieuwen van het token voor de Tenant{2}, bestaat niet in het beleid{3}of een van de basis beleidsregels. |
...
| `AADB2C99006` | Azure AD B2C kan de extensie-app met de App-ID{0}niet vinden. Ga naar https://go.microsoft.com/fwlink/?linkid=851224 voor meer informatie. |
...
| `AADB2C99015` | Voor het profiel{0}in het beleid{1}in de Tenant{2}ontbreekt alle InputClaims die vereist zijn voor de referentie stroom van het wacht woord voor de resource-eigenaar. |
