...
In een verwant artikel maakt u [een rest-service](rest-api-claims-exchange-dotnet.md) die samenwerkt met Azure Active Directory B2C (Azure AD B2C).
...
>[!NOTE]
>U kunt ook uw REST-service beveiligen met behulp van [http-basis verificatie](secure-rest-api-dotnet-basic-auth.md). HTTP Basic-verificatie wordt echter minder veilig beschouwd via een client certificaat. Onze aanbeveling is de REST-service te beveiligen met behulp van verificatie via client certificaten, zoals beschreven in dit artikel.
...
* Voer de stappen in het artikel [integratie rest API claims uitwissels](rest-api-claims-exchange-dotnet.md) uit.
...
7. Typ in het vak **wacht** woord het wacht woord van het certificaat.
    ...
    ![Beleids sleutel uploaden op de pagina een sleutel maken in Azure Portal](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-cert-upload.png)
...
5. Voeg direct na het afsluitende `<Metadata>` element het volgende XML-fragment toe:
    ...
    ![XML-elementen voor ClientCertificate-verificatie instellen](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-cert-tech-profile.png)
...
3. Test het proces door **test** te typen in het vak **naam** .
    ...
    ![De opgegeven naam is gemarkeerd en de invoer validatie fout wordt weer gegeven](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-test.png)
...
![Certificaat validatie code toevoegen](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-code.png)
...
Als u deze stap moet oplossen, raadpleegt u [Logboeken verzamelen met behulp van Application Insights](troubleshoot-with-application-insights.md).
