...
- Het type van de `DisplayControl` is `VerificationControl`.
- `DisplayClaims`
  ...
  - **Verificatie code** -de claim van de verificatie code die door de gebruiker wordt verstrekt nadat de code is verzonden. Deze claim moet worden ingesteld als vereist en het `ControlClaimType` moet zijn ingesteld op `VerificationCode`.
