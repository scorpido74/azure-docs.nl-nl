---
title: 'Zelfstudie: Uw virtuele hub-netwerk beveiligen met de preview-proefversie van Azure Firewall Manager'
description: In deze zelfstudie leert u hoe u uw virtuele netwerk beveiligt met Azure Firewall Manager met behulp van de Azure-portal.
...
---
# <a name="tutorial-secure-your-hub-virtual-network-using-azure-firewall-manager-preview"></a>Zelfstudie: Uw virtuele hub-netwerk beveiligen met de preview-proefversie van Azure Firewall Manager 
...
Met Azure Firewall Manager Preview u een virtueel hubnetwerk maken om uw hybride netwerkverkeer te beveiligen dat is bestemd voor privé-IP-adressen, Azure PaaS en internet. U Azure Firewall Manager gebruiken om netwerktoegang in een hybride netwerk te beheren met behulp van beleid dat toegestaan en geweigerd netwerkverkeer definieert.

Firewall Manager ondersteunt ook een beveiligde virtuele hub-architectuur. Zie [Wat zijn de azure Firewall Manager-architectuuropties voor](vhubs-and-vnets.md) een vergelijking van de beveiligde virtuele hub- en hubtypen voor virtuele netwerkarchitectuur?
...
- Als u het gesproken subnetverkeer via de hubfirewall wilt routeren, hebt u een UDR (User Defined route) nodig die naar de firewall verwijst en de instelling voor het uitzetten van de **virtuele netwerkgatewayroute** is uitgeschakeld. Met deze optie wordt de routeverdeling naar de spaaksubnetten voorkomen. Dit voorkomt dat geleerde routes in strijd zijn met uw UDR.
...
>
>Azure Firewall kan worden geconfigureerd ter ondersteuning van gedwongen tunneling. Zie [Azure Firewall forced tunneling](../firewall/forced-tunneling.md)voor meer informatie.
...
2. Typ **Firewallbeheer** in de zoekbalk van azure-portal en druk op **Enter**.
3. Selecteer **Azure-firewallbeleid weergeven**op de pagina Azure Firewall.

   ![Firewallbeleid](media/tutorial-hybrid-portal/firewall-manager-policy.png)

1. Selecteer **Azure Firewall-beleid maken**.
1. Selecteer uw abonnement en selecteer voor resourcegroep **Nieuw maken** en maak een brongroep met de naam **FW-Hybrid-Test**.
2. Typ **Pol-Net01**voor de beleidsnaam .
3. Selecteer **Oost-VS**voor Regio .
4. Selecteer **Volgende:Regels**.
5. Selecteer **Een regelverzameling toevoegen**.
6. Typ **RCNet01**voor **naam**.
7. Selecteer **Netwerk**voor **regelverzamelingstype**.
8. Voor **prioriteit**, type **100**.
9. Bij **Actie** selecteert u **Toestaan**.
10. Typ **AllowWeb**onder **Regels**voor **Naam**.
11. Voor **bronadressen**typt u **192.168.1.0/24**.
12. Bij **Protocol** selecteert u **TCP**.
13. Voor **bestemmingspoorten**typt u **80**.
14. Selecteer **IP-adres**voor **doeltype**.
15. Voor **Bestemming**typt u **10.6.0.0/16**.
16. Voer in de volgende regelrij de volgende gegevens in:
 
    Naam: typ **AllowRDP**<br>
    Ip-adres van de bron: type **192.168.1.0/24**.<br>
    Protocol, selecteer **TCP**<br>
    Bestemmingspoorten, type **3389**<br>
    Doeltype, **IP-adres** selecteren<br>
    Typ voor Bestemming **10.6.0.0/16**

1. Selecteer **Toevoegen**.
2. Selecteer **Controleren + maken**.
3. Bekijk de details en selecteer **Maak .**

## <a name="create-the-firewall-hub-virtual-network"></a>Het virtuele hub-netwerk voor de firewall maken

> [!NOTE]
> De grootte van het subnet AzureFirewallSubnet is /26. Zie [Veelgestelde vragen](../firewall/firewall-faq.md#why-does-azure-firewall-need-a-26-subnet-size)over azure firewall voor meer informatie over de subnetgrootte.
...
Zet het **virtuele netwerk van de VNet-Hub** om in een virtueel *hubnetwerk* en beveilig het met Azure Firewall.

1. Typ **Firewallbeheer** in de zoekbalk van azure-portal en druk op **Enter**.
3. Selecteer op de pagina Azure Firewall Manager onder **Beveiliging toevoegen aan virtuele netwerken**de optie Virtuele netwerken **weergeven**.
4. Selecteer **Virtuele netwerken converteren**.
5. Selecteer **VNet-hub** en selecteer **Volgende: Azure Firewall**.
6. Selecteer **Pol-Net01**voor het **firewallbeleid**.
7. Selecteer **Volgende " Controleren + bevestigen**
8. Bekijk de details en selecteer **Bevestigen**.


   Dit duurt een paar minuten om te implementeren.
7. Nadat de implementatie is voltooid, gaat u naar de brongroep **FW-Hybrid-Test** en selecteert u de firewall.
9. Let op het **privé-IP-adres firewall** op de pagina **Overzicht.** U zult het later gebruiken wanneer u de standaardroute maakt.
...
5. Selecteer **voor Regio** **(VS) Oost-VS**.
...
10. Selecteer voor **openbaar IP-adres****Bestaande gebruiken*en selecteer **VNet-Onprem-GW-pip** voor de naam.
11. Accepteer de resterende standaardinstellingen en selecteer **Controleren + maken**.
12. Controleer de configuratie en selecteer **Vervolgens Maken**.
...
8. Selecteer **voor Locatie** **(VS) Oost-VS)**.
9. Selecteer **Maken**.
...
4. Selecteer **VNet-hub**onder **Virtueel netwerk**.
5. Selecteer **GatewaySubnet**onder **Subnet**.
6. Selecteer **OK**.
...
2. Selecteer **Associate**.
4. Selecteer **VNet-spaaks**onder **Virtueel netwerk**.
...
4. Selecteer **Volgende: schijven**.
5. Accepteer de standaardinstellingen en selecteer **Volgende:Netwerken**.
6. Selecteer **VNet-Onprem** voor virtueel netwerk en controleer of het subnet **SN-Corp**is.
7. Selecteer RdP (3389) **voor** **openbare binnenkomende poorten**en selecteer vervolgens **RDP (3389)**
8. Selecteer **Volgende:Beheer**.
9. Selecteer **Uitschakelen** **voor diagnostische gegevens.**
...
1. Let eerst op het privé-IP-adres voor virtuele VM-Spoke-01-machine op de pagina VM-Spoke-01 Overview.
...
3. Open een webbrowser op **VM-OnPrem** en blader naar http://\<privé-IP-adres VM-spoke-01\>.
   ...
   U ziet de **VM-spoke-01** ![webpagina: VM-Spoke-01 webpagina](media/secure-hybrid-network/vm-spoke-01-web.png)
...
1. Open de **brongroep FW-Hybrid-Test** en selecteer het **pol-Net01-firewallbeleid.**
2. Selecteer **Onder Instellingen**de optie **Regels**.
3. Selecteer **onder Netwerkregels**de regelverzameling **RCNet01,** selecteer de ellips (...)en selecteer **Bewerken**.
4. Selecteer **Weigeren**voor **regelverzamelingsactie**.
5. Selecteer **Opslaan**.

Sluit bestaande externe bureaubladen en browsers op **VM-Onprem** voordat u de gewijzigde regels test. Nadat de update voor regelverzameling is voltooid, voert u de tests opnieuw uit. Ze moeten deze keer allemaal mislukken.
...
> [Zelfstudie: Uw virtuele WAN beveiligen met de preview-proefversie van Azure Firewall Manager](secure-cloud-network.md)