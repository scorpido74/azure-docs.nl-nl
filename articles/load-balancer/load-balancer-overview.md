...
>[!NOTE]
> Azure biedt een pakket volledig beheerde oplossingen voor taakverdeling voor uw scenario's. Zie [Wat is Azure-toepassing gateway](../application-gateway/application-gateway-introduction.md)als u op zoek bent naar Transport Layer Security (TLS) Termination (' SSL offload ') of per HTTP/HTTPS-aanvraag, verwerking van de toepassingslaag. Als u op zoek bent naar globale DNS-taak verdeling, raadpleegt u [Wat is Traffic Manager](../traffic-manager/traffic-manager-overview.md). Uw end-to-end-scenario's kunnen van pas komen bij het combi neren van deze oplossingen.
