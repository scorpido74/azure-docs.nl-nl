...
- **Aanbeveling**: werk de Azure-functie bij om een geldige JSON-nettolading te retour neren. Een C# functie kan bijvoorbeeld `(ActionResult)new<OkObjectResult("{`\"id retour neren\":\"123\"`}");`.
...
- **Oorzaak**: onjuiste indeling of ontbrekende definitie van een eigenschap.
...
- **Oorzaak**: de in de gekoppelde service van de AzureML-service ingevoerde referentie is ongeldig of heeft geen machtiging voor de bewerking.

- **Aanbeveling**: Controleer of de referentie in de gekoppelde service geldig is en of het toegang heeft tot de AzureML-service.
...
- **Oorzaak**: de uitvoering van de AzureML-pipeline is mislukt.
...
- **Oorzaak**: time-out bij het verzenden van taken. 
...
- **Oorzaak**: er worden op hetzelfde moment te veel taken verzonden naar HDInsight.

- **Aanbeveling**: overweeg het aantal gelijktijdige taken dat naar HDInsight moet worden verzonden te beperken. Raadpleeg Data Factory gelijktijdige activiteit als de taken worden verzonden door dezelfde activiteit. Wijzig de triggers zodat de gelijktijdige pijplijn uitvoeringen in de loop van de tijd worden verdeeld. Raadpleeg de HDInsight-documentatie om `templeton.parallellism.job.submit` aan te passen terwijl de fout wordt voorgesteld.
...
- **Aanbeveling**: Deze uitzonde ring is afkomstig uit HDInsight. Hiermee wordt het werkelijke probleem verborgen. Neem contact op met het HDInsight-team voor ondersteuning. Geef de cluster naam en het tijds bereik voor het uitvoeren van de activiteit op.
...
- **Oorzaak**: web activity biedt geen ondersteuning voor de HTTP-methode die is opgegeven in de nettolading van de activiteit.
...
- **Oorzaak**: ontbrekende vereiste machtigingen.
...
- **Oorzaak**: er kunnen meerdere redenen zijn voor deze fout, zoals de netwerk verbinding, DNS-fout, validatie van het server certificaat of de time-out.
