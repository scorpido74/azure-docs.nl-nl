...
U kunt opgeven welke kolommen u wilt toewijzen in de eigenschap Copy `translator`  ->  `mappings` activity->. In het volgende voor beeld wordt een Kopieer activiteit in een pijp lijn gedefinieerd om gegevens van gescheiden tekst te kopiëren naar Azure SQL Database.
...
| culturele  | Cultuur van de kolom Source of sink. <br>Toep assen wanneer type `Datetime` of `Datetimeoffset`is. De standaardwaarde is `en-us`. | Nee       |
| formaat   | Indelings teken reeks die moet worden gebruikt `Datetime` wanneer `Datetimeoffset`type of is. Raadpleeg de [aangepaste datum-en tijd notatie teken reeksen](https://docs.microsoft.com/dotnet/standard/base-types/custom-date-and-time-format-strings) voor het opmaken van DateTime. | Nee       |
...
De Kopieer activiteit ondersteunt de volgende tussenliggende gegevens typen: 
