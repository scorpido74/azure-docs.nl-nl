...
* Azure Storage-account. U gebruikt Blob Storage als een brongegevens opslag. Zie [een opslag account maken](../storage/common/storage-account-create.md)als u geen Azure Storage-account hebt.
* Azure Storage Explorer. Zie [Azure Storage Explorer](https://storageexplorer.com/)als u dit hulp programma wilt installeren.
* Azure SQL Database. U gebruikt de database als sink-gegevensopslag. Als u geen Azure SQL Database hebt, raadpleegt u [een Azure SQL database maken](../sql-database/sql-database-get-started-portal.md).
...
1. Voeg deze statische variabelen toe aan `Program` de klasse. Vervang de plaatsaanduidingen door uw eigen waarden.
...
1. Voeg de volgende regel toe aan `Main` de methode waarmee een Data Factory wordt gemaakt:
...
1. Voeg de volgende regel toe aan `Main` de methode waarmee een Azure Storage gekoppelde service wordt gemaakt:
...
1. Voeg de volgende code toe aan `Main` de methode waarmee zowel Azure Blob-bron-als Sink-gegevens sets worden gemaakt.
...
Maak in uw C#-project een klasse met `EmailRequest`de naam. Deze klasse definieert welke eigenschappen de pijp lijn in de hoofd aanvraag verzendt wanneer een e-mail wordt verzonden. In deze zelfstudie verzendt de pijplijn vier eigenschappen van de pijplijn naar het e-mailbericht:
...
Maak in de [Azure Portal](https://portal.azure.com)een Logic apps werk stroom met de naam *CopySuccessEmail*. Definieer de werk stroom trigger `When an HTTP request is received`als. Als aanvraagtrigger vult u het `Request Body JSON Schema` met de volgende JSON:
...
Deze JSON-inhoud wordt uitgelijnd met `EmailRequest` de klasse die u hebt gemaakt in de vorige sectie.

Voeg een actie van `Office 365 Outlook – Send an email`toe. Voor de actie **een E-mail verzenden** kunt u aanpassen hoe u het e-mail bericht wilt opmaken, met behulp van de eigenschappen die worden door gegeven in het JSON-schema van de aanvraag **tekst** . Hier volgt een voorbeeld:
...
Ga terug naar uw project in Visual Studio. We voegen nu de code toe die een pijp lijn maakt met een Kopieer activiteit `DependsOn` en-eigenschap. In deze zelf studie bevat de pijp lijn één activiteit, een Kopieer activiteit, die in de BLOB-gegevensset als bron wordt gebruikt en een andere blob-gegevensset als sink. Als de Kopieer activiteit slaagt of mislukt, worden verschillende e-mail taken aangeroepen.
...
1. Voeg de volgende regel toe aan `Main` de methode waarmee de pijp lijn wordt gemaakt:
...
Plak in `Url` de-eigenschap de eind punten van de **http-post-URL** uit uw Logic apps-werk stromen. Geef in `Body` de eigenschap een exemplaar van de `EmailRequest` klasse door. De e-mailaanvraag bevat de volgende eigenschappen:

* Bericht. Geeft de waarde `@{activity('CopyBlobtoBlob').output.dataWritten`van. Hiermee opent u een eigenschap van de vorige Kopieer activiteit en geeft u de `dataWritten`waarde van op. In het geval waarin het kopiëren mislukt, wordt de uitvoer van de fout doorgegeven in plaats van `@{activity('CopyBlobtoBlob').error.message`.
* Naam data factory. Met de waarde `@{pipeline().DataFactory}` van deze systeem variabele geeft u toegang tot de bijbehorende Data Factory naam. Zie [systeem variabelen](control-flow-system-variables.md)voor een lijst met systeem variabelen.
* Naam van de pijp lijn. Geeft de waarde `@{pipeline().Pipeline}`van. Met deze systeem variabele kunt u toegang krijgen tot de bijbehorende naam van de pijp lijn.
* Ontvangst. Geeft de waarde `"@pipeline().parameters.receiver"`van. Toegang tot de pijplijn parameters.
...
Uw laatste `Main` methode moet er als volgt uitzien.
...
1. Voeg de volgende code toe aan `Main` de methode waarmee details van de uitvoer van de Kopieer activiteit worden opgehaald, bijvoorbeeld grootte van de gegevens die zijn gelezen/geschreven:
