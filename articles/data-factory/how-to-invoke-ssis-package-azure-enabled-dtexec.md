...
Het hulp programma wordt geïnstalleerd `{SSMS Folder}\Common7\IDE\CommonExtensions\Microsoft\SSIS\150\Binn`op. U kunt het pad toevoegen aan de omgevings variabele PATH zodat het kan worden aangeroepen vanaf elke locatie.
...
- **/Set**: onderdrukt de configuratie van een para meter, variabele, eigenschap, container, logboek provider, foreach-enumerator of verbinding in uw pakket. Deze optie kan meerdere keren worden opgegeven. Geef als volgt de waarde voor deze optie op `property_path;value`:. De waarde van `\package.variables[counter].Value;1` `counter` de variabele wordt bijvoorbeeld vervangen door 1. Met de wizard **pakket configuratie** kunt u de waarde van `property_path` items in uw pakket zoeken, kopiëren en plakken waarvan u de waarde wilt overschrijven. Zie de [wizard pakket configureren](https://docs.microsoft.com/sql/integration-services/package-configuration-wizard-ui-reference?view=sql-server-2014)voor meer informatie.
