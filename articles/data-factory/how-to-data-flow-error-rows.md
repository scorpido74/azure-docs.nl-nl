...
1. Er is een Azure SQL database-doel tabel met een ```nvarchar(5)``` kolom met de naam ' name '.
...
2. Met deze Conditional Split trans formatie wordt de maximum lengte van ' title ' gedefinieerd als vijf. Een rij die kleiner dan of gelijk aan vijf is, gaat in de ```GoodRows``` stroom. Een rij die groter is dan vijf, gaat naar de ```BadRows``` stroom.
