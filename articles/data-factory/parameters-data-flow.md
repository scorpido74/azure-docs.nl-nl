...
Een gemeen schappelijk patroon is het door geven van een kolom naam als een parameter waarde. Gebruik de `byName()` functie om te verwijzen naar de kolom die aan de para meter is gekoppeld. Vergeet niet om de kolom naar het juiste type te casten met een cast `toString()`-functie zoals.
