...
| clientId | De client-ID die is gekoppeld aan uw HubSpot-toepassing.  | Ja |
...
| accessToken | Het toegangs token dat is verkregen bij het verifiëren van uw OAuth-integratie. Markeer dit veld als SecureString om het veilig op te slaan in Data Factory, of om te [verwijzen naar een geheim dat is opgeslagen in azure Key Vault](store-credentials-in-key-vault.md). | Ja |
