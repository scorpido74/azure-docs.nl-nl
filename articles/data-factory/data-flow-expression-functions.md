...
Retourneert altijd een waarde ONWAAR. Gebruik de functie syntaxis (false ()) als er een kolom met de naam False is.  
* ``(10 + 20 > 30) -> false``  
* ``(10 + 20 > 30) -> false()``  
___
### <code>filter</code>
...
Wijst elk element van de matrix toe aan een nieuw element met behulp van de gegeven expressie. De toewijzing verwacht een verwijzing naar één element in de expressie functie als #item en een verwijzing naar de element index als #index.  
...
Trekt getallen af. Aftrekken van het datum aantal dagen. De duur aftrekken van een tijds tempel. Trek twee time Stamps af om het verschil in milliseconden te verkrijgen. Hetzelfde als de-operator.  
* ``minus(20, 10) -> 10``  
* ``20 - 10 -> 10``  
...
Normaliseren van de teken reeks waarde voor het scheiden van geaccentde Unicode-tekens.  
...
Retourneert een NULL-waarde. Gebruik de functie syntaxis (Null ()) als er een kolom is met de naam null. Elke bewerking die gebruikmaakt, resulteert in een NULL.  
...
Hiermee wordt de grootte van een matrix of kaart type gezocht * ``size(['element1', 'element2']) -> 2``
* ``size([1,2,3]) -> 3``
___
### <code>slice</code>
