---
title: Gegevens kopiëren van QuickBooks Online met Azure Data Factory (Preview)
...
| consumerSecret | De consumentgeheim voor OAuth 1.0-verificatie. Markeer dit veld als een SecureString om het veilig op te slaan in Data Factory, of [verwijs naar een geheim dat is opgeslagen in Azure Key Vault](store-credentials-in-key-vault.md). | Ja |
| accessToken | Het toegangstoken voor verificatie voor OAuth 1.0. Markeer dit veld als een SecureString om het veilig op te slaan in Data Factory, of [verwijs naar een geheim dat is opgeslagen in Azure Key Vault](store-credentials-in-key-vault.md). | Ja |
| accessTokenSecret | De access-tokengeheim voor OAuth 1.0-verificatie. Markeer dit veld als een SecureString om het veilig op te slaan in Data Factory, of [verwijs naar een geheim dat is opgeslagen in Azure Key Vault](store-credentials-in-key-vault.md). | Ja |
...
Als u wilt kopiëren van gegevens uit QuickBooks Online, stelt u het brontype in de kopieeractiviteit naar **QuickBooksSource**. De volgende eigenschappen worden ondersteund in de kopieeractiviteit **source** sectie:
