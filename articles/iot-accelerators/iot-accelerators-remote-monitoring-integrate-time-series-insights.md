...
1. Als u uw Time Series Insights omgeving wilt maken, gebruikt u de waarden in de volgende tabel:
    ...
    | Instelling | Waarde |
    | ------- | ----- |
    | Omgevings naam | De volgende scherm afbeelding maakt gebruik van de naam **contorosrmtsi**. Kies uw eigen unieke naam wanneer u deze stap uitvoert. |
...
1. Als u uw IoT-hub als een nieuwe gebeurtenis bron wilt configureren, gebruikt u de waarden in de volgende tabel:
    ...
    | Instelling | Waarde |
    | ------- | ----- |
    | Bron naam van gebeurtenis | De volgende scherm afbeelding maakt gebruik van de naam **contosorm-IOT-hub**. Gebruik uw eigen unieke naam wanneer u deze stap uitvoert. |
    | Bron | **IoT Hub** |
    ...
    | Beleids naam van IOT-hub | **iothubowner** Zorg ervoor dat het gebruikte beleid een eigenaars beleid is. |
...
Configureer de omgeving van `basic` implementatie voor de bijgewerkte micro Services.
...
1. Navigeer naar **instellingen** > **sleutels** en maak vervolgens een nieuwe sleutel voor uw toepassing. Zorg ervoor dat u de sleutel waarde naar een veilige locatie kopieert.
...
1. Wanneer u verbinding hebt gemaakt, typt u `cd /app`.
...
1. Ga naar de **service ASA Manager** en bewerk het bestand docker opstellen door `PCS_TELEMETRY_STORAGE_TYPE`toe te voegen.

1. Start de docker-containers opnieuw met `sudo ./start.sh` van de virtuele machine.
...
Configureer de omgeving van `standard` implementatie voor de bijgewerkte micro services hierboven

1. Voer `kubectl proxy`uit vanaf de opdracht regel. Zie [toegang tot de Kubernetes-API](https://kubernetes.io/docs/tasks/access-kubernetes-api/http-proxy-access-api/#using-kubectl-to-start-a-proxy-server)voor meer informatie.
