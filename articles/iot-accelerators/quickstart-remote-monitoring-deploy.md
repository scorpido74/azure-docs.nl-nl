...
Deze quickstart implementeert een **Basic**-versie van de oplossingsverbetering voor test-en demonstratiedoeleinden waarmee de kosten worden geminimaliseerd. Zie [Basic- en Standard-implementaties](iot-accelerators-remote-monitoring-deploy-cli.md#basic-and-standard-deployments)voor meer informatie over de verschillende versies die u kunt implementeren.
...
Kies **C# Microservices** als de **Implementatieopties**. De Java- en C#-implementaties hebben dezelfde functies.
...
Selecteer het **Abonnement** en de **Regio** die u wilt gebruiken om de oplossingsverbetering te implementeren. U kiest meestal de regio die zich het meest in uw buurt bevindt. Voor deze quickstart maken we gebruik van **VS - oost**.
...
Als u de taakstatus wilt bekijken, gaat u terug naar de **Onderhoudspagina** en bekijkt u de lijst met taken in de weergave **Taken**. Mogelijk moet u een aantal seconden wachten voordat u ziet dat de taak is uitgevoerd:
...
![Schermopname van de pagina 'Ingerichte oplossingen' met de oplossingsversneller gemarkeerd.](media/quickstart-remote-monitoring-deploy/deletesolution.png)
