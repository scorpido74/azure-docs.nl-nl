...
Als u de oplossings versneller wilt uitvoeren, gaat u naar de **services\\scripts\\lokale** map in uw opdracht regel omgeving en voert u de volgende opdracht uit:
...
U kunt een afzonderlijke shell gebruiken om de logboeken van de container weer te geven. Zoek eerst de container-ID met behulp van de opdracht `docker ps`. Gebruik vervolgens `docker logs {container-id} --tail 1000` om de laatste 1000 vermeldingen voor de opgegeven container weer te geven.
...
Om toegang te krijgen tot het dash board van de oplossing voor externe controle, navigeert u naar `http://localhost:8080` in uw browser. U kunt nu de Web-UI en de lokale micro Services gebruiken.
