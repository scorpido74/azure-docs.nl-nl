...
Maak de gateway van het virtuele netwerk voor TestVNet1. BGP vereist een op route gebaseerde VPN-gateway. U hebt ook de aanvullende para meter `-Asn` nodig om het autonome systeem nummer (ASN) voor TestVNet1 in te stellen. Het maken van een gateway kan enige tijd duren (45 minuten of langer). 
...
Voer de volgende opdracht uit en controleer de sectie `bgpSettings` boven aan de uitvoer:
...
## <a name ="crossprembgp"></a>Een cross-premises verbinding tot stand brengen met BGP
...
In deze stap maakt u de verbinding van TestVNet1 naar site5. U moet de para meter `--enable-bgp` opgeven om BGP in te scha kelen voor deze verbinding. 
...
## <a name ="v2vbgp"></a>Een VNet-naar-VNet-verbinding tot stand brengen met BGP
...
In dit voor beeld maken de virtuele netwerken deel uit van hetzelfde abonnement. U kunt VNet-naar-VNet-verbindingen tussen verschillende abonnementen instellen. Zie [een vnet-naar-VNet-verbinding configureren](vpn-gateway-howto-vnet-vnet-cli.md)voor meer informatie. Zorg ervoor dat u `-EnableBgp $True` toevoegt tijdens het maken van de verbindingen om BGP in te scha kelen.
...
In deze stap maakt u de verbinding van TestVNet1 naar site5. Als u BGP wilt inschakelen voor deze verbinding, moet u de para meter `--enable-bgp` opgeven.
