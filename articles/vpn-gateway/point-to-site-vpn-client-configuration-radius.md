...
### <a name="usernamefiles"></a>1. configuratie bestanden voor VPN-clients genereren
...
Als u al client configuratie bestanden hebt gemaakt, kunt u deze ophalen met behulp van de cmdlet `Get-AzVpnClientConfiguration`. Maar als u wijzigingen aanbrengt in uw P2S VPN-configuratie, zoals het type VPN-protocol of het verificatie type, wordt de configuratie niet automatisch bijgewerkt. U moet de `New-AzVpnClientConfiguration`-cmdlet uitvoeren om een nieuwe configuratie-down load te maken.
...
#### <a name="adwincli"></a>Setup van Windows VPN-client
...
2. Dubbel klik op het pakket om het te installeren. Als er een pop-upvenster van SmartScreen wordt weer gegeven, selecteert u **meer info** > **toch worden uitgevoerd**.
...
#### <a name="admaccli"></a>Setup van Mac OS X-VPN-client
...
#### <a name="adlinuxcli"></a>Setup van Linux VPN-client via strongSwan
...
5. Open het bestand **VpnSettings. XML** in de map **generic** van de gedownloade client configuratie bestanden. Zoek het label `VpnServer` en kopieer de naam, te beginnen met `azuregateway` en eindigt met `.cloudapp.net`.
...
### <a name="certfiles"></a>1. configuratie bestanden voor VPN-clients genereren
...
#### <a name="certmaccli"></a>Setup van Mac OS X-VPN-client
...
3. Open het dialoog venster **netwerk** onder **netwerk voorkeuren**. Selecteer **+** om een nieuw VPN-client verbindings profiel te maken voor een P2S-verbinding met het virtuele Azure-netwerk.
...
## <a name="otherauth"></a>Werken met andere verificatie typen of-protocollen
...
1. Gebruik de cmdlet `Get-AzVpnClientConfiguration` om de VPN-client configuratie voor EapMSChapv2 te genereren.
