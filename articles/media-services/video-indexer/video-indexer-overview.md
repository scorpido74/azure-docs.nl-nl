...
* **Miniatuur extractie voor gezichten** ("beste gezicht"): identificeert automatisch het beste vastgelegde gezicht in elke groep gezichten (op basis van kwaliteit, grootte en frontale positie) en extraheer dit als een afbeeldings Asset.
...
* **Multi-language-identificatie en transcriptie** (preview-versie): identificeert automatisch de gesp roken taal in verschillende segmenten van de audio, waarbij elk segment van het Media bestand dat moet worden getranscribeerd, wordt verzonden en de transcriptie weer kan worden gecombineerd tot één Unified transcriptie. Zie voor meer informatie [automatisch inhoud identificeren en transcriberen in meerdere talen](multi-language-identification-transcription.md).
...
* **Detectie van Emotion**: identificeert emoties op basis van spraak (wat er wordt genoemd) en de gesp roken Toon (hoe dit wordt genoemd). De EMOTION kan Joy, verdriet, boosheid of vrezen zijn.
