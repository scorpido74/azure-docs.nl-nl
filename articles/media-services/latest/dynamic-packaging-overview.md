...
## <a name="to-prepare-your-source-files-for-delivery"></a><a id="delivery-protocols"/>Voor bereiding van de bron bestanden voor levering
...
> [!NOTE]
> Smooth Streaming vereist dat audio en video aanwezig zijn in de stroom.

## <a name="on-demand-streaming-workflow"></a>Werk stroom voor streaming op aanvraag

In de volgende stappen ziet u een gemeen schappelijke werk stroom voor Media Services stromen waarbij dynamische pakketten worden gebruikt in combi natie met de standaard encoder in Azure Media Services.

1. Upload een invoer bestand zoals een QuickTime/MOV-of MXF-bestand. Dit bestand wordt ook wel het mezzanine-of bron bestand genoemd. Zie indelingen die worden [ondersteund door de Standard encoder](media-encoder-standard-formats.md)voor een lijst met ondersteunde indelingen.
...
Zie de lijst met standaard [indelingen en codecs](media-encoder-standard-formats.md)voor encoders.
...
Als u een Smooth Streaming stroom speelt, worden in `Accessibility` het manifest waarden en `Role` kenmerken voor dat audio spoor vermeld. In het `StreamIndex` - `Role="alternate" Accessibility="description"` element zou bijvoorbeeld worden toegevoegd om aan te geven dat het een audio beschrijving is.
...
Voor HLS V7 en hoger `(format=m3u8-cmaf)`zou de afspeel lijst moeten `AUTOSELECT=YES,CHARACTERISTICS="public.accessibility.describes-video"` worden uitgevoerd wanneer het audio beschrijvings spoor wordt gesignaleerd.
