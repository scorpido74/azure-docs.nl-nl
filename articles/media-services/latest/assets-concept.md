...
Nadat de digitale bestanden zijn geüpload naar de opslag en zijn gekoppeld aan een Asset, kunnen ze worden gebruikt in de Media Services encoding, streaming, en analyse van inhouds werk stromen. Een van de algemene Media Services werk stromen is het uploaden, coderen en streamen van een bestand. In deze sectie vindt u een overzicht van de algemene stappen.
...
Voor een volledig .NET-voor beeld waarin wordt getoond hoe u de Asset maakt, een Beschrijf bare SAS-URL naar de container van het activum in de opslag ruimte, uploadt u het bestand in de container in Storage met behulp van de SAS URL. Zie [een taak invoer maken op basis van een lokaal bestand](job-input-from-local-file-how-to.md).

### <a name="create-a-new-asset"></a>Een nieuwe Asset maken

> [!NOTE]
> De eigenschappen van het Asset van het type datetime zijn altijd in UTC-indeling.
...
|naam: (uniek) Zie [naamgevings conventies](media-services-apis-overview.md#naming-conventions) ||
...
|storageEncryptionFormat| Opties (opties voor maken)|
|Type||
