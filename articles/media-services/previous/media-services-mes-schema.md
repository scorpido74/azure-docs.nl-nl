...
## <a name="Preset"></a>Vooraf ingesteld (hoofd element)
...
| Naam | Type | Beschrijving |
| --- | --- | --- |
| **Versie**<br/><br/> Verplicht |**XS: decimaal** |De vooraf ingestelde versie. De volgende beperkingen zijn van toepassing: XS: fractionDigits-waarde = "1" en XS: minInclusive-waarde = "1" bijvoorbeeld **Version = "1.0"** . |

## <a name="Encoding"></a>Gecodeerd
...
| **KeyFrameInterval**<br/><br/> minOccurs="0"<br/><br/> **default="00:00:02"** |**XS: tijd** |Bepaalt de vaste afstand tussen IDR frames in eenheden van seconden. Wordt ook wel de GOP terug-duur genoemd. Zie **SceneChangeDetection** om te bepalen of het coderings programma kan afwijken van deze waarde. |
...
| **Complexe**<br/><br/> minOccurs="0"<br/><br/> default="Balanced" |**XS: teken reeks** |Hiermee bepaalt u de afweging tussen de snelheid en video kwaliteit van de code ring. Dit kan een van de volgende waarden zijn: **snelheid**, **gebalanceerd**of **kwaliteit**<br/><br/> Standaard: **evenwichtig** |
...
| Naam | Type | Beschrijving |
| --- | --- | --- |
| **Regeling** |**XS: teken reeks** | Wanneer de invoer geen video bevat, wilt u mogelijk het coderings programma dwingen om een monochroom video spoor in te voegen. Gebruik hiervoor voor waarde = "InsertBlackIfNoVideoBottomLayerOnly" (om een video te plaatsen bij alleen de laagste bitrate) of voor waarde = "InsertBlackIfNoVideo" (om een video in te voegen bij alle uitvoer bitrates). Raadpleeg [dit artikel](media-services-advanced-encoding-with-mes.md#no_video) voor meer informatie.|

## <a name="H264Layers"></a>H264Layers

Standaard, als u een invoer verzendt naar het coderings programma dat alleen audio bevat en geen video, bevat de uitvoer Asset alleen bestanden met audio gegevens. Sommige spelers kunnen dergelijke uitvoer stromen mogelijk niet verwerken. U kunt de H264Video's **InsertBlackIfNoVideo** -kenmerk instelling gebruiken om ervoor te zorgen dat het coderings programma een video spoor toevoegt aan de uitvoer in dat scenario. Raadpleeg [dit artikel](media-services-advanced-encoding-with-mes.md#no_video) voor meer informatie.
...
| **BufferWindow**<br/><br/> minOccurs="0"<br/><br/> default="00:00:05" |**XS: tijd** |Lengte van de video buffer. |
...
| **Segment**<br/><br/> minOccurs="0"<br/><br/> default="0" |**XS: int** |Hiermee wordt bepaald op hoeveel segmenten een frame is verdeeld. U wordt aangeraden standaard te gebruiken. |
...
| **Hebben** |**XS: duration** |Hiermee geeft u de duur van een presentatie op (bijvoorbeeld de weer gave van een overlay in de video). |
...
| Naam | Type | Beschrijving |
| --- | --- | --- |
| **FileName** |**XS: teken reeks** |De naam van het uitvoer bestand.<br/><br/> U kunt de macro's die in de volgende tabel worden beschreven, gebruiken om de namen van de uitvoer bestanden te maken. Bijvoorbeeld:<br/><br/> **"Uitvoer": [{"FileName": "{Basenaam} *{Resolution}* {bitrate}. mp4", "Format": {"type": "MP4Format"}}]** |

### <a name="macros"></a>Schrijft

| Macro | Beschrijving |
| --- | --- |
| **{Basename}** |Als u VoD-code ring uitvoert, is {basispad} de eerste 32 tekens van de eigenschap AssetFile.Name van het primaire bestand in de invoer Asset.<br/><br/> Als het invoer element een live archief is, wordt {basispad} afgeleid van de kenmerken voor naam bijhouden in het server manifest. Als u een subclip verzendt met behulp van de TopBitrate, zoals in: "< VideoStream\>TopBitrate </VideoStream\>" en het uitvoer bestand bevat video, is {basispad} de eerste 32 tekens van de naam van de videolaag met de hoogste bitsnelheid.<br/><br/> Als u in plaats daarvan een subfragment verzendt met behulp van alle invoer bitrates, zoals "< VideoStream\>* </VideoStream\>" en het uitvoer bestand bevat een video, is {basispad} de eerste 32 tekens van de naam van de track van de bijbehorende videolaag. |
| **{Codec}** |Wordt toegewezen aan ' H264 ' voor video en ' AAC ' voor audio. |
| **{Bitrate}** |De doel video-bitsnelheid als het uitvoer bestand video en audio bevat of als het uitvoer bestand alleen audio bevat. De gebruikte waarde is de bitsnelheid in kbps. |
...
| **{Height}** |De hoogte van de video, in pixels, in het uitvoer bestand, als het bestand video bevat. |
...
| **{Index}** |Verplicht voor miniatuur. Mag slechts één keer worden weer gegeven. |

## <a name="Video"></a>Video (complex type neemt van codec over)
### <a name="attributes"></a>Kenmerken

| Naam | Type | Beschrijving |
| --- | --- | --- |
| **Beginnen** |**XS: teken reeks** | |
| **Wizardstap** |**XS: teken reeks** | |
| **Range** |**XS: teken reeks** | |
...
| **Breedte**<br/><br/> minOccurs="0" |**XS: int** | |
| **Hoogte**<br/><br/> minOccurs="0" |**XS: int** | |
| **Borg**<br/><br/> minOccurs="0" |**XS: int** |Geldige waarden: 1 (slechtste)-100 (beste) |
...
## <a name="BmpImage"></a>BmpImage (complex type neemt uit video over)
### <a name="elements"></a>Elementen
...
## <a name="JpgImage"></a>JpgImage (complex type neemt uit video over)
### <a name="elements"></a>Elementen
...
## <a name="PngImage"></a>PngImage (complex type neemt uit video over)
### <a name="elements"></a>Elementen
