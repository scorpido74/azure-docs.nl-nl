...
- Zie ook voor meer informatie de [Azure AD-verificatie gebruiken om toegang te krijgen tot de Media Services-API met het rest](https://docs.microsoft.com/en-us/azure/media-services/previous/media-services-rest-connect-with-aad) -artikel.
...
3. Controleer het bestand **AzureMediaServices. postman_environment. json** . U ziet dat bijna alle bewerkingen in de verzameling een test script uitvoeren. De scripts hebben een aantal waarden die worden geretourneerd door het antwoord en stellen geschikte omgevings variabelen in.
    ...
    ```
4. Klik aan de linkerkant van het venster **postman** op **1. Het AAD-verificatie Token ophalen** -> **Azure AD-Token ophalen voor de Service-Principal**.
...
1. Selecteer **AccessPolicy** -> **AccessPolicy maken voor het uploaden**.
2. Druk op **Verzenden**.
...
1. Selecteer **activa** -> **activa maken**.
2. Druk op **Verzenden**.
...
1. Selecteer **Locator** -> **SAS-Locator te maken**.
2. Druk op **Verzenden**.
...
- [Het Azure Storage REST API gebruiken](https://docs.microsoft.com/azure/storage/common/storage-rest-api-auth?toc=%2fazure%2fstorage%2fblobs%2ftoc.json)
- [BLOB plaatsen](https://docs.microsoft.com/rest/api/storageservices/put-blob)
- [Blobs uploaden naar Blob Storage](https://docs.microsoft.com/previous-versions/azure/storage/storage-use-azcopy#upload-blobs-to-blob-storage)
...
1. Druk op **+** om een nieuw tabblad aanvraag te maken.
