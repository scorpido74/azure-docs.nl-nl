...
Een machine of VM die onderdeel uitmaakt van een cluster, heet een knooppunt. Aan elk knooppunt wordt een knooppuntnaam toegewezen (een tekenreeks). Knooppunten hebben kenmerken zoals plaatsingseigenschappen. Elke machine of VM heeft een Windows-service die automatisch wordt gestart, `FabricHost.exe`, die tijdens het opstarten wordt gestart en twee uitvoer bare bestanden worden gestart: `Fabric.exe` en `FabricGateway.exe`. Deze twee uitvoer bare bestanden vormen het knoop punt. Voor ontwikkelings-en test scenario's kunt u meerdere knoop punten op één computer of virtuele machine hosten door meerdere exemplaren van `Fabric.exe` en `FabricGateway.exe`uit te voeren.
...
[Uw eerste Service Fabric zelfstandige cluster maken](service-fabric-cluster-creation-for-windows-server.md)
...
* Status query's (via [Power shell](/powershell/module/ServiceFabric/), [cli](service-fabric-sfctl.md), [ C# de FabricClient api's](/dotnet/api/system.fabric.fabricclient.healthclient) en [Java FabricClient-api's](/java/api/system.fabric)of [rest api's](/rest/api/servicefabric)).
