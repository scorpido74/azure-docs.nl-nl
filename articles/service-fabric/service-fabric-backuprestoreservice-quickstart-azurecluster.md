...
`Include backup restore service` inschakelen selectie vakje onder `+ Show optional settings` op `Cluster Configuration` tabblad.
...
2. Schakel nu de _service voor back-up en herstel_ in door de volgende `addonFeatures` sectie toe te voegen onder `properties` sectie, zoals wordt weer gegeven in het volgende code fragment: 
    ...
    ```
3. Het X. 509-certificaat configureren voor het versleutelen van referenties. Dit is belang rijk om ervoor te zorgen dat de referenties die zijn verschaft om verbinding te maken met de opslag, worden versleuteld voordat ze worden bewaard. Configureer het versleutelings certificaat door de volgende `BackupRestoreService` toe te voegen aan de sectie `fabricSettings`, zoals wordt weer gegeven in het volgende code fragment: 
...
Gebruik voor back-upopslag het Azure Storage account dat hierboven is gemaakt. Container `backup-container` is geconfigureerd voor het opslaan van back-ups. Er wordt een container met deze naam gemaakt, als deze nog niet bestaat, tijdens het uploaden van back-ups. Vul `ConnectionString` met een geldige connection string voor het Azure Storage-account, vervang `account-name` door de naam van uw opslag account en `account-key` met de sleutel van uw opslag account.
...
Voer de volgende Power shell-cmdlets uit om een nieuw back-upbeleid te maken. Vervang `account-name` door de naam van uw opslag account en `account-key` met de sleutel van uw opslag account.
...
Voer het volgende Power shell-script uit om de vereiste REST API aan te roepen om een nieuw beleid te maken. Vervang `account-name` door de naam van uw opslag account en `account-key` met de sleutel van uw opslag account.
