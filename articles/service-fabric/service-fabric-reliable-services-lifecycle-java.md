...
* Tijdens het opstarten:
  ...
  * De methode van `runAsync` de service wordt aangeroepen, zodat de service langlopende of achtergrond taken kan uitvoeren.
* Tijdens afsluiten:
  * Het annulerings token dat is door `runAsync` gegeven aan is geannuleerd en de listeners zijn gesloten.
...
2. Deze gebeurtenissen treden parallel op:
    ...
    - De methode van `runAsync` de service`StatelessService.runAsync()`() wordt aangeroepen.
3. Indien aanwezig, wordt de eigen `onOpenAsync` methode van de service aangeroepen. Dit `StatelessService.onOpenAsync()` wordt specifiek genoemd. Dit is een ongebruikelijke onderdrukking, maar deze is wel beschikbaar.

Het is belang rijk te weten dat er geen volg orde is tussen de oproep om de listeners te maken en te openen en `runAsync`de aanroep naar. De listeners worden mogelijk geopend voordat `runAsync` deze wordt gestart. Dit `runAsync` kan ook worden opgeroepen voordat de communicatie-listeners zijn geopend of voordat ze zelfs zijn gebouwd. Als een synchronisatie is vereist, moet deze door de implementeerder worden uitgevoerd. Hier volgen enkele veelvoorkomende oplossingen:
...
* Soms wordt de code `runAsync` in niet gestart totdat de listeners zijn geopend. In dit geval is verdere coördinatie nood zakelijk. Een veelvoorkomende oplossing is het toevoegen van een vlag in de listeners. De vlag geeft aan wanneer de listeners zijn voltooid. Deze `runAsync` methode controleert dit voordat de werkelijke hoeveelheid werk wordt voortgezet.
...
1. Deze gebeurtenissen treden parallel op:
    ...
    - Het annulerings token dat is door `runAsync()` gegeven aan, is geannuleerd. Het controleren van de eigenschap `isCancelled` van het `true`annulerings token retourneert, en als deze `throwIfCancellationRequested` wordt aangeroepen, genereert `CancellationException`de methode van het token een.
2. Wanneer `closeAsync()` op elke listener is voltooid en `runAsync()` ook is voltooid, wordt de methode van `StatelessService.onCloseAsync()` de service aangeroepen als deze aanwezig is. Opnieuw is dit geen veelvoorkomende onderdrukking, maar kan ook worden gebruikt om resources veilig te sluiten, de achtergrond verwerking te stoppen, de externe status op te slaan of bestaande verbindingen te sluiten.
...
3. Deze gebeurtenissen treden parallel op:
    - `StatefulServiceBase.createServiceReplicaListeners()`is aangeroepen. 
      ...
      - Als de service een secundaire service is, worden alleen listeners die zijn `listenOnSecondary = true` gemarkeerd als geopend. Luister aars die open zijn op de secundaire zones, zijn minder gangbaar.
...
1. Deze gebeurtenissen treden parallel op:
    - Alle geopende listeners worden gesloten. `CommunicationListener.closeAsync()`wordt aangeroepen voor elke listener.
    - Het annulerings token dat is door `runAsync()` gegeven aan, is geannuleerd. Een aanroep van de `isCancelled()` methode van het annulerings `true`token retourneert, en als deze wordt aangeroepen `throwIfCancellationRequested()` , genereert de methode `OperationCanceledException`van het token een.
2. Nadat `closeAsync()` de service `StatefulServiceBase.onChangeRoleAsync()` is voltooid op elke listener `runAsync()` , wordt deze ook wel voltooid. Deze aanroep wordt niet vaak overschreven in de service.
   ...
   > [!NOTE]  
   > `runAsync` Wachten op volt ooien is alleen nodig als deze replica een primaire replica is.
...
1. Deze gebeurtenissen treden parallel op:
    - Alle geopende listeners worden gesloten. `CommunicationListener.closeAsync()`wordt aangeroepen voor elke listener.
    - Het annulerings token dat is door `runAsync()` gegeven aan, is geannuleerd. Een controle van de `isCancelled()` methode van het annulerings `true`token retourneert. Als wordt aangeroepen, genereert de `throwIfCancellationRequested()` methode van het token `OperationCanceledException`een.
2. Nadat `closeAsync()` de service `StatefulServiceBase.onChangeRoleAsync()` is voltooid op elke listener `runAsync()` , wordt deze ook wel voltooid. Deze aanroep wordt niet vaak overschreven in de service.
...
* Het is geldig om een service te volt `runAsync()` ooien en terug te keren. Dit wordt niet beschouwd als een fout voorwaarde. Het vertegenwoordigt het achtergrond werk van de service wordt voltooid. Voor stateful Reliable Services `runAsync()` zou opnieuw worden aangeroepen als de service wordt gedegradeerd van primair en vervolgens weer wordt teruggedraaid naar primair.
...
* Er zijn fouten `onCloseAsync()` opgetreden in `onAbort()` het pad naar de aanroep. Deze aanroep is een kans op een beste kans voor de service om alle resources die ze hebben geclaimd, op te schonen en vrij te geven. Dit wordt meestal opgeroepen wanneer een permanente fout op het knoop punt wordt gedetecteerd of wanneer Service Fabric de levens cyclus van het service-exemplaar niet betrouwbaar kunt beheren als gevolg van interne storingen.
