...
1. Controleer of de `apiversion` is ingesteld op `2017-07-01-preview` of hoger voor de `Microsoft.ServiceFabric/clusters` resource, en als dit niet het geval is, werkt u deze bij, zoals wordt weer gegeven in het volgende voor beeld:
...
2. Schakel nu de DNS-service in op een van de volgende manieren:
   ...
   - Als u de DNS-service met de standaard instellingen wilt inschakelen, voegt u deze toe aan de sectie `addonFeatures` in de sectie `properties`, zoals wordt weer gegeven in het volgende voor beeld:
   ...
   - Als u de service met andere dan de standaard instellingen wilt inschakelen, voegt u een sectie `DnsService` toe aan de sectie `fabricSettings` in het gedeelte `properties`. In dit geval hoeft u de DNS niet toe te voegen aan `addonFeatures`. Zie [DNS-service-instellingen](./service-fabric-cluster-fabric-settings.md#dnsservice)voor meer informatie over de eigenschappen die kunnen worden ingesteld voor de DNS-service.
       ...
       ```
3. Nadat u de cluster sjabloon met uw wijzigingen hebt bijgewerkt, past u deze toe en laat u de upgrade volt ooien. Wanneer de upgrade is voltooid, wordt de DNS-systeem service gestart in uw cluster. De service naam is `fabric:/System/DnsService`en u kunt deze vinden onder de sectie **systeem** service in service Fabric Explorer. 
...
Het is sterk aan te raden om een naam schema van `<ServiceDnsName>.<AppInstanceName>`te gebruiken. bijvoorbeeld `service1.application1`. Als een toepassing wordt geïmplementeerd met behulp van docker opstellen, worden aan services automatisch DNS-namen toegewezen met dit naamgevings schema.
...
Open uw project in Visual Studio of uw favoriete editor en open het bestand ApplicationManifest. XML. Ga naar de sectie standaard services en voeg voor elke service het kenmerk `ServiceDnsName` toe. In het volgende voor beeld ziet u hoe u de DNS-naam van de service instelt op `service1.application1`
...
- Gebruik `backendrangedschemesvc-0.application`om de partitie "0" van een service met DNS-naam `backendrangedschemesvc.application` die gebruikmaakt van een gepartitioneerd partitie schema, op te lossen.
- Als u de partitie ' First ' van een service met DNS-naam `backendnamedschemesvc.application` wilt omzetten die gebruikmaakt van een benoemde partitie schema, gebruikt u `backendnamedschemesvc-first.application`.
