...
`InstanceCount` voor de service geeft u het gewenste aantal exemplaren op dat moet worden uitgevoerd. Wanneer een (of alle) van de instanties mislukt, Service Fabric reageert door automatisch vervangende instanties te maken op andere knoop punten. Service Fabric gaat door met het maken van vervangingen totdat de service terugkeert naar het aantal gewenste exemplaren.
Een ander voor beeld: als de stateless service `InstanceCount` van-1 heeft, betekent dit dat er één exemplaar moet worden uitgevoerd op elk knoop punt in het cluster. Als sommige van deze instanties mislukken, dan detecteert Service Fabric dat de service niet de gewenste status heeft en wordt geprobeerd om de exemplaren te maken op de knoop punten waar ze ontbreken.

#### <a name="stateful-services"></a>Stateful Services
Er zijn twee soorten stateful-Services:
1. Stateful met persistente status
2. Stateful met niet-persistente status (status wordt opgeslagen in het geheugen)

De herstel status van stateful service fouten is afhankelijk van het type stateful service en het aantal replica's dat de service had en de hoeveelheid die is mislukt.

Wat is quorum?
In een stateful service worden inkomende gegevens gerepliceerd tussen replica's (primair en ActiveSecondary). Als de meeste replica's de gegevens ontvangen, worden gegevens beschouwd als vastgelegde quorum (voor vijf replica's, 3 is een _quorum_). Dit betekent dat het op een bepaald moment zeker is dat er ten minste een quorum van replica's met de meest recente gegevens wordt weer gegeven. Als er een fout optreedt in een replica (2 van 5 replica's), kunnen we gebruikmaken van de quorum waarde om te berekenen of het herstellen (sinds de resterende 3 van vijf replica's nog steeds actief zijn) gegarandeerd dat ten minste één replica volledige gegevens bevat).

Wat is quorum verlies?
Wanneer het quorum van replica's mislukt, wordt de partitie met de status quorum verlies gedeclareerd. Stel dat een partitie 5 replica's bevat. Dit betekent dat ten minste 3 volledige gegevens zijn gegarandeerd. Als het quorum (3 uitgaande 5) van replica's mislukt, kan Service Fabric niet bepalen of de resterende replica's (2 uitgaand 5) voldoende gegevens hebben om de partitie te herstellen.

Vaststellen of een nood geval is opgetreden voor een stateful service en het beheer ervan drie fasen volgt:
...
2. Vaststellen of het quorum verlies permanent is of niet
   - De meeste tijd zijn fouten tijdelijk. Processen worden opnieuw gestart, knoop punten opnieuw worden gestart, Vm's worden opnieuw gestart, het herstellen van netwerk partities. Soms zijn fouten permanent. 
     - Voor services zonder persistente status, een fout in een quorum of meer van replica's resulteert _onmiddellijk_ in permanent quorum verlies. Wanneer Service Fabric quorum verlies detecteert in een stateful, niet-persistente service, gaat het onmiddellijk door naar stap 3 door het declareren van (potentieel) gegevens verlies. Door gaan met gegevens verlies is zinvol omdat Service Fabric weet dat er niet wordt gewacht tot de replica's terugkomen, omdat zelfs als ze zijn hersteld vanwege een niet-blijvende aard van de service, de gegevens verloren zijn gegaan.
     - Als er sprake is van stateful permanente Services, treedt er een fout op in een quorum of meer van replica's, Service Fabric te wachten tot de replica's terugkomen en het quorum herstellen. Dit resulteert in een service onderbreking voor _schrijf bewerkingen_ naar de betrokken partitie (of ' replicaset ') van de service. Lees bewerkingen kunnen echter nog steeds mogelijk zijn met beperkte consistentie garanties. De standaard hoeveelheid tijd die Service Fabric wacht totdat het quorum is hersteld, is oneindig, omdat het volgen van een (mogelijke) gegevens verlies gebeurtenis plaatsvindt en andere Risico's ondervindt.
3. Vaststellen of er daad werkelijk gegevens verlies is en herstel van back-ups
   ...
   1. Eerst moet u vastleggen dat `OnDataLossAsync` is geactiveerd en alle benodigde beheerders waarschuwingen starten.
   1. Normaal gesp roken wordt u op dit moment gepauzeerd en gewacht op verdere beslissingen en hand matige acties moeten worden ondernomen. Dit komt doordat zelfs als er back-ups beschikbaar zijn, moeten deze mogelijk worden voor bereid. Als bijvoorbeeld twee verschillende Services gegevens coördineren, moeten deze back-ups mogelijk worden gewijzigd om ervoor te zorgen dat de gegevens die in de twee services relevant zijn, consistent zijn. 
   1. Vaak is er ook enige andere telemetrie of het afbreken van de service. Deze meta gegevens kunnen in andere services of in Logboeken zijn opgenomen. Deze informatie kan worden gebruikt om te bepalen of er aanroepen zijn ontvangen en verwerkt op de primaire server die niet aanwezig waren in de back-up of die zijn gerepliceerd naar deze specifieke replica. Deze moeten mogelijk opnieuw worden afgespeeld of worden toegevoegd aan de back-up voordat herstellen haalbaar is.  
   1. Vergelijkingen van de resterende status van de replica naar die zijn opgenomen in back-ups die beschikbaar zijn. Als u de betrouw bare verzamelingen van Service Fabric gebruikt, zijn er hulpprogram ma's en processen beschikbaar om dit te doen, zoals beschreven in [dit artikel](service-fabric-reliable-services-backup-restore.md). Het doel is om te zien of de status binnen de replica voldoende is of dat de back-up mogelijk ontbreekt.
   1. Zodra de vergelijking is uitgevoerd, en, indien nodig, de herstel bewerking is voltooid, moet de service code de waarde True retour neren als er status wijzigingen zijn aangebracht. Als de replica heeft vastgesteld dat het de beste beschik bare kopie van de status heeft en geen wijzigingen heeft aangebracht, retourneert onwaar. Waar geeft aan dat alle _Overige_ replica's mogelijk nu inconsistent zijn met deze waarde. Ze worden verwijderd en opnieuw opgebouwd op basis van deze replica. ONWAAR geeft aan dat er geen status wijzigingen zijn aangebracht, zodat de andere replica's kunnen blijven wat ze hebben. 
...
Replica's kunnen tijdelijk worden afgesloten vanwege een tijdelijke fout. Wacht enige tijd, omdat Service Fabric probeert deze naar boven te brengen. Als replica's langer dan de verwachte duur zijn, voert u de onderstaande stappen voor het oplossen van problemen uit.
- De replica's kunnen vastlopen. Controleer status rapporten op replica niveau en uw toepassings Logboeken. Verzamel crash dumps en neem de nodige maat regelen om te herstellen.
- Het kan zijn dat het replica proces niet meer reageert. Controleer uw toepassings Logboeken om dit te controleren. Verzamel de proces dump en beëindig het niet-reagerende proces. Service Fabric maakt een vervangend proces en wordt geprobeerd de replica terug te brengen.
- Knoop punten die de replica's hosten, zijn mogelijk niet beschikbaar. Start de onderliggende VM opnieuw op om de knoop punten omhoog te brengen.

Er kunnen zich situaties voordoen waarin het niet mogelijk is om replica's te herstellen, maar de stations zijn mogelijk mislukt of de computers zijn niet fysiek te reageren. In deze gevallen moet Service Fabric worden verteld dat er niet moet worden gewacht op replica herstel.
Gebruik deze methoden niet als potentiële gegevens verlies niet acceptabel is om de service online te brengen, in welk geval alle inspanningen moeten worden uitgevoerd om fysieke machines te herstellen.

De volgende stappen kunnen resulteren in dataloss. Controleer voordat u deze volgt:
   
> [!NOTE]
> Het is _nooit_ veilig om deze methoden te gebruiken, maar niet op een gerichte manier voor specifieke partities. 
>

- Gebruik `Repair-ServiceFabricPartition -PartitionId` of `System.Fabric.FabricClient.ClusterManagementClient.RecoverPartitionAsync(Guid partitionId)` API. Met deze API kunt u de ID opgeven van de partitie die moet worden hersteld van de QuorumLoss met mogelijke gegevens verlies.
- Als uw cluster veelvuldige storingen ondervindt, waardoor Services de status van quorum verlies kunnen verliezen en mogelijke _gegevens verlies acceptabel is_, kunt u een geschikte [QuorumLossWaitDuration](https://docs.microsoft.com/powershell/module/servicefabric/update-servicefabricservice?view=azureservicefabricps) -waarde opgeven om uw service automatisch te herstellen. Service Fabric wacht op de beschik bare QuorumLossWaitDuration (de standaard waarde is oneindig) voordat het herstel wordt uitgevoerd. Deze methode wordt _niet aanbevolen_ omdat dit kan leiden tot onverwachte gegevens verlies.
