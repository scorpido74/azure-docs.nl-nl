...
Het certificaat moet worden geïnstalleerd op elk knoop punt in het cluster `/var/lib/sfcerts`onder. Het gebruikers account waarmee de service wordt uitgevoerd (standaard sfuser) **moet lees toegang hebben** tot het geïnstalleerde certificaat (dat wil zeggen, `/var/lib/sfcerts/TestCert.pem` voor het huidige voor beeld).
