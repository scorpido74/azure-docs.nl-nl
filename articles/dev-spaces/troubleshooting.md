...
Voor de Visual Studio-uitbrei ding `MS_VS_AZUREDEVSPACES_TOOLS_LOGGING_ENABLED` stelt u de omgevings variabele in op 1. Zorg ervoor dat u Visual Studio opnieuw start om de omgevings variabele van kracht te laten worden. Wanneer deze functie is ingeschakeld, worden gedetailleerde logboeken naar uw `%TEMP%\Microsoft.VisualStudio.Azure.DevSpaces.Tools` Directory geschreven.

In de CLI kunt u meer informatie tijdens het uitvoeren van de opdracht uitvoeren met `--verbose` behulp van de switch. U kunt ook meer gedetailleerde logboeken bekijken `%TEMP%\Azure Dev Spaces`in. Op een Mac kan de map *temp* worden gevonden door vanuit een `echo $TMPDIR` Terminal venster te worden uitgevoerd. Op een Linux-computer is de map *temp* doorgaans `/tmp`. Controleer bovendien of logboek registratie is ingeschakeld in uw [Azure cli-configuratie bestand](/cli/azure/azure-cli-configuration?view=azure-cli-latest#cli-configuration-values-and-environment-variables).
...
Met het bovenstaande bestand maakt u een *AzurePodIdentityException* -object voor de *azds-injecter-webhook*. Als u dit object wilt implementeren `kubectl`, gebruikt u:
...
Gebruik `kubectl`de volgende opties om de *identiteit* -en *AzureIdentityBinding* -objecten te implementeren:
...
Deze fout wordt mogelijk weer gegeven bij het uitvoeren van het fout opsporingsprogramma voor Visual Studio-code. De VS code-uitbrei ding maakt `src` standaard gebruik van de werkmap voor het project in de container. Als u uw `Dockerfile` hebt bijgewerkt om een andere werkmap op te geven, wordt deze fout weer geven.

U kunt dit probleem oplossen door het `launch.json` bestand onder de `.vscode` submap van de projectmap bij te werken. Wijzig de `configurations->cwd` instructie zodat deze verwijst naar de map die is `WORKDIR` `Dockerfile`gedefinieerd in het project. Mogelijk moet u ook de `configurations->program` richt lijn bijwerken.
...
1. Controleer de locatie% Program Files%/Microsoft SDKs\Azure\Azure dev Spaces `azds.exe`cli for. Als dat het geval is, voegt u die locatie toe aan de omgevings variabele PATH.
...
* De container wordt nog steeds gemaakt en geïmplementeerd. Dit probleem kan zich voordoen als `azds up` u het fout opsporingsprogramma uitvoert of start en vervolgens probeert toegang te krijgen tot de container voordat deze is geïmplementeerd.
...
1. Controleer de poort configuratie in de volgende assets:
    * **[Helm-grafiek](https://docs.helm.sh):** opgegeven door `service.port` de `deployment.containerPort` en in values. yaml met `azds prep` de opdracht.
...
Als u Azure-ontwikkel ruimten wilt inschakelen voor een bestaande naam ruimte in een `use-dev-spaces` AKS- `kubectl` cluster, voert u uit en gebruikt u dit om alle peulen in die naam ruimte opnieuw te starten.
...
Na [het draaien van de certificaten in uw AKS-cluster](../aks/certificate-rotation.md), kunnen bepaalde `azds space list` bewerkingen `azds up` , zoals en, mislukken. U moet ook de certificaten op uw Azure dev Space-controller vernieuwen na het draaien van de certificaten in uw cluster.

U kunt dit probleem oplossen door te controleren of uw *kubeconfig* de bijgewerkte `az aks get-credentials` certificaten heeft met `azds controller refresh-credentials` behulp van de opdracht. Bijvoorbeeld:
