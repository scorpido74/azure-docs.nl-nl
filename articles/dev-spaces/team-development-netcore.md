...
ms.date: 07/09/2018
ms.topic: tutorial
description: In deze zelfstudie ziet u hoe u Azure Dev Spaces en Visual Studio Code gebruikt voor teamontwikkeling in een .NET Core-toepassing in Azure Kubernetes Service
...
Ga naar het venster van VS Code voor `mywebapi` en bewerk de code op basis van de `string Get(int id)`-methode in `Controllers/ValuesController.cs`, bijvoorbeeld:
