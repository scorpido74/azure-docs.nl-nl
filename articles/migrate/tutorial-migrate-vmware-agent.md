...
3. Geef in **Projectdetails** de projectnaam en geografie op waarin u het project wilt maken en klik op **Volgende**. Bekijk ondersteunde geografies voor [open bare](migrate-support-matrix.md#supported-geographies-public-cloud) en [overheids Clouds](migrate-support-matrix.md#supported-geographies-azure-government).
...
U kunt het replicatie apparaat op een aantal manieren instellen.

- Instellen met een gedownloade eicellen-sjabloon (open Virtualization Application). U importeert de sjabloon in VMware en maakt de replicatie apparaat-VM. Dit is de methode die in deze zelf studie wordt gebruikt.
- Instellen met een script.
...
5. Voer een naam in die moet worden gebruikt voor het registreren van het apparaat met Azure Migrate server migratie. Klik vervolgens op **Volgende**.
...
10. Voer de referenties in voor het account dat u hebt [gemaakt](#prepare-an-account-for-automatic-discovery) voor VMware-detectie. Selecteer **Add** > **door gaan**toevoegen.
