...
U kunt bijvoorbeeld het dubbele pad naar `properties.desired.chiller-water` instellen en vervolgens de volgende JSON-inhoud opgeven: 
...
Als u een bestaande eigenschap wilt verwijderen, geeft u de eigenschaps waarde op die moet worden `null`.
...
Als u een metriek bouwt om te rapporteren over geconfigureerde modules, selecteert u `moduleId` van `devices.modules`. Bijvoorbeeld:
...
2. Geef een **doel voorwaarde** op om te bepalen welke apparaten of modules met deze configuratie worden benaderd. De voor waarde is gebaseerd op dubbele Tags of dubbele gerapporteerde eigenschappen en moet overeenkomen met de indeling van de expressie. 
   ...
   Voor automatische apparaatconfiguratie kunt u alleen de tag of de gerapporteerde eigenschap op het doel opgeven. Bijvoorbeeld `tags.environment='test'` of `properties.reported.chillerProperties.model='4000x'`. U kunt `*` opgeven voor alle apparaten. 
