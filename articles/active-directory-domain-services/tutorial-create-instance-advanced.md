...
description: In deze zelfstudie leert u hoe u een Azure Active Directory Domain Services-exemplaar maakt en configureert en geavanceerde configuratieopties opgeeft met behulp van de Azure-portal.
author: iainfoulds
manager: daveba
...
ms.locfileid: "79239145"
---
# <a name="tutorial-create-and-configure-an-azure-active-directory-domain-services-instance-with-advanced-configuration-options"></a>Zelfstudie: Een Azure Active Directory Domain Services-exemplaar maken en configureren met geavanceerde configuratieopties
...
U [een beheerd domein maken met standaardconfiguratieopties][tutorial-create-instance] voor netwerken en synchronisatie, of deze instellingen handmatig definiëren. In deze zelfstudie ziet u hoe u deze geavanceerde configuratieopties definieert om een Azure AD DS-exemplaar te maken en te configureren met behulp van de Azure-portal.
...
## <a name="create-an-instance-and-configure-basic-settings"></a>Een instantie maken en basisinstellingen configureren
...
1. Een *forest* is een logische constructie die wordt gebruikt door Active Directory Domain Services om een of meer domeinen te groeperen. Standaard wordt een door Azure AD DS beheerd domein gemaakt als *gebruikersforest.* Dit type forest synchroniseert alle objecten uit Azure AD, inclusief gebruikersaccounts die zijn gemaakt in een on-premises AD DS-omgeving. Een *resourceforest* synchroniseert alleen gebruikers en groepen die rechtstreeks in Azure AD zijn gemaakt. Resourceforests zijn momenteel in preview. Zie overzicht van [Azure AD DS-bronforests][resource-forests]voor meer informatie over *resourceforests,* waaronder waarom u er een gebruiken en hoe u forestvertrouwensrelaties maken met on-premises AD DS-domeinen.
    ...
    ![Basisinstellingen configureren voor een Azure AD Domain Services-exemplaar](./media/tutorial-create-instance-advanced/basics-window.png)

1. Als u extra opties handmatig wilt configureren, kiest u **Volgende - Netwerken**. Selecteer anders **Controleren + maken** om de standaardconfiguratieopties te accepteren en ga vervolgens naar de sectie naar Uw [beheerde domein implementeren.](#deploy-the-managed-domain) De volgende standaardinstellingen worden geconfigureerd wanneer u deze optie voor het maken kiest:

    * Hiermee maakt u een virtueel netwerk met de naam *aadds-vnet* dat gebruikmaakt van het IP-adresbereik van *10.0.1.0/24*.
    * Hiermee maakt u een subnet met de naam *aadds-subnet* met het IP-adresbereik van *10.0.1.0/24*.
...
Om connectiviteit te bieden, zijn een virtueel Azure-netwerk en een speciaal subnet nodig. Azure AD DS is ingeschakeld in dit virtuele netwerksubnet. In deze zelfstudie maakt u een virtueel netwerk, maar u er in plaats daarvan voor kiezen om een bestaand virtueel netwerk te gebruiken. In beide benaderingen moet u een speciaal subnet maken voor gebruik door Azure AD DS.

Enkele overwegingen voor dit speciale virtuele netwerksubnet zijn de volgende gebieden:

* Het subnet moet ten minste 3-5 beschikbare IP-adressen in het adresbereik hebben om de Ad DS-bronnen van Azure te ondersteunen.
* Selecteer niet het subnet *Gateway* voor het implementeren van Azure AD DS. Azure AD DS wordt niet in een *gatewaysubnet* geïmplementeerd.
* Implementeer geen andere virtuele machines op het subnet. Toepassingen en VM's maken vaak gebruik van netwerkbeveiligingsgroepen om de connectiviteit te beveiligen. Als u deze workloads in een apart subnet uitvoert, u deze netwerkbeveiligingsgroepen toepassen zonder de verbinding met uw beheerde domein te verstoren.
* U uw beheerde domein niet verplaatsen naar een ander virtueel netwerk nadat u Azure AD DS hebt ingeschakeld.

Zie [netwerkoverwegingen voor Azure Active Directory Domain Services voor][network-considerations]meer informatie over het plannen en configureren van het virtuele netwerk.

Vul de velden in het *netwerkvenster* als volgt in:

1. Kies **op** de pagina Netwerk een virtueel netwerk om Azure AD DS in te zetten in het vervolgkeuzemenu of selecteer **Nieuw maken**.
    1. Als u ervoor kiest een virtueel netwerk te maken, voert u een naam in voor het virtuele netwerk, zoals *myVnet,* en geeft u een adresbereik op, zoals *10.0.1.0/24*.
    1. Maak een speciaal subnet met een duidelijke naam, zoals *DomainServices.* Geef een adresbereik op, zoals *10.0.1.0/24*.
    ...
    Zorg ervoor dat u een adresbereik kiest dat binnen uw privé-IP-adresbereik valt. IP-adresbereiken waarvan u niet de eigenaar bent en die zich in de openbare adresruimte bevinden, veroorzaken fouten in Azure AD DS.

1. Selecteer een virtueel netwerksubnet, zoals *DomainServices*.
1. Kies Volgende **- Administratie**wanneer u klaar bent.

## <a name="configure-an-administrative-group"></a>Een beheergroep configureren

Een speciale beheergroep met de naam *AAD DC Administrators* wordt gebruikt voor beheer van het Azure AD DS-domein. Leden van deze groep krijgen administratieve machtigingen voor VM's die zijn verbonden met het domein van het beheerde domein. Op door het domein verbonden VM's wordt deze groep toegevoegd aan de groep lokale beheerders. Leden van deze groep kunnen ook Extern bureaublad gebruiken om op afstand verbinding te maken met vm's die zijn verbonden met domeinen.

U hebt geen machtigingen *voor domeinbeheerder* of *bedrijfsbeheerder* op een beheerd domein met Azure AD DS. Deze machtigingen worden gereserveerd door de service en worden niet beschikbaar gesteld aan gebruikers binnen de tenant. In plaats daarvan u met de groep *AAD DC-beheerders* een aantal bevoorrechte bewerkingen uitvoeren. Deze bewerkingen omvatten het behoren tot de beheergroep op met domeinen verbonden VM's en het configureren van groepsbeleid.

De wizard maakt automatisch de groep *AAD DC-beheerders* in uw Azure AD-map. Als u een bestaande groep met deze naam in uw Azure AD-map hebt, selecteert de wizard deze groep. U er optioneel voor kiezen om extra gebruikers toe te voegen aan deze *aaddc-beheerdersgroep* tijdens het implementatieproces. Deze stappen kunnen later worden voltooid.

1. Als u extra gebruikers wilt toevoegen aan deze groep *AAD DC-beheerders,* selecteert u **Groepslidmaatschap beheren**.

    ![Groepslidmaatschap configureren van de groep AAD DC-beheerders](./media/tutorial-create-instance-advanced/admin-group.png)

1. Selecteer de knop **Leden toevoegen** en zoek vervolgens naar en selecteer gebruikers uit uw Azure AD-map. Zoek bijvoorbeeld naar uw eigen account en voeg deze toe aan de groep *AAD DC-beheerders.*
1. Wijzig of voeg desgewenst extra ontvangers voor meldingen toe wanneer er waarschuwingen zijn in het beheerde Azure AD DS-domein waarvoor aandacht nodig is.
1. Kies Volgende **- Synchronisatie wanneer**u klaar bent.

## <a name="configure-synchronization"></a>Synchronisatie configureren

Met Azure AD DS u *alle* gebruikers en groepen synchroniseren die beschikbaar zijn in Azure AD of een *scoped* synchronisatie van alleen specifieke groepen. Als u ervoor kiest om *alle* gebruikers en groepen te synchroniseren, u er niet later voor kiezen om alleen een scoped synchronisatie uit te voeren. Zie [Azure AD Domain Services scoped synchronization][scoped-sync]voor meer informatie over scoped synchronization.

1. Kies voor deze zelfstudie om **Alle** gebruikers en groepen te synchroniseren. Deze synchronisatiekeuze is de standaardoptie.
...
1. Het inrichten van uw beheerde domein kan tot een uur duren. Er wordt een melding weergegeven in de portal die de voortgang van uw Azure AD DS-implementatie weergeeft. Selecteer de melding om de gedetailleerde voortgang voor de implementatie te bekijken.
    ...
    ![Melding in de Azure-portal van de lopende implementatie](./media/tutorial-create-instance-advanced/deployment-in-progress.png)
...
1. Selecteer uw resourcegroep, zoals *myResourceGroup,* en kies vervolgens uw Azure AD DS-exemplaar in de lijst met Azure-bronnen, zoals *aaddscontoso.com*. Op het tabblad **Overzicht** ziet u dat het beheerde domein momenteel *wordt geïmplementeerd.* U het beheerde domein pas configureren als het volledig is ingericht.
    ...
    ![Status domeinservices tijdens de indpositie](./media/tutorial-create-instance-advanced/provisioning-in-progress.png)
...
1. Wanneer het beheerde domein volledig is ingericht, wordt op het tabblad **Overzicht** de domeinstatus weergegeven als *Actief*.
    ...
    ![Status domeinservices eenmaal ingericht](./media/tutorial-create-instance-advanced/successfully-provisioned.png)
...
1. Op het tabblad **Overzicht** voor uw beheerde domein worden enkele **vereiste configuratiestappen**weergegeven. De eerste configuratiestap is het bijwerken van DNS-serverinstellingen voor uw virtuele netwerk. Zodra de DNS-instellingen correct zijn geconfigureerd, wordt deze stap niet meer weergegeven.
    ...
    De vermelde adressen zijn de domeincontrollers voor gebruik in het virtuele netwerk. In dit voorbeeld zijn deze adressen *10.1.0.4* en *10.1.0.5*. U deze IP-adressen later vinden op het tabblad **Eigenschappen.**
