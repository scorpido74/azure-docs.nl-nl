---
title: Controle - Azure Database voor MySQL
description: In dit artikel worden de statistieken beschreven voor het bewaken en waarschuwen voor Azure Database voor MySQL, waaronder CPU-, opslag- en verbindingsstatistieken.
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: conceptual
...
ms.locfileid: "79537088"
---
# <a name="monitoring-in-azure-database-for-mysql"></a>Controle in Azure Database voor MySQL
Door gegevens over uw servers te bewaken, u problemen oplossen en optimaliseren voor uw werkbelasting. Azure Database voor MySQL biedt verschillende statistieken die inzicht geven in het gedrag van uw server.

## <a name="metrics"></a>Metrische gegevens
Alle Azure-statistieken hebben een frequentie van één minuut en elke statistiek biedt 30 dagen geschiedenis. U waarschuwingen configureren op de statistieken. Zie [Waarschuwingen instellen](howto-alert-on-metric.md)voor stapsgewijze richtlijnen. Andere taken zijn het instellen van geautomatiseerde acties, het uitvoeren van geavanceerde analyses en het archiveren van geschiedenis. Zie het overzicht [van Azure Metrics](../monitoring-and-diagnostics/monitoring-overview-metrics.md)voor meer informatie.

### <a name="list-of-metrics"></a>Lijst met statistieken
Deze statistieken zijn beschikbaar voor Azure Database voor MySQL:
...
|seconds_behind_master|Replicatievertraging in seconden|Count|Het aantal seconden dat de replicaserver achterloopt op de hoofdserver.|
...
U langzame query- en controlelogboekregistratie op uw server inschakelen. Deze logboeken zijn ook beschikbaar via Azure Diagnostic Logs in Azure Monitor logs, Event Hubs en Storage Account. Ga voor meer informatie over logboekregistratie naar de [controlelogboeken](concepts-audit-logs.md) en artikelen [voor slow querylogboeken.](concepts-server-logs.md)

## <a name="query-store"></a>Query Store
[Query Store](concepts-query-store.md) is een functie die de queryprestaties in de loop van de tijd bijhoudt, inclusief queryruntimestatistieken en wachtgebeurtenissen. De functie blijft queryruntime-prestatiegegevens in het **mysql-schema** bevatten. U het verzamelen en opslaan van gegevens regelen via verschillende configuratieknoppen.

## <a name="query-performance-insight"></a>Inzicht in queryprestaties
[QueryPerformance Insight](concepts-query-performance-insight.md) werkt samen met de Query Store om visualisaties te bieden die toegankelijk zijn vanuit de Azure-portal. Met deze grafieken u belangrijke query's identificeren die van invloed zijn op de prestaties. Query Performance Insight is toegankelijk in de sectie **Intelligent Performance** van de portalpagina van uw Azure Database for MySQL-server.
...
- Zie [Waarschuwingen instellen](howto-alert-on-metric.md) voor richtlijnen voor het maken van een waarschuwing op een statistiek.
...
- Lees onze blog over [best practices voor het bewaken van uw server.](https://azure.microsoft.com/blog/best-practices-for-alerting-on-metrics-with-azure-database-for-mysql-monitoring/)
