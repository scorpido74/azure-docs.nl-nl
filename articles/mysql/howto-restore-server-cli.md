---
title: Back-up maken en herstellen - Azure CLI - Azure Database voor MySQL
description: Meer informatie over het maken van back-ups en het herstellen van een server in Azure Database voor MySQL met behulp van de Azure CLI.
author: ajlam
ms.author: andrela
ms.service: mysql
ms.devlang: azurecli
...
---
# <a name="how-to-back-up-and-restore-a-server-in-azure-database-for-mysql-using-the-azure-cli"></a>Een back-up maken en een server herstellen in Azure Database voor MySQL met azure cli
...
Tijdens het maken `az mysql server create` van een `--geo-redundant-backup` server via de opdracht bepaalt de parameter de optie Back-upredundantie. Als `Enabled`, geo redundante back-ups worden genomen. Of `Disabled` als lokaal redundante back-ups worden genomen. 

De back-upbewaarperiode `--backup-retention`wordt ingesteld door de parameter . 

Zie de [Azure Database voor MySQL-server CLI Quickstart voor](quickstart-create-mysql-server-database-using-azure-cli.md)meer informatie over het instellen van deze waarden tijdens het maken.
...
Als u de server wilt herstellen, gebruikt u de opdracht Azure CLI [az mysql-serverrestore.](/cli/azure/mysql/server#az-mysql-server-restore)
...
De `az mysql server restore` opdracht vereist de volgende parameters:
...
Als u een server wilt maken met `az mysql server georestore` een georedundante back-up, gebruikt u de opdracht Azure CLI.
...
De `az mysql server georestore` opdracht vereist de volgende parameters:
