---
title: 'Zelfstudie: Een server ontwerpen - Azure-portal - Azure Database voor MySQL'
description: In deze zelfstudie wordt uitgelegd hoe u Azure Database voor MySQL-server en -database maakt en beheert met Azure-portal.
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: tutorial
...
2. Selecteer **Azure** > **Database databases voor MySQL**. Als u MySQL Server niet vinden onder de categorie **Databases,** klikt u op **Alles weergeven** om alle beschikbare databaseservices weer te geven. U kunt ook **Azure Database voor MySQL** in het zoekvak typen om de service snel te vinden.
   
   ![Navigeren naar MySQL](./media/tutorial-design-database-using-portal/1-Navigate-to-MySQL.png)

3. Klik op Azure Database voor de tegel **MySQL.** Vul het Azure Database for MySQL-formulier in.
...
In dit voorbeeld wordt de servernaam *mydemoserver.mysql.database.azure.com*en is de login van de serverbeheerder *myadmin\@mydemoserver*.
...
Zodra u bent verbonden met de server, maakt u een lege database om mee te werken.
...
2. Vul het **formulier Herstellen** in met de vereiste informatie.
...
3. Klik op **OK** om de server te herstellen [om te herstellen naar een tijdstip](./howto-restore-server-portal.md) voordat de tabel is verwijderd. Als u een server herstelt, wordt er een nieuwe kopie van de server gemaakt, vanaf het tijdstip dat u opgeeft.
