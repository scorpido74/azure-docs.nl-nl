...
De POST-URI heeft `{subscriptionId}`, `{vaultName}`, `{vaultresourceGroupName}`en `{fabricName}` para meters. In ons voor beeld is de waarde voor de verschillende para meters als volgt:
