...
description: Een inleiding tot het maken van back-ups van SQL Server data bases met behulp van Azure Backup Server
ms.reviewer: kasinh
author: dcurwin
manager: carmonm
ms.service: backup
...
3. MABS toont het Start scherm met de richt lijnen voor het maken van een **beveiligings groep**. Klik op **Volgende**.
4. Selecteer **servers**.
...
8. Klik op **Volgende**
    ...
    MABS toont de totale beschik bare opslag ruimte en het mogelijke schijfruimte gebruik.

    ![Schijf toewijzing](./media/backup-azure-backup-sql/pg-storage.png)

    MABS maakt standaard één volume per gegevens bron (SQL Server-Data Base) die wordt gebruikt voor de eerste back-upkopie. Met deze benadering beperkt de Logical Disk Manager (LDM) MABS-beveiliging tot 300-gegevens bronnen (SQL Server-data bases). Als u deze beperking wilt omzeilen, selecteert u de optie **gegevens samen voegen in DPM-opslag groep**. Als u deze optie gebruikt, gebruikt MABS één volume voor meerdere gegevens bronnen, waarmee MABS Maxi maal 2000 SQL-data bases kan beveiligen.

    Als **de optie volumes automatisch verg Roten** is geselecteerd, kan MABS het verhoogde back-upvolume accounts maken als de productie gegevens groeien. Als **de optie volumes automatisch verg Roten** is niet geselecteerd, beperkt MABS de back-upopslag die wordt gebruikt voor de gegevens bronnen in de beveiligings groep.
...
10. Kies wanneer u wilt dat de consistentie controle wordt uitgevoerd en klik op **volgende**.
    ...
    MABS kan een consistentie controle uitvoeren om de integriteit van het back-uppunt te controleren. De controlesom van het back-upbestand wordt berekend op de productie server (SQL Server computer in dit scenario) en de gegevens van de back-up voor dat bestand op MABS. Als er een conflict optreedt, wordt ervan uitgegaan dat het back-upbestand op MABS beschadigd is. Met MABS wordt de back-upgegevens hersteld door de blokken te verzenden die overeenkomen met de controlesom die niet overeenkomt. Omdat de consistentie controle een prestatie-intensieve bewerking is, kunnen beheerders de consistentie controle plannen of automatisch uitvoeren.
