...
De agent wordt weer gegeven in de lijst met geïnstalleerde Program ma's. Ga naar **configuratie scherm** > **Program Ma's** > **Program ma's en onderdelen**om de lijst met geïnstalleerde Program ma's weer te geven.
...
Alle wijzigingen worden aangebracht in dit lokale Power shell-object ```$setting``` en vervolgens wordt het volledige object doorgevoerd naar DPM en Azure Backup om ze op te slaan met de cmdlet [set-DPMCloudSubscriptionSetting](https://docs.microsoft.com/powershell/module/dataprotectionmanager/set-dpmcloudsubscriptionsetting?view=systemcenter-ps-2019) . U moet de vlag ```–Commit``` gebruiken om ervoor te zorgen dat de wijzigingen behouden blijven. De instellingen worden niet toegepast en worden gebruikt door Azure Backup tenzij ze worden vastgelegd.
...
In het onderstaande voor beeld wordt met de eerste opdracht de teken reeks geconverteerd ```passphrase123456789``` naar een veilige teken reeks en wordt de beveiligde teken reeks toegewezen aan de variabele met de naam ```$Passphrase```. met de tweede opdracht wordt de beveiligde teken reeks in ```$Passphrase``` ingesteld als wacht woord voor het versleutelen van back-ups.
...
Op dit moment moet u alle vereiste wijzigingen in het ```$setting```-object hebben aangebracht. Vergeet niet de wijzigingen door te voeren.
...
Haal nu de lijst met gegevens bronnen op ```$server``` op met behulp van de cmdlet [Get-DPMDatasource](https://docs.microsoft.com/powershell/module/dataprotectionmanager/get-dpmdatasource?view=systemcenter-ps-2019) . In dit voor beeld wordt gefilterd op het volume *D:\\* die u wilt configureren voor back-up. Deze gegevens bron wordt vervolgens toegevoegd aan de beveiligings groep met behulp van de cmdlet [add-DPMChildDatasource](https://docs.microsoft.com/powershell/module/dataprotectionmanager/add-dpmchilddatasource?view=systemcenter-ps-2019) . Gebruik het *aanpas* bare beveiligings groeps object ```$MPG``` om de toevoegingen te maken.
...
In het bovenstaande voor beeld is ```$onlineSch``` een matrix met vier elementen die het bestaande schema voor online beveiliging voor de beveiligings groep in het GFS-schema bevat:
...
Als u de wekelijkse planning wilt wijzigen, moet u dus verwijzen naar het ```$onlineSch[1]```.
...
* de gegevens bronnen ophalen die overeenkomen met de ```$PG[0]```
...
* De matrix met back-uppunten ophalen met behulp van de cmdlet ```Get-DPMRecoveryPoint```.
