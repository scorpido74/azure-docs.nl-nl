...
description: Overzicht en conceptuele informatie over Azure Active Directory, inclusief terminologie, welke licenties beschikbaar zijn en een lijst met bijbehorende functies met koppelingen voor meer informatie.
...
ms.locfileid: "79240784"
---
# <a name="what-is-azure-active-directory"></a>What is Azure Active Directory? (Engelstalig)

Azure Active Directory (Azure AD) is de cloudgebaseerde service voor identiteits- en toegangsbeheer van Microsoft, waarmee uw werknemers zich kunnen aanmelden en toegang krijgen tot bronnen in:
...
- **App-ontwikkelaars.** Als app-ontwikkelaar u Azure AD gebruiken als een op standaarden gebaseerde benadering voor het toevoegen van single sign-on (SSO) aan uw app, zodat deze kan werken met de reeds bestaande referenties van een gebruiker. Azure AD biedt ook API's waarmee u gepersonaliseerde app-ervaringen maken met behulp van bestaande organisatiegegevens. U kunt aan de slag als u zich aanmeldt voor een [gratis proefversie van Azure Active Directory Premium van dertig dagen](https://azure.microsoft.com/trial/get-started-active-directory/). Voor meer informatie kunt u ook [Azure Active Directory voor ontwikkelaars](../develop/index.yml) raadplegen.
...
Voor zakelijke Microsoft Online-services, zoals Office 365 of Microsoft Azure, hebt u Azure AD nodig om u aan te melden en voor de identiteitsbescherming. Als u zich abonneert op een Zakelijke Microsoft Online-service, krijgt u automatisch Azure AD met toegang tot alle gratis functies.

Als u uw Azure AD-implementatie wilt verbeteren, u ook betaalde mogelijkheden toevoegen door te upgraden naar Azure Active Directory Premium P1- of Premium P2-licenties. Azure AD-betaalde licenties zijn gebouwd bovenop uw bestaande gratis directory en bieden selfservice, verbeterde bewaking, beveiligingsrapportage en veilige toegang voor uw mobiele gebruikers.
...
>
>Azure Active Directory Premium P1 en Premium P2 worden momenteel niet ondersteund in China. Neem contact op met het [Azure Active Directory Forum](https://azure.microsoft.com/support/community/?product=active-directory)voor meer informatie over de AD-prijzen van Azure .

- **Azure Active Directory Free.** Biedt gebruikers- en groepsbeheer, on-premises adreslijstsynchronisatie, basisrapporten, zelfservicewachtwoordwijziging voor cloudgebruikers en eenmalige aanmelding in Azure, Office 365 en veel populaire SaaS-apps.

- **Azure Active Directory Premium P1.** Naast de gratis functies biedt P1 uw hybride gebruikers ook toegang tot zowel on-premises als cloudbronnen. P1 biedt tevens ondersteuning voor geavanceerd beheer, zoals dynamische groepen, selfservice voor groepsbeheer, Microsoft Identity Manager (een on-premises pakket voor identiteits- en toegangsbeheer) en cloudfuncties voor terugschrijven, die selfservice voor wachtwoordherstel voor on-premises gebruikers mogelijk maken.

- **Azure Active Directory Premium P2.** Naast de functies Gratis en P1 biedt P2 ook [Azure Active Directory Identity Protection](../identity-protection/overview-identity-protection.md) om op risico's gebaseerde voorwaardelijke toegang tot uw apps en kritieke bedrijfsgegevens en privileged identity [management](../privileged-identity-management/pim-getting-started.md) te bieden om beheerders en hun toegang tot bronnen te ontdekken, te beperken en te controleren en om just-in-time toegang te bieden wanneer dat nodig is.
...
Om Azure AD en de bijbehorende documentatie beter te begrijpen, raden we u aan de volgende voorwaarden te bekijken.

|Term of concept|Beschrijving|
|---------------|-----------|
|Identiteit| Iets dat geverifieerd kan worden. Een identiteit kan een gebruiker zijn met een gebruikersnaam en wachtwoord. Identiteiten omvatten ook toepassingen of andere servers waarvoor verificatie nodig is via geheime sleutels of certificaten.|
|Account| Een identiteit met gegevens die eraan zijn gekoppeld. U geen account hebben zonder een identiteit.|
...
|Globale Azure AD-beheerder|Deze beheerdersrol wordt automatisch toegewezen aan personen die de Azure AD-tenant hebben gemaakt. Globale beheerders kunnen alle beheerfuncties voor Azure AD uitvoeren en alle services die worden weergegeven in Azure AD, zoals Exchange Online, SharePoint Online en Skype voor Bedrijven Online. Er kunnen meerdere globale beheerders zijn, maar alleen globale beheerders kunnen beheerdersrollen aan gebruikers toewijzen (inclusief het toewijzen van andere globale beheerders).<br><br>**Opmerking**<br>Deze beheerdersrol wordt globale beheerder genoemd in de Azure-portal, maar wordt **bedrijfsbeheerder** genoemd in de Microsoft Graph API en Azure AD PowerShell.<br><br>Zie [Machtigingen van de rol beheerder in Azure Active Directory](../users-groups-roles/directory-assign-admin-roles.md) voor meer informatie over de verschillende beheerdersrollen.|
...
Nadat u uw Azure AD-licentie hebt gekozen, krijgt u toegang tot enkele of alle volgende functies voor uw organisatie:
...
|Privileged Identity Management (PIM)|Beheer, controleer en bewaak de toegang binnen uw organisatie. Deze functie omvat toegang tot bronnen in Azure AD en Azure en andere Microsoft Online Services, zoals Office 365 of Intune. Zie [Azure AD Privileged Identity Management](../privileged-identity-management/index.yml) voor meer informatie.|
