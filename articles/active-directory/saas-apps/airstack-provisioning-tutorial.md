...
5. Selecteer in de sectie **beheerders referenties** de `https://api-as.lenovosoftware.com/0/as/common/scim` invoer in de **Tenant-URL**. Voer de waarde voor het **scim-verificatie token** in die eerder is opgehaald in het **geheime token**. Klik op **verbinding testen** om te controleren of Azure AD verbinding kan maken met de luchtstack. Als de verbinding mislukt, zorg er dan voor dat het account van de netstack beheerders machtigingen heeft en probeer het opnieuw.
