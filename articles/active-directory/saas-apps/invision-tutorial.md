...
1. Voer op de pagina **eenmalige aanmelding** de volgende stappen uit:
    ...
    b. Voer in het tekstvak **naam** de naam bijvoorbeeld in `azureadsso`.
    ...
    f. Gebruik `Unspecified` in het tekstvak **naam-id-indeling** de **notatie naam-id**.

    g. Selecteer **SHA-256** in de vervolg keuzelijst voor het **hash-algoritme**.

    h. Voer de juiste naam in voor het label voor de **SSO-knop**.

    i. Zorg ervoor **dat just-in-time-inrichting is toegestaan** op.
