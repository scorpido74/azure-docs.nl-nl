...
5. Selecteer in de sectie **beheerders referenties** de `https://api.spaceiq.com/scim` invoer in de **Tenant-URL**. Voer de waarde voor het **scim-verificatie token** in die eerder is opgehaald in het **geheime token**. Klik op **verbinding testen** om te controleren of Azure AD verbinding kan maken met SpaceIQ. Als de verbinding mislukt, zorg er dan voor dat uw SpaceIQ-account beheerders machtigingen heeft en probeer het opnieuw.
