...
5. Selecteer in de sectie **beheerders referenties** de `https://api.promapp.com/api/scim` invoer in de **Tenant-URL**. Voer de waarde voor het **scim-verificatie token** in die eerder is opgehaald in het **geheime token**. Klik op **verbinding testen** om te controleren of Azure AD verbinding kan maken met Promapp. Als de verbinding mislukt, zorg er dan voor dat uw Promapp-account beheerders machtigingen heeft en probeer het opnieuw.
