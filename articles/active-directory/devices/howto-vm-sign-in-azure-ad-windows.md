...
If you choose to install and use the CLI locally, this article requires that you are running the Azure CLI version 2.0.31 or later. Voer az --version uit om de versie te zoeken. If you need to install or upgrade, see the article [Install Azure CLI](https://docs.microsoft.com/cli/azure/install-azure-cli).
...
1. In the **Select** field, select a user, group, service principal, or managed identity. Als u de beveiligings-principal niet in de lijst ziet staan, kunt u tekst typen in het vak **Selecteren** om te zoeken naar weergavenamen, e-mailadressen en object-id's.
...
1. Open a command prompt on the VM and verify these queries against the Instance Metadata Service (IMDS) Endpoint running on the Azure host returns:
   ...
   | Command to run | Verwachte uitvoer |
   | --- | --- |
