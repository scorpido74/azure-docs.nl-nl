...
> [!NOTE]
> Vervang `contoso.corp.com` in het volgende voor beeld door de naam van uw on-premises Active Directory domein.
...
- Controleer de huidige status door `dsregcmd /status` te typen in een opdracht venster en controleer of in zowel *AzureAdJoined* als *DomainJoined* *Ja*wordt weer gegeven.
...
Zorg ervoor dat er voldoende domein controllers zijn geïnstalleerd om te reageren op de tijd om uw resource aanvraag te verwerken. Controleer de uitvoer van `nltest /dsgetdc:contoso /keylist /kdc`om te controleren of u een domein controller kunt zien waarop de functie wordt uitgevoerd.
