---
title: Een API-connector toevoegen aan een gebruikers stroom
...
Aangepaste kenmerken kunnen worden gemaakt voor de gebruiker met de **indeling \<aad-extensions-app-id> extension_ \<AttributeName> _** . Uw API moet verwachten dat er claims in dezelfde geserialiseerde indeling worden ontvangen. Uw API kan claims in dezelfde indeling retour neren, maar zonder de `<aad-extensions-app-id>` . <!--TODO: update once fix is rolled out--> Zie voor meer informatie over aangepaste kenmerken [aangepaste kenmerken definiëren voor Self-service aanmeldingen](user-flow-add-custom-attributes.md).
...
- Meer informatie [over hoe uw API moet reageren](self-service-sign-up-add-api-connector.md#expected-response-types-from-the-web-api)
- Meer informatie over [hoe u een API-connector kunt inschakelen](api-connectors-overview.md#where-you-can-enable-an-api-connector-in-a-user-flow)
