...
Als u een samen vatting van uw configuratie-instellingen wilt weer geven, opent u het hulp programma Azure AD Connect en kiest u de extra taak met de naam: huidige configuratie weer geven of exporteren. Er wordt een snelle samen vatting van uw instellingen weer gegeven, samen met de mogelijkheid om de volledige configuratie van uw server te exporteren. 

Standaard worden de instellingen geëxporteerd naar **%ProgramData%\AADConnect**, maar u kunt ervoor kiezen om de instellingen op te slaan op een beveiligde locatie om te zorgen voor Beschik baarheid in het geval van een ramp. Instellingen worden geëxporteerd met de JSON-bestands indeling en moeten niet hand matig worden gemaakt of bewerkt om logische consistentie te garanderen. Een door de import hand gemaakte of bewerkte bestand wordt niet ondersteund en kan leiden tot onverwachte resultaten. 

## <a name="importing-azure-ad-connect-settings"></a>Azure AD Connect-instellingen importeren 

Ga als volgt te werk om eerder geëxporteerde instellingen te importeren:
 
1. Installeer **Azure AD Connect** op een nieuwe server. 
2. Selecteer de optie **aanpassen** na de **welkomst** pagina. 
3. Klik op **synchronisatie-instellingen importeren**. Blader naar het eerder geëxporteerde bestand met JSON-instellingen.  
4. Klik op **Install**.

![Onderdelen installeren](media/how-to-connect-import-export-config/import1.png)

> [!NOTE]
> U moet de instellingen op deze pagina negeren, zoals het gebruik van SQL Server in plaats van LocalDB of het gebruik van een bestaand service account in plaats van een standaard LEVERANCIERSPECIFIEKE, enzovoort. Deze instellingen worden niet geïmporteerd uit het configuratie-instellingen bestand, maar er zijn er informatie en vergelijkings doeleinden.

### <a name="import-installation-experience"></a>Installatie-ervaring importeren 

Het importeren van de installatie kan opzettelijk eenvoudig worden gehouden met minimale invoer van de gebruiker om de reproduceer baarheid van een bestaande server eenvoudig te bieden.  

Dit zijn de enige wijzigingen die tijdens de installatie-ervaring kunnen worden aangebracht. Alle andere wijzigingen kunnen na de installatie van de wizard Azure AD Connect worden aangebracht.: 
- **Azure Active Directory referenties**   – de account naam voor de Azure Global-beheerder die wordt gebruikt voor het configureren van de oorspronkelijke server wordt standaard voorgesteld en **moet**   worden gewijzigd als u informatie wilt synchroniseren met een nieuwe map.
- **Gebruikers aanmelding**   : de aanmeldings opties die zijn geconfigureerd voor de oorspronkelijke server worden standaard geselecteerd en vragen om referenties of andere informatie die nodig is tijdens de configuratie. In zeldzame gevallen is het mogelijk dat er een server met verschillende opties moet worden ingesteld om te voor komen dat het gedrag van de actieve server wordt gewijzigd. Klik anders op volgende om dezelfde instellingen te gebruiken. 
- Referenties voor on **-premises Directory**   – voor elke on-premises map die is opgenomen in uw synchronisatie-instellingen, moet u referenties opgeven voor het maken van een synchronisatie account of een vooraf gemaakte, aangepaste synchronisatie account opgeven. Deze procedure is identiek aan de ervaring voor schone installatie met de uitzonde ring u kunt geen mappen toevoegen of verwijderen. 
...
Als een bestaande server het beheer van instellingen niet ondersteunt, kunt u ervoor kiezen om de server in-place te upgraden of de instellingen te migreren voor gebruik op een nieuwe staging-server.  

Voor de migratie moet een Power shell-script worden uitgevoerd waarmee de bestaande instellingen worden geëxtraheerd voor gebruik in een nieuwe installatie.Deze methode wordt aanbevolen om de instellingen van de bestaande server te catalogiseren en vervolgens toe te passen op een nieuw geïnstalleerde tijdelijke server.Als u de instellingen voor de oorspronkelijke server met de nieuwe server vergelijkt, worden de wijzigingen tussen de servers snel gevisualiseerd.Volg altijd het certificerings proces van uw organisatie om ervoor te zorgen dat er geen aanvullende configuratie is vereist.  
