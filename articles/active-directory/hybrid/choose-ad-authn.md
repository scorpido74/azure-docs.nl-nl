...
Voor een niet-routeerbaar domein dat niet kan worden geverifieerd in azure AD, hebt u extra configuratie nodig om gebruikers-ID-aanmelding te implementeren. Deze vereiste wordt ook wel alternatieve aanmeldings-ID-ondersteuning genoemd. Zie [alternatieve aanmeldings-id configureren](/windows-server/identity/ad-fs/operations/configuring-alternate-login-id) voor beperkingen en vereisten. Als u ervoor kiest een multi-factor Authentication-provider met Federatie van derden te gebruiken, moet u ervoor zorgen dat de provider WS-Trust ondersteunt zodat apparaten kunnen deel nemen aan Azure AD.
