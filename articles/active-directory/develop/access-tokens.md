...
> [!IMPORTANT]
> Toegangs tokens worden gemaakt op basis van de *doel groep* van het token, wat betekent dat de toepassing die eigenaar is van de scopes in het token.  Zo kan een bron instelling `accessTokenAcceptedVersion` in het [app-manifest](reference-app-manifest.md#manifest-reference) naar `2` toestaan dat een client het v 1.0-eind punt aanroept om een v 2.0-toegangs token te ontvangen.  Op deze manier is het wijzigen van de [optionele claims](active-directory-optional-claims.md) voor het toegangs token voor uw client geen invloed op het toegangs token dat wordt ontvangen wanneer een token wordt aangevraagd voor `user.read`, dat eigendom is van de MS Graph-resource.  
...
| `x5t` | Tekenreeks | Werkt hetzelfde (in gebruik en waarde) als `kid`. `x5t` is een verouderde claim die alleen in v 1.0-toegangs tokens wordt verzonden voor compatibiliteits doeleinden. |
...
| `onprem_sid`| Teken reeks, in [sid-indeling](https://docs.microsoft.com/windows/desktop/SecAuthZ/sid-components) | In gevallen waarin de gebruiker een on-premises verificatie heeft, levert deze claim de SID. U kunt `onprem_sid` gebruiken voor autorisatie in oudere toepassingen.|
...
#### <a name="the-amr-claim"></a>De claim `amr`
...
De `alg`-claim geeft het algoritme aan dat is gebruikt voor het ondertekenen van het token, terwijl de `kid` claim duidt op de specifieke open bare sleutel die is gebruikt om het token te valideren.
...
* Bevat een `jwks_uri`, waarmee de locatie wordt opgegeven van de set open bare sleutels die worden gebruikt voor het ondertekenen van tokens. Het JSON-document op de `jwks_uri` bevat alle informatie over de open bare sleutel die op dat moment wordt gebruikt op dat specifieke moment. Uw app kan de claim `kid` in de JWT-header gebruiken om te selecteren welke open bare sleutel in dit document is gebruikt voor het ondertekenen van een bepaald token. Vervolgens kan de hand tekening worden gevalideerd met de juiste open bare sleutel en het aangegeven algoritme.

> [!NOTE]
> Het eind punt v 1.0 retourneert zowel het `x5t`-als het `kid`-claim, terwijl het v 2.0-eind punt antwoordt met alleen de `kid`-claim. We raden u aan om de `kid`-claim te gebruiken om uw token te valideren.
...
Als uw app aangepaste handtekening sleutels heeft als gevolg van het gebruik van de functie voor het [toewijzen van claims](active-directory-claims-mapping.md) , moet u een para meter voor de `appid`-query met de app-id toevoegen om een `jwks_uri` te krijgen met de handtekening sleutel gegevens van uw app, die moeten worden gebruikt voor de validatie. Bijvoorbeeld: `https://login.microsoftonline.com/{tenant}/.well-known/openid-configuration?appid=6731de76-14a6-49ae-97bc-6eba6914391e` bevat een `jwks_uri` van `https://login.microsoftonline.com/{tenant}/discovery/keys?appid=6731de76-14a6-49ae-97bc-6eba6914391e`.
...
* Controleer de claim `scp` of `roles` om te controleren of alle huidige bereiken overeenkomen met die van uw API, en laat de client de aangevraagde actie uitvoeren.
* Zorg ervoor dat de aanroepende client uw API mag aanroepen met behulp van de claim `appid`.
* Valideer de verificatie status van de aanroepende client met `appidacr`. Dit mag niet 0 zijn als open bare clients uw API niet mogen aanroepen.
* Controleer op een lijst met `nonce` claims om te controleren of het token niet opnieuw wordt afgespeeld.
* Controleer of de `tid` overeenkomt met een Tenant die uw API mag aanroepen.
* Gebruik de claim `acr` om te controleren of de gebruiker MFA heeft uitgevoerd. Dit moet worden afgedwongen met behulp van [voorwaardelijke toegang](https://docs.microsoft.com/azure/active-directory/conditional-access/overview).
* Als u het `roles`-of `groups`-claim in het toegangs token hebt aangevraagd, controleert u of de gebruiker zich in de groep bevindt die deze actie mag uitvoeren.
...
* De claims `sub` en `oid` zijn hetzelfde. 
