...
Als u tokens wilt verkrijgen voor een toepassing die v 1.0-tokens accepteert (bijvoorbeeld de Microsoft Graph-API, dat https://graph.microsoft.com)wil zeggen, moet u eerst `scopes` een gewenste resource-id met een gewenste OAuth2-machtiging voor die resource maken.
...
Als u wilt lezen en schrijven met MSAL.NET Azure Active Directory met behulp van de Microsoft Graphhttps://graph.microsoft.com/)-API (, maakt u een lijst met bereiken zoals in het volgende code fragment:
...
In ADAL.NET v2. X werden de vernieuwings tokens beschikbaar gemaakt, zodat u oplossingen kunt ontwikkelen voor het gebruik van deze tokens door ze in `AcquireTokenByRefreshToken` de cache op te slaan en de methoden te gebruiken die worden geleverd door ADAL 2. x.
