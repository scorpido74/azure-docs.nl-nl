...
>      - Selecteer `Register`.
> 1. Selecteer `iOS` **Manage** `Authentication`  > in de `Add Platform`sectie beheren.  > 
...
> 1. Alleen voor iOS, onder het hoofd knooppunt dict, vervangt `CFBundleURLSchemes` u door de ***bundel-id*** die u hebt ingevoerd in de portal.
>
>    ```xml
...
> [!NOTE]
> Als `UISceneDelegate` u in plaats van de code `UIApplicationDelegate`van Ios 13 + inschakelt, plaatst `scene:openURLContexts:` u deze in plaats daarvan in de call back (Zie [de documentatie van Apple](https://developer.apple.com/documentation/uikit/uiscenedelegate/3238059-scene?language=objc)).
...
> |---------|---------|
> | `scopes` | Bevat de bereiken die worden aangevraagd (dat wil zeggen `[ "user.read" ]` , voor Microsoft Graph `[ "<Application ID URL>/scope" ]` of voor aangepaste web-`api://<Application ID>/access_as_user`api's () |
> | `account` | Het account waarvoor een token wordt aangevraagd. Deze Quick start is over een enkele account toepassing. Als u een app met meerdere accounts wilt maken, moet u de logica definiëren om te bepalen welk account moet worden gebruikt voor token aanvragen `accountsFromDeviceForParameters:completionBlock:` met en de juiste door geven`accountIdentifier` |

## <a name="next-steps"></a>Volgende stappen

Raadpleeg de zelf studie voor iOS en macOS voor een volledige stapsgewijze hand leiding voor het bouwen van toepassingen, met inbegrip van een volledige uitleg van deze Snelstartgids.
...
> [Graph API zelf studie voor iOS en macOS bellen](https://docs.microsoft.com/azure/active-directory/develop/guidedsetups/active-directory-ios)
