...
Access_tokens kort in de weg staan en u moet ze vernieuwen nadat ze verlopen zijn om door te gaan met de toegang tot resources. U kunt dit doen door een andere `POST` aanvraag in te `/token` dienen bij het eind punt, `refresh_token` in plaats van `code`de.  Vernieuwings tokens zijn geldig voor alle machtigingen die uw client al toestemming heeft gegeven, waardoor een vernieuwings token dat is uitgegeven `scope=mail.read` op een aanvraag voor, kan worden gebruikt om een `scope=api://contoso.com/api/UseResource`nieuw toegangs token aan te vragen.
