...
* Richt uw app in met behulp van de verouderde portal voor Azure AD. Zie [een app registreren](quickstart-register-app.md) voor meer informatie
...
Wanneer u de rechten hebt ingesteld op de juiste wijze, ziet u een bestand in de projectmap met de titel `entitlements.plist` dat iets bevat dat er ongeveer als volgt uitziet:
...
Zodra u de sleutel hanger hebt ingeschakeld in elk van uw toepassingen en u klaar bent om SSO te gebruiken, vertelt u de identiteits-SDK over uw sleutel hanger met behulp van de volgende instelling in uw `ADAuthenticationSettings` met de volgende instelling:
...
```
Met de instelling `AD_CREDENTIALS_AUTO` kan de SDK proberen contact te maken met de Broker, `AD_CREDENTIALS_EMBEDDED` voor komt dat de SDK naar de Broker aanroept.
...
Deze omleidings-URI moet worden opgegeven in de registratie van uw app met behulp van de [Azure Portal](https://portal.azure.com/). Zie [integreren met Azure Active Directory](active-directory-how-to-integrate.md)voor meer informatie over Azure AD-App-registratie.
...
* Meer informatie over het [SAML-protocol voor eenmalige aanmelding](single-sign-on-saml-protocol.md)
