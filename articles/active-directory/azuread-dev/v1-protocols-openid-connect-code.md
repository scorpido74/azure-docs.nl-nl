...
Door de machtigings bereiken in de aanvraag in te `response_type=code+id_token`stellen en `authorize` te gebruiken, zorgt het eind punt ervoor dat de gebruiker heeft ingestemd `scope` met de machtigingen die zijn aangegeven in de query parameter en moet uw app een autorisatie code voor uitwisseling voor een toegangs token retour neren.
