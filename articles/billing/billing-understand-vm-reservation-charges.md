...
Voor gereserveerde capaciteit voor SQL-databases, zie [Inzicht in korting voor gereserveerde instanties in Azure](billing-understand-reservation-charges.md).
...
![Schermopname van één toegepaste reservering en twee gekoppelde VM-instanties](./media/billing-reserved-vm-instance-application/billing-reserved-vm-instance-application.png)
...
Voor meer informatie over de toepassing van uw Azure-reserveringen in uw gebruiksrapporten voor facturering, raadpleegt u [Inzicht in reserveringsgebruik](billing-understand-reserved-instance-usage-ea.md).

## <a name="reservation-discount-for-windows-vms"></a>Reserveringskorting voor Windows-VM's

Wanneer u Windows-VM-instanties uitvoert, wordt de reservering toegepast om de infrastructuurkosten te dekken. De toepassing van de reservering op de infrastructuurkosten voor Windows-VM’s werkt hetzelfde als voor niet-Windows-VM's. Voor Windows-software worden de kosten afzonderlijk gefactureerd op vCPU-basis. Zie [Windows-softwarekosten met reserveringen](billing-reserved-Instance-windows-software-costs.md). U kunt uw Windows-licentiekosten dekken met [Azure Hybrid Benefit for Windows Server](../virtual-machines/windows/hybrid-use-benefit-licensing.md).
