...
In dit artikel wordt ervan uitgegaan dat u de `aci-helloworld:v1` container installatie kopie die is opgeslagen in het REGI ster. In de voor beelden wordt de register naam *myContainerRegistry*gebruikt. Vervang door uw eigen register-en afbeeldings namen in latere stappen.
...
Verifieer eerst bij de Azure CLI met [AZ login][az-login], met behulp van de identiteit die u hebt geconfigureerd op de virtuele machine. Voor `<userID>`vervangt u de ID van de identiteit die u in een vorige stap hebt opgehaald. 

```azurecli
az login --identity --username <userID>
```

Verifieer vervolgens bij het REGI ster met [AZ ACR login][az-acr-login]. Wanneer u deze opdracht gebruikt, gebruikt de CLI het Active Directory-token dat is gemaakt tijdens het uitvoeren van `az login` om uw sessie naadloos te verifiëren met het container register. (Afhankelijk van de installatie van uw virtuele machine moet u deze opdracht en docker-opdrachten mogelijk uitvoeren met `sudo`.)

```azurecli
az acr login --name myContainerRegistry
```

Er moet een `Login succeeded` bericht worden weer gegeven. U kunt vervolgens `docker`-opdrachten uitvoeren zonder referenties op te geven. Voer bijvoorbeeld [docker-pull][docker-pull] uit om de `aci-helloworld:v1` installatie kopie te halen, waarbij u de naam van de aanmeldings server van het REGI ster opgeeft. De naam van de aanmeldings server bestaat uit de naam van de container register (alle kleine letters), gevolgd door `.azurecr.io`, bijvoorbeeld `mycontainerregistry.azurecr.io`.
