...
De Azure CLI-opdracht `az acr pack build` gebruikt het [`pack`](https://github.com/buildpack/pack) cli-hulp programma, van [Buildpacks](https://buildpacks.io/), om een app te bouwen en de installatie kopie naar een Azure container Registry te pushen. Deze functie biedt een optie om snel een container installatie kopie te bouwen op basis van de bron code van uw toepassing in node. js, Java en andere talen, zonder dat u een Dockerfile hoeft te definiëren.
...
Voer de opdracht [AZ ACR Pack build][az-acr-pack-build] uit om een container installatie kopie te bouwen en te pushen met behulp van de native Buildpacks van de Cloud. Terwijl de opdracht [AZ ACR build][az-acr-build] een installatie kopie van een Dockerfile-bron en gerelateerde code bouwt en pusht, met `az acr pack build` kunt u rechtstreeks een toepassings bron structuur opgeven.
