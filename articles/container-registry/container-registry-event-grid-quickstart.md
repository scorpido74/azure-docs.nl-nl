...
Implementeer vervolgens een container register in de resource groep met de volgende opdrachten. Voordat u de opdracht [AZ ACR Create][az-acr-create] uitvoert, stelt u `ACR_NAME` in op een naam voor het REGI ster. De naam moet uniek zijn binnen Azure en mag Maxi maal 5-50 alfanumerieke tekens bevatten.
...
Voer de volgende Azure CLI-opdracht uit om een container installatie kopie te bouwen op basis van de inhoud van een GitHub-opslag plaats. ACR-taken pushen automatisch een geslaagde installatie kopie naar uw REGI ster, waardoor de `ImagePushed` gebeurtenis wordt gegenereerd.
...
U hebt nu een installatie kopie naar het REGI ster gepusht en vervolgens verwijderd. Ga naar de Web-App van Event Grid viewer en u moet zowel `ImageDeleted` als `ImagePushed` gebeurtenissen zien. U ziet mogelijk ook een validatie gebeurtenis voor een abonnement die wordt gegenereerd door de opdracht uit te voeren in het gedeelte [Abonneren op register gebeurtenissen](#subscribe-to-registry-events) .
...
Gefeliciteerd. Als u de gebeurtenissen `ImagePushed` en `ImageDeleted` ziet, stuurt het REGI ster gebeurtenissen naar Event Grid en Event Grid stuurt deze gebeurtenissen door naar het eind punt van uw web-app.
