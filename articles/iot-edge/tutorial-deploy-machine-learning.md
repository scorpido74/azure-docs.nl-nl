...
3. Geef de volgende naam op voor de `Azure/ai-toolkit-iot-edge`github-opslag plaats:. Schakel het selectie vakje **openbaar** uit als u uw project privé wilt laten blijven. Selecteer **importeren**.
...
12. Voer de cellen in het notitie blok uit door ze te **Run** selecteren en uitvoeren `Shift + Enter`of ingedrukt te houden.
...
Mogelijk moet u voor verhoogde `sudo` machtigingen gebruiken om opdrachten uit te `iotedge` voeren. Als u zich afmeldt en opnieuw aanmeldt bij uw apparaat, worden uw machtigingen automatisch bijgewerkt.
