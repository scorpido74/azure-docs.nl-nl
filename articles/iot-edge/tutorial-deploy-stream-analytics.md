...
![Diagram-zelf studie architectuur: fase en implementatie van ASA-taak](./media/tutorial-deploy-stream-analytics/asa-architecture.png)
</center>
...
1. Wanneer de Stream Analytics-module wordt toegevoegd aan de lijst met modules, selecteert u de naam ervan om te zien hoe deze is gestructureerd en de instellingen ervan bij te werken op de pagina **IOT Edge module bijwerken** .

   Het tabblad **module-instellingen** bevat de **afbeeldings-URI** die verwijst naar een standaard Azure stream Analytics-installatie kopie. Deze ene installatie kopie wordt gebruikt voor elke Stream Analytics module die wordt geïmplementeerd op een IoT Edge apparaat.

   Op het tabblad **dubbele instellingen** voor de module wordt de JSON weer gegeven die de eigenschap Azure stream Analytics (ASA) met de naam **ASAJobInfo**definieert. De waarde van deze eigenschap verwijst naar de taakdefinitie in uw opslagcontainer. Met deze eigenschap wordt aangegeven hoe de Stream Analytics installatie kopie wordt geconfigureerd met uw specifieke taak Details.
...
1. Noteer de naam van de module Stream Analytics, omdat u deze in de volgende stap nodig hebt. vervolgens selecteert u **volgende: routes** om door te gaan.

1. Op het tabblad **routes** definieert u hoe berichten worden door gegeven tussen modules en de IOT hub. Berichten worden gemaakt met behulp van naam/waarde-paren. Vervang de standaard `route` `upstream` naam en-waarden door de paren die in de volgende tabel worden weer gegeven, de volgende naam/waarde-paren, waarbij exemplaren van _{modulenaam}_ worden vervangen door de naam van uw Azure stream Analytics-module.
...
1. Op het tabblad **controleren en maken** ziet u hoe de gegevens die u in de wizard hebt verstrekt, worden geconverteerd naar een JSON-implementatie manifest. Wanneer u klaar bent met het bekijken van het manifest, selecteert u **maken**.

1. U keert terug naar de pagina met details van het apparaat. Selecteer **vernieuwen**.  
