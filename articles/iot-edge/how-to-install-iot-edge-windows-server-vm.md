...
1. Een nieuwe virtuele machine maken:
   ...
   * Met deze opdracht wordt u gevraagd om een wacht woord, maar u kunt de optie `--admin-password` toevoegen om het eenvoudiger in een script in te stellen
   * De Windows Server Core-installatie kopie heeft alleen opdracht regel ondersteuning met extern bureau blad, dus als u de volledige bureaublad ervaring wilt, geeft u `MicrosoftWindowsServer:WindowsServer:2019-Datacenter-with-Containers:latest` op als de installatie kopie
