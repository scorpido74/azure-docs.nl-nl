...
IoT Edge Security daemon biedt een beheer-API, die wordt aangeroepen door de IoT Edge agent bij het maken/starten/stoppen/verwijderen van een IoT Edge module. De Security daemon slaat registraties op voor alle actieve modules. Deze registraties wijzen de identiteit van een module toe aan bepaalde eigenschappen van de module. Enkele voor beelden van deze eigenschappen zijn de proces-id (PID) van het proces dat wordt uitgevoerd in de container of de hash van de inhoud van de docker-container.
...
De beheer-API is een geautoriseerde API die alleen kan worden aangeroepen vanuit de IoT Edge-agent.  Sinds de IoT Edge beveiligings-daemon Boots traps en start de IoT Edge agent, kan hij een impliciete registratie voor de IoT Edge agent maken, nadat deze heeft verklaard dat er niet met de IoT Edge agent is geknoeid. Hetzelfde Attestation-proces dat door de workload API wordt gebruikt, beperkt ook de toegang tot de beheer-API tot alleen de IoT Edge-agent.
