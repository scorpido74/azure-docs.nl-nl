...
De Azure IoT Edge security manager is de kern van een goed begrensde beveiliging voor het beveiligen van de IoT Edge-apparaat en alle bijbehorende onderdelen door om de hardware van de beveiligde silicon te abstraheren. Security Manager is het brand punt voor beveiligings beveiliging en biedt technologie-integratie punt aan Original Equipment Manufacturers (OEM).
...
IoT Edge Security daemon biedt een beheer-API, die wordt aangeroepen door de IoT Edge agent bij het maken/starten/stoppen/verwijderen van een IoT Edge module. De Security daemon slaat registraties op voor alle actieve modules. Deze registraties toewijzen van een module identiteit aan enkele eigenschappen van de module. Voor beelden zijn deze module-eigenschappen bevatten de proces-id (PID) van het proces dat wordt uitgevoerd in de container en de hash van de inhoud van de docker-container.
