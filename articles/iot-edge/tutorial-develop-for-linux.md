...
In de Quick Start hebt u een IoT Edge apparaat gemaakt met behulp van een virtuele Linux-machine en een module geïmplementeerd vanuit Azure Marketplace. In deze zelf studie wordt uitgelegd hoe u uw eigen code ontwikkelt en implementeert op een IoT Edge apparaat. Dit artikel is een nuttige vereiste voor de andere zelf studies, die meer details over specifieke programmeer talen of Azure-Services gaan.

In deze zelf studie wordt het voor beeld van de implementatie van een **C#-module op een Linux-apparaat**gebruikt. Dit voor beeld is gekozen omdat het het meest voorkomende ontwikkelaars scenario is voor IoT Edge oplossingen. Ook als u van plan bent een andere taal te gebruiken of een Azure-service te implementeren, is deze zelf studie nog steeds nuttig voor meer informatie over de ontwikkel hulpprogramma's en-concepten. Voltooi deze inleiding in het ontwikkelings proces en kies vervolgens de gewenste taal of Azure-service om de details te bekijken.
...
In deze zelf studie worden Linux-apparaten met IoT Edge als doel gebruikt. U kunt uw favoriete besturings systeem gebruiken zolang de ontwikkel machine Linux-containers uitvoert. We raden u aan om Visual Studio code te gebruiken voor het ontwikkelen van Linux-apparaten, zodat deze zelf studie wordt gebruikt. U kunt ook Visual Studio gebruiken, hoewel er verschillen zijn in ondersteuning tussen de twee hulpprogram ma's.
...
IoT Edge-modules worden verpakt als containers, dus u hebt een container engine op uw ontwikkel computer nodig om deze te bouwen en te beheren. Het is aan te raden docker Desktop voor ontwikkeling te gebruiken vanwege de ondersteuning en populariteit van de functies. Met docker desktop in Windows kunt u scha kelen tussen Linux-containers en Windows-containers, zodat u eenvoudig modules kunt ontwikkelen voor verschillende soorten IoT Edge apparaten.
...
6. In het opdracht palet zoekt en selecteert u **Azure: Meld**u aan. Volg de aanwijzingen om u aan te melden bij uw Azure-account.
...
8. Open de sectie Explorer van Visual Studio code door het pictogram te selecteren in de activiteiten balk aan de linkerkant of door **weer gave** > **Verkenner**te selecteren.

9. Onder aan de sectie Explorer vouwt u het menu samengevouwen **Azure IOT Hub-apparaten** uit. Als het goed is, ziet u de apparaten en IoT Edge apparaten die zijn gekoppeld aan de IoT-hub die u hebt geselecteerd in het opdracht palet.
...
Voor deze zelf studie gebruiken we de sjabloon C#-module omdat dit de meestgebruikte sjabloon is.
...
* De **implementatie. debug. sjabloon. json** bestand en **implementatie. bestand sjabloon. json** zijn sjablonen die u helpen bij het maken van een implementatie manifest. Een *implementatie manifest* is een bestand dat precies aangeeft welke modules u wilt implementeren op een apparaat, hoe ze moeten worden geconfigureerd en hoe ze met elkaar en de Cloud kunnen communiceren. De sjabloon bestanden gebruiken verwijzingen voor bepaalde waarden. Wanneer u de sjabloon transformeert in een waar implementatie manifest, worden de aanwijzers vervangen door waarden die zijn overgenomen uit andere oplossings bestanden. Zoek de twee gebruikelijke tijdelijke aanduidingen in uw implementatie sjabloon:
  ...
  * In de sectie register referenties wordt het adres aangevuld met de informatie die u hebt ingevoerd tijdens het maken van de oplossing. De gebruikers naam en het wacht woord verwijzen echter naar de variabelen die zijn opgeslagen in het. env-bestand. Deze configuratie is voor beveiliging, omdat het. env-bestand Git wordt genegeerd, maar de implementatie sjabloon niet.
...
Momenteel kan Visual Studio code C#-modules ontwikkelen voor Linux AMD64-en ARM32v7-apparaten. U moet selecteren welke architectuur u wilt richten op elke oplossing, omdat dit van invloed is op hoe de container wordt gebouwd en wordt uitgevoerd. De standaard waarde is Linux AMD64.
...
De C#-voorbeeld code die wordt geleverd met de project sjabloon maakt gebruik van de [ModuleClient-klasse](https://docs.microsoft.com/dotnet/api/microsoft.azure.devices.client.moduleclient?view=azure-dotnet) van de IOT hub SDK voor .net.
...
8. Zoek de gewenste eigenschappen voor de module **$edgeHub** aan de onderkant van het bestand.
