---
title: Zelfstudie - Subnetverkeer routering configureren met Azure Traffic Manager
description: In deze zelfstudie wordt uitgelegd hoe u Traffic Manager configureert om verkeer van gebruikerssubnetten naar specifieke eindpunten te routeren.
...
---
# <a name="tutorial-direct-traffic-to-specific-endpoints-based-on-user-subnet-using-traffic-manager"></a>Zelfstudie: Direct verkeer naar specifieke eindpunten op basis van gebruikerssubnet met Traffic Manager
...
In deze sectie installeert u de IIS-server op de twee VM's - *myIISVMEastUS* & *myIISVMWestEurope*- en werkt u vervolgens de standaardwebsitepagina bij. De aangepaste websitepagina geeft de naam weer van de virtuele machine waarmee u verbinding maakt als u de website in een webbrowser bezoekt.
...
9. Herhaal stap 1-6 met door een RDP-verbinding te maken met de VM *myIISVMWestEurope* binnen de *myResourceGroupTM2-brongroep* om IIS te installeren en de standaardwebpagina aan te passen.
10. Start Windows PowerShell op *myIISVMWestEurope* VM en gebruik de volgende opdrachten om de IIS-server te installeren en het standaard htm-bestand bij te werken.
...
In deze sectie maakt u een VM (*myVMEastUS* en *myVMWestEurope*) in elke Azure-regio **(Oost-VS** en **West-Europa).** U gebruikt deze VM's om te testen hoe Traffic Manager gebruikersverkeer doorstuurt op basis van het subnet van de query van de gebruiker.
...
Voeg de twee VM's met de IIS-servers toe - *myIISVMEastUS* & *myIISVMWestEurope* om gebruikersverkeer te routeren op basis van het subnet van de query van de gebruiker.
...
3. Voer de volgende gegevens in of selecteer de volgende gegevens, accepteer de standaardinstellingen voor de overige instellingen en selecteer **OK:**
    ...
    |  Instellingen voor subnetroutering    |   Voeg het IP-adres van *myVMEastUS-testVM* toe. Elke gebruikersquery die afkomstig is van deze vm wordt doorgestuurd naar het *myInternalWebSiteEndpoint.*    |

4. Herhaal stap 2 en 3 om een ander eindpunt met de naam *myProdWebsiteEndpoint* toe te voegen voor het openbare IP-adres *myIISVMWestEurope-ip* dat is gekoppeld aan de IIS-server VM met de naam *myIISVMWestEurope*. Voeg voor **subnetrouteringsinstellingen**het IP-adres van de testVM - *myVMWestEurope*toe . Elke gebruikersquery van deze test-VM zal naar het eindpunt *myProdWebsiteEndpoint* worden gerouteerd.
...
4. Selecteer **OK**.
5. Er wordt mogelijk een certificaatwaarschuwing weergegeven tijdens het aanmelden. Als u de waarschuwing ontvangt, selecteert u **Ja** of **Doorgaan** om door te gaan met de verbinding.
6. Typ in een webbrowser op de VM *myVMEastUS* de DNS-naam van uw Traffic Manager-profiel om uw website weer te geven. Aangezien het VM *myVMEastUS* IP-adres is gekoppeld aan het eindpunt *myInternalWebsiteEndpoint,* lanceert de webbrowser de Test website server - *myIISVMEastUS*.

7. Maak vervolgens verbinding met de VM *myVMWestEurope* in **West-Europa** met stap 1-5 en blader naar de domeinnaam Traffic Manager-profiel van deze VM. Aangezien het VM *myVMWestEurope* IP-adres is gekoppeld aan het eindpunt *myProductionWebsiteEndpoint,* lanceert de webbrowser de Test website server - *myIISVMWestEurope*.
