...
Als u wilt weten of de Nic's van de virtuele Azure-machine ondersteuning bieden voor RSS, voert u de opdracht `Get-SmbClientNetworkInterface` als volgt uit en controleert u het veld `RSS Capable`: 
...
## <a name="is-advanced-networking-recommended"></a>Wordt Geavanceerd netwerk aanbevolen?

Voor maximale prestaties is het raadzaam om waar mogelijk geavanceerde netwerken te configureren. Houd rekening met de volgende aandachtspunten:  

* De Azure Portal maakt standaard gebruik van geavanceerde netwerken voor virtuele machines die deze functie ondersteunen.  Andere implementatie methoden, zoals Ansible en vergelijk bare configuratie hulpprogramma's, zijn echter niet mogelijk.  Fout bij het inschakelen van geavanceerde netwerken kan de prestaties van een machine hobble.  
* Als geavanceerd netwerk niet is ingeschakeld op de netwerk interface van een virtuele machine vanwege een gebrek aan ondersteuning voor een exemplaar type of-grootte, blijft het uitgeschakeld met grotere instantie typen. In die gevallen hebt u hand matige interventie nodig.
