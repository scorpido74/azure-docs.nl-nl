...
1. Als u een metrische grafiek wilt maken, selecteert u **metrische** naam en relevante **aggregatie** per metriek. De **resource** -en **metrische naam ruimte** -kiezers zijn vooraf geselecteerd voor uw Azure Data Explorer-cluster. Zie [ondersteunde Azure Data Explorer-metrische](#supported-azure-data-explorer-metrics)gegevens voor meer informatie over de verschillende gegevens.
1. Selecteer de knop **metriek toevoegen** om meerdere metrische gegevens weer te geven die in dezelfde grafiek zijn getekend.
1. Selecteer de knop **+ nieuwe grafiek** om meerdere grafieken in één weer gave te zien.
...
**Gegevens** | **Teleenheid** | **Aggregatie** | **Beschrijving van metrische gegevens**
|---|---|---|---|
Doorlopend export aantal geëxporteerde records    | Aantal | Sum | Totaal aantal records dat is geëxporteerd uit het cluster. |
Maximale achterstand voor continue export |    Aantal   | Max.   | De maximale waarde in minuten van geëxporteerde records.|
Aantal doorlopend exporteren in behandeling | Aantal | Max.   | De maximale waarde van de export bewerkingen in behandeling.
Resultaat doorlopend exporteren    | Aantal |   Aantal   | Het totale aantal doorlopende export bewerkingen, op basis van het resultaat. Metric bevat een doorlopende Export naam en-data base. 
Gebruik exporteren |    Procent | Max.   | Het gebruik van een gedefinieerde sleuf voor export bewerkingen.
| | | | |
...
**Gegevens** | **Teleenheid** | **Aggregatie** | **Beschrijving van metrische gegevens**
|---|---|---|---|
| Verwerkte gebeurtenissen (voor gebeurtenis/IoT-hubs) | Aantal | Max, min, Sum | Totaal aantal gebeurtenissen dat is gelezen vanuit Event hubs en verwerkt door het cluster. De gebeurtenissen worden gesplitst in gebeurtenissen die zijn afgewezen en gebeurtenissen die door de Cluster-Engine worden geaccepteerd. |
...
| Opname volume (in MB) | Aantal | Max, Sum | De totale grootte van de gegevens die zijn opgenomen in het cluster (in MB) vóór compressie. |
| | | | |  
...
**Gegevens** | **Teleenheid** | **Aggregatie** | **Beschrijving van metrische gegevens**
|---|---|---|---|
| Queryduur | Milliseconden | Gem, min, maximum, Sum | Totale tijd tot de query resultaten zijn ontvangen (geen netwerk latentie inbegrepen). |
...
**Gegevens** | **Teleenheid** | **Aggregatie** | **Beschrijving van metrische gegevens**
|---|---|---|---|
Gegevens frequentie van streaming opname |    Aantal   | RateRequestsPerSecond | Het totale volume van de gegevens die zijn opgenomen in het cluster. |
Opname duur van streaming   | Milliseconden  | Avg, Max, Min | Totale duur van alle aanvragen voor streaming-opname. |
Aanvraag frequentie voor streaming-opname   | Aantal | Aantal, Gem, Max, min, som | Totaal aantal aanvragen voor streaming-opname. |
Resultaat van streaming-opname | Aantal | Gemiddeld   | Totaal aantal streaming-opname aanvragen per resultaat type. |
| | | | |
