...
|-------------------------------|-------------|-----------------|
|Effectief cap-resource percentage | *Effectief cap resource percent* is een vaste limiet voor het percentage resources dat door de werkbelasting groep toegankelijk is, waarbij rekening wordt gehouden met een *effectief min resource percentage* dat is toegewezen aan andere werkbelasting groepen. De *werkelijke Cap-resource procentwaarde* wordt geconfigureerd met `CAP_PERCENTAGE_RESOURCE` behulp van de para meter in de syntaxis voor het maken van een [werkbelasting groep](/sql/t-sql/statements/create-workload-group-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest) .  De geldige waarde wordt hier beschreven.<br><br>Als een werkbelasting `DataLoads` groep bijvoorbeeld is gemaakt met `CAP_PERCENTAGE_RESOURCE` = 100 en een andere werkbelasting groep wordt gemaakt met een effectief min resource percentage van 25%, is het percentage van de `DataLoads` *effectief cap-resource* voor de werkbelasting groep 75%.<br><br>Het *resource percentage van de effectief cap* bepaalt de bovengrens van gelijktijdigheid (en dus mogelijk door Voer) die een werkbelasting groep kan behalen.  Als er nog een extra door Voer is vereist dan wat momenteel wordt gerapporteerd door het *effectiefste Cap* -percentage `CAP_PERCENTAGE_RESOURCE`voor resources, `MIN_PERCENTAGE_RESOURCE` verhoogt u de, verlaagt u de andere werkbelasting groepen of verkleint u het exemplaar om meer resources toe te voegen.  Het verminderen van `REQUEST_MIN_RESOURCE_GRANT_PERCENT` de kan gelijktijdigheid verhogen, maar kan de algemene door Voer niet verhogen.| Min, Gem, Max |
|Effectief min resource percentage |*Effectief min resource percentage* is het minimale percentage resources dat voor de werkbelasting groep is gereserveerd en geïsoleerd, waarbij rekening wordt gehouden met het minimum service niveau.  De werkelijke minimale resource procentuele waarde wordt geconfigureerd met behulp van de `MIN_PERCENTAGE_RESOURCE` para meter in de syntaxis [groep maken](/sql/t-sql/statements/create-workload-group-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest) .  De geldige waarde wordt [hier](/sql/t-sql/statements/create-workload-group-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest#effective-values)beschreven.<br><br>Gebruik het aggregatie type Sum wanneer deze metriek niet is gefilterd en ongesplitst om de totale isolatie van de werk belasting te bewaken die op het systeem is geconfigureerd.<br><br>Het *werkelijke minimale resource percentage* bepaalt de laagste grens van gegarandeerde gelijktijdigheid (en dus gegarandeerde door Voer) die een werkbelasting groep kan belasten.  Als er extra gegarandeerde resources nodig zijn dan wordt gerapporteerd door de *werkelijke minimale resource procentuele* waarde, verhoogt u `MIN_PERCENTAGE_RESOURCE` de para meter die is geconfigureerd voor de werkbelasting groep.  Het verminderen van `REQUEST_MIN_RESOURCE_GRANT_PERCENT` de kan gelijktijdigheid verhogen, maar kan de algemene door Voer niet verhogen. |Min, Gem, Max|
...
|Toewijzing van werkbelasting groep op Maxi maal resource percentage |Met deze metriek wordt het percentage toegewezen resources weer gegeven ten opzichte van het *resource percentage van de effectief cap* per werkbelasting groep.  Deze metrische gegevens bieden het effectief gebruik van de werkbelasting groep.<br><br>Overweeg een werkbelasting `DataLoads` groep met een *effectief cap-resource percentage* van 75% `REQUEST_MIN_RESOURCE_GRANT_PERCENT` en een geconfigureerd op 25%.  De *werkbelasting groep toegewezen door de maximale* waarde voor resource `DataLoads` percentage gefilterd op zou 33% zijn (25%/75%) Als er één query werd uitgevoerd in deze werkbelasting groep.<br><br>Gebruik deze metrische gegevens om het gebruik van een werkbelasting groep te identificeren.  Een waarde dicht bij 100% geeft aan dat alle resources die beschikbaar zijn voor de werkbelasting groep, worden gebruikt.  Daarnaast zou de *werkbelasting groep query's metrieken* voor dezelfde werkbelasting groep met een waarde die groter is dan nul zou aangeven dat de werkbelasting groep extra resources zou gebruiken als ze worden toegewezen.  Als deze metriek echter voortdurend laag is en de *werkbelasting groep actieve query's* laag is, wordt de werkbelasting groep niet gebruikt.  Deze situatie is vooral problematisch als een *effectief cap-resource percentage* groter is dan nul, omdat dit zou wijzen op [isolatie van werk belastingen](#underutilized-workload-isolation).|Min, Gem, Max |
|Toewijzing van werkbelasting groep per systeem percentage | Met deze metriek wordt het percentage toegewezen resources ten opzichte van het hele systeem weer gegeven.<br><br>Overweeg een werkbelasting `DataLoads` groep met `REQUEST_MIN_RESOURCE_GRANT_PERCENT` een configuratie die is ingesteld op 25%.  De waarde *van de werkbelasting groep per systeem percentage* gefilterd op `DataLoads` zou 25% zijn (25%/100%) Als er één query werd uitgevoerd in deze werkbelasting groep.|Min, Gem, Max |
|Time-outs query werkbelasting groep |Query's voor de werkbelasting groep waarvoor een time-out is opgetreden.  Querytime-time-outs die door deze metrische gegevens worden gerapporteerd, worden slechts eenmaal gestart nadat de query is uitgevoerd (de wacht tijd is niet opgenomen als gevolg van vergren deling of het wachten op de bron).<br><br>Querytime-out wordt geconfigureerd met `QUERY_EXECUTION_TIMEOUT_SEC` behulp van de para meter in de syntaxis voor het maken van een [werkbelasting groep](/sql/t-sql/statements/create-workload-group-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest) .  Het verhogen van de waarde kan het aantal querytime-outs verminderen.<br><br>U kunt de `REQUEST_MIN_RESOURCE_GRANT_PERCENT` para meter voor de werkbelasting groep verhogen om de hoeveelheid time-outs te verminderen en meer resources per query toe te wijzen.  Houd er rekening `REQUEST_MIN_RESOURCE_GRANT_PERCENT` mee dat de hoeveelheid gelijktijdigheid voor de werkbelasting groep wordt verminderd. |Sum |
|Werkbelasting groep in wachtrij geplaatste query's | Query's voor de werkbelasting groep die momenteel in de wachtrij staan om de uitvoering te starten.  Query's kunnen een wachtrij zijn omdat ze wachten op resources of vergren delingen.<br><br>Query's kunnen om talloze redenen worden gewacht.  Als het systeem overbelast is en de gelijktijdigheids vraag groter is dan beschikbaar is, worden query's in de wachtrij geplaatst.<br><br>Overweeg om meer resources aan de werkbelasting groep toe te `CAP_PERCENTAGE_RESOURCE` voegen door de para meter in de instructie [Create workload Group](/sql/t-sql/statements/create-workload-group-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest) te verhogen.  Als `CAP_PERCENTAGE_RESOURCE` dit groter is dan het *werkelijke Cap-percentage* van de resource, is de geconfigureerde werk belasting isolatie voor een andere werkbelasting groep van invloed op de resources die zijn toegewezen aan deze werkbelasting groep.  Overweeg om andere `MIN_PERCENTAGE_RESOURCE` werkbelasting groepen te verlagen of het exemplaar omhoog te schalen om meer resources toe te voegen. |Sum |
...
Houd rekening met de volgende groeps beleidsobjecten en classificatie configuratie `wgPriority` waarbij een werkbelasting groep met de naam wordt gemaakt `wcCEOPriority` en *TheCEO* `membername` wordt toegewezen aan de werk belasting met behulp van de classificatie werk belasting.  Voor `wgPriority` de werkbelasting groep is 25% werk belasting isolatie geconfigureerd`MIN_PERCENTAGE_RESOURCE` (= 25).  Elke query die wordt verzonden door *TheCEO* , krijgt 5% van de`REQUEST_MIN_RESOURCE_GRANT_PERCENT` systeem bronnen (= 5).
...
Metriek 1: *effectief min resource percentage* (gem. `blue line`aggregatie)<br>
Metriek 2: *toewijzing van werkbelasting groepen op systeem percentage* (gem. `purple line`aggregatie)<br>
...
Houd rekening met de volgende werkbelasting groep en classificatie configuratie waarbij een `wgDataAnalyst` werkbelasting groep met de naam wordt gemaakt en de *DataAnalyst* `membername` wordt toegewezen aan deze met behulp van de classificatie van de `wcDataAnalyst` werk belasting.  Voor `wgDataAnalyst` de werkbelasting groep is 6% isolatie van de werk`MIN_PERCENTAGE_RESOURCE` belasting geconfigureerd (= 6) en een resource limiet van`CAP_PERCENTAGE_RESOURCE` 9% (= 9).  Elke query die wordt verzonden door de *DataAnalyst* , krijgt 3% systeem bronnen`REQUEST_MIN_RESOURCE_GRANT_PERCENT` (= 3).
...
De onderstaande grafiek wordt als volgt geconfigureerd:<br>
Metriek 1: *percentage van effectief cap-resource* (gem. `blue line`aggregatie)<br>
Metriek 2: *toewijzing van werkbelasting groepen op Maxi maal resource percentage* (gem `purple line`. aggregatie)<br>
...
![met fles hals-WG](./media/sql-data-warehouse-workload-management-portal-monitor/bottle-necked-wg.png) in het diagram ziet u dat de werkbelasting groep met een limiet van 9% op resources 90% + wordt gebruikt (van de *werkbelasting groep toewijzing op basis van het percentage metrische resources*).  Er is een stabiele wachtrij met query's, zoals wordt weer gegeven in de *groep workload query's*in de wachtrij.  In dit geval is het verhogen `CAP_PERCENTAGE_RESOURCE` van de naar een hogere waarde dan 9% dat er meer query's gelijktijdig kunnen worden uitgevoerd.  Het verg `CAP_PERCENTAGE_RESOURCE` Roten van de veronderstelt dat er voldoende bronnen beschikbaar zijn en niet worden geïsoleerd door andere werkbelasting groepen.  Controleer of de limiet is verhoogd door de *werkelijke Cap-waarde voor het resource percentage*te controleren.  Als u meer door voer wilt doen, kunt u `REQUEST_MIN_RESOURCE_GRANT_PERCENT` ook overwegen om de waarde te verhogen naar een hoger dan 3.  Het verg `REQUEST_MIN_RESOURCE_GRANT_PERCENT` Roten van het uitvoeren van query's kan leiden tot snellere uitvoering.
