...
- Synapse SQL: Complete T-SQL gebaseerde analytics 
...
Synapse SQL maakt gebruik van een node-gebaseerde architectuur. Toepassingen verbinden en geven T-SQL-opdrachten uit met een Control-knooppunt, het enige toegangspunt voor een Synapse SQL-groep. Het control-knooppunt voert de MPP-engine uit, die query's optimaliseert voor parallelle verwerking en vervolgens bewerkingen doorgeeft aan Compute-knooppunten om hun werk parallel te doen. 
...
Een distributie is de basiseenheid voor opslag en verwerking van parallelle query's die op gedistribueerde gegevens worden uitgevoerd. Wanneer een query wordt uitgevoerd, wordt het werk verdeeld in 60 kleinere query's die parallel worden uitgevoerd. 
