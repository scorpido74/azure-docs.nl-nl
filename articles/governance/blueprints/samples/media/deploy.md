---
title: Voorbeeld van mediablauwdruk implementeren
description: Stappen implementeren voor het voorbeeld van de blauwdruk van de media, inclusief details van de parameterparameters voor blauwdrukken.
ms.date: 02/25/2020
ms.topic: sample
...
---
# <a name="deploy-the-media-blueprint-sample"></a>Het voorbeeld van de mediablauwdruk implementeren

Als u het voorbeeld van de mediablauwdruk wilt implementeren, moeten de volgende stappen worden genomen:
...
1. Zoek het voorbeeld van de **blauwdruk media** onder _Andere voorbeelden_ en selecteer Dit **voorbeeld gebruiken**.
...
|\[Voorbeeld:\]Besturingselementen voor media controleren en specifieke VM-extensies implementeren om controlevereisten te ondersteunen |Beleidstoewijzing |Log Analytics-werkruimte-id waarvoor VM's moeten worden geconfigureerd |Dit is de ID (GUID) van de Log Analytics-werkruimte waarvoor de VM's moeten worden geconfigureerd. |
|\[Voorbeeld:\]Besturingselementen voor media controleren en specifieke VM-extensies implementeren om controlevereisten te ondersteunen |Beleidstoewijzing |Lijst met resourcetypen waarmee diagnostische logboeken zijn ingeschakeld |Lijst met resourcetypen om te controleren of diagnostische logboekinstelling niet is ingeschakeld. Acceptabele waarden zijn te vinden op [azure monitor diagnostische logboekenschema's.](../../../../azure-monitor/platform/diagnostic-logs-schema.md#supported-log-categories-per-resource-type) |
|\[Voorbeeld:\]Besturingselementen voor media controleren en specifieke VM-extensies implementeren om controlevereisten te ondersteunen |Beleidstoewijzing |Beheerdersgroep |Groep. Voorbeeld: `Administrator; myUser1; myUser2` |
|\[Voorbeeld:\]Besturingselementen voor media controleren en specifieke VM-extensies implementeren om controlevereisten te ondersteunen |Beleidstoewijzing |Lijst met gebruikers die moeten worden opgenomen in de groep Windows VM-beheerders |Een lijst met gescheiden punten van puntkomma's die moeten worden opgenomen in de lokale groep Administrators. Voorbeeld: `Administrator; myUser1; myUser2` |
...
Nu u de stappen hebt bekeken om het voorbeeld media te implementeren, gaat u naar de volgende artikelen voor meer informatie over het overzicht en de controletoewijzing:

> [!div class="nextstepaction"]
> [Media blauwdrukken - Overzicht](./index.md)
> [Media blauwdrukken - Controle mapping](./control-mapping.md)
