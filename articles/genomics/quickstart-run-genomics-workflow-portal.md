...
Gebruik python `pip` om de Microsoft Genomics-client `msgen`te installeren. Bij de volgende instructies wordt ervan uitgegaan dat Python al op het systeempad staat. Als u problemen ondervindt met `pip` de installatie wordt niet herkend, moet u python en de submap scripts toevoegen aan het systeempad.
...
Als u niet wilt installeren als `msgen` een systeem bestand binair en Python-pakketten voor het hele systeem wilt wijzigen, gebruikt `–-user` u de `pip`vlag with.
 ...
 * [Aanvragen](https://pypi.python.org/pypi/requests). 
...
Als u GATK4 wilt uitvoeren, stelt u de `process_name` para meter in `gatk4`op.

De Genomics-service voert standaard VCF-bestanden uit. Als u een gVCF-uitvoer wilt gebruiken in plaats van een VCF-uitvoer `-emitRefConfidence` (vergelijkbaar met in GATK 3 `emit-ref-confidence` . x en in GATK 4. x) `emit_ref_confidence` , voegt u de para meter toe aan het *bestand config. txt* en stelt u deze in op `gvcf`, zoals wordt weer gegeven in de vorige afbeelding.  Als u wilt terugkeren naar de VCF-uitvoer, moet u deze verwijderen uit het bestand *config. txt* `emit_ref_confidence` of `none`de para meter instellen op. 
...
In dit artikel hebt u voor beelden van invoer gegevens geüpload naar Azure Storage en een werk stroom verzonden naar de Microsoft Genomics `msgen` -service via de python-client. Voor meer informatie over andere typen invoer bestanden die kunnen worden gebruikt met de Microsoft Genomics-service, raadpleegt u de volgende pagina's: [gekoppelde FASTQ](quickstart-input-pair-FASTQ.md) | [BAM](quickstart-input-BAM.md) | [meerdere FASTQ of BAM](quickstart-input-multiple.md). U kunt ook deze zelfstudie lezen met behulp van de [Azure notebook tutorial](https://aka.ms/genomicsnotebook) (Zelfstudie in Azure-notitieblokken).
