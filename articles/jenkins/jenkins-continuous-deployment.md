...
- Een [AKS-cluster](../aks/kubernetes-walkthrough.md) en `kubectl` geconfigureerd met de [AKS-cluster referenties](/cli/azure/aks#az-aks-get-credentials).

- Een [Azure container Registry-REGI ster (ACR)](../container-registry/container-registry-get-started-azure-cli.md), de naam van de ACR-aanmeldings server en het AKS-cluster dat is geconfigureerd voor [verificatie met het ACR-REGI ster](../aks/cluster-container-registry-integration.md).

- De Azure CLI-versie 2.0.46 of hoger is geïnstalleerd en geconfigureerd. Voer  `az --version` uit om de versie te bekijken. Als u wilt installeren of upgraden, raadpleegt u [Azure cli installeren](/cli/azure/install-azure-cli).

- [Docker geïnstalleerd](https://docs.docker.com/install/) in uw ontwikkel systeem

- Een GitHub-account, een [persoonlijk toegangs token van github](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)en een Git-client die is geïnstalleerd in uw ontwikkel systeem

- Als u uw eigen Jenkins-exemplaar opgeeft in plaats van deze voorbeeld script manier om Jenkins te implementeren, moet uw Jenkins-exemplaar [docker geïnstalleerd en geconfigureerd](https://docs.docker.com/install/) en [kubectl](https://kubernetes.io/docs/tasks/tools/install-kubectl/)hebben.
...
Voordat u de container installatie kopie van *Azure-stem voor front* kunt pushen naar ACR, moet u de ACR-aanmeldings server ophalen met de opdracht [AZ ACR List](/cli/azure/acr#az-acr-list) . In het volgende voor beeld wordt het adres van de ACR-aanmeldings server opgehaald voor een REGI ster in de resource groep met de naam *myResourceGroup*:

```azurecli
az acr list --resource-group myResourceGroup --query "[].{acrLoginServer:loginServer}" --output table
```

Gebruik de opdracht [docker tag](https://docs.docker.com/engine/reference/commandline/tag/) om de installatie kopie te labelen met de naam van de ACR-aanmeldings server en een versie nummer van `v1`. Geef uw eigen `<acrLoginServer>` naam op die u in de vorige stap hebt verkregen:
...
Gebruik vervolgens de opdracht [kubectl Toep assen](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#apply) om de toepassing te implementeren in uw AKS-cluster:

```console
kubectl apply -f azure-vote-all-in-one-redis.yaml
```

Er wordt een Kubernetes-load balancer service gemaakt om de toepassing beschikbaar te maken voor Internet. Dit proces kan enkele minuten duren. Als u de voortgang van de load balancer-implementatie wilt controleren, gebruikt u de opdracht [kubectl Get service](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#get) met het argument `--watch`. Zodra het *EXTERNE IP-adres* is gewijzigd van *in behandeling* in een *IP-adres*, gebruikt u `Control + C` om het controleproces van kubectl te stoppen.
...
![Azure-voorbeeld toepassing die wordt uitgevoerd in AKS](media/jenkins-continuous-deployment/azure-vote.png)
...
- Onder **globale eigenschappen**selecteert u **omgevings variabelen**. Voeg een variabele met de naam `ACR_LOGINSERVER` en de waarde van uw ACR-aanmeldings server toe.
    ...
    ![Omgevings variabelen Jenkins](media/jenkins-continuous-deployment/env-variables.png)
...
Maak eerst een service-principal met behulp van de opdracht [AZ AD SP create-for-RBAC](/cli/azure/ad/sp#az-ad-sp-create-for-rbac) :
...
Haal de bron-ID van uw ACR-REGI ster op met de opdracht [AZ ACR show](/cli/azure/acr#az-acr-show) en sla deze op als een variabele. Geef de naam van de resource groep en de ACR op:
...
![Een Jenkins-referentie object maken met de gegevens van de Service-Principal](media/jenkins-continuous-deployment/acr-credentials.png)
...
1. Selecteer **webhook toevoegen**.
    ...
    ![Een GitHub-webhook maken voor Jenkins](media/jenkins-continuous-deployment/webhook.png)
...
![Voor beeld van Azure stem in AKS bijgewerkt door de Jenkins-build-taak](media/jenkins-continuous-deployment/azure-vote-updated.png)
