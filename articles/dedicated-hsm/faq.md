...
De specifieke HSM van Azure is de juiste keuze voor ondernemingen die migreren naar Azure on-premises toepassingen die gebruikmaken van Hsm's. De toegewezen Hsm's bevat een optie voor het migreren van een toepassing met minimale wijzigingen. Als cryptografische bewerkingen worden uitgevoerd in de code van de toepassing die wordt uitgevoerd in een virtuele machine of Web-App van Azure, kunnen ze speciale HSM gebruiken. In het algemeen kan software die wordt uitgevoerd in IaaS-modellen (Infrastructure as a Service), die ondersteuning bieden voor Hsm's als een sleutel archief, gebruikmaken van HSM, zoals Application Gateway of Traffic Manager, voor minder SSL, ADCS (Active Directory Certificate Services) of Vergelijk bare PKI-hulpprogram ma's, hulpprogram ma's/toepassingen die worden gebruikt voor het ondertekenen van documenten, ondertekening van programma code of een SQL Server (IaaS) die zijn geconfigureerd met TDE (transparante database versleuteling) met hoofd sleutel in een HSM met behulp van een EKM-provider (Extensible Key Management). Azure Key Vault is geschikt voor toepassingen die in de cloud worden uitgevoerd of voor versleuteling in rest scenario's waarin klant gegevens worden verwerkt door PaaS (platform as a Service) of SaaS (Software as a Service)-scenario's zoals Office 365-klant sleutel, Azure Information Protection , Azure Disk Encryption, Azure Data Lake Store versleuteling met door de klant beheerde sleutel, Azure Storage versleuteling met door de klant beheerde sleutel en Azure SQL met door de klant beheerde sleutel.
...
* Application Gateway of Traffic Manager voor minder dan SSL
...
Micro soft heeft geen beheer-of cryptografische controle over de HSM. Micro soft heeft toegang op monitor niveau via een seriële poort verbinding om eenvoudige telemetrie te verkrijgen, zoals de status van de Tempe ratuur en het onderdeel. Hierdoor kan micro soft proactieve meldingen over status problemen bieden. Indien nodig kan de klant dit account uitschakelen.
