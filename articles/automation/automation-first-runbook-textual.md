...
1. In dit geval gaat u een [Power shell workflow-runbook](automation-runbook-types.md#powershell-workflow-runbooks)maken. Selecteer **Power shell-werk stroom** voor het **type Runbook**.
...
1. Uw runbook is momenteel leeg met alleen de vereiste `Workflow` sleutel woord, de naam van het runbook en de accolades die de hele werk stroom omsluiten.
...
1. Wanneer de runbook-taak is voltooid, wordt de uitvoer weer gegeven in het deel venster testen. In dit geval ziet u `Hello World`.
...
1. Schuif naar links om het runbook weer te geven op de pagina **Runbooks** en houd er rekening mee dat het veld **ontwerp status** is ingesteld op **gepubliceerd**.
1. Schuif terug naar rechts om de pagina voor **MyFirstRunbook-workflow**weer te geven.
...
1. Zodra de status van het runbook wordt weer gegeven `Completed`, klikt u op **uitvoer**. De uitvoer pagina wordt geopend, waar u uw `Hello World` bericht kunt zien.

   ![Taakoverzicht](media/automation-first-runbook-textual/job-pane-status-blade-outputtile.png)

1. Sluit de uitvoer pagina.

1. Klik op **Alle logboeken** om het deelvenster Streams voor de runbooktaak te openen. In de uitvoer stroom ziet u alleen `Hello World`. Houd er rekening mee dat in het deel venster stromen andere stromen kunnen worden weer gegeven voor een runbook-taak, zoals uitgebreide en fout stromen, als het runbook hiernaar schrijft.
...
1. Voeg variabelen voor de para meters `VMName` en `ResourceGroupName` toe aan het runbook en gebruik de variabelen met de `Start-AzVM` cmdlet zoals hieronder wordt weer gegeven.
