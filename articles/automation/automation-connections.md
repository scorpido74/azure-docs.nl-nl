...
De interne cmdlet in de volgende tabel wordt gebruikt voor toegang tot verbindingen in uw runbooks en DSC-configuraties. Deze cmdlet wordt geleverd met de globale `Orchestrator.AssetManagement.Cmdlets`module. Zie [interne cmdlets](shared-resources/modules.md#internal-cmdlets)voor meer informatie.
...
>[!NOTE]
>Vermijd het gebruik van variabelen `Name` met de `Get-AutomationConnection`para meter van. Het gebruik van variabelen in dit geval kan de detectie van afhankelijkheden tussen runbooks of DSC-configuraties en verbindings assets tijdens het ontwerpen bemoeilijken.
...
Een nieuwe verbinding maken met Windows Power shell met `New-AzAutomationConnection` de cmdlet. Deze cmdlet heeft een `ConnectionFieldValues` para meter die verwacht dat een hashtabel waarden definieert voor elk van de eigenschappen die worden gedefinieerd door het verbindings type.
...
Het volgende voor beeld is een sjabloon in de **JSON** -bestands indeling die de gebruikers naam-en wachtwoord eigenschappen definieert voor een aangepast `MyModuleConnection`verbindings type met de naam:
...
Een verbinding in een runbook of DSC-configuratie ophalen met de `Get-AutomationConnection` interne cmdlet. Deze cmdlet heeft de voor keur `Get-AzAutomationConnection` boven de cmdlet, omdat deze de verbindings waarden ophaalt in plaats van informatie over de verbinding. 
...
In de volgende afbeelding ziet u een voor beeld van het gebruik van een verbindings object in een grafisch runbook. In dit voor beeld `Constant value` wordt de gegevensset voor `Get RunAs Connection` de activiteit gebruikt, waarbij een verbindings object voor verificatie wordt gebruikt. Er wordt hier een [pijplijn koppeling](automation-graphical-authoring-intro.md#links-and-workflow) gebruikt omdat de `ServicePrincipalCertificate` parameterset een enkel object verwacht.
