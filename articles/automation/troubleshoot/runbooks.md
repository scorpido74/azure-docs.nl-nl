...
ms.locfileid: "82508782"
---
# <a name="troubleshoot-runbook-errors"></a>Problemen met runbook oplossen

 In dit artikel worden de verschillende runbook-fouten beschreven die zich kunnen voordoen en hoe u deze kunt oplossen.
...
## <a name="diagnosing-runbook-issues"></a>Problemen met runbook diagnosticeren

Wanneer u fouten ontvangt tijdens het uitvoeren van een runbook in Azure Automation, kunt u de volgende stappen gebruiken om te helpen bij het vaststellen van de problemen.
...
1. Ga naar het Automation uitvoeren als-account en kopieer de toepassings-ID en vinger afdruk.
    ...
    ![ID en vinger afdruk kopiëren](../media/troubleshoot-runbooks/collect-app-id.png)
...
U ontvangt de volgende fout bij het werken met `Select-AzureSubscription`de `Select-AzureRMSubscription`-, `Select-AzSubscription` -of-cmdlet:
...
2. Zorg ervoor dat uw script de cmdlet [Connect-AzAccount](https://docs.microsoft.com/powershell/module/Az.Accounts/Connect-AzAccount?view=azps-3.7.0) uitvoert voordat u de `Select-*` cmdlet uitvoert.
3. Voeg `Disable-AzContextAutosave –Scope Process` aan het begin van uw runbook toe. Met deze cmdlet-aanroep zorgt u ervoor dat alle referenties alleen van toepassing zijn op de uitvoering van het huidige runbook.
4. Als dit fout bericht nog steeds wordt weer gegeven, wijzigt u de code `AzContext` door de `Connect-AzAccount`para meter toe te voegen voor en voert u de code uit.
...
## <a name="scenario-authentication-to-azure-failed-because-multi-factor-authentication-is-enabled"></a><a name="auth-failed-mfa"></a>Scenario: authenticatie naar Azure is mislukt omdat multi-factor Authentication is ingeschakeld
...
## <a name="scenario-you-see-an-error-in-your-job-streams-about-the-get_serializationsettings-method"></a><a name="get-serializationsettings"></a>Scenario: er wordt een fout in uw taak stromen weer geven over de get_SerializationSettings methode
...
## <a name="scenario-term-not-recognized-as-the-name-of-a-cmdlet-function-script"></a><a name="not-recognized-as-cmdlet"></a>Scenario: term wordt niet herkend als de naam van een cmdlet, functie, script
...
## <a name="scenario-the-runbook-job-start-was-attempted-three-times-but-failed-to-start-each-time"></a><a name="job-attempted-3-times"></a>Scenario: het starten van de runbook-taak is drie keer geprobeerd, maar kan niet elke keer worden gestart
...
* Te veel uitzonderings gegevens. Er is een limiet van 1 MB voor de taak uitvoer stroom. Zorg ervoor dat uw runbook aanroepen naar een uitvoerbaar bestand of `try` subproces met en `catch` blokkeert. Als de bewerkingen een uitzonde ring veroorzaken, laat u de code het bericht van de uitzonde ring in een Automation-variabele schrijven. Met deze techniek wordt voor komen dat het bericht naar de uitvoer stroom van de taak wordt geschreven.
...
Er wordt een van de volgende fouten weer gegeven wanneer u `Connect-AzAccount` met de cmdlet werkt:
...
U ontvangt deze fout bij het werken `Add-AzAccount`met, een alias voor de `Connect-AzAccount` cmdlet:
...
U ontvangt de volgende fout bij het aanroepen van een onderliggend runbook met de `Wait` para meter en de uitvoer stroom bevat een object:
...
`Start-AzAutomationRunbook`de uitvoer stroom wordt niet correct verwerkt als de stroom objecten bevat.

### <a name="resolution"></a>Oplossing

Het is raadzaam om een polling logica te implementeren en de cmdlet [Get-AzAutomationJobOutput](https://docs.microsoft.com/powershell/module/Az.Automation/Get-AzAutomationJobOutput?view=azps-3.7.0) te gebruiken om de uitvoer op te halen. Hieronder wordt een voor beeld van deze logica gedefinieerd.
...
## <a name="scenario-runbook-job-fails-because-allocated-quota-exceeded"></a><a name="quota-exceeded"></a>Scenario: de Runbook-taak is mislukt omdat het toegewezen quotum is overschreden
...
## <a name="scenario-status-400-bad-request-when-calling-a-webhook"></a><a name="expired webhook"></a>Scenario: status: 400 ongeldige aanvraag bij het aanroepen van een webhook
...
## <a name="scenario-429-the-request-rate-is-currently-too-large"></a><a name="429"></a>Scenario: 429: het aanvraag aantal is momenteel te groot...
...
## <a name="scenario-powershell-job-fails-with-error-cannot-invoke-method"></a><a name="cannot-invoke-method"></a>Scenario: Power shell-taak mislukt met fout: kan methode niet aanroepen
...
## <a name="scenario-invalid-status-code-forbidden-when-using-key-vault-inside-a-runbook"></a>Scenario: ongeldige status code ' verboden ' bij het gebruiken van Key Vault in een runbook

### <a name="issue"></a>Probleem

Wanneer u probeert toegang te krijgen tot Key Vault via een Azure Automation runbook, wordt de volgende fout weer geven:
