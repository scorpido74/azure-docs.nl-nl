---
title: Private Link - Azure portal - Azure Database voor MariaDB
description: Meer informatie over het configureren van privékoppeling voor Azure Database voor MariaDB vanuit Azure-portal
author: kummanish
ms.author: manishku
ms.service: mariadb
ms.topic: conceptual
...
---
# <a name="create-and-manage-private-link-for-azure-database-for-mariadb-using-portal"></a>Private Link voor Azure Database voor MariaDB maken en beheren met Portal

Een privéeindpunt is de fundamentele bouwsteen voor privékoppelingen in Azure. Hiermee kunnen Azure-resources, zoals Virtuele Machines (VM's), privé communiceren met privékoppelingsbronnen.  In dit artikel leert u hoe u de Azure-portal gebruiken om een VM te maken in een Azure Virtual Network en een Azure Database voor MariaDB-server met een Azure-privéeindpunt.
...
In deze sectie maakt u een virtueel netwerk en het subnet om de VM te hosten die wordt gebruikt om toegang te krijgen tot uw Private Link-bron (een MariaDB-server in Azure).
...
In deze sectie maakt u een Azure Database voor MariaDB-server in Azure. 

1. Selecteer linksboven in het scherm in de Azure-portal de optie Een**Azure-database** > voor **bronnendatabases** > **maken voor MariaDB**.

1. Geef in **Azure Database voor MariaDB** de volgende gegevens op:
    ...
    | Locatie | Selecteer een Azure-gebied waar u wilt dat uw MariaDB-server zich bevindt. |
    |Versie  | Selecteer de databaseversie van de MariaDB-server die vereist is.|
...
In deze sectie maakt u een privéeindpunt voor de MariaDB-server. 
...
6. Voer in **Een privéeindpunt maken - Resource**deze gegevens in of selecteer deze:
    ...
    | Abonnement| Selecteer uw abonnement. |
    | Resourcetype | Selecteer **Microsoft.DBforMariaDB/servers**. |
    | Resource |MyServer *selecteren*|
    |Subresource target |*MariadbServer selecteren*|
    |||
...
2. Wanneer u het **doorgegeven bericht Validatie ziet,** selecteert u **Maken**. 
    ...
    ![Private Link gemaakt](media/concepts-data-access-and-security-private-link/show-mariadb-private-link.png)
...
2. Voer `nslookup mydemomserver.privatelink.mariadb.database.azure.com`. 
...
7. Sluit de verbinding met extern bureaublad met myVm.

## <a name="clean-up-resources"></a>Resources opschonen
Wanneer u klaar bent met het privéeindpunt, de MariaDB-server en de VM, verwijdert u de brongroep en alle bronnen die deze bevat:
