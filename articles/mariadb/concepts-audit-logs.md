---
title: Controlelogboeken - Azure Database voor MariaDB
description: Beschrijft de controlelogboeken die beschikbaar zijn in Azure Database voor MariaDB en de beschikbare parameters voor het inschakelen van logboeken.
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
---
# <a name="audit-logs-in-azure-database-for-mariadb"></a>Controlelogboeken in Azure-database voor MariaDB

In Azure Database voor MariaDB is het controlelogboek beschikbaar voor gebruikers. Het controlelogboek kan worden gebruikt om activiteiten op databaseniveau bij te houden en wordt vaak gebruikt voor naleving.
...
- `audit_log_include_users`: MariaDB-gebruikers worden opgenomen voor het loggen. De standaardwaarde voor deze parameter is leeg, waaronder alle gebruikers die moeten worden logboekregistratie. Dit heeft een `audit_log_exclude_users`hogere prioriteit boven . De maximale lengte van de parameter is 512 tekens.
> [!Note]
...
- `audit_log_exclude_users`: MariaDB-gebruikers worden uitgesloten van logboekregistratie. Zorgt voor ten hoogste vier gebruikers. De maximale lengte van de parameter is 256 tekens.
...
Controlelogboeken zijn geïntegreerd met Azure Monitor Diagnostic Logs. Zodra u controlelogboeken op uw MariaDB-server hebt ingeschakeld, u deze verzenden naar Azure Monitor-logboeken, gebeurtenishubs of Azure Storage. Zie het artikel over de [controlelogboekportal](howto-configure-audit-logs-portal.md#set-up-diagnostic-logs)voor meer informatie over het inschakelen van diagnostische logboeken in de Azure-portal.

## <a name="diagnostic-logs-schemas"></a>Schema's voor diagnostische logboeken

In de volgende secties wordt beschreven wat de uitvoer van MariaDB-controlelogboeken is op basis van het gebeurtenistype. Afhankelijk van de uitvoermethode kunnen de opgenomen velden en de volgorde waarin ze worden weergegeven, variëren.
...
| `connection_id_d` | Unieke verbindings-ID gegenereerd door MariaDB |
| `host_s` | Blank |
| `ip_s` | IP-adres van de klant die verbinding maakt met MariaDB |
...
| `event_time` | Querystartseconden in UNIX-tijdstempel |
...
- Gecontroleerde gebeurtenissen weergeven voor alle MariaDB-servers met diagnostische logboeken ingeschakeld voor controlelogboeken
