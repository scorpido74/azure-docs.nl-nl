---
title: Replica's lezen - Azure Database voor MariaDB
description: "Meer informatie over leesreplica's in Azure Database voor MariaDB: regio's kiezen, replica's maken, verbinding maken met replica's, replicatie bewaken en replicatie stoppen."
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
Met de functie leesreplica kunt u gegevens van een Azure Database for MariaDB-server repliceren naar een alleen-lezen server. U kunt van de hoofdserver naar maximaal vijf replica's repliceren. Replica's worden asynchroon bijgewerkt met behulp van de binaire log (binlog) bestandspositietechnologie van de MariaDB-engine met wereldwijde transactie-ID (GTID). Zie het overzicht van de [binlogreplicatie](https://mariadb.com/kb/en/library/replication-overview/)voor meer informatie over binlog-replicatie.

Replica's zijn nieuwe servers die u beheert die vergelijkbaar zijn met gewone Azure Database voor MariaDB-servers. Voor elke gelezen replica wordt u gefactureerd voor de ingerichte compute in vCores en opslag in GB/maand.

Zie de [replicatiedocumentatie van MariaDB](https://mariadb.com/kb/en/library/gtid/)voor meer informatie over GTID-replicatie.
...
* Regionale beschikbaarheid: Azure Database voor MariaDB is beschikbaar in West US 2, France Central, UAE North en Germany Central. Hun gekoppelde regio's zijn echter niet beschikbaar.
...
Wanneer u de replicawerk voor maken start, wordt een lege Azure-database voor MariaDB-server gemaakt. De nieuwe server is gevuld met de gegevens die zich op de hoofdserver bevond. De creatietijd is afhankelijk van de hoeveelheid gegevens op de master en de tijd sinds de laatste wekelijkse volledige back-up. De tijd kan variëren van enkele minuten tot enkele uren.

> [!NOTE]
> Als u geen opslagwaarschuwing op uw servers hebt ingesteld, raden we u aan dit te doen. De waarschuwing informeert u wanneer een server de opslaglimiet nadert, wat van invloed is op de replicatie.
...
U verbinding maken met de replica met behulp van de hostnaam en een geldig gebruikersaccount, zoals u zou doen op een gewone Azure-database voor MariaDB-server. Voor een server met de naam **myreplica** met de gebruikersnaam **myadmin,** u verbinding maken met de replica met behulp van de mysql CLI:
...
Azure Database voor MariaDB biedt de **replicatievertraging in seconden** in Azure Monitor. Deze statistiek is alleen beschikbaar voor replica's.

Deze statistiek wordt `seconds_behind_master` berekend met behulp van `SHOW SLAVE STATUS` de statistiek die beschikbaar is in de opdracht van MariaDB.
...
Een gelezen replica wordt gemaakt als een nieuwe Azure-database voor MariaDB-server. Een bestaande server kan niet worden omgezet in een replica. U geen replica van een andere gelezen replica maken.

### <a name="replica-configuration"></a>Replicaconfiguratie

Er wordt een replica gemaakt met dezelfde serverconfiguratie als de stramien. Nadat een replica is gemaakt, kunnen verschillende instellingen onafhankelijk van de hoofdserver worden gewijzigd: rekengeneratie, vCores, opslag, back-upbewaarperiode en MariaDB-engineversie. De prijscategorie kan ook onafhankelijk worden gewijzigd, behalve van of naar de basislaag.
...
- In-memory tabellen kunnen ertoe leiden dat replica's niet synchroon lopen. Dit is een beperking van de MariaDB replicatietechnologie.
