...
In dit voor beeld wordt een pool gemaakt die begint met 25 knoop punten met lage prioriteit. Telkens wanneer een knoop punt met een lage prioriteit is afgebroken, wordt het vervangen door een toegewezen knoop punt. Net als in het eerste voor beeld wordt met de variabele `maxNumberofVMs` voor komen dat de groep meer dan 25 Vm's overschrijdt. Dit voor beeld is handig voor het gebruik van Vm's met lage prioriteit, terwijl u er ook voor zorgt dat alleen een vast aantal preemptions plaatsvindt voor de levens duur van de groep.
...
De *doubleVecList* -waarde wordt voor de evaluatie geconverteerd naar één *doubleVec* . Als `v = [1,2,3]`bijvoorbeeld, wordt het aanroepen van `avg(v)` gelijk aan het aanroepen van `avg(1,2,3)`. Het aanroepen van `avg(v, 7)` is gelijk aan het aanroepen van `avg(1,2,3,7)`.
