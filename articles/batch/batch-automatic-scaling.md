...
De *doubleVecList* -waarde wordt voor de evaluatie geconverteerd naar één *doubleVec* . Als `v = [1,2,3]` bijvoorbeeld is, wordt het aanroepen van `avg(v)` gelijk aan het aanroepen van `avg(1,2,3)`. Het aanroepen van `avg(v, 7)` is gelijk aan het aanroepen van `avg(1,2,3,7)`.
