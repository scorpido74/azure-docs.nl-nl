---
title: 'Zelf studie: on-premises gegevens migreren naar Azure Storage met AzCopy | Microsoft Docs'
...
---
#  <a name="tutorial-migrate-on-premises-data-to-cloud-storage-with-azcopy"></a>Zelf studie: on-premises gegevens migreren naar Cloud opslag met AzCopy
...
* Vervang de tijdelijke aanduiding `<container-name>` door de naam van de container die u hebt gemaakt.

Als u de inhoud van de opgegeven map recursief naar Blob Storage wilt uploaden, geeft u de optie `--recursive` op. Wanneer u AzCopy met deze optie uitvoert, worden ook alle submappen en de bijbehorende bestanden geüpload.
...
Zie [bestanden synchroniseren](storage-use-azcopy-blobs.md#synchronize-files)voor meer informatie over de `sync` opdracht.
...
In deze voor beelden wordt ervan uitgegaan dat uw map de naam `myFolder`heeft, of de naam van het opslag account `mystorageaccount` is en dat de naam van de container `mycontainer`is.
