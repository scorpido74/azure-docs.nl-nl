...
Installeer de Azure Blob Storage-client bibliotheek voor Java script-pakket met behulp van de `npm install` opdracht terwijl u nog steeds in de map *BLOB-Quick Start-V12* wordt uitgevoerd. Met deze opdracht wordt het bestand *package. json* gelezen en wordt de Azure Blob Storage-client bibliotheek V12 voor Java script-pakket en alle bibliotheken waarvan het afhankelijk is, geïnstalleerd.
...
* [BlobClient](/javascript/api/@azure/storage-blob/blobclient): met `BlobClient` de klasse kunt u Azure Storage blobs bewerken.
...
Voeg deze hulp functie *after* toe na `main` de functie:
