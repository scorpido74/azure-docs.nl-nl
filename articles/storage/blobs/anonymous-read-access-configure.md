...
description: Meer informatie over het in-of uitschakelen van anonieme toegang tot BLOB-gegevens voor het opslag account. Stel de instelling voor open bare toegang van de container in om containers en blobs beschikbaar te maken voor anonieme toegang.
services: storage
author: tamram
ms.service: storage
ms.topic: how-to
...
## <a name="enable-or-disable-public-read-access-for-a-storage-account"></a>Open bare Lees toegang voor een opslag account in-of uitschakelen

Open bare toegang is standaard ingeschakeld voor een opslag account. Als u open bare toegang uitschakelt, wordt voor komen dat alle anonieme toegang tot containers en blobs in dat account. Voor een betere beveiliging raadt micro soft u aan om open bare toegang voor uw opslag accounts uit te scha kelen, tenzij uw scenario vereist dat gebruikers anoniem toegang hebben tot BLOB-resources.

> [!WARNING]
> Als u open bare toegang uitschakelt voor een opslag account, worden de instellingen voor open bare toegang overschreven voor alle containers in dat opslag account. Wanneer open bare toegang voor het opslag account is uitgeschakeld, zullen toekomstige anonieme aanvragen voor dat account mislukken.

Als u open bare toegang voor een opslag account wilt in-of uitschakelen, gebruikt u de Azure Portal of Azure CLI om de **blobPublicAccess** -eigenschap van het account te configureren. Deze eigenschap is beschikbaar voor alle opslag accounts die zijn gemaakt met het Azure Resource Manager-implementatie model. Zie [overzicht van opslag accounts](../common/storage-account-overview.md)voor meer informatie.

# <a name="azure-portal"></a>[Azure Portal](#tab/portal)

Voer de volgende stappen uit om open bare toegang voor een opslag account in de Azure Portal in of uit te scha kelen:

1. Ga in Azure Portal naar uw opslagaccount.
1. Zoek de **configuratie** -instelling onder **instellingen**.
1. Stel **open bare BLOB-toegang** in op **uitgeschakeld** of **ingeschakeld**.

    :::image type="content" source="media/anonymous-read-access-configure/blob-public-access-portal.png" alt-text="Scherm afbeelding die laat zien hoe u open bare BLOB-toegang voor account kunt in-of uitschakelen":::

# <a name="azure-cli"></a>[Azure CLI](#tab/azure-cli)

Als u open bare toegang voor een opslag account met Azure CLI wilt in-of uitschakelen, moet u eerst de resource-ID voor uw opslag account ophalen door de opdracht [AZ resource show](/cli/azure/resource#az-resource-show) aan te roepen. Vervolgens roept u de opdracht [AZ resource update](/cli/azure/resource#az-resource-update) aan om de eigenschap **allowBlobPublicAccess** voor het opslag account in te stellen. Als u open bare toegang wilt inschakelen, stelt u de eigenschap **allowBlobPublicAccess** in op True (waar). Als u wilt uitschakelen, stelt u deze in op **Onwaar**.

In het volgende voor beeld wordt de toegang voor open bare blobs uitgeschakeld voor het opslag account. Vergeet niet om de waarden van de tijdelijke aanduidingen tussen vier Kante haken te vervangen door uw eigen waarden:
...
Als u wilt controleren of open bare toegang is ingeschakeld met Azure CLI, roept u de opdracht [AZ resource show](/cli/azure/resource#az-resource-show) en de query voor de eigenschap **allowBlobPublicAccess** aan:
...
> [!NOTE]
> Het uitschakelen van open bare toegang voor een opslag account heeft geen invloed op alle statische websites die worden gehost in dat opslag account. De **$Web** container is altijd openbaar toegankelijk.

## <a name="check-the-public-access-setting-for-a-storage-account"></a>Controleer de instelling voor open bare toegang voor een opslag account

Als u de instelling voor open bare toegang voor een opslag account wilt controleren, haalt u de waarde van de eigenschap **allowBlobPublicAccess** op. Als u deze eigenschap in één keer wilt controleren op een groot aantal opslag accounts, gebruikt u de Azure resource Graph Explorer.

De eigenschap **allowBlobPublicAccess** is niet standaard ingesteld en retourneert geen waarde voordat u deze expliciet hebt ingesteld. Het opslag account is standaard ingesteld op het toestaan van open bare toegang wanneer de waarde van de eigenschap Null is.

### <a name="check-the-public-access-setting-for-a-single-storage-account"></a>Controleer de instelling voor open bare toegang voor één opslag account

Als u de instelling voor open bare toegang voor één opslag account wilt controleren met behulp van Azure CLI, roept u de opdracht [AZ resource show](/cli/azure/resource#az-resource-show) en de query voor de eigenschap **allowBlobPublicAccess** aan:
...
### <a name="check-the-public-access-setting-for-a-set-of-storage-accounts"></a>Controleer de instelling voor open bare toegang voor een set opslag accounts
...
Als u anonieme gebruikers lees toegang wilt verlenen voor een container en de bijbehorende blobs, moet u eerst open bare toegang voor het opslag account inschakelen en vervolgens het open bare toegangs niveau van de container instellen. Als de open bare toegang voor het opslag account is uitgeschakeld, kunt u de open bare toegang voor een container niet configureren.

Wanneer open bare toegang is ingeschakeld voor een opslag account, kunt u een container configureren met de volgende machtigingen:
...
Wanneer open bare toegang voor het opslag account is uitgeschakeld, kan het open bare toegangs niveau van een container niet worden ingesteld. Als u het open bare toegangs niveau van de container probeert in te stellen, ziet u dat de instelling is uitgeschakeld, omdat open bare toegang is verboden voor het account.

:::image type="content" source="media/anonymous-read-access-configure/container-public-access-blocked.png" alt-text="Scherm opname waarin wordt getoond dat het niveau van open bare toegang tot de container wordt geblokkeerd wanneer open bare toegang is uitgeschakeld":::
