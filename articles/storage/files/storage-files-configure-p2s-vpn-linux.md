...
Als u wilt dat VPN-verbindingen van uw on-premises Linux-machines worden geverifieerd om toegang te krijgen tot uw virtuele netwerk, moet u twee certificaten maken: een basis certificaat, dat wordt meegeleverd met de gateway van de virtuele machine en een client certificaat, dat wordt ondertekend met het basis certificaat. Met het volgende script worden de vereiste certificaten gemaakt.
...
De gateway van het virtuele netwerk van Azure maakt een downloadbaar pakket met configuratie bestanden die nodig zijn om de VPN-verbinding op uw on-premises Linux-computer te initialiseren. Het volgende script plaatst de certificaten die u op de juiste plaats hebt gemaakt en configureert het `ipsec.conf` bestand met de juiste waarden uit het configuratie bestand in het Download bare pakket.
