...
* [Meer informatie over het aanpassen van de grootte van een Windows-Server op basis van het aantal items (bestanden en mappen) dat u wilt synchroniseren.](storage-sync-files-planning.md#recommended-system-resources)
    ...
    **Let op:** Het eerder gekoppelde artikel bevat een tabel met een bereik voor Server geheugen (RAM). Richten naar het grote aantal voor de Azure VM. U kunt naar het kleinere nummer voor uw on-premises machine richten.
...
> [!IMPORTANT]
> Vaak wordt een on-premises Windows-Server gebruikt voor uw on-premises StorSimple-apparaat. In een dergelijke configuratie is het mogelijk om de functie '[gegevensontdubbeling](https://docs.microsoft.com/windows-server/storage/data-deduplication/install-enable)' in te scha kelen op die Windows-Server. **Als u Gegevensontdubbeling hebt gebruikt met uw StorSimple-gegevens, moet u ervoor zorgen dat u ook Gegevensontdubbeling op deze virtuele machine van Azure inschakelt.** Verwar deze ontdubbeling op bestands niveau niet met StorSimples ingebouwde ontdubbeling op blok niveau, waarvoor geen actie nood zakelijk is.

> [!IMPORTANT]
> Implementeer een **snelle besturingssysteem schijf** voor uw virtuele machine in de cloud om de prestaties te optimaliseren. U slaat de synchronisatie database op de besturingssysteem schijf op voor al uw gegevens volumes. Zorg er bovendien voor dat u een **grote besturingssysteem schijf**maakt. Afhankelijk van het aantal items (bestanden en mappen) op uw StorSimple-volumes, heeft de besturingssysteem schijf mogelijk een **aantal honderd GiB** ruimte nodig om de synchronisatie database te kunnen ondersteunen.

### <a name="expose-the-storsimple-8020-volumes-to-the-azure-vm"></a>De StorSimple 8020-volumes beschikbaar maken voor de Azure VM
