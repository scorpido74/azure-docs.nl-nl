---
title: Cloud-init gebruiken om de hostnaam in te stellen voor een virtuele Linux-machine in azure
...
De hostnaam is standaard hetzelfde als de VM-naam wanneer u een nieuwe virtuele machine in azure maakt.  Als u een Cloud-init-script wilt uitvoeren om deze standaard-hostnaam te wijzigen wanneer u een virtuele machine in azure maakt met [AZ VM Create](/cli/azure/vm), geeft u het Cloud-init-bestand op met de schakel optie `--custom-data`.  
...
Maak nu een virtuele machine met [AZ VM Create](/cli/azure/vm) en geef het bestand Cloud-init met `--custom-data cloud_init_hostname.txt` als volgt op:
