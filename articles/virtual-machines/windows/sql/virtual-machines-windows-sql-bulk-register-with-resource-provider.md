...
In dit artikel wordt beschreven hoe u uw virtuele SQL Server Virtual Machine (VM) in azure bulksgewijs kunt registreren met de resource provider `Register-SqlVMs` van de SQL-VM met behulp van de Power shell-cmdlet.
...
Als u het script wilt maken, kopieert u het [volledige script](#full-script) uit het einde van dit artikel en slaat `RegisterSqlVMs.psm1`u het lokaal op als. 
...
Open een Power shell-terminal voor beheer en navigeer naar de `RegisterSqlVMs.psm1` locatie waar u het bestand hebt opgeslagen. Voer vervolgens de volgende Power shell-cmdlet uit om het script te importeren als een module: 
...
Elke keer dat de `Register-SqlVMs` cmdlet wordt gebruikt, wordt zowel een rapport als een logboek bestand gegenereerd. 
...
| Aantal Vm's dat is geregistreerd | Het aantal virtuele machines dat is geregistreerd na het uitvoeren van de cmdlet. Geeft een lijst van de geregistreerde virtuele machines `SubscriptionID, Resource Group, Virtual Machine`in de indeling. | 
...
Kopieer het volledige script en sla het op `RegisterSqLVMs.psm1`als.
