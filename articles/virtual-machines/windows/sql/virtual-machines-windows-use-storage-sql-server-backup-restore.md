...
* **Beheerde hardware**: Er is geen overhead van het hardwarematige beheer met Azure-Services. Azure-Services beheren de hardware en bieden geo-replicatie voor redundantie en bescherming tegen hardwarestoringen.
...
* **Beschik baarheid van back-up**: Back-ups die zijn opgeslagen in azure blobs zijn overal en op elk gewenst moment beschikbaar en kunnen eenvoudig worden teruggezet naar een on-premises SQL Server of een andere SQL Server die wordt uitgevoerd op een virtuele machine van Azure, zonder dat de data base moet worden gekoppeld/losgekoppeld of gedownload en koppelen van de VHD.
* **Kosten**: Betaal alleen voor de service die wordt gebruikt. Kan rendabel zijn als een off-site-en back-uparchief optie. Raadpleeg de prijzen(https://go.microsoft.com/fwlink/?LinkId=277060 "Calculator")van [Azure prijs calculator]en het [](https://go.microsoft.com/fwlink/?LinkId=277059 "artikel") prijzen voor Azure-prijzen voor meer informatie.
* **Opslag momentopnamen**: Wanneer database bestanden worden opgeslagen in een Azure-Blob en u SQL Server 2016 gebruikt, kunt u [back-ups van bestands momentopnamen](https://msdn.microsoft.com/library/mt169363.aspx) gebruiken om bijna momentane back-ups en snelle herstel bewerkingen uit te voeren.
...
| **Container** |Een container biedt een groepering van een set blobs en kan een onbeperkt aantal blobs bevatten. Als u een SQL Server back-up naar een Azure-Blob service wilt schrijven, moet u ten minste de basis container hebben gemaakt. |
| **Blob** |Een bestand van elk type en elke grootte. Blobs zijn adresseerbaar met behulp van de volgende URL-indeling: **https://[Storage account]. blob. core. Windows. net/[container]/[BLOB]** . Zie [Wat zijn blok-en pagina](https://msdn.microsoft.com/library/azure/ee691964.aspx) -blobs? voor meer informatie over pagina-blobs. |
...
> [!NOTE]
> SQL Server 2016 is bijgewerkt ter ondersteuning van blok-blobs. Raadpleeg [de zelf studie: Gebruik de Microsoft Azure Blob Storage-service met SQL Server 2016](https://msdn.microsoft.com/library/dn466438.aspx) -data bases voor meer informatie.
> 
> 
...
2. Ga vervolgens door met een van de volgende zelf studies, waarmee u een opslag account maakt en een herstel bewerking uitvoert.
   ...
   * **SQL Server 2014**: [Vind SQL Server 2014 back-up en herstel naar Microsoft Azure](https://msdn.microsoft.com/library/jj720558\(v=sql.120\).aspx)Blob Storage service.
   * **SQL Server 2016**: [Vind De Microsoft Azure Blob Storage-service gebruiken met SQL Server 2016-data bases](https://msdn.microsoft.com/library/dn466438.aspx)
...
Zie [back-up en herstel voor SQL Server in Azure virtual machines](virtual-machines-windows-sql-backup-recovery.md)voor andere SQL Server opties voor back-up en herstel.
