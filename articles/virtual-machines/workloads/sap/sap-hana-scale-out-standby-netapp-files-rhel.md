...
Voor deze voorbeeld configuratie zijn de subnetten:  
  ...
  - `storage` 10.9.3.0/26  
  - `hana` 10.9.2.0/26  
  - `anf` 10.9.0.0/26 (overgedragen subnet naar Azure NetApp Files)
...
5. Implementeer Azure NetApp Files volumes door de instructies in [een NFS-volume maken voor Azure NetApp files](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-create-volumes)te volgen.  
   ...
   Wanneer u de volumes implementeert, moet u ervoor zorgen dat u de **nfsv 4.1** -versie selecteert. Implementeer de volumes in het aangewezen Azure NetApp Files [subnet](https://docs.microsoft.com/rest/api/virtualnetwork/subnets). 
...
3. **[A]** Voeg een netwerk route toe zodat de communicatie met de Azure NetApp files via de netwerk interface van de opslag.  
   ...
   In dit voor beeld wordt `Networkmanager` gebruikt voor het configureren van de extra netwerk route. Bij de volgende instructies wordt ervan uitgegaan dat de opslag netwerk interface is `eth1`.  
   Bepaal eerst de naam van de verbinding voor apparaat `eth1`. In dit voor beeld wordt de naam van de verbinding voor apparaat `eth1` `Wired connection 1`.  
   ...
   Configureer vervolgens extra route naar het Azure NetApp Files gedelegeerde netwerk via `eth1`.  
...
5. **[A]** Wijzig de sunrpc-instellingen, zoals aanbevolen in de [SAP Hana op NetApp aff-systemen met NFS-configuratie handleiding](https://www.netapp.com/us/media/tr-4435.pdf).  
...
6. **[A]** Red Hat voor Hana-configuratie.
    ...
    > [!NOTE]
    > Als u HANA 2,0 SP04 installeert, moet u de pakket `compat-sap-c++-7` installeren, zoals beschreven in SAP Note [2593824], voordat u SAP Hana kunt installeren. 
