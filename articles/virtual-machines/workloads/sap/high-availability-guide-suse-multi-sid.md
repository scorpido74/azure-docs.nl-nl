...
* Front-end configuratie
  ...
  * IP-adres voor NW1 10.3.1.17
  * IP-adres voor NW1 10.3.1.19
* Back-end-configuratie
  * Verbonden met primaire netwerk interfaces van alle virtuele machines die deel moeten uitmaken van het (A) SCS/ERS-cluster
* Test poort
...
Een andere mogelijkheid is om de shares te implementeren op de [NFS-volumes van Azure NetApp files](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-create-volumes).  Met Azure NetApp Files krijgt u ingebouwde hoge Beschik baarheid voor de SAP NFS-shares.
...
2. **[A]** naam omzetting instellen voor de extra SAP-systemen. U kunt de DNS-server gebruiken of `/etc/hosts` op alle knoop punten wijzigen. In dit voor beeld ziet u hoe u het `/etc/hosts`-bestand gebruikt.  Pas de IP-adressen en de hostnamen aan uw omgeving aan. 
...
4. **[A]** Configureer `autofs` om de/sapmnt/sid-en/usr/sap/sid/sys-bestands systemen te koppelen voor de extra SAP-systemen die u op het cluster implementeert. In dit voor beeld **NW2** en **NW3**.  
...
3. **[1]** Maak een virtuele IP-en status test cluster bronnen voor het ers-exemplaar van het extra SAP-systeem dat u naar het cluster implementeert. Het voor beeld dat hier wordt weer gegeven, is voor **NW2** en **NW3** ers met behulp van Maxi maal beschik bare NFS-server. 
   ...
   Zorg er vervolgens voor dat de resources van de zojuist gemaakte ERS-groep worden uitgevoerd op het cluster knooppunt, tegengesteld aan het cluster knooppunt waar het ASCS-exemplaar voor hetzelfde SAP-systeem is geïnstalleerd.  Als bijvoorbeeld NW2 ASCS is geïnstalleerd op `slesmsscl1`, moet u ervoor zorgen dat de groep NW2 ERS wordt uitgevoerd op `slesmsscl2`.  U kunt de groep NW2 ERS migreren naar `slesmsscl2` door de volgende opdracht uit te voeren: 
...
1. Knoop punt crash simuleren
   ...
   Voer de volgende opdracht uit als root op het knoop punt waarop ten minste één ASCS-exemplaar wordt uitgevoerd. In dit voor beeld hebben we de opdracht uitgevoerd op `slesmsscl2`, waarbij de ASCS-instanties voor NW1 en NW3 worden uitgevoerd.  
