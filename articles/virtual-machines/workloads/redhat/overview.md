...
RHEL-installatie kopieën die zijn verbonden met RHUI update, zijn standaard de meest recente secundaire `yum update` versie van RHEL wanneer een wordt uitgevoerd. Dit gedrag houdt in dat een RHEL 7,4 VM kan worden bijgewerkt naar RHEL 7,7 als `yum update` er een bewerking wordt uitgevoerd. Dit gedrag is inherent aan het ontwerp voor RHUI. Als u dit upgrade gedrag wilt beperken, schakelt u van normale RHEL-opslag plaatsen naar [uitgebreide ondersteuning voor update-opslag](./redhat-rhui.md#rhel-eus-and-version-locking-rhel-vms)plaatsen.
...
* Meer informatie over de [aanbieding voor Red Hat Gold`rhel-byos`image ()](./byos.md).
