...
| --------- | ---- | ----------- |
|name|tekenreeks|Vereist. De naam voor de worker-rol. De naam van de rol moet uniek zijn.|
|enableNativeCodeExecution|booleaans|Optioneel. De standaard waarde is `true`; uitvoering van systeem eigen code en volledig vertrouwen zijn standaard ingeschakeld. Stel dit kenmerk in `false` op het uitschakelen van systeem eigen code voor de worker-rol en gebruik in plaats daarvan Azure gedeeltelijk vertrouwen.|
...
|Protocolsubstatus|tekenreeks|Vereist. Het transport protocol voor het externe eind punt. `HTTP`Voor een werk rollen zijn mogelijke waarden, `HTTPS` `UDP`, of. `TCP`|
...
|setReadyOnProcessStart|booleaans|Vereist. Hiermee geeft u op of de rolinstantie wacht totdat het opdracht regel programma een signaal start. Deze waarde moet worden ingesteld op `true` op dit moment. Het instellen van de `false` waarde op is gereserveerd voor toekomstig gebruik.|
