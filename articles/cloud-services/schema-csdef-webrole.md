...
In de volgende tabel worden de kenmerken van het element `WebRole` beschreven.

| Kenmerk | Type | Beschrijving |  
| --------- | ---- | ----------- |  
|name|string|Vereist. De naam voor de webrole. De naam van de rol moet uniek zijn.|  
|enableNativeCodeExecution|booleaans|Optioneel. De standaard waarde is `true`; uitvoering van systeem eigen code en volledig vertrouwen zijn standaard ingeschakeld. Stel dit kenmerk in op `false` om het uitvoeren van systeem eigen code voor de webfunctie uit te scha kelen en in plaats daarvan Azure gedeeltelijk vertrouwen te gebruiken.|  
...
Het `ConfigurationSettings`-element beschrijft de verzameling configuratie-instellingen voor een webrole. Dit element is de bovenliggende elementen van het `Setting`-element.
...
Het `InputEndpoint`-element beschrijft een extern eind punt naar een webrole.
...
|protocol|string|Vereist. Het transport protocol voor het externe eind punt. Voor een webfunctie zijn mogelijke waarden `HTTP`, `HTTPS`, `UDP`of `TCP`.|  
...
|ignoreRoleInstanceStatus|booleaans|Optioneel. Wanneer de waarde van dit kenmerk is ingesteld op `true`, wordt de status van een service genegeerd en wordt het eind punt niet verwijderd door de load balancer. Stel deze waarde in op `true` nuttig is voor het opsporen van fouten in de actieve instanties van een service. De standaardwaarde is `false`. **Opmerking:**  Een eind punt kan nog steeds verkeer ontvangen, zelfs wanneer de rol niet de status gereed heeft.|  
...
|protocol|string|Vereist. Het transport protocol voor het interne eind punt. Mogelijke waarden zijn `HTTP`, `TCP`, `UDP`of `ANY`.<br /><br /> Een waarde van `ANY` geeft aan dat elk protocol een wille keurige poort is toegestaan.|  
...
|protocol|string|Vereist. Het transport protocol voor het interne eind punt. Mogelijke waarden zijn `udp` en `tcp`. Gebruik `tcp` voor verkeer op basis van http/https.|  
...
Het `Runtime`-element beschrijft een verzameling instellingen voor een omgevings variabele voor een webrole waarmee de runtime omgeving van het proces van Azure host wordt beheerd. Dit element is de bovenliggende elementen van het `Environment`-element. Dit element is optioneel en een rol kan slechts één runtime blok hebben.
...
| Kenmerk | Type | Beschrijving |  
| --------- | ---- | ----------- |  
|assemblyName|string|Vereist. Het pad en de bestands naam van de assembly met het toegangs punt. Het pad is relatief ten opzichte van de map **\\%ROLEROOT%\Approot** ( **\\%ROLEROOT%\Approot** niet opgeven in `commandLine`, wordt ervan uitgegaan dat dit wordt gebruikt). **% ROLEROOT%** is een omgevings variabele die wordt onderhouden door Azure en vertegenwoordigt de locatie van de hoofdmap voor uw rol. De map **\\%ROLEROOT%\Approot** vertegenwoordigt de toepassingsmap voor uw rol.<br /><br /> Voor HWC-rollen is het pad altijd relatief ten opzichte van de **\\%ROLEROOT%\Approot\bin** -map.<br /><br /> Als de assembly niet kan worden gevonden ten opzichte van **\\%ROLEROOT%\Approot** map, wordt de **\\%ROLEROOT%\Approot\bin** doorzocht voor volledige IIS-en IIS Express webrollen.<br /><br /> Het gedrag van deze terugvallen voor volledige IIS is niet een aanbevolen best practice en kan in toekomstige versies worden verwijderd.|  
...
Het `Sites`-element beschrijft een verzameling websites en webtoepassingen die worden gehost in een webrole. Dit element is de bovenliggende elementen van het `Site`-element. Als u geen `Sites` element opgeeft, wordt uw webrole gehost als verouderde webrol. u kunt slechts één website hebben gehost in uw webrole. Dit element is optioneel en een rol kan slechts één site blok hebben.

Het `Sites`-element is alleen beschikbaar via de Azure SDK-versie 1,3 of hoger.

##  <a name="Site"></a>Site  
Het `Site`-element geeft een website of webtoepassing op die deel uitmaakt van de webrole.

Het `Site`-element is alleen beschikbaar via de Azure SDK-versie 1,3 of hoger.

In de volgende tabel worden de kenmerken van het element `Site` beschreven.
...
In de volgende tabel worden de kenmerken van het element `VirtualApplication` beschreven.
...
Het element `VirtualDirectory` geeft een mapnaam (ook wel pad genoemd) op die u in IIS opgeeft en wijst deze toe aan een fysieke map op een lokale of externe server.

Het `VirtualDirectory`-element is alleen beschikbaar via de Azure SDK-versie 1,3 of hoger.

In de volgende tabel worden de kenmerken van het element `VirtualDirectory` beschreven.
...
Het `Bindings`-element beschrijft een verzameling bindingen voor een website. Het is het bovenliggende element van het element `Binding`. Het element is vereist voor elk `Site`-element. Zie [communicatie inschakelen voor rolinstanties](cloud-services-enable-communication-role-instances.md)voor meer informatie over het configureren van eind punten.

Het `Bindings`-element is alleen beschikbaar via de Azure SDK-versie 1,3 of hoger.

##  <a name="Binding"></a>Dwingen  
Het `Binding`-element geeft configuratie gegevens op die nodig zijn voor aanvragen om te communiceren met een website of webtoepassing.
...
Het element `Contents` beschrijft het verzamelen van inhoud voor een webrole. Dit element is de bovenliggende elementen van het `Content`-element.
...
| Kenmerk | Type | Beschrijving |  
| --------- | ---- | ----------- |  
|Pad|string|Vereist. Relatief of absoluut pad van een lokale map waarvan de inhoud wordt gekopieerd naar de virtuele machine van Azure. De uitbrei ding van omgevings variabelen in het mappad wordt ondersteund.|  
