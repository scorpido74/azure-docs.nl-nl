...
Als u deze fout wilt oplossen, gaat u naar de `server parameters` van uw MySQL-exemplaar en wijzigt u de `time_zone` waarde van *systeem* naar *+ 0:00*.
...
De naam van het Azure veer Cloud service-exemplaar wordt gebruikt voor het aanvragen van een subdomeinnaam onder `azureapps.io`, zodat de installatie mislukt als de naam in conflict is met een bestaande. Mogelijk vindt u meer informatie in de activiteiten Logboeken.
...
1. Als het eind punt voor de installatie van de veer boot is ingeschakeld, gaat u naar de Azure Portal en zoekt u naar de pagina configuratie van uw toepassing.  Voeg een omgevings variabele toe met de naam `MANAGEMENT_ENDPOINTS_WEB_EXPOSURE_INCLUDE` en de waarde `*`. 
...
Als u de metrische gegevens van _JVM_ kunt zien, maar geen metrische gegevens van _Tomcat_, controleert u of de `spring-boot-actuator` afhankelijkheid is ingeschakeld in uw toepassings pakket en of deze correct wordt opgestart.
