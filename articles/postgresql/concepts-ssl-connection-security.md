...
In het volgende voor beeld ziet u hoe u verbinding maakt met uw PostgreSQL-server met behulp van het opdracht regel programma psql. Gebruik de `sslmode=verify-full` instelling Connection String voor het afdwingen van SSL-certificaat verificatie. Geef het pad van het lokale certificaat bestand `sslrootcert` door aan de para meter.
