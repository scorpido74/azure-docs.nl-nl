...
3. Rol `<user>@yourtenant.onmicrosoft.com` in Azure Database for PostgreSQL maken.
4. Maak `<user>@yourtenant.onmicrosoft.com` lid van de rol azure_ad_user. Dit moet alleen worden verleend aan Azure AD-gebruikers.
...
Wanneer u de `psql` opdracht regel-client gebruikt, moet het toegangs token worden door gegeven via de `PGPASSWORD` omgevings variabele, omdat het toegangs token de wachtwoord lengte overschrijdt die `psql` rechtstreeks kan accepteren:
