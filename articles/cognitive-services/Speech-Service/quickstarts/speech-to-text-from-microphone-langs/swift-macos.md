...
1. De app moet ook het gebruik van de microfoon in het bestand declareren `Info.plist` . Klik op het bestand in het overzicht en voeg de sleutel ' beschrijving privacy-microfoon gebruik ' toe, met een waarde als ' microfoon is vereist voor spraak herkenning '.
...
1. `AppDelegate.swift`Vervang in de teken reeks door `YourSubscriptionKey` de sleutel van uw abonnement.
...
1. Bouw de voorbeeld code en voer deze **Product**  >  uit door in het menu product**uitvoeren** te selecteren of te klikken op de knop **afspelen** .
