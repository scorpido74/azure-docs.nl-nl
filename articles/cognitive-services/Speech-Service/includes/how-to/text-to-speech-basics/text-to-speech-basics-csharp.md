...
Als u de voorbeelden in dit `using` artikel wilt uitvoeren, neemt u de volgende instructies boven aan uw script op.
...
Vervolgens maakt u [`SpeechSynthesizer`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechsynthesizer?view=azure-dotnet) een object dat tekst-naar-spraakconversies en -uitvoer uitvoert naar luidsprekers, bestanden of andere uitvoerstromen. De [`SpeechSynthesizer`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechsynthesizer?view=azure-dotnet) accepteert als params het [`SpeechConfig`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechconfig?view=azure-dotnet) object gemaakt in [`AudioConfig`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.audio.audioconfig?view=azure-dotnet) de vorige stap, en een object dat aangeeft hoe de uitvoerresultaten moeten worden behandeld.

Als u wilt `AudioConfig` beginnen, maakt u `.wav` een om `FromWavFileOutput()` de uitvoer automatisch naar een `using` bestand te schrijven, met behulp van de functie, en het te instantiëren met een instructie. Een `using` instructie in deze context beschikt automatisch over onbeheerde resources en zorgt ervoor dat het object na verwijdering buiten het bereik valt.
...
Vervolgens instantiate `SpeechSynthesizer` een `using` met een andere verklaring. Geef `config` uw object `audioConfig` en het object als params. Vervolgens is het uitvoeren van spraaksynthese en het `SpeakTextAsync()` schrijven naar een bestand net zo eenvoudig als het uitvoeren met een reeks tekst.
...
Deze keer slaat u het [`SpeechSynthesisResult`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechsynthesisresult?view=azure-dotnet) resultaat op een variabele. De `AudioData` eigenschap `byte []` bevat een van de uitvoergegevens. U hiermee `byte []` handmatig werken of u [`AudioDataStream`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.audiodatastream?view=azure-dotnet) de klasse gebruiken om de in-memory stream te beheren. In dit voorbeeld `AudioDataStream.FromResult()` gebruikt u de statische functie om een stroom uit het resultaat te halen.
...
In dit voorbeeld geeft u een hifi-RIFF-indeling `Riff24Khz16BitMonoPcm` op door de `SpeechSynthesisOutputFormat` indeling op het object in te `SpeechConfig` stellen. Net als in het voorbeeld in [`AudioDataStream`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.audiodatastream?view=azure-dotnet) de vorige sectie, gebruikt u om een in-memory stream van het resultaat te krijgen en vervolgens naar een bestand te schrijven.
...
Vervolgens moet u het verzoek voor spraaksynthese wijzigen om naar uw XML-bestand te verwijzen. De aanvraag is meestal hetzelfde, `SpeakTextAsync()` maar in `SpeakSsmlAsync()`plaats van de functie te gebruiken, gebruikt u . Deze functie verwacht een XML-tekenreeks, zodat u eerst uw `File.ReadAllText()`SSML-config als tekenreeks laadt met behulp van . Vanaf hier is het resultaatobject precies hetzelfde als eerdere voorbeelden.

> [!NOTE]
> Als u Visual Studio gebruikt, vindt uw buildconfig waarschijnlijk uw XML-bestand niet standaard. Als u dit wilt oplossen, klikt u met de rechtermuisknop op het XML-bestand en selecteert u **Eigenschappen**. Wijzig **Actie voor bouwen** in *inhoud*en wijzig Kopiëren **in uitvoermap** om *altijd te kopiëren*.
