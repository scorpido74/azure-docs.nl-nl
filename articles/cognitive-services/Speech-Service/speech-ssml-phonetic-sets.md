...
| Middel      | `k`   | **c**UT   | Sla-**verzonken** | Ira**q**   |
| Kg      | `g`   | **g**o    | a**g**o     | DRA**g**   |
...
De telefoonset van de spraak service plaatst stress na de klinker van de benadrukte letter greep. de `fr-FR` Speech Service Phone set biedt geen ondersteuning voor de IPA substress ' ˌ '. Als de IPA-subbelasting nodig is, moet u de IPA rechtstreeks gebruiken.
...
| ls      | `f`   | **f**Emme   | su**ff**IXe   | Bo**f**                          |
| Kg      | `g`   | **g**auche  | é**g**ALE     | BA**gu**e                        |
| lopen     | `ŋ`   |             |               | [<sup>1</sup>](#fr-1)Park**ING** |
| hy     | `ɥ`   | h**u**/estand   | n**u**Ierland     |                                  |
| Middel      | `k`   | **c**Arte   | é**c**Aille   | **c** zijn                          |
...
<a id="fr-1"></a>alleen 
**1** *voor een aantal vreemde woorden.*

> [!TIP]
> De `fr-FR` Speech Service Phone set biedt geen ondersteuning voor de volgende Franse liasions, `n‿`, `t‿`en `z‿`. Als ze nodig zijn, kunt u overwegen om de IPA rechtstreeks te gebruiken.
...
| ls | `f` | **F**ahrtdauer | Angri**ff**slustig | abbruchrei**f** |  |
| Kg | `g` | **g**UT |  | [<sup>6</sup>](#de-c-6) GRE**g** |  |
| h | `h` | **H**ausanbau |  |  |  |
| Y | `j` | **J**od | Reakt | hu |  |
| Middel | `k` | **K**oma | Aspe**k**t | FLEC**k** |  |
...
<a id="de-c-6"></a>
**6** *woord-terminal alleen in woorden van vreemde oorsprong zoals GRE**g**.*<br>
<a id="de-c-7"></a>
**7** *alleen in woorden van vreemde oorsprong, zoals: **aardgas**Uyen.*<br>
<a id="de-c-8"></a>
**8** *alleen in woorden van vreemde oorsprong, zoals: **S**taccato.*<br>
<a id="de-c-9"></a>
**9** *alleen in woorden van vreemde oorsprong, zoals: go**OO**ve.*<br>
<a id="de-c-10"></a>
**10** *de IPA `x` is een harde ' Ch ' na alle niet-voorste klinkers (a, AA, Oh, wat,man en diphthong AW).*<br>
<a id="de-c-11"></a>
**11** *het IPA `ç` is een zacht ' Ch ' na de IH, de iy, eh, AE, uy, UE, OE, EU ook in diphthongs ay, Oy) en mede klinkers*<br>
<a id="de-c-12"></a>
...
> [!NOTE]
> We moeten een [GS\]-telefoon toevoegen tussen twee afzonderlijke klinkers, met uitzonde ring van de twee klinkers van een authentiek diphthong. Deze mondelinge medeklinker is een glottis slag. Zie <a href="http://en.wikipedia.org/wiki/Glottal_stop" target="_blank">e glottis slag <span class="docon docon-navigate-external x-hidden-focus"></a></a>voor meer informatie.
...
| Ik      | `i`   | **Ik**bérica  | AV**i**Spa    | belastingen     |
| &      | `e`   | **e**LeFante | bij**e**nKlik    | Elefant**e** |
...
| d      | `d`        | **d**Edo   |                | portlan**d**   |
|        | `ð`        |            | de**d**o       | Verda**d**     |
| ls      | `f`        | **f**ácil  | ele**vooraf**   | PU**f**        |
| Kg      | `g`        | **g**Anga  |                | dópin**g**     |
|        | `ɣ`        |            | een**g**UA       | tuare**g**     |
| "J"      | `j`        | **Ik**Odo   | CAL **-** Ente   | opnieuw**y**        |
| JJ     | `j.j` `jj` |            | VI**ll**a      |                |
| Middel      | `k`        | **c**oche  | Bo**c**a       | titáni**c**    |
...
> [!TIP]
> De `es-ES` Speech Service Phone set biedt geen ondersteuning voor de volgende Spaanse IPA, `β`, `ð`en `ɣ`. Als ze nodig zijn, kunt u overwegen om de IPA rechtstreeks te gebruiken.

# <a name="zh-cn"></a>[zh-CN](#tab/zh-CN)

De telefoonset spraak service voor `zh-CN` is gebaseerd op de ingebouwde telefoon <a href="https://en.wikipedia.org/wiki/Pinyin" target="_blank">- <span class="docon docon-navigate-external x-hidden-focus"></span> pinyinset</a> .
...
De telefoonset spraak service voor `zh-TW` is gebaseerd op de systeem eigen telefoon- <a href="https://en.wikipedia.org/wiki/Bopomofo" target="_blank">Bopomofo <span class="docon docon-navigate-external x-hidden-focus"></span> </a> -set.
...
De telefoonset spraak service voor `ja-JP` is gebaseerd op de systeem eigen <a href="https://en.wikipedia.org/wiki/Kana" target="_blank">telefoon <span class="docon docon-navigate-external x-hidden-focus"></span> Kana</a> -set.
