...
Deze quickstart vereist C# 7.1 of hoger. Er zijn een paar manieren om de C#-versie voor uw project te wijzigen. In deze handleiding laten we u zien `translate-sample.csproj` hoe u het bestand aanpassen. Zie [De C#-taalversie selecteren](https://docs.microsoft.com/dotnet/csharp/language-reference/configure-language-version)voor alle beschikbare opties, zoals het wijzigen van de taal in Visual Studio.

Open uw project `translate-sample.csproj`en open . Zorg ervoor `LangVersion` dat dit is ingesteld op 7.1 of hoger. Als er geen eigenschapsgroep voor de taalversie is, voegt u de volgende regels toe:
...
Vervolgens gaan we een reeks klassen maken die worden gebruikt bij het deserialiseren van de JSON-respons die wordt geretourneerd door de Translator Text API.
...
Maak `Program` in de klasse een `TranslateTextRequest()`asynchrone functie genaamd . Deze functie heeft `subscriptionKey`vier `host` `route`argumenten `inputText`nodig: , , en .
...
In de laatste stap roept u `TranslateTextRequest()` aan in de functie `Main`. In deze steekproef vertalen we naar`de`Duitse (`it`),`ja`Italiaanse (`th`), Japanse ( ) en Thaise ( ). Zoek `static void Main(string[] args)` en vervang deze door deze code:
...
U zult merken `Main`dat in , `subscriptionKey`je `endpoint`verklaart `route`, , en . Daarnaast vraagt u de gebruiker om `Console.Readline()` invoer met en `textToTranslate`toewijzen van de waarde aan .
