...
U `Main`ziet dat in, u `subscriptionKey`declareert `endpoint`, en `route`. Daarnaast vraagt u de gebruiker om invoer met `Console.Readline()` en wijst u de waarde toe aan. `textToTranslate`
