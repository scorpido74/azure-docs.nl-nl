...
Voor deze snelstart is C# 7.1 of hoger vereist. Er zijn een aantal manieren om de C#-versie voor uw project te wijzigen. In deze handleiding laten we u zien hoe u het `sentences-sample.csproj`-bestand kunt aanpassen. Zie [De C#-taalversie selecteren](https://docs.microsoft.com/dotnet/csharp/language-reference/configure-language-version) voor alle beschikbare opties, zoals het wijzigen van de taal in Visual Studio.

Open uw project en open vervolgens `sentences-sample.csproj`. Zorg ervoor dat `LangVersion` is ingesteld op 7.1 of hoger. Als er geen eigenschapsgroep bestaat voor de taalversie, voegt u deze regels toe:
...
Maak in de `Program`-klasse een nieuwe functie met de naam `BreakSentenceRequest()`. Deze functie gebruikt vier argumenten: `subscriptionKey`, `endpoint`, `route` en `inputText`.
...
## <a name="serialize-the-break-sentence-request"></a>De aanvraag voor een BreakSentence serialiseren
...
U ziet dat u in `Main` `subscriptionKey`, `endpoint`, `route` en de tekst declareert om `breakSentenceText` te evalueren.
