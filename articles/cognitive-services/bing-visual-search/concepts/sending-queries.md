...
Hier volgen de headers die in uw aanvraag moeten worden opgegeven. De `Content-Type` koptekst `Ocp-Apim-Subscription-Key` en zijn de enige vereiste headers, maar u moet `User-Agent`ook, `X-MSEdge-ClientID` `X-MSEdge-ClientIP`, en. `X-Search-Location`
...
Als de installatie kopie een herkende entiteit bevat, zoals een cultuur bekende/populaire persoon, plaats of ding, kan een van de Tags een entiteit inzicht bevatten. De `mainEntity` velden `data` en zijn alleen beschikbaar als het `enableEntityData` kenmerk in de `Content-Type` kop is ingesteld op `true`.
