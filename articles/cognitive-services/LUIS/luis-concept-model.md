...
De volgende utterance is een **positief voor beeld** voor `OrderBook` de intentie en een **negatief voor beeld** voor `ShippingLocation` de `None` doel stellingen: 
...
* TravelOrder (door machine geleerde entiteit)
    ...
    * Locatie (door machine geleerde entiteit)
        * Oorsprong (rol gevonden door context zoals `from`)
        * Doel (rol gevonden door context zoals `to`)
...
Sommige van deze gegevens, zoals de oorsprongs locatie en doel locatie, moeten worden geleerd uit de context van de utterance, mogelijk met een dergelijke formule `from` ring `to`en. Andere delen van gegevens kunnen worden geëxtraheerd met exact overeenkomende teken reeksen (`Vegan`) of vooraf samengestelde instanties (geographyV2 van `Seattle` en `Cairo`). 
...
Voor een lijst met lucht havens is de vermelding in de lijst voor Seattle de `Seattle` plaatsnaam en de synoniemen voor Seattle bevatten de luchthaven code voor Seattle samen met de Omringende steden en steden:

|`Seattle`Synoniemen van lijst entiteit|
|--|
|`Sea`|
|`seatac`|
|`Bellevue`|
