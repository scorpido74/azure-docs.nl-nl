...
[Reference documentation](https://docs.microsoft.com/python/api/overview/azure/cognitiveservices/textanalytics?view=azure-python) | Rubygems-voor[beelden](https://github.com/Azure-Samples/cognitive-services-quickstart-code) ([Source code](https://github.com/Azure/azure-sdk-for-ruby/tree/master/data/azure_cognitiveservices_textanalytics) | [package)](https://rubygems.org/gems/azure_cognitiveservices_textanalytics) | voor referentie documentatie
...
Maak in een console venster (zoals cmd, Power shell of bash) een nieuwe map voor uw app en navigeer ernaar. Maak vervolgens een bestand met `GemFile`de naam en een ruby-bestand voor uw code.

```console
mkdir myapp && cd myapp
```

Voeg in `GemFile`uw de volgende regels toe om de client bibliotheek toe te voegen als een afhankelijkheid.
...
Maak een klasse met `TextAnalyticsClient`de naam. 
...
Maak in deze klasse een functie die wordt `initialize` aangeroepen om de client te verifiëren met behulp van uw sleutel en eind punt. 

[!code-ruby[initialize function for authentication](~/cognitive-services-ruby-sdk-samples/samples/text_analytics.rb?name=initialize)]

Buiten de-klasse gebruikt u de functie van `new()` de client om deze te instantiëren.
...
Maak in het client object een functie met de `AnalyzeSentiment()` naam die een lijst met invoer documenten gebruikt die later zullen worden gemaakt. Roep de functie van `sentiment()` de client aan en ontvang het resultaat. Herhaal vervolgens de resultaten en druk de document-ID en sentiment Score af. Een score dichter bij 0 geeft aan dat er een negatieve sentiment is, terwijl een score dichter bij 1 wijst op een positieve sentiment.
...
Maak buiten de client functie een nieuwe functie met de naam `SentimentAnalysisExample()` die het `TextAnalyticsClient` object maakt dat eerder is gemaakt. Maak een lijst met `MultiLanguageInput` objecten die de documenten bevatten die u wilt analyseren. Elk object bevat een `id`, en `Language` een `text` -kenmerk. Het `text` kenmerk slaat de tekst op die moet worden geanalyseerd `language` , is de taal van het document en de `id` kan een wille keurige waarde zijn. Roep vervolgens de functie van `AnalyzeSentiment()` de client aan.
...
Maak in het client object een functie met de `DetectLanguage()` naam die een lijst met invoer documenten gebruikt die later zullen worden gemaakt. Roep de functie van `detect_language()` de client aan en ontvang het resultaat. Herhaal vervolgens de resultaten en druk de document-ID en de gedetecteerde taal af.
...
Maak buiten de client functie een nieuwe functie met de naam `DetectLanguageExample()` die het `TextAnalyticsClient` object maakt dat eerder is gemaakt. Maak een lijst met `LanguageInput` objecten die de documenten bevatten die u wilt analyseren. Elk object bevat een `id`, en een `text` -kenmerk. Het `text` kenmerk slaat de tekst die moet worden geanalyseerd en de `id` kan een wille keurige waarde zijn. Roep vervolgens de functie van `DetectLanguage()` de client aan.
...
Maak in het client object een functie met de `RecognizeEntities()` naam die een lijst met invoer documenten gebruikt die later zullen worden gemaakt. Roep de functie van `entities()` de client aan en ontvang het resultaat. Herhaal vervolgens de resultaten en druk de ID van elk document af en de herkende entiteiten.
...
Maak in het client object een functie met de `ExtractKeyPhrases()` naam die een lijst met invoer documenten gebruikt die later zullen worden gemaakt. Roep de functie van `key_phrases()` de client aan en ontvang het resultaat. Herhaal vervolgens de resultaten en druk de document-ID en de geëxtraheerde sleutel zinnen af.
...
Maak buiten de client functie een nieuwe functie met de naam `KeyPhraseExtractionExample()` die het `TextAnalyticsClient` object maakt dat eerder is gemaakt. Maak een lijst met `MultiLanguageInput` objecten die de documenten bevatten die u wilt analyseren. Elk object bevat een `id`, een `language`en een `text` -kenmerk. Het `text` kenmerk slaat de tekst op die moet worden geanalyseerd `language` , is de taal van de tekst en het `id` kan een wille keurige waarde zijn. Roep vervolgens de functie van `ExtractKeyPhrases()` de client aan.
