...
|Header|Beschrijving|  
|------------|-----------------|  
|zodat|Optionele aanvraagheader.<br /><br /> Het standaard media type is Application/JSON. Als u wilt opgeven dat het antwoord [JSON-LD](https://json-ld.org/)gebruikt, stelt u de Accept-header in op Application/LD + JSON.|  
...
|----------|-----------|----------|--------------|
|aantal <a name="count" />|Het aantal resultaten dat moet worden geretourneerd, te beginnen met de index die is opgegeven door de para meter `offset`.|Tekenreeks|Nee|   
...
|<a name="offset"/>offset|De index voor het starten van resultaten die zijn opgegeven met de para meter `count`.|Geheel getal|Nee|  
...
|entityPresentationInfo|Aanvullende informatie over de entiteit, zoals hints, die u kunt gebruiken om het type van de entiteit te bepalen. Bijvoorbeeld of het nu gaat om een restaurant of Hotel. Het veld `entityScenario` wordt ingesteld op lijst item.|EntityPresentationInfo|  
