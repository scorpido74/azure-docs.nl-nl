...
U kunt onderhoud starten op een VM als **IsCustomerInitiatedMaintenanceAllowed** is ingesteld op **True**. Gebruik de cmdlet [set-AzVmss](/powershell/module/az.compute/set-azvmss) met de para meter `-PerformMaintenance`.
...
De volgende aanroep initieert onderhoud op een VM-exemplaar als `IsCustomerInitiatedMaintenanceAllowed` is ingesteld op **True**:
