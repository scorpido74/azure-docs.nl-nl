...
* Windows Power shell, versie 3,0 of 4,0. Als u de versie van Windows Power shell wilt zoeken`$PSVersionTable` , typt u: en `PSVersion` verifieert u de waarde van 3,0 of 4,0. Zie [Windows Management framework 3,0](https://www.microsoft.com/download/details.aspx?id=34595) of [windows Management Framework 4,0](https://www.microsoft.com/download/details.aspx?id=40855)als u een compatibele versie wilt installeren.

Gebruik de cmdlet Get-Help voor meer informatie over de cmdlets die u in deze zelf studie ziet.

    Get-Help <cmdlet-name> -Detailed

Als u bijvoorbeeld hulp voor de `New-AzRedisCache` cmdlet wilt weer geven, typt u:

    Get-Help New-AzRedisCache -Detailed

### <a name="how-to-connect-to-other-clouds"></a>Verbinding maken met andere Clouds
De Azure-omgeving is `AzureCloud`standaard, die de algemene Azure Cloud-instantie vertegenwoordigt. Als u verbinding wilt maken met een ander exemplaar `Connect-AzAccount` , gebruikt u `-Environment` de opdracht`EnvironmentName` met de of-opdracht regel optie met de gewenste omgeving of omgevings naam.

Voer de `Get-AzEnvironment` cmdlet uit om de lijst met beschik bare omgevingen weer te geven.
...
> [!IMPORTANT]
> De eerste keer dat u een Azure-cache maakt voor redis in een abonnement met behulp van de Azure Portal `Microsoft.Cache` , registreert de Portal de naam ruimte voor dat abonnement. Als u probeert de eerste Azure-cache voor redis in een abonnement te maken met behulp van Power shell, moet u die naam ruimte eerst registreren met de volgende opdracht. andere cmdlets, `New-AzRedisCache` zoals en `Get-AzRedisCache` mislukken.
> 
> `Register-AzResourceProvider -ProviderNamespace "Microsoft.Cache"`
> 
> 

Voer de volgende opdracht uit om een lijst met beschik `New-AzRedisCache`bare para meters en beschrijvingen ervan weer te geven voor.
...
Als u een Premium-cache wilt maken, geeft u een grootte van P1 (6 GB-60 GB), P2 (13 GB-130 GB), P3 (26 GB-260 GB) of P4 (53 GB-530 GB). Als u Clustering wilt inschakelen, geeft u een aantal `ShardCount` Shard op met behulp van de para meter. In het volgende voor beeld wordt een P1 Premium-cache gemaakt met 3 Shards. Een P1 Premium-cache is 6 GB groot en omdat we drie Shards hebben opgegeven, is de totale grootte 18 GB (3 x 6 GB).

    New-AzRedisCache -ResourceGroupName myGroup -Name mycache -Location "North Central US" -Sku Premium -Size P1 -ShardCount 3

Als u waarden voor de `RedisConfiguration` para meter wilt opgeven, moet `{}` u de waarden in als sleutel `@{"maxmemory-policy" = "allkeys-random", "notify-keyspace-events" = "KEA"}`/waarde-paren. In het volgende voor beeld wordt een standaard cache van `allkeys-random` 1 GB gemaakt met maxmemory-beleid en `KEA`-meldingen die zijn geconfigureerd met. Zie voor meer informatie de [ruimte meldingen (geavanceerde instellingen)](cache-configure.md#keyspace-notifications-advanced-settings) en het [geheugen beleid](cache-configure.md#memory-policies).
...
Zie voor meer informatie over `databases` de eigenschap [standaard Azure-cache voor redis-server configuratie](cache-configure.md#default-redis-server-configuration). Voor meer informatie over het maken van een cache met behulp van de cmdlet [New-AzRedisCache](https://docs.microsoft.com/powershell/module/az.rediscache/new-azrediscache) raadpleegt u de sectie voor het maken van een Azure-cache voor redis.
...
Voer de volgende opdracht uit om een lijst met beschik `Set-AzRedisCache`bare para meters en beschrijvingen ervan weer te geven voor.
...
De `Set-AzRedisCache` cmdlet kan worden gebruikt voor het bijwerken van eigenschappen `Size`zoals `Sku`, `EnableNonSslPort`, en de `RedisConfiguration` waarden. 
...
`Set-AzRedisCache`kan worden gebruikt voor het schalen van een Azure-cache voor redis `Size`- `Sku`instantie wanneer `ShardCount` de, of-eigenschappen worden gewijzigd. 
...
In het volgende voor beeld ziet u hoe u een `myCache` cache kunt schalen met de naam in een cache van 2,5 GB. Houd er rekening mee dat deze opdracht voor zowel een basis-als een standaard cache werkt.

    Set-AzRedisCache -ResourceGroupName myGroup -Name myCache -Size 2.5GB

Nadat deze opdracht is gegeven, wordt de status van de cache geretourneerd (vergelijkbaar met aanroepen `Get-AzRedisCache`). Houd er rekening `ProvisioningState` mee `Scaling`dat.
...
Voer de volgende opdracht uit om een lijst met beschik `Get-AzRedisCache`bare para meters en beschrijvingen ervan weer te geven voor.
...
Om informatie over alle caches in een specifieke resource groep te retour neren `Get-AzRedisCache` , voert `ResourceGroupName` u uit met de para meter.

    Get-AzRedisCache -ResourceGroupName myGroup

Om informatie over een specifieke cache te retour neren `Get-AzRedisCache` , voert `Name` u uit met de para meter met de naam van `ResourceGroupName` de cache en de para meter met de resource groep die die cache bevat.
...
Voer de volgende opdracht uit om een lijst met beschik `Get-AzRedisCacheKey`bare para meters en beschrijvingen ervan weer te geven voor.
...
Als u de sleutels voor uw cache wilt ophalen, `Get-AzRedisCacheKey` roept u de cmdlet aan en geeft u de naam van de cache door in de naam van de resource groep die de cache bevat.
...
Voer de volgende opdracht uit om een lijst met beschik `New-AzRedisCacheKey`bare para meters en beschrijvingen ervan weer te geven voor.
...
Als u de primaire of secundaire sleutel voor uw cache opnieuw wilt genereren, `New-AzRedisCacheKey` roept u de cmdlet aan en geeft u de naam en de resource `Primary` groep `Secondary` op, `KeyType` en geeft u of voor de para meter op. In het volgende voor beeld wordt de secundaire toegangs sleutel voor een cache opnieuw gegenereerd.
...
Voer de volgende opdracht uit om een lijst met beschik `Remove-AzRedisCache`bare para meters en beschrijvingen ervan weer te geven voor.
...
In het volgende voor beeld wordt de naam `myCache` van de cache verwijderd.
...
U kunt gegevens importeren in een Azure-cache voor redis-exemplaar `Import-AzRedisCache` met behulp van de-cmdlet.
...
Voer de volgende opdracht uit om een lijst met beschik `Import-AzRedisCache`bare para meters en beschrijvingen ervan weer te geven voor.
...
U kunt gegevens exporteren vanuit een Azure-cache voor een redis- `Export-AzRedisCache` exemplaar met behulp van de-cmdlet.
...
Voer de volgende opdracht uit om een lijst met beschik `Export-AzRedisCache`bare para meters en beschrijvingen ervan weer te geven voor.
...
U kunt uw Azure-cache opnieuw opstarten voor redis- `Reset-AzRedisCache` exemplaar met behulp van de-cmdlet.
...
Voer de volgende opdracht uit om een lijst met beschik `Reset-AzRedisCache`bare para meters en beschrijvingen ervan weer te geven voor.
