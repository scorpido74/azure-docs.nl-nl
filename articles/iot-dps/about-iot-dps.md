...
description: Beschrijft apparaatinrichting in Azure met de DPS (Device Provisioning Service) en IoT Hub
author: wesmc7777
ms.author: wesmc
ms.date: 04/04/2019
ms.topic: overview
...
Er zijn veel inrichtingsscenario's waarin DPS een uitstekende keuze is om apparaten te verbinden en configureren op IoT Hub, zoals:
...
Alle scenario's in de vorige sectie kunnen worden uitgevoerd met DPS voor zero-touch provisioning met dezelfde stroom. Veel van de handmatige stappen die traditioneel betrokken zijn bij de inrichting worden geautomatiseerd met DPS om de tijd te verkorten om IoT-apparaten te implementeren en het risico op handmatige fouten te verlagen. In de volgende sectie wordt beschreven wat er achter de schermen gebeurt om een apparaat in te richten. De eerste stap is handmatig, alle overige gebeuren automatisch.
...
2. Apparaat neemt contact op met het DPS-eindpunt dat in de fabriek is ingesteld. Het apparaat geeft de identificerende informatie door aan DPS om zijn identiteit te bewijzen.
3. DPS valideert de identiteit van het apparaat door de registratie-id en -sleutel te valideren aan de hand van de vermelding van de inschrijvingslijst met behulp van een nonce-uitdaging[(Trusted Platform Module)](https://trustedcomputinggroup.org/work-groups/trusted-platform-module/)of standaard X.509-verificatie (X.509).
4. DPS registreert het apparaat met een IoT-hub en vult de [gewenste dubbele status](../iot-hub/iot-hub-devguide-device-twins.md)van het apparaat in.
5. De IoT-hub retourneert apparaat-id-informatie naar DPS.
6. DPS retourneert de informatie over de IoT-hub-verbinding naar het apparaat. Het apparaat kan nu rechtstreeks gegevens gaan verzenden naar de IoT-hub.
...
Er zijn twee verschillende stappen in het implementatieproces van een apparaat waarin DPS een onderdeel neemt dat onafhankelijk van elkaar kan worden uitgevoerd:
...
Beide stappen sluiten naadloos aan op bestaande productie- en implementatieprocessen. DPS vereenvoudigt zelfs bepaalde implementatieprocessen die handmatig werk omvatten om verbindingsinformatie op het apparaat te krijgen.
...
DPS introduceert geen nieuwe stap in het productieproces; in plaats daarvan sluit het aan bij de bestaande stap die de initiële software installeert en (idealiter) de HSM op het apparaat. In plaats van een apparaat-id te maken in deze stap, wordt het apparaat geprogrammeerd met de gegevens van de inrichtingsservice, waardoor het apparaat bij inschakeling de service kan aanroepen om de verbindingsgegevens/toewijzing van de IoT-oplossing op te vragen.
...
* **Meerdere toewijzingsbeleidsregels** om te bepalen hoe DPS apparaten toewijst aan IoT-hubs ter ondersteuning van uw scenario's: laagste latentie, gelijkmatige gewogen verdeling (standaard) en statische configuratie via de inschrijvingslijst. Latentie wordt bepaald met dezelfde methode als [Traffic Manager](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-routing-methods#performance).
* **Controle en logboekregistratie van diagnostische gegevens** om ervoor te zorgen dat alles goed werkt.
* **Multihub-ondersteuning** stelt DPS in staat om apparaten aan meer dan één IoT-hub toe te wijzen. DPS kan met hubs praten op meerdere Azure-abonnementen.
* **Met ondersteuning voor meerdere regio's** kan DPS apparaten toewijzen aan IoT-hubs in andere regio's.
* **Versleuteling voor gegevens in rust** maakt het mogelijk gegevens in DPS te versleutelen en transparant te decoderen met behulp van 256-bits AES-versleuteling, een van de sterkste blokcijfers die beschikbaar zijn, en is FIPS 140-2-compatibel.
...
Net als alle Azure IoT-services werkt DPS cross-platform met verschillende besturingssystemen. Azure biedt open source-SDK's in een veelheid aan [talen](https://github.com/Azure/azure-iot-sdks) ter bevordering van het verbinden van apparaten en het beheren van de service. DPS ondersteunt de volgende protocollen voor het verbinden van apparaten:
...
> [!NOTE]
> DPS is globaal en niet gebonden aan een locatie. U moet echter een gebied opgeven waarin de metagegevens die aan uw DPS-profiel zijn gekoppeld, zich bevinden.

## <a name="availability"></a>Beschikbaarheid
Er is een 99,9% Service Level Agreement voor DPS, en u [de SLA lezen.](https://azure.microsoft.com/support/legal/sla/iot-hub/) In de volledige [Azure SLA](https://azure.microsoft.com/support/legal/sla/) wordt de gegarandeerde beschikbaarheid van Azure als geheel uitgelegd.
...
DPS automatiseert het inrichten van apparaten met Azure IoT Hub. Lees hier meer informatie over [IoT Hub](https://docs.microsoft.com/azure/iot-hub/).
...
> [IoT Hub Device Provisioning Service instellen met de Azure-portal](quick-setup-auto-provision.md)
> Een gesimuleerd apparaat
> maken[en inrichten](quick-create-simulated-device.md)[Set up device for provisioning](tutorial-set-up-device.md)
