...
* [Visual Studio Code](https://code.visualstudio.com/): De gratis, licht gewichts-en open-source-tekst editor van micro soft met de Power shell-extensie die een volledige Power shell-ontwikkel ervaring biedt.
* Een Power shell-console: Fout opsporing met behulp van dezelfde opdrachten die u zou gebruiken voor het opsporen van fouten in andere Power shell-processen.
...
De functie code in `run.ps1` ziet eruit als het volgende script:
...
U hoeft alleen maar een aanroep toe te voegen aan de `Wait-Debugger`-cmdlet, net boven de `if`-instructie, als volgt:
...
Fout opsporing begint bij de instructie `if`. 

Als `Wait-Debugger` is geïmplementeerd, kunt u de functies nu fouten opsporen met behulp van Visual Studio code of een Power shell-console.
...
1. Druk op F1 om de opdracht pallet weer te geven en zoek naar `Session`.

1. Kies **PowerShell: Sessie menu weer geven @ no__t-0.

1. Als uw **huidige sessie** niet **Power shell Core 6**is, kiest u **Switch: Power shell Core 6 @ no__t-0.
...
Controleer of `Wait-Debugger` is ingesteld in de functie waaraan u het fout opsporingsprogramma wilt koppelen.  Als `Wait-Debugger` is toegevoegd, kunt u fouten opsporen in uw functie-app met Visual Studio code.
...
* Voert `func host start` uit in de terminal om de functie-app in de functions-host te starten.
...
U ziet dat een antwoord niet onmiddellijk wordt geretourneerd. Dat komt omdat `Wait-Debugger` de fout opsporing en Power shell-uitvoering heeft bijgevoegd zodra dit zou kunnen. Dit komt door het [BreakAll-concept](#breakall-might-cause-your-debugger-to-break-in-an-unexpected-place), dat later wordt uitgelegd. Nadat u op de knop `continue` hebt geklikt, wordt het fout opsporingsprogramma nu direct na `Wait-Debugger` op de regel afgebroken.
...
* De Power shell-console met de `Invoke-RestMethod` heeft een resultaat geretourneerd
* De geïntegreerde Power shell-console in Visual Studio code wacht tot een script wordt uitgevoerd

Later wanneer u dezelfde functie aanroept, wordt het fout opsporingsprogramma in Power shell-uitbrei ding direct na de `Wait-Debugger` afgebroken.
...
Met de functie-app die wordt uitgevoerd en de `Wait-Debugger`, kunt u aan het proces koppelen. U hebt nog twee Power shell-consoles nodig.
...
U kunt ook onderbrekings punten op dit niveau instellen met de cmdlet `Set-PSBreakpoint`.

Nadat u het script hebt door lopen en volledig hebt aangeroepen, ziet u het volgende:

* De Power shell-console waarin u `Invoke-RestMethod` hebt uitgevoerd, heeft nu een resultaat geretourneerd.
* De Power shell-console waarin u `Debug-Runspace` hebt uitgevoerd, wacht tot een script is uitgevoerd.

U kunt dezelfde functie opnieuw aanroepen (met behulp van `Invoke-RestMethod`) en het fout opsporingsprogramma wordt direct na de `Wait-Debugger` opdracht afgebroken.
...
### <a name="breakall-might-cause-your-debugger-to-break-in-an-unexpected-place"></a>`BreakAll` kan ertoe leiden dat uw fout opsporingsprogramma op een onverwachte plek breekt
...
De Azure Functions runtime voert enkele opdrachten uit voordat u het script van de `run.ps1` daad werkelijk aanroept. het is dus mogelijk dat het fout opsporingsprogramma eindigt op de `Microsoft.Azure.Functions.PowerShellWorker.psm1` of `Microsoft.Azure.Functions.PowerShellWorker.psd1`.
