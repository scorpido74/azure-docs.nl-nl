...
Blader naar de locatie van de functie code onder _src/main/Java_, open het *functie. java* -project bestand en voeg de volgende para meter toe aan de definitie van de `run`-methode:

```java
@QueueOutput(name = "msg", queueName = "outqueue", connection = "AzureWebJobsStorage") OutputBinding<String> msg
```

De para meter `msg` is een type [`OutputBinding<T>`](/java/api/com.microsoft.azure.functions.outputbinding) dat een verzameling teken reeksen vertegenwoordigt die als berichten naar een uitvoer binding worden geschreven wanneer de functie is voltooid. In dit geval is de uitvoer een opslag wachtrij met de naam `outqueue`. De connection string voor het opslag account wordt ingesteld door de `connection`-methode. In plaats van de connection string zelf, geeft u de toepassings instelling door die het opslag account bevat connection string.
...
U kunt nu de nieuwe para meter `msg` gebruiken om te schrijven naar de uitvoer binding vanuit uw functie code. Voeg de volgende regel code toe vóór het succes van de reactie om de waarde van `name` toe te voegen aan de `msg`-uitvoer binding.
...
De methode `run` moet er nu uitzien als in het volgende voor beeld:
...
Omdat de archetype ook een reeks tests maakt, moet u deze tests bijwerken om de nieuwe para meter `msg` in de hand tekening van de `run`-methode te verwerken.  

Blader naar de locatie van de test code onder _src/test/java_, open het *functie. java* -project bestand en vervang de regel met code onder `//Invoke` door de volgende code.
