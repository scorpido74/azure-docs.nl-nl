...
| **package** | A value that is the Java package for the generated function code. Gebruik de standaard. The examples in this quickstart use `com.fabrikam.functions`. |
...
| **appRegion** | Kies een [regio](https://azure.microsoft.com/regions/) in de buurt of in de buurt van andere services die door uw functie worden gebruikt. De standaardwaarde is `westus`. Run this [Azure CLI] command to get a list of all regions:<br/>`az account list-locations --query '[].{Name:name}' -o tsv` |
...
```
The [function key](functions-bindings-http-webhook.md#authorization-keys) isn't required when running locally. Gebruik `Ctrl+C` in de terminal om de functiecode te stoppen.
