...
Met Visual Studio kunt u C#-klassen bibliotheek functies ontwikkelen, testen en implementeren in Azure. Als deze ervaring uw eerste met Azure Functions is, kunt u meer te weten komen over [een inleiding tot Azure functions](functions-overview.md).
...
De project-sjabloon maakt een C#-project, `Microsoft.NET.Sdk.Functions` installeert het NuGet-pakket en stelt het doel raamwerk in. Het nieuwe project heeft de volgende bestanden:

* **host. json**: Hiermee kunt u de host voor functies configureren. Deze instellingen zijn van toepassing op zowel lokaal als in Azure. Zie voor meer informatie [host. json Reference](functions-host-json.md).
...
3. Bekijk de zojuist toegevoegde klasse. U ziet een statische methode **Run** , die wordt voorzien van het kenmerk **functie naam** . Dit kenmerk geeft aan dat de methode het toegangs punt voor de functie is.
    ...
    De volgende C#-klasse vertegenwoordigt bijvoorbeeld een eenvoudige, getriggerde functie voor wachtrij opslag:
