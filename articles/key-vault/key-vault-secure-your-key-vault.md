...
In het beheervlak gebruikt u RBAC(Role Based Access Control) om de bewerkingen te autoriseren die een beller kan uitvoeren. In het RBAC-model heeft elk Azure-abonnement een exemplaar van Azure AD. U verleent toegang tot gebruikers, groepen en toepassingen vanuit deze map. Toegang wordt verleend voor het beheren van resources in het Azure-abonnement die gebruikmaken van het Azure Resource Manager-implementatiemodel. Als u toegang wilt verlenen, gebruikt u de [Azure-portal](https://portal.azure.com/), de [Azure CLI,](../cli-install-nodejs.md) [Azure PowerShell](/powershell/azureps-cmdlets-docs)of de [API's](https://msdn.microsoft.com/library/azure/dn906885.aspx)azure resource manager .
...
U de toegang tot gegevensvlak beperken door [eindpunten van virtuele netwerkservices voor Azure Key Vault](key-vault-overview-vnet-service-endpoints.md)te gebruiken. U [firewalls en virtuele netwerkregels](key-vault-network-security.md) configureren voor een extra beveiligingslaag.
...
Het beveiligingsteam maakt een sleutelkluis en stelt registratie- en toegangsmachtigingen in. Zie [Over Azure Key Vault-sleutels, -geheimen en -certificaten](about-keys-secrets-and-certificates.md)voor meer informatie over toegangsmachtigingen voor Key Vault.
...
We raden u aan extra beveiligde toegang tot uw sleutelkluis in te stellen door [Key Vault-firewalls en virtuele netwerken te configureren.](key-vault-network-security.md)
...
Zie Wat is Azure Key Vault voor een zelfstudie voor een [beheerder.](key-vault-overview.md)

Zie [Azure Key Vault-logboekregistratie](key-vault-logging.md)voor meer informatie over use logging voor Key Vault.
