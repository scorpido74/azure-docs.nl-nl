...
Dit script heeft geen transformatie stappen. Het Lees programma van het bron bestand `SearchLog.tsv`met de naam schematizes en schrijft de rijenset terug naar een bestand met de naam SearchLog-First-u-SQL. CSV.
...
* **Variabelen van de rijenset**: elke query-expressie die een rijenset produceert, kan worden toegewezen aan een variabele. U-SQL volgt het naamgevings patroon T-SQL-`@searchlog`variabele (bijvoorbeeld) in het script.
...
Het volgende absolute bestandspad verwijst naar een bestand in een Data Lake Store met de `mystore`naam:
