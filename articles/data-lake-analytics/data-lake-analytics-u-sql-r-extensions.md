...
* Gebruik de `REFERENCE ASSEMBLY` instructie om R-extensies in te scha kelen voor het U-SQL-script.
* Gebruik de `REDUCE` bewerking voor het partitioneren van de invoer gegevens op een sleutel.
* De R-uitbrei dingen voor U-SQL bevatten een ingebouwde versmaller () waarmee R-`Extension.R.Reducer`code wordt uitgevoerd op elk hoek punt dat aan de verminderr is toegewezen. 
* Het gebruik van toegewezen benoemde gegevens frames `inputFromUSQL` met `outputToUSQL` de naam en respectievelijk door geven van gegevens tussen U-SQL en R. invoer-en uitvoer data frame-id-id's zijn vast (dat wil zeggen, gebruikers kunnen deze vooraf gedefinieerde namen van invoer-en uitvoer data frame-id's niet wijzigen).

## <a name="embedding-r-code-in-the-u-sql-script"></a>R-code in het U-SQL-script insluiten

U kunt de R-code van uw U-SQL-script inline met behulp van `Extension.R.Reducer`de opdracht parameter van de. U kunt bijvoorbeeld het R-script declareren als een teken reeks variabele en dit door geven als een para meter voor de reducer.
...
* U-SQL-teken reeksen kunnen worden geconverteerd naar factoren in R-code, wanneer U-SQL een R-invoer data frame maakt of door `stringsAsFactors: true`de para meter reducer in te stellen.
