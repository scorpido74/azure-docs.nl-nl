...
Listeners die aan Hybride verbindingen zijn gekoppeld met HTTP-ondersteuning, moeten `request` de penbeweging afhandelen. Een listener die geen afhandelt `request` en daarom herhaalde time-outfouten veroorzaakt tijdens de verbinding, kan in de toekomst worden geblack door de service.
...
De `namespace-address` is de Fully Qualified Domain name van de Azure relay naam ruimte die als host fungeert voor de hybride verbinding, `{myname}.servicebus.windows.net`doorgaans van het formulier.
...
Hetzelfde geldt voor de `Sec-WebSocket-Extensions` koptekst. Als het Framework een uitbrei ding ondersteunt, moet de header worden ingesteld op het antwoord aan de server zijde `Sec-WebSocket-Extensions` van de vereiste Handshake voor de uitbrei ding.
...
##### <a name="rejecting-the-socket"></a>De socket weigeren
 ...
 Voor het afwijzen van de socket `accept` na het inspecteren van het bericht is een vergelijk bare Handshake vereist zodat de status code en beschrijving die de reden voor de weigering door gegeven, naar de afzender kunnen stromen.
 ...
 Om de socket af te wijzen, gebruikt de client de adres- `accept` URI uit het bericht en worden er twee query teken reeks parameters aan toegevoegd:
...
Als er geen hoofd tekst is, worden de hoofd frames wegge laten. De indicator voor of een hoofd tekst aanwezig is, is `body` de Booleaanse eigenschap in het aanvraag bericht.
...
De JSON-inhoud `request` voor is als volgt:

* **adres** -URI-teken reeks. Dit is het adres van de rendez dat voor deze aanvraag moet worden gebruikt. Als de binnenkomende aanvraag groter is dan 64 kB, wordt de rest van dit bericht leeg gelaten en moet de client een hand Shake van het rendez-vous `accept` initiëren dat overeenkomt met de onderstaande bewerking. De service plaatst vervolgens de volledige `request` op de tot stand gebrachte WebSocket. Als de reactie naar verwachting groter is dan 64 kB, moet de listener ook een Shake van het rendez-vous initiëren en vervolgens het antwoord over de ingestelde Web-socket overdragen.
* **id** : teken reeks. De unieke id voor deze aanvraag.
* **RequestHeaders** : dit object bevat alle HTTP-headers die door de afzender aan het eind punt zijn door gegeven, met uitzonde ring van autorisatie-informatie zoals [hierboven](#request-operation)beschreven en headers die strikt verband houden met de verbinding met de gateway. In het bijzonder worden alle headers die zijn [RFC7230](https://tools.ietf.org/html/rfc7230)gedefinieerd of gereserveerd `Via`in RFC7230, met uitzonde ring van, verwijderd en niet doorgestuurd:
...
Het antwoord is een JSON-object met de naam ' Response '. De regels voor het afhandelen van de inhoud van `request` de hoofd tekst zijn precies `body` hetzelfde als het bericht en op basis van de eigenschap.

* **aanvraag** -waarde – string. Vereist. De `id` eigenschaps waarde van `request` het bericht waarop wordt gereageerd.
...
* **gegevensservicehost** : http-headers die moeten worden ingesteld in een extern http-antwoord.
  Net als met `request`de, RFC7230 gedefinieerde HEADERS mogen niet worden gebruikt.
...
Voor antwoorden die 64 kB overschrijdt, moet het antwoord worden verzonden via een rendez van het andere. Als de aanvraag meer dan 64 kB overschrijdt en de `request` enige adres veld bevat, moet u een socket van rendez-vous instellen om de `request`te verkrijgen. Zodra een rendez van de client is ingesteld, moeten reacties op de respectievelijke clients en volgende aanvragen van die respectieve clients worden geleverd via de socket van de rendez-out.

De `address` URL in de `request` moet worden gebruikt als-is voor het tot stand brengen van de disrendez-socket, maar bevat de volgende para meters:
...
Wanneer het listener-token bijna is verlopen, kan het worden vervangen door een tekst frame bericht naar de service te verzenden via het ingestelde besturings kanaal. Het bericht bevat een JSON-object `renewToken`met de naam, waarmee de volgende eigenschap op dit moment wordt gedefinieerd:
...
De _naam ruimte-adres_ is de Fully Qualified Domain name van de Azure relay naam ruimte die als host fungeert voor de hybride verbinding `{myname}.servicebus.windows.net`, doorgaans van het formulier.
...
| `sb-hc-id`     | Nee        | Een optionele ID waarmee end-to-end diagnostische tracering is ingeschakeld en beschikbaar wordt gesteld aan de listener tijdens het accepteren van de handshake.
 ...
 Het `{path}` is het pad naar de URL-code naam ruimte van de vooraf geconfigureerde hybride verbinding waarop u deze listener wilt registreren. De `path` expressie kan worden uitgebreid met een achtervoegsel en een query reeks expressie om verder te communiceren. Als de hybride verbinding onder het pad `hyco`is geregistreerd, kan `path` de expressie worden `hyco/suffix?param=value&...` gevolgd door de query teken reeks parameters die hier zijn gedefinieerd. Een volledige expressie kan er dan als volgt uitzien:
...
De aanvraag kan wille keurige extra HTTP-headers bevatten, inclusief door de toepassing gedefinieerde items. Alle opgegeven headers, met uitzonde ring van degene die rechtstreeks zijn gedefinieerd in RFC7230 (Zie [aanvraag bericht](#Request message)) stroom naar de listener `requestHeader` en kunnen worden gevonden op het object van het **aanvraag** bericht.
...
Omdat de service effectief fungeert als een proxy, zelfs als dit niet zo is als een echte HTTP-proxy, `Via` voegt deze een kop toe of `Via` maakt een aantekeningen voor de bestaande header die voldoet aan [RFC7230, in de sectie 5.7.1](https://tools.ietf.org/html/rfc7230#section-5.7.1).
De service voegt de hostnaam van de relay `Via`-naam ruimte toe aan.
...
Als er een fout optreedt, kan de service als volgt reageren. Of het antwoord afkomstig is van de service of van de listener, kan worden geïdentificeerd door de aanwezigheid van `Via` de header. Als de header aanwezig is, is de reactie van de listener.
