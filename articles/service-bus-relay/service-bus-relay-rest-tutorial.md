...
1. Definieer direct na de openings accolade van de naam ruimte declaratie een nieuwe interface met de `IImageContract` naam en pas `ServiceContractAttribute` het kenmerk toe op de interface met de `https://samples.microsoft.com/ServiceModel/Relay/RESTTutorial1`waarde. 
...
1. Het `<system.serviceModel>` XML-element is een WCF-element waarmee een of meer services worden gedefinieerd. Hier wordt de service naam en het eind punt gedefinieerd. Voeg aan de onderkant van `<system.serviceModel>` het element, maar nog `<system.serviceModel>`steeds een `<bindings>` element toe met de volgende inhoud:
