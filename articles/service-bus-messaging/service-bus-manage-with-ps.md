...
* `$Namespace` is de naam van de Service Bus naam ruimte waarmee we willen werken.
* `$Location` identificeert het Data Center waarin de naam ruimte wordt ingericht.
* `$CurrentNamespace` slaat de referentie naam ruimte op die we ophalen (of maken).

`$Namespace` en `$Location` kunnen worden door gegeven als para meters.
