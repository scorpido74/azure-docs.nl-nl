...
In dit artikel worden enkele veelvoorkomende problemen besproken die kunnen optreden bij het verplaatsen van gegevens tussen R en Azure SQL Database in [Machine Learning Services (met R) in Azure SQL database](machine-learning-services-overview.md). De ervaring die u in deze oefening krijgt, biedt essentiële achtergrond bij het werken met gegevens in uw eigen script.
...
- Als u de voorbeeld code in deze oefeningen wilt uitvoeren, moet u eerst [Azure SQL database hebben met Machine Learning Services (met R)](machine-learning-services-overview.md) ingeschakeld.
...
Het antwoord kan meestal worden gevonden met behulp van de R- `str()` opdracht. Voeg de functie `str(object_name)` overal in uw R-script toe zodat het gegevens schema van het opgegeven R-object als een informatief bericht wordt geretourneerd. U kunt de berichten weer geven op het tabblad **berichten** in SSMS.

Als u wilt weten waarom voor beeld 1 en voor beeld 2 dergelijke verschillende resultaten hebben, voegt u de regel `str(OutputDataSet)` aan het einde van de definitie van de `@script` variabele in elke instructie in, zoals:
...
> [!TIP]
> U kunt ook R-identiteits functies, zoals `is.matrix` , gebruiken `is.vector` om informatie over de interne gegevens structuur te retour neren.
...
Onder de kaften wordt de kolom van drie waarden geconverteerd naar een matrix met één kolom. Omdat een matrix slechts een speciaal geval van een matrix in R is, wordt de matrix `y` impliciet gedwongen naar een matrix met één kolom om de twee argumenten te laten overeenkomen.
...
Houd er echter rekening mee wat er gebeurt wanneer u de grootte van de matrix wijzigt `y` .
...
Het volgende script definieert bijvoorbeeld een numerieke matrix met de lengte 6 en slaat deze op in de R-variabele `df1` . De numerieke matrix wordt vervolgens gecombineerd met de gehele getallen van de RTestData-tabel (gemaakt hierboven) die drie (3) waarden bevat, om een nieuw gegevens frame te maken `df2` .
...
Om het gegevens frame in te vullen, herhaalt R de elementen die worden opgehaald uit RTestData zo vaak als nodig is om overeen te komen met het aantal elementen in de matrix `df1` .
...
Nadat u de query hebt uitgevoerd, controleert u de resultaten van de `str` functie om te zien hoe R de invoer gegevens verwerkt.
...
- Geef de kolommen in de invoer gegevens bron afzonderlijk op, in plaats van met en u `SELECT *` weet hoe elke kolom wordt verwerkt.
