...
Wanneer u een nieuwe server maakt in Azure SQL Database of Azure Synapse met de naam *MySqlServer*, bijvoorbeeld een firewall op server niveau, blokkeert de toegang tot het open bare eind punt voor de server (die toegankelijk is op *MySqlServer.database.Windows.net*). *SQL database* wordt gebruikt om te verwijzen naar zowel SQL database als Azure Synapse (voorheen Azure SQL Data Warehouse).

> [!IMPORTANT]
> Dit artikel is *niet* van toepassing op een door *Azure SQL beheerd exemplaar*. Zie [uw toepassing verbinden met Azure SQL Managed instance](../managed-instance/connect-application-instance.md)voor meer informatie over de netwerk configuratie.
>
...
Met deze regels kunnen clients toegang krijgen tot uw hele server, dat wil zeggen, alle data bases die worden beheerd door de-server. De regels worden opgeslagen in de Data Base *Master* . U kunt Maxi maal 128 IP-firewall regels op server niveau voor een server hebben. Als u de instelling **Azure-Services en-resources toegang tot deze server toestaan** hebt ingeschakeld, telt dit als een enkele firewall regel voor de-server.
...
1. Als u een IP-firewall regel op server niveau wilt instellen op de overzichts pagina van de data base, selecteert u **Server firewall instellen** op de werk balk, zoals wordt weer gegeven in de volgende afbeelding.
    ...
    De pagina **firewall-instellingen** voor de server wordt geopend.
