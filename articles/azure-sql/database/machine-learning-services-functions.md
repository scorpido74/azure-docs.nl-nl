...
Als u deze regel van R vanuit T-SQL wilt aanroepen, voert u `sp_execute_external_script` de r-functie uit in de para meter r-script en voegt u deze toe als volgt:
...
- De regel die begint met `@params` definieert alle variabelen die worden gebruikt door de R-code en de bijbehorende SQL-gegevens typen.
...
Het `utils` pakket dat standaard wordt geïnstalleerd, biedt diverse hulp functies voor het onderzoeken van de huidige R-omgeving. Deze functies kunnen nuttig zijn als u verschillen zoekt in de manier waarop uw R-code wordt uitgevoerd in SQL en buiten omgevingen. U kunt bijvoorbeeld de functie R gebruiken `memory.limit()` om geheugen voor de huidige R-omgeving op te halen.
