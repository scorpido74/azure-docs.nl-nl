...
|`avg_storage_percent`|Totale opslag ruimte die wordt gebruikt door gegevens in alle data bases binnen een elastische pool. Bevat geen lege ruimte in database bestanden. Beschikbaar in de weer gave [sys.elastic_pool_resource_stats](/sql/relational-databases/system-catalog-views/sys-elastic-pool-resource-stats-azure-sql-database) in de- `master` Data Base. Deze metrische gegevens worden ook verzonden naar Azure Monitor, waar ze worden [genoemd](../../azure-monitor/platform/metrics-supported.md#microsoftsqlserverselasticpools) `storage_percent` en kunnen worden weer gegeven in azure Portal.|Minder dan 80%. Kan 100% voor Pools benaderen zonder gegevens groei.|
|`avg_allocated_storage_percent`|Totale opslag ruimte die wordt gebruikt door database bestanden in opslag in alle data bases binnen een elastische pool. Bevat lege ruimte in database bestanden. Beschikbaar in de weer gave [sys.elastic_pool_resource_stats](/sql/relational-databases/system-catalog-views/sys-elastic-pool-resource-stats-azure-sql-database) in de- `master` Data Base. Deze metrische gegevens worden ook verzonden naar Azure Monitor, waar ze worden [genoemd](../../azure-monitor/platform/metrics-supported.md#microsoftsqlserverselasticpools) `allocated_data_storage_percent` en kunnen worden weer gegeven in azure Portal.|Minder dan 90%. Kan 100% voor Pools benaderen zonder gegevens groei.|
