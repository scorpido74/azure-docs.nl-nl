...
* [Geo-replicatie](active-geo-replication-overview.md), voor het maken van asynchroon gerepliceerde, alleen-lezen-secundairen voor alle data bases. Tijdens een onderbreking voert u een failover uit naar de replica's in de herstel regio.  Nadat de onderbreking is opgelost, wordt er een failback uitgevoerd naar de data bases in de oorspronkelijke regio zonder gegevens verlies.
...
* [Herstel functies van Shard-beheer](elastic-database-recovery-manager.md), om database vermeldingen in de catalogus tijdens herstel en repatriër te wijzigen. Met deze functies kan de app verbinding maken met Tenant databases, ongeacht de locatie zonder de app opnieuw te hoeven configureren.
* [DNS-aliassen van SQL Server](../../sql-database/dns-alias-overview.md), om het naadloos inrichten van nieuwe tenants mogelijk te maken, ongeacht in welke regio de app wordt uitgevoerd. DNS-aliassen worden ook gebruikt om het synchronisatie proces van de catalogus toe te staan om verbinding te maken met de actieve catalogus, ongeacht de locatie.
...
1. Open in uw webbrowser de Wingtip tickets-gebeurtenissen hub ( http://events.wingtip-dpt.&lt ; User &gt; . trafficmanager.net-replace &lt; User door de &gt; gebruikers waarde van uw implementatie).
    ...
    *Tip: Houd de muis aanwijzer boven de locatie om de weer gave te verg Roten.* 
     ![ Gebeurtenissen hub in de oorspronkelijke regio in orde](./media/saas-dbpertenant-dr-geo-replication/events-hub-original-region.png)
...
1. Bewaakt de service om te bepalen wanneer failover van data bases is uitgevoerd. Als er een failover van een Tenant database is uitgevoerd, wordt de catalogus bijgewerkt om de herstel status van de Tenant database vast te leggen en de Tenant als online te markeren.
...
1. Zodra de catalogus database is hersteld, vernieuwt u de Wingtip tickets-hub van gebeurtenissen in uw webbrowser.
   ...
   * Als u rechtstreeks op de pagina gebeurtenissen van een offline Tenant opent, wordt de melding ' Tenant offline ' weer gegeven. Als contoso concert hal bijvoorbeeld offline is, opent u de http://events.wingtip-dpt.&lt &gt; pagina gebruikers. Trafficmanager.net/contosoconcerthall ![ Contoso offline](./media/saas-dbpertenant-dr-geo-replication/dr-in-progress-offline-contosoconcerthall.png) 
...
4. Wanneer het script wordt uitgevoerd, vernieuwt u de pagina Events hub ( http://events.wingtip-dpt.&lt ; User &gt; . trafficmanager.net)
...
Meer informatie over de technologieën Azure SQL Database biedt u de mogelijkheid om bedrijfs continuïteit in te scha kelen in de [overzichts documentatie van bedrijfs continuïteit](business-continuity-high-availability-disaster-recover-hadr-overview.md) .
