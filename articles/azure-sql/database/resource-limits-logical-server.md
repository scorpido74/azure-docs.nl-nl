...
Nadat de data base-engine is gestart, worden gegevens in het geheugen agressief opgeslagen in de data base-engine, omdat de werk belasting gegevens uit de opslag begint te lezen. Na deze eerste opvolgings periode is het gebruikelijk en verwacht dat de `avg_memory_usage_percent` kolommen en worden weer gegeven `avg_instance_memory_percent` in [sys.dm_db_resource_stats](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database) sluitend of gelijk zijn aan 100%, met name voor data bases die niet inactief zijn en niet volledig in het geheugen passen.
