...
2. **MFA configureren** : Zie [wat is Azure multi-factor Authentication?](../../active-directory/authentication/multi-factor-authentication.md), [voorwaardelijke toegang (MFA) met Azure SQL database en Data Warehouse](conditional-access-configure.md)voor stapsgewijze instructies. (Voor volledige voorwaardelijke toegang is een Premium-Azure Active Directory vereist (Azure AD). Beperkte MFA is beschikbaar met een standaard Azure AD.)
