...
In deze zelfstudie ziet u hoe u een gegevensgestuurde web-app van Python [Django](https://www.djangoproject.com/) naar [Azure App Service](app-service-linux-intro.md) implementeert en deze koppelt aan een Azure Database for Postgres-database. App Service biedt een uiterst schaalbare webhostingservice met self-patchfunctie.
...
- De [prijscategorie](../../postgresql/concepts-pricing-tiers.md) B_Gen4_1 (Basic, Gen4, 1 kern)  die hier gebruikt wordt is de minst voordelige. Laat voor productiedatabases het argument `--sku-name` weg om de prijscategorie GP_Gen5_2 (Algemeen gebruik, Gen 5, 2 kernen) te gebruiken.
...
- In uw Python-code opent u deze instellingen als omgevingsvariabelen met instructies zoals `os.environ.get('DJANGO_ENV')`. Zie [Omgevingsvariabelen openen](how-to-configure-python.md#access-environment-variables) voor meer informatie.
