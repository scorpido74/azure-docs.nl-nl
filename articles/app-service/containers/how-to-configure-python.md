...
description: Meer informatie over het configureren van een vooraf gebouwde Python-container voor uw app. In dit artikel worden de meest voorkomende configuratietaken weergegeven.
ms.topic: quickstart
ms.date: 03/28/2019
...
In dit artikel wordt beschreven hoe Python-apps worden uitgevoerd in [Azure App Service](app-service-linux-intro.md) en hoe u het gedrag van Azure App Service zo nodig kunt aanpassen. Python-apps moeten worden geïmplementeerd met alle vereiste [pip-modules.](https://pypi.org/project/pip/)

De App Service-implementatieengine activeert automatisch `pip install -r requirements.txt` een virtuele omgeving en wordt voor u uitgevoerd wanneer u een [Git-repository](../deploy-local-git.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json)of een [Zip-pakket](../deploy-zip.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json) implementeert waarbij buildprocessen zijn ingeschakeld.

Deze handleiding biedt belangrijke concepten en instructies voor Python-ontwikkelaars die een ingebouwde Linux-container in App Service gebruiken. Als u Azure App Service nog nooit hebt gebruikt, moet u eerst de [Python quickstart](quickstart-python.md) en [Python volgen met De zelfstudie van PostgreSQL.](tutorial-python-postgresql-app.md)

> [!NOTE]
> Linux is momenteel de aanbevolen optie voor het uitvoeren van Python-apps in App Service. Zie Python op de [Windows-smaak van App Service](https://docs.microsoft.com/visualstudio/python/managing-python-on-azure-app-service)voor meer informatie over de Windows-optie.
>
...
1. Als *manage.py* wordt gevonden in de wortel van de opslagplaats, voert u *manage.py collecte uit.* Als deze `DISABLE_COLLECTSTATIC` stap `true`is ingesteld op, wordt deze stap echter overgeslagen.
1. Aangepaste script uitvoeren `POST_BUILD_SCRIPT_PATH`als dit is opgegeven door .

`PRE_BUILD_COMMAND`, `POST_BUILD_COMMAND`en `DISABLE_COLLECTSTATIC` zijn standaard lege omgevingsvariabelen. Als u pre-build-opdrachten `PRE_BUILD_COMMAND`wilt uitvoeren, definieert u . Als u opdrachten na het `POST_BUILD_COMMAND`bouwen wilt uitvoeren, definieert u . Als u het uitvoeren van collectstatic wilt uitschakelen bij het bouwen van Django-apps, stelt u . `DISABLE_COLLECTSTATIC=true`
...
Zie [Oryx-documentatie: Hoe Python-apps worden gedetecteerd en gebouwd voor](https://github.com/microsoft/Oryx/blob/master/doc/runtimes/python.md)meer informatie over hoe App Service Python-apps in Linux draait en bouwt.

## <a name="container-characteristics"></a>Containerkenmerken

Python-apps die zijn geïmplementeerd voor App Service op Linux, worden uitgevoerd in een Docker-container die is gedefinieerd in de [App Service Python GitHub-repository.](https://github.com/Azure-App-Service/python) U de afbeeldingsconfiguraties vinden in de versiespecifieke mappen.
...
Als u meer specifieke controle wilt over de opstartopdracht, gebruikt u een [aangepaste opstartopdracht](#customize-startup-command) en vervangt u `<module>` de naam van de module die *wsgi.py*bevat.
...
Als uw hoofdappmodule zich in een ander bestand bevindt, een andere naam voor het app-object gebruikt of als u aanvullende argumenten aan Gunicorn wilt geven, gebruikt u een [aangepaste opstartopdracht.](#customize-startup-command)
...
U kunt het opstartgedrag van de container sturen door een aangepaste Gunicorn-opstartopdracht te geven. Voer hiervoor de volgende opdracht uit in de [Cloud Shell:](https://shell.azure.com)

```azurecli-interactive
az webapp config set --resource-group <resource-group-name> --name <app-name> --startup-file "<custom-command>"
```

Als u bijvoorbeeld een Flask-app hebt waarvan de hoofdmodule *hello.py* is `myapp`en het object * \<Flask-app* in dat bestand de naam heeft, is de>van de aangepaste opdracht als volgt:
...
U ook eventuele aanvullende argumenten voor Gunicorn toevoegen `--workers=4`aan * \<>op maat, *zoals. Zie voor meer informatie [Running Gunicorn](https://docs.gunicorn.org/en/stable/run.html) (Gunicorn uitvoeren, docs.gunicorn.org).

Als u een niet-Gunicorn-server wilt gebruiken, zoals [aiohttp,](https://aiohttp.readthedocs.io/en/stable/web_quickstart.html)u>op * \<maat* vervangen door iets als dit:
...
- Als uw bestanden bestaan, heeft App Service uw specifieke opstartbestand niet kunnen identificeren. Controleer of uw app is gestructureerd zoals App Service verwacht voor [Django](#django-app) of [Flask,](#flask-app)of gebruik een [aangepaste opstartopdracht.](#customize-startup-command)
...
- Controleer of uw app is gestructureerd zoals App Service verwacht voor [Django](#django-app) of [Flask,](#flask-app)of gebruik een [aangepaste opstartopdracht.](#customize-startup-command)
