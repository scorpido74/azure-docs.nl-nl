---
title: 'Zelfstudie: Een app voor meerdere containers maken'
description: Meer informatie over het gebruik van een multicontainer-app op Azure App Service die een WordPress-app en een MySQL-container bevat, en de WordPress-app configureren.
...
Om deze zelfstudie te voltooien, heb je ervaring nodig met [Docker Compose.](https://docs.docker.com/compose/)

## <a name="download-the-sample"></a>Het voorbeeld downloaden

Voor deze zelfstudie gebruikt u het compositiebestand van [Docker,](https://docs.docker.com/compose/wordpress/#define-the-project)maar u wijzigt het om Azure Database voor MySQL, permanente opslag en Redis op te nemen. U vindt het configuratiebestanden in [Azure-voorbeelden](https://github.com/Azure-Samples/multicontainerwordpress). Zie [Docker Compose-opties voor](configure-custom-container.md#docker-compose-options)ondersteunde configuratieopties.
...
Maak in Cloud Shell een App Service-abonnement [`az appservice plan create`](/cli/azure/appservice/plan?view=azure-cli-latest#az-appservice-plan-create) in de resourcegroep met de opdracht.
...
**Gefeliciteerd,** je hebt een multi-container app gemaakt in Web App voor containers. Vervolgens configureert u uw app voor gebruik met Azure Database for MySQL. Installeer WordPress nu nog niet.
...
Maak een Azure Database voor [`az mysql server create`](/cli/azure/mysql/server?view=azure-cli-latest#az-mysql-server-create) MySQL-server met de opdracht.

Vervang in de volgende opdracht de naam van uw MySQL-server waarbij de `a-z` `0-9` `-` _ &lt;mysql-servernaam>_ tijdelijke aanduiding (geldige tekens zijn , en ). Deze naam maakt deel uit van de hostnaam van de MySQL-server (`<mysql-server-name>.database.windows.net`) en moet globaal uniek zijn.
...
Zie [Omgevingsvariabelen configureren](configure-custom-container.md#configure-environment-variables)voor meer informatie over omgevingsvariabelen.
...
Meld u aan bij WordPress-beheerder. Selecteer in de linkernavigatie **Plug-ins**en selecteer **Geïnstalleerde plug-ins**.
