...
2. Nadat uw ILB ASE is gemaakt, wordt een SSL-certificaat geüpload dat overeenkomt met uw ILB ASE-domein.

3. Het geüploade SSL-certificaat wordt toegewezen aan de ILB ASE als het 'standaard' SSL-certificaat.  Dit certificaat wordt gebruikt voor SSL-verkeer naar apps op de ILB ASE wanneer ze het `https://someapp.mycustomrootdomain.com`algemene hoofddomein gebruiken dat is toegewezen aan de ASE (bijvoorbeeld).
...
Een SSL-certificaat moet aan de ASE worden gekoppeld als het 'standaard' SSL-certificaat dat wordt gebruikt om SSL-verbindingen met apps tot stand te brengen. Als het standaard DNS-achtervoegsel *internal-contoso.com*van de `https://some-random-app.internal-contoso.com` ASE internal-contoso.com is, vereist een verbinding een SSL-certificaat dat geldig is voor **.internal-contoso.com*. 

Verkrijg een geldig SSL-certificaat door gebruik te maken van interne certificaatautoriteiten, een certificaat te kopen bij een externe uitgever of een zelfondertekend certificaat te gebruiken. Ongeacht de bron van het SSL-certificaat moeten de volgende certificaatkenmerken correct zijn geconfigureerd:

* **Onderwerp**: Dit kenmerk moet worden ingesteld op **.your-root-domain-here.com*.
* **Alternatieve naam van het onderwerp:** dit kenmerk moet zowel **.your-root-domain-here.com* als **.scm.your-root-domain-here.com*bevatten. SSL-verbindingen met de SCM/Kudu-site die aan elke app zijn gekoppeld, gebruiken een adres van het formulier *your-app-name.scm.your-root-domain-here.com*.

Met een geldig SSL-certificaat in de hand zijn twee extra voorbereidende stappen nodig. Converteer het SSL-certificaat naar een .pfx-bestand of sla het certificaat in deze indeling op. Houd er rekening mee dat het .pfx-bestand alle tussenliggende en hoofdcertificaten moet bevatten. Beveilig het bestand met een wachtwoord.
...
Nadat het SSL-certificaat is gegenereerd en geconverteerd naar een door een basis64-gecodeerde tekenreeks, gebruikt u het voorbeeld resourcebeheersjabloon [Configureer het standaard SSL-certificaat][quickstartconfiguressl] op GitHub. 
...
Nadat het bestand *azuredeploy.parameters.json* is ingevuld, configureert u het standaard SSL-certificaat met behulp van het PowerShell-codefragment. Wijzig de bestandspaden die overeenkomen met de plaats waar de sjabloonbestanden van Resource Manager zich op uw machine bevinden. Vergeet niet om uw eigen waarden te leveren voor de implementatienaam Resource Manager en de naam van de resourcegroep:
...
Nadat de sjabloon is voltooid, zijn apps op de ILB ASE toegankelijk via HTTPS. De verbindingen worden beveiligd met behulp van het standaard SSL-certificaat. Het standaard SSL-certificaat wordt gebruikt wanneer apps op de ILB ASE worden geadresseerd met behulp van een combinatie van de toepassingsnaam plus de standaardhostnaam. `https://mycustomapp.internal-contoso.com` Gebruikt bijvoorbeeld het standaard SSL-certificaat voor **.internal-contoso.com*.

Net als apps die op de openbare multitenant-service worden uitgevoerd, kunnen ontwikkelaars echter aangepaste hostnamen configureren voor afzonderlijke apps. Ze kunnen ook unieke SNI SSL-certificaatbindingen configureren voor afzonderlijke apps.
