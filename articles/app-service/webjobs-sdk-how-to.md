...
De host is een runtime container voor functions.  Het luistert naar triggers en aanroepen-functies. In versie 3. *x*, de host is een implementatie van `IHost`. In versie 2. *x*gebruikt u het object `JobHost`. U maakt een exemplaar van een host in uw code en schrijft code om het gedrag aan te passen.
...
De klasse `JobHostConfiguration` heeft een `UseDevelopmentSettings`-methode waarmee de ontwikkelings modus wordt ingeschakeld.  In het volgende voor beeld ziet u hoe u ontwikkelings instellingen gebruikt. Als `config.IsDevelopment` `true` retour neren wanneer het lokaal wordt uitgevoerd, stelt u een lokale omgevings variabele met de naam `AzureWebJobsEnv` in met de waarde `Development`.
...
Alle uitgaande HTTP-aanvragen die u vanuit een functie maakt met behulp van `HttpClient`, loopt door `ServicePointManager`. Nadat u de waarde hebt ingesteld in `DefaultConnectionLimit`, wordt in `ServicePointManager` aanvragen in de wachtrij geplaatst voordat ze worden verzonden. Stel dat uw @no__t 0 is ingesteld op 2 en dat uw code 1.000 HTTP-aanvragen maakt. In eerste instantie worden slechts twee aanvragen toegestaan via het besturings systeem. De andere 998 worden in de wachtrij geplaatst totdat er ruimte is. Dit betekent dat uw `HttpClient` mogelijk een time-out veroorzaakt omdat het lijkt alsof de aanvraag is gedaan, maar de aanvraag werd nooit verzonden door het besturings systeem naar de doel server. Het is dus mogelijk dat uw lokale `HttpClient` 10 seconden duurt om een aanvraag te volt ooien, maar uw service retourneert elke aanvraag in 200 MS. 

De standaard waarde voor ASP.NET-toepassingen is `Int32.MaxValue` en dat is waarschijnlijk goed geschikt voor webtaken die worden uitgevoerd in een basis-of hoger App Service plan. Webjobs hebben doorgaans de instelling altijd aan nodig en alleen ondersteund door basis-en snellere App Service plannen.
...
Functies moeten open bare methoden zijn en moeten één trigger kenmerk of het kenmerk [`NoAutomaticTrigger`](#manual-triggers) hebben.
...
Het kenmerk `QueueTrigger` vertelt de runtime om de functie aan te roepen wanneer er een wachtrij bericht wordt weer gegeven in de wachtrij van `myqueue-items`. Het kenmerk `Blob` vertelt de runtime om het wachtrij bericht te gebruiken voor het lezen van een BLOB in de *werk items-* container. De inhoud van het wachtrij bericht dat is door gegeven aan de functie in de para meter `myQueueItem`, is de naam van de blob.
...
In versie 3. *x*, de opslag bindingen zijn opgenomen in het `Microsoft.Azure.WebJobs.Extensions.Storage`-pakket. Roep de `AddAzureStorage`-extensie methode aan in de `ConfigureWebJobs`-methode, zoals hier wordt weer gegeven:
...
Als u andere typen triggers en bindingen wilt gebruiken, installeert u het NuGet-pakket dat deze bevat en roept u de extensie methode van `Add<binding>` aan die in de uitbrei ding is geïmplementeerd. Als u bijvoorbeeld een Azure Cosmos DB binding wilt gebruiken, installeert u `Microsoft.Azure.WebJobs.Extensions.CosmosDB` en roept u `AddCosmosDB` aan, bijvoorbeeld:
...
Als u de timer trigger of de binding van de bestanden wilt gebruiken die deel uitmaken van de kern Services, roept u de methoden van de `AddTimers`-of `AddFiles`-extensie aan.
...
Als u de bestanden binding wilt gebruiken, installeert u `Microsoft.Azure.WebJobs.Extensions` en roept u `UseFiles` aan.
...
Roep de `AddExecutionContextBinding`-extensie methode aan in de `ConfigureWebJobs`-methode, zoals hier wordt weer gegeven:
...
Het eerder genoemde `Microsoft.Azure.WebJobs.Extensions`-pakket biedt ook een speciaal bindings type dat u kunt registreren door de `UseCore`-methode aan te roepen. Met deze binding kunt u een para meter [`ExecutionContext`] definiëren in de hand tekening van de functie. Dit is als volgt ingeschakeld:
...
* **Versie 3. *x*:** Stel configuratie in wanneer de methode `Add<Binding>` wordt aangeroepen in `ConfigureWebJobs`.
* **Versie 2. *x*:** Stel de configuratie in door eigenschappen in te stellen in een configuratie object dat u door geeft aan `JobHost`.
...
U kunt dit doen door een `NameResolver`-object in te geven in het `JobHostConfiguration`-object. U neemt tijdelijke aanduidingen op in de para meters voor de trigger-of bindings kenmerken en de code van @no__t 0 levert de werkelijke waarden die worden gebruikt in plaats van deze tijdelijke aanduidingen. U identificeert tijdelijke aanduidingen door deze te omheen met een percentage (%) tekens, zoals hier wordt weer gegeven:
...
Er is een standaard `NameResolver` dat van kracht wordt als u geen aangepaste versie opgeeft. De standaard waarden worden opgehaald uit de app-instellingen of omgevings variabelen.

De klasse `NameResolver` haalt de naam van de wachtrij op uit `appSettings`, zoals hier wordt weer gegeven:
...
U voegt de resolver toe door de [`ConfigureServices`-] uitbreidings methode aan te roepen op [`HostBuilder`](/dotnet/api/microsoft.extensions.hosting.hostbuilder), zoals in dit voor beeld:
...
Geef uw `NameResolver`-klasse door aan het object `JobHost`, zoals hier wordt weer gegeven:
...
* [Voor beelden](../azure-functions/functions-bindings-storage-queue.md#trigger---example). Code voorbeelden. Het C# voor beeld van de klassen bibliotheek is van toepassing op de webjobs SDK. Laat gewoon het kenmerk `FunctionName` weg.
...
In dit voor beeld wordt slechts één exemplaar van de functie @no__t 0 op een bepaald moment uitgevoerd:
...
Het standaard bereik voor een vergren deling is `SingletonScope.Function`, wat betekent dat het vergrendelings bereik (het pad van de BLOB-lease) is gekoppeld aan de volledig gekwalificeerde functie naam. Als u de functies wilt vergren delen, geeft u `SingletonScope.Host` op en gebruikt u een scope-ID met dezelfde naam voor alle functies die u niet tegelijkertijd wilt uitvoeren. In het volgende voor beeld wordt slechts één exemplaar van `AddItem` of `RemoveItem` uitgevoerd op een tijdstip:
...
Elk logboek dat is gemaakt met een `ILogger`-exemplaar heeft een `Category` en `Level`. [`LogLevel`](/dotnet/api/microsoft.extensions.logging.loglevel) is een inventarisatie en de gehele code geeft de relatieve urgentie aan:
...
U kunt elke categorie onafhankelijk filteren op een bepaald [`LogLevel`](/dotnet/api/microsoft.extensions.logging.loglevel). Het is bijvoorbeeld mogelijk dat u alle logboeken wilt weer geven voor de verwerking van BLOB-triggers, maar alleen `Error` en hoger voor alle andere.

#### <a name="version-3x"></a>Versie 3. *x*

Versie 3. *x* van de SDK is afhankelijk van de filters die zijn ingebouwd in .net core. Met de klasse `LogCategories` kunt u categorieën definiëren voor specifieke functies, triggers of gebruikers. Het definieert ook filters voor specifieke host-provincies, zoals `Startup` en `Results`. Zo kunt u de uitvoer van logboek registratie nauw keurig afstemmen. Als er geen overeenkomst wordt gevonden binnen de gedefinieerde categorieën, wordt het filter terugvallen op de waarde van @no__t 0 wanneer u beslist of het bericht moet worden gefilterd.

voor `LogCategories` is het volgende met de instructie vereist:

```cs
using Microsoft.Azure.WebJobs.Logging; 
```

In het volgende voor beeld wordt een filter gemaakt dat standaard alle logboeken op het niveau @no__t 0 filtert. De categorieën `Function` en `results` (gelijk aan `Host.Results` in versie 2). *x*) worden gefilterd op het niveau van de `Error`. Met het filter wordt de huidige categorie vergeleken met alle geregistreerde niveaus in het `LogCategories`-exemplaar en wordt de langste overeenkomst gekozen. Dit betekent dat het op @no__t 0 geregistreerde niveau voor `Host.Triggers` overeenkomt met `Host.Triggers.Queue` of `Host.Triggers.Blob`. Zo kunt u grotere categorieën beheren zonder dat u deze hoeft toe te voegen.
...
In versie 2. *x* van de SDK gebruikt u de klasse `LogCategoryFilter` om het filteren te beheren. De `LogCategoryFilter` heeft een eigenschap `Default` met een begin waarde van `Information`, wat inhoudt dat alle berichten op de niveaus `Information`, `Warning`, `Error` of `Critical` worden geregistreerd, maar alle berichten op de `Debug`-of `Trace`-niveaus worden gefilterd.

Net als bij `LogCategories` in versie 3. *x*, met de eigenschap `CategoryLevels` kunt u logboek niveaus voor specifieke categorieën opgeven, zodat u de logboek registratie van de uitvoer kunt aanpassen. Als er geen overeenkomst wordt gevonden in de `CategoryLevels`-woorden lijst, wordt het filter terugvallen op de waarde van `Default` wanneer u bepaalt of het bericht moet worden gefilterd.

In het volgende voor beeld wordt een filter gemaakt dat standaard alle logboeken op het niveau van @no__t 0 filtert. De categorieën `Function` en `Host.Results` worden gefilterd op het niveau van `Error`. Met de `LogCategoryFilter` wordt de huidige categorie vergeleken met alle geregistreerde `CategoryLevels` en wordt de langste overeenkomst gekozen. Het niveau @no__t 0 dat voor `Host.Triggers` is geregistreerd, komt dus overeen met `Host.Triggers.Queue` of `Host.Triggers.Blob`. Zo kunt u grotere categorieën beheren zonder dat u deze hoeft toe te voegen.
...
Met de volgende aangepaste implementatie van [`ITelemetryInitializer`] kunt u uw eigen [`ITelemetry`](/dotnet/api/microsoft.applicationinsights.channel.itelemetry) toevoegen aan de standaard [`TelemetryConfiguration`].
...
Roep [`ConfigureServices`] aan in de opbouw functie om uw aangepaste [`ITelemetryInitializer`] toe te voegen aan de pijp lijn.
...
In versie 3. *x*, hoeft u de [`TelemetryClient`] niet langer leeg te laten wanneer de host wordt gestopt. Het geregistreerde `ApplicationInsightsLoggerProvider`, waarmee de [`TelemetryClient`]wordt leeg gemaakt, wordt automatisch verwijderd.

#### <a name="version-2x"></a>Versie 2. *x*

In versie 2. *x*, de [`TelemetryClient`] die intern is gemaakt door de Application Insights provider voor de webjobs SDK [, gebruikt `ServerTelemetryChannel`](https://github.com/Microsoft/ApplicationInsights-dotnet/blob/develop/src/ServerTelemetryChannel/ServerTelemetryChannel.cs). Wanneer het Application Insights-eind punt niet beschikbaar is of het beperken van binnenkomende aanvragen, slaat dit kanaal [aanvragen op in het bestands systeem van de web-app en verzendt deze later opnieuw](https://apmtips.com/blog/2015/09/03/more-telemetry-channels).
...
Het `SamplingPercentageEstimatorSettings`-object configureert [adaptieve steek proeven](https://docs.microsoft.com/azure/application-insights/app-insights-sampling). Dit betekent dat in bepaalde scenario's met grote volumes, toepassingen Insights een geselecteerde subset van telemetriegegevens naar de server verzendt.
