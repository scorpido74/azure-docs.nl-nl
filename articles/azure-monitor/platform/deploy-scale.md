...
[Diagnostische instellingen](./diagnostic-settings.md) verzamelen resource logboeken en metrische gegevens van Azure-resources op meerdere locaties, meestal naar een log Analytics-werk ruimte, waarmee u de gegevens kunt analyseren met [logboek query's](../log-query/log-query-overview.md) en [logboek waarschuwingen](alerts-log.md). Gebruik beleid om automatisch een diagnostische instelling te maken telkens wanneer u een resource maakt.
...
## <a name="azure-monitor-for-vms-and-virtual-machine-agents"></a>Azure Monitor voor VM's en de agents van de virtuele machine
[Azure monitor voor VM's](../insights/vminsights-overview.md) is het primaire hulp programma in azure monitor voor het bewaken van virtuele machines en virtuele-machine schaal sets. Als u Azure Monitor voor VM's wilt inschakelen, moet u zowel de Log Analytics agent als de afhankelijkheids agent op elke client installeren. U kunt ook de Log Analytics-agent zelf installeren ter ondersteuning van andere bewakings scenario's. In plaats van deze taken hand matig uit te voeren, gebruikt u Azure Policy om ervoor te zorgen dat elke virtuele machine is geconfigureerd terwijl u deze maakt.
...
Zie [Azure monitor voor VM's inschakelen met behulp van Azure Policy](../insights/vminsights-enable-at-scale-policy.md)voor meer informatie over dit proces.

![Azure Monitor voor VM's beleid](../platform/media/deploy-scale/vminsights-policy.png)
