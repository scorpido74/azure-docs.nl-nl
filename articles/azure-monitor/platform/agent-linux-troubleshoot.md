...
* Ga naar de pagina met Log Analytics feedback om de verzonden ideeën en bugs [https://aka.ms/opinsightsfeedback](https://aka.ms/opinsightsfeedback) te bekijken of een nieuwe bestand te openen.  
...
| 2 | Er is een ongeldige optie aan de shell-bundel door gegeven. `sudo sh ./omsagent-*.universal*.sh --help` uitvoeren voor gebruik |
| 3 | Er is geen optie aan de shell-bundel door gegeven. Voer `sudo sh ./omsagent-*.universal*.sh --help` uit voor gebruik. |
...
| 5 | De shell bundel moet worden uitgevoerd als root of er is een 403-fout geretourneerd tijdens het onboarden. Voer uw opdracht uit met behulp van `sudo`. |
...
| Fout code | Betekenis |
| --- | --- |
| 2 | Er is een ongeldige optie aan het omsadmin-script door gegeven. Voer `sudo sh /opt/microsoft/omsagent/bin/omsadmin.sh -h` uit voor gebruik. |
| 3 | Er is een ongeldige configuratie aan het omsadmin-script door gegeven. Voer `sudo sh /opt/microsoft/omsagent/bin/omsadmin.sh -h` uit voor gebruik. |
| 4 | Er is een ongeldige proxy aan het omsadmin-script door gegeven. Controleer de proxy en Raadpleeg onze [documentatie voor het gebruik van een HTTP-proxy](log-analytics-agent.md#network-firewall-requirements). |
...
| 34 | Het script voor het genereren van de configuratie is niet aanwezig. Voer onboarding opnieuw uit met `sudo sh /opt/microsoft/omsagent/bin/omsadmin.sh -w <Workspace ID> -s <Workspace Key>`. |
...
* Controleer of de volgende Azure Monitor-eind punten white list zijn:
    ...
    |*.azure-automation.net | Poort 443| Inkomend en uitgaand | 
...
3. Call stack wordt gedumpt in het omiagent_trace-bestand, als u veel krul-en NSS-functie aanroepen ziet, volgt u de onderstaande stappen voor de oplossing.

### <a name="resolution-step-by-step"></a>Oplossing (stapsgewijze stap)

1. Voer een upgrade uit voor het pakket nss-PEM naar [v 1.0.3 -5. EL7 _ 6.1](https://centos.pkgs.org/7/centos-x86_64/nss-pem-1.0.3-7.el7.x86_64.rpm.html). <br/>
...
* Als `omsconfig.log` logboek bestand niet aangeeft dat `PerformRequiredConfigurationChecks` bewerkingen periodiek op het systeem worden uitgevoerd, is er mogelijk een probleem met de cron-taak/-service. Zorg ervoor dat de cron-taak bestaat onder `/etc/cron.d/OMSConsistencyInvoker`. Voer, indien nodig, de volgende opdrachten uit om de cron-taak te maken:
...
* In sommige gevallen is het mogelijk dat de Log Analytics agent voor Linux-configuratie agent geen communicatie kan maken met de Portal Configuration-service, wat resulteert in de meest recente configuratie die niet wordt toegepast.
  1. Controleer of de `omsconfig`-agent is geïnstalleerd door `dpkg --list omsconfig` of `rpm -qi omsconfig`uit te voeren.  Als deze niet is geïnstalleerd, installeert u de nieuwste versie van de Log Analytics-agent voor Linux opnieuw.
...
* Log Analytics agent voor Linux-gebruikers `omsagent` heeft geen toegang tot het aangepaste logboek vanwege machtigingen of niet gevonden.  Mogelijk worden de volgende fouten weer geven:
...
### <a name="resolution"></a>Resolutie
1. Controleer of het volgende bestand bestaat: `/etc/opt/microsoft/omsagent/<workspace id>/conf/omsadmin.conf`om de onboarding naar Azure Monitor te controleren. Als dat niet het geval is, hetzij:  
...
**Achtergrond:** In plaats van de Log Analytics-agent voor Linux dat wordt uitgevoerd als een bevoegde gebruiker-`root`, wordt de agent uitgevoerd als de `omsagent` gebruiker. In de meeste gevallen moet er expliciete machtiging aan deze gebruiker worden verleend, zodat bepaalde bestanden kunnen worden gelezen. Voer de volgende opdrachten uit om een machtiging te verlenen aan `omsagent` gebruiker:
