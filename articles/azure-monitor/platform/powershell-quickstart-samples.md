...
Als u de geschiedenis voor een specifieke waarschuwings regel wilt weer geven, kunt `Get-AzAlertHistory` u de cmdlet gebruiken en de resource-id van de waarschuwings regel door geven.
...
U kunt e-mail en eigenschappen van webhooks `New-AzAlertRuleEmail` maken `New-AzAlertRuleWebhook`met respectievelijk. Wijs in de cmdlet waarschuwings regel deze eigenschappen toe als acties voor de eigenschap **Actions** van de waarschuwings regel.
...
Een volledige lijst met beschik bare `Get-AzMetricDefinition` opties voor is beschikbaar op [Get-MetricDefinitions](https://msdn.microsoft.com/library/mt282458.aspx).
...
De aanvullende eigenschappen van de webhook zijn optioneel. U kunt de inhoud van een waarschuwing voor een activiteiten logboek weer `Get-AzActivityLogAlert`geven met behulp van.
