...
`Get-AzLog`ondersteunt veel andere para meters. Zie de `Get-AzLog` verwijzing voor meer informatie.

> [!NOTE]
> `Get-AzLog`biedt alleen 15 dagen aan geschiedenis. Met de para meter **-MaxRecords** kunt u een query uitvoeren op de laatste N gebeurtenissen, na 15 dagen. Als u toegang wilt hebben tot gebeurtenissen die ouder zijn dan 15 dagen, gebruikt u de REST API of SDK (C#-voor beeld met de SDK). Als u **StartTime**niet opneemt, wordt de standaard waarde **EndTime** min één uur. Als u geen **EndTime**opgeeft, is de standaard waarde huidige tijd. Alle tijden zijn in UTC.
> 
> 
...
Als u de geschiedenis voor een specifieke waarschuwings regel wilt weer geven, kunt u de `Get-AzAlertHistory` cmdlet gebruiken en de resource-id van de waarschuwings regel door geven.

```powershell
Get-AzAlertHistory -ResourceId /subscriptions/s1/resourceGroups/rg1/providers/microsoft.insights/alertrules/myalert -StartTime 2016-03-1 -Status Activated
```

De `Get-AzAlertHistory` cmdlet ondersteunt verschillende para meters. Zie [Get-AlertHistory](https://msdn.microsoft.com/library/mt282453.aspx)voor meer informatie.
...
U kunt de `Add-AlertRule` cmdlet gebruiken om een waarschuwings regel te maken, bij te werken of uit te scha kelen.

U kunt e-mail en eigenschappen van webhooks maken met `New-AzAlertRuleEmail` `New-AzAlertRuleWebhook` respectievelijk. Wijs in de cmdlet waarschuwings regel deze eigenschappen toe als acties voor de eigenschap **Actions** van de waarschuwings regel.
...
U kunt de `Get-AzMetricDefinition` cmdlet gebruiken om de lijst met alle metrische gegevens voor een specifieke resource weer te geven.
...
Een volledige lijst met beschik bare opties voor `Get-AzMetricDefinition` is beschikbaar op [Get-MetricDefinitions](https://msdn.microsoft.com/library/mt282458.aspx).

## <a name="create-and-manage-activity-log-alerts"></a>Waarschuwingen voor activiteiten logboeken maken en beheren
U kunt de `Set-AzActivityLogAlert` cmdlet gebruiken om een waarschuwing voor een activiteiten logboek in te stellen. Een waarschuwing voor een activiteiten logboek vereist dat u eerst uw voor waarden definieert als een woorden lijst en vervolgens een waarschuwing maakt die deze voor waarden gebruikt.
...
De aanvullende eigenschappen van de webhook zijn optioneel. U kunt de inhoud van een waarschuwing voor een activiteiten logboek weer geven met behulp van `Get-AzActivityLogAlert` .
...
U kunt de `Get-Autoscalesetting` cmdlet gebruiken om meer informatie over de instelling voor automatisch schalen op te halen.
...
U kunt de `Remove-Autoscalesetting` cmdlet gebruiken om een instelling voor automatisch schalen te verwijderen.
