...
Het schema voor de diagnostische logboeken van de resource is afhankelijk van het bron type dat is gedefinieerd door de eigenschap `resourceId`) en de `category` eigenschappen. In de volgende lijst ziet u alle Azure-Services die ondersteuning bieden voor bron logboeken met koppelingen naar de service en het specifieke schema voor de categorie waar beschikbaar.
