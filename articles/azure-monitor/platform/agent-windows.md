...
Als u klaar bent wordt de **MMA** in het **Configuratiescherm** weergegeven. To confirm it is reporting to Log Analytics, review [Verify agent connectivity to Log Analytics](#verify-agent-connectivity-to-log-analytics). 
...
|MMA-specific options                   |Opmerkingen         |
|---------------------------------------|--------------|
...
Ga in de computer naar **Configuratiescherm** en zoek het item **Microsoft Monitoring Agent**.  Select it and on the **Azure Log Analytics** tab, the agent should display a message stating: **The Microsoft Monitoring Agent has successfully connected to the Microsoft Operations Management Suite service.**<br><br> ![Verbindingsstatus van MMA met Log Analytics](media/agent-windows/log-analytics-mma-laworkspace-status.png)

You can also perform a simple log query in the Azure portal.  

1. Klik in Azure Portal op **Alle services**. In the list of resources, type **Azure Monitor**. Als u begint te typen, wordt de lijst gefilterd op basis van uw invoer. Select **Azure Monitor**.  
