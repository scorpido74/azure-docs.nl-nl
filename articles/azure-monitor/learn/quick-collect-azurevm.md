---
title: Gegevens verzamelen van een virtuele Azure-machine met Azure Monitor | Microsoft Documenten
...
[Azure Monitor](../overview.md) kan gegevens rechtstreeks van uw Virtuele Azure-machines verzamelen in een Log Analytics-werkruimte voor gedetailleerde analyse en correlatie. Als u de VM-extensie Log Analytics voor [Windows](../../virtual-machines/extensions/oms-windows.md) en [Linux](../../virtual-machines/extensions/oms-linux.md) installeert, kan Azure Monitor gegevens uit uw Azure VM's verzamelen. Deze quickstart laat u zien hoe u gegevens van uw Azure Linux- of Windows VM's configureert en verzamelt met behulp van de VM-extensie met een paar eenvoudige stappen.  
...
1. Selecteer **alle services** in de linkerbovenhoek in de Azure-portal. Typ in de lijst met resources **Log Analytics**. Als u begint te typen, wordt de lijst gefilterd op basis van uw invoer. Selecteer **Logboekanalysewerkruimten**.

2. Selecteer in de lijst met Log Analytics-werkruimten de *StandaardLAWerkruimte* die u eerder hebt gemaakt.

3. Selecteer **virtuele machines**in het linkermenu onder Werkruimtegegevensbronnen .  
...
5. Selecteer **Verbinding maken** in de details voor de virtuele machine. De agent wordt automatisch geïnstalleerd en geconfigureerd voor uw Log Analytics-werkruimte. Dit proces duurt een paar minuten, gedurende welke tijd de **status** **Verbinding toont.**
...
Azure Monitor kan gebeurtenissen verzamelen van de Windows-gebeurtenislogboeken of Linux Syslog- en prestatiemeteritems die u opgeeft voor analyse en rapportage op langere termijn, en actie ondernemen wanneer een bepaalde voorwaarde wordt gedetecteerd. Volg deze stappen voor het verzamelen van gebeurtenissen uit het Windows-gebeurtenislogboek en de Linux Syslog en diverse algemene prestatiemeters om mee te beginnen.  
...
3. U kunt een gebeurtenislogboek toevoegen door de naam van het logboek te typen.  Typ **Systeem** en selecteer **+** het plusteken .
...
7. Selecteer **Opslaan** boven aan de pagina om de configuratie op te slaan.
...
Wanneer deze niet langer nodig is, verwijdert u de Log Analytics-werkruimte. Selecteer hiervoor de werkruimte Log Analytics die u eerder hebt gemaakt en selecteer **Verwijderen**op de resourcepagina .
