...
1. Open de ESXi host-firewall voor syslog. **Configuratie van ESXi** > **Software** > **beveiligings profiel** > **firewall** en open **Eigenschappen**.  
...
1. Nadat de Log Analytics-agent voor Linux is geïnstalleerd, gaat u naar de map/etc/opt/Microsoft/omsagent/sysconf/omsagent.d en kopieert u het bestand vmware_esxi. conf naar de map/etc/opt/Microsoft/omsagent/conf/omsagent.d en wijzigt u de eigenaar/groep en machtigingen van het bestand. Bijvoorbeeld:
...
| Onafhankelijk | Log Analytics-agent voor Linux | SCOM-agent | Azure Storage | SCOM vereist? | SCOM-agent gegevens die via een beheer groep zijn verzonden | verzamelings frequentie |
| --- | --- | --- | --- | --- | --- | --- |
...
Wanneer u op een ESXi-hostnaam klikt, ziet u informatie van die ESXi-host. Als u de resultaten van het gebeurtenis type wilt beperken, voegt u `“ProcessName_s=EVENT TYPE”` toe in uw zoek query. U kunt **proces** naam selecteren in het zoek filter. Zo beperkt u de informatie voor u.
