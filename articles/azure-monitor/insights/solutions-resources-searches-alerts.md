...
[Log Analytics opgeslagen Zoek](../../azure-monitor/log-query/log-query-overview.md) resources een type `Microsoft.OperationalInsights/workspaces/savedSearches` hebben en de volgende structuur hebben. Dit omvat algemene variabelen en para meters, zodat u dit code fragment kunt kopiëren en plakken in het oplossings bestand en de parameter namen wijzigt.
...
Een opgeslagen zoek opdracht kan een of meer schema's bevatten met elk schema dat een afzonderlijke waarschuwings regel vertegenwoordigt. De planning bepaalt hoe vaak de zoek actie wordt uitgevoerd en het tijds interval waarover de gegevens worden opgehaald. Resources plannen hebben een type `Microsoft.OperationalInsights/workspaces/savedSearches/schedules/` en hebben de volgende structuur. Dit omvat algemene variabelen en para meters, zodat u dit code fragment kunt kopiëren en plakken in het oplossings bestand en de parameter namen wijzigt.
