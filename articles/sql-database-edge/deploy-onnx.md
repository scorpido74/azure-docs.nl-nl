...
Sla het model op in Azure SQL Database rand, in `models` een tabel in een `onnx`data base. Geef in het connection string het **server adres**, de **gebruikers naam**en het **wacht woord**op.
...
Ten slotte kunt `sqlalchemy` u gebruiken om `x_train` de `y_train` en Panda dataframes in te voegen in de tabellen `features` en `target`respectievelijk. 
