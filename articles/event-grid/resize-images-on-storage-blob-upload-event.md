...
U moet de vorige zelf studie over het Blob-opslag hebben voltooid: [afbeeldings gegevens uploaden in de Cloud met Azure Storage][previous-tutorial].
...
3. Gebruik de instellingen voor het gebeurtenisabonnement zoals die zijn opgegeven in de tabel onder de afbeelding.
    ...
    | **Endpoints** | automatisch gegenereerd | Gebruik de eindpunt-URL die voor u wordt gegenereerd. |
...
5. Selecteer **Maken** om het gebeurtenisabonnement toe te voegen. Hiermee maakt u een gebeurtenis abonnement waarmee de functie `Thumbnail` wordt geactiveerd wanneer een BLOB wordt toegevoegd aan de `images`-container. De functie past de afbeelding in grootte aan en voegt deze toe aan de container `thumbnails`.
