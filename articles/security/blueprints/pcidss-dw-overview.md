---
title: Azure-blauwdruk voor beveiliging en naleving-data warehouse voor PCI DSS
description: Azure-blauwdruk voor beveiliging en naleving-data warehouse voor PCI DSS
services: security
author: meladie
...
ms.locfileid: "71259762"
---
# <a name="azure-security-and-compliance-blueprint-data-warehouse-for-pci-dss"></a>Azure-blauwdruk voor beveiliging en naleving: Data Warehouse voor PCI DSS

## <a name="overview"></a>Overzicht

Deze Azure-blauwdruk voor beveiliging en naleving biedt richt lijnen voor de implementatie van een Data Warehouse-architectuur in azure die helpt bij de vereisten van de beveiliging van de gegevens van de betaal kaart (PCI DSS 3,2). Er wordt een algemene referentie architectuur gedemonstreerd en de juiste verwerking van creditcard gegevens (inclusief kaart nummer, verval datum en verificatie gegevens) in een beveiligde, compatibele omgeving met meerdere lagen. In deze blauw druk ziet u hoe klanten aan specifieke vereisten voor beveiliging en naleving voldoen en als basis voor klanten kunnen worden gebruikt om hun eigen data warehouse-oplossingen in azure te bouwen en te configureren.
...
![Diagram van data warehouse voor PCI DSS referentie architectuur](images/pcidss-dw-architecture.png "Diagram van data warehouse voor PCI DSS referentie architectuur")
...
**Azure Disk Encryption**: [Azure Disk Encryption](../azure-security-disk-encryption-overview.md) maakt gebruik van de BitLocker-functie van Windows om volume versleuteling te bieden voor besturings systeem-en gegevens schijven. De oplossing kan worden geïntegreerd met Azure Key Vault om de versleutelings sleutels voor de schijf te controleren en te beheren.
...
Het gegevensstroom diagram voor deze referentie architectuur is beschikbaar voor [downloaden](https://aka.ms/pcidss-dw-tm) of kan hieronder worden weer gegeven. Dit model kan klanten helpen om inzicht te krijgen in de punten van potentieel risico in de systeem infrastructuur bij het aanbrengen van wijzigingen.

![Diagram van data warehouse voor PCI DSS referentie architectuur](images/pcidss-dw-threat-model.png "PaaS-webtoepassing voor PCI DSS Threat model")

## <a name="compliance-documentation"></a>Documentatie voor naleving

De [Azure-blauwdruk voor beveiliging en naleving-PCI DSS matrix van de klant verantwoordelijkheid](https://aka.ms/pcidss-crm) bevat verantwoordelijkheden voor alle PCI DSS 3,2-vereisten.

De [Azure-blauwdruk voor beveiliging en naleving-PCI DSS Data Warehouse-implementatie matrix](https://aka.ms/pcidss-dw-cim) bevat informatie over welke PCI DSS 3,2-vereisten worden verholpen door de Data Warehouse-architectuur, met inbegrip van gedetailleerde beschrijvingen van de manier waarop de implementatie voldoet aan de vereisten van elke gedekte controle.
...
-   [Azure Data Factory](https://docs.microsoft.com/azure/data-factory/introduction) is een beheerde Cloud service die is gebouwd voor complexe Hybrid extract-trans formatie-load-en gegevens integratie projecten. Azure Data Factory heeft mogelijkheden om gegevens van de kaart houder te traceren en te vinden, inclusief visualisatie-en controle Programma's om te bepalen wanneer de gegevens zijn aangekomen en waar deze vandaan komen. Klanten kunnen met behulp van Azure Data Factory gegevensgestuurde werk stromen maken en plannen die gegevens uit verschillende gegevens archieven opnemen. Met deze pijp lijnen kunnen klanten gegevens opnemen van zowel interne als externe bronnen. Klanten kunnen vervolgens de gegevens verwerken en transformeren voor uitvoer naar gegevens archieven zoals Azure SQL Data Warehouse.
