...
Wanneer u de toegang uitbreidt, krijgt u de rol [beheerder voor gebruikers toegang](built-in-roles.md#user-access-administrator) toegewezen in Azure in het hoofd`/`bereik (). Hiermee kunt u alle resources weer geven en toegang toewijzen in een abonnement of beheer groep in de Directory. De toewijzingen van beheerders rollen voor gebruikers toegang kunnen worden verwijderd met behulp van Power shell.
...
Ga als volgt te werk om een toewijzing van de rol beheerder voor gebruikers toegang voor`/`een gebruiker in het hoofd bereik () te verwijderen.
...
1. Met behulp van `elevateAccess`rest, bellen, waarmee u de rol van beheerder voor gebruikers toegang verleent`/`aan het hoofd bereik ().

   ```http
   POST https://management.azure.com/providers/Microsoft.Authorization/elevateAccess?api-version=2016-07-01
   ```

1. Maak een [roltoewijzing](/rest/api/authorization/roleassignments) om een wille keurige rol aan elk bereik toe te wijzen. In het volgende voor beeld worden de eigenschappen weer gegeven voor het toewijzen van de rol {Roledefinitionid hebben`/`} aan het hoofd bereik ():
...
- Roep [Get](/rest/api/authorization/roleassignments/listforscope) roleAssignmentse `{objectIdOfUser}` op, waarbij de object-id is van de gebruiker wiens roltoewijzingen u wilt ophalen.
...
Wanneer u aanroept `elevateAccess`, maakt u een roltoewijzing voor uzelf, zodat u die bevoegdheden kunt intrekken die u nodig hebt om de toewijzing te verwijderen.

1. Roep [Get](/rest/api/authorization/roledefinitions/get) roleDefinitionse `roleName` aan, waarbij is gelijk aan de beheerder van de gebruikers toegang om de naam-id van de beheerder van de gebruikers toegang te bepalen.
    ...
    Sla de id op uit `name` de para meter in dit `18d7d88d-d35e-4fb5-a5c3-7773c20a72d9`geval.

2. U moet ook de roltoewijzing voor de Directory-beheerder in het bereik van de Directory weer geven. Geef alle toewijzingen op Directory-bereik weer `principalId` voor de Directory-beheerder die de toegangs oproep ter verhoging heeft gemaakt. Hiermee worden alle toewijzingen in de map voor de objectid weer geven.
   ...
   1. De vorige oproepen retour neren een lijst met roltoewijzingen. Zoek naar de roltoewijzing waar de scope zich `"/"` bevindt `roleDefinitionId` en eindigt met de rolnaam-id die u in stap `principalId` 1 hebt gevonden en overeenkomt met de objectId van de Directory-beheerder. 
      ...
      Sla de id opnieuw op uit de `name` para meter, in dit geval e7dd75bc-06f6-4e71-9014-ee96a929d099.

   1. Gebruik ten slotte de roltoewijzings-ID om de toewijzing te verwijderen die `elevateAccess`is toegevoegd door:
