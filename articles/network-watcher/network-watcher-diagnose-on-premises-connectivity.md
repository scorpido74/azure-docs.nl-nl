...
U kunt gedetailleerde informatie ophalen uit de Scrubbed-wfpdiag. txt over de fout, zoals in dit geval wordt vermeld dat er `ERROR_IPSEC_IKE_POLICY_MATCH` is dat de verbinding niet goed werkt.

Een andere algemene onjuiste configuratie is het opgeven van onjuiste gedeelde sleutels. Als u in het voor gaande voor beeld verschillende gedeelde sleutels hebt opgegeven, wordt de volgende fout weer gegeven in IKEErrors. txt: `Error: Authentication failed. Check shared key`.
