---
title: Verwijzing naar OData-filter-Azure Search
description: OData-taal verwijzing voor filter syntaxis in Azure Search query's.
ms.date: 06/13/2019
services: search
ms.service: search
ms.topic: conceptual
...
ms.locfileid: "69647596"
---
# <a name="odata-filter-syntax-in-azure-search"></a>OData-$filter syntaxis in Azure Search

Azure Search gebruikt [OData-filter expressies](query-odata-filter-orderby-syntax.md) om aanvullende criteria toe te passen op een zoek opdracht Naast zoek termen in volledige tekst. In dit artikel wordt de syntaxis van filters in detail beschreven. Zie [filters in azure Search](search-filters.md)voor meer algemene informatie over welke filters en hoe u deze kunt gebruiken voor het realiseren van specifieke query scenario's.
...
> [Syntaxis diagram van OData voor Azure Search](https://azuresearch.github.io/odata-syntax-diagram/#boolean_expression)
...
- Verzamelings filter expressies `any` met `all`of. Deze filter criteria Toep assen op verzamelings velden. Zie OData-verzamelings [operatoren in azure Search](search-query-odata-collection-operators.md)voor meer informatie.
- Logische expressies die andere Boole-expressies combi neren met `and`behulp van `not`de Opera Tors, `or`en. Zie [logische OData-Opera tors in azure Search](search-query-odata-logical-operators.md)voor meer informatie.
- Vergelijkings expressies, waarmee velden of bereik variabelen worden vergeleken met constante waarden `eq`met behulp `lt`van `ge`de Opera `le`tors `gt`, `ne`,,, en. Zie OData-vergelijkings [operatoren in azure Search](search-query-odata-comparison-operators.md)voor meer informatie. Vergelijkings expressies worden ook gebruikt om de afstand tussen geo-ruimtelijke coördinaten `geo.distance` te vergelijken met behulp van de functie. Zie [geo-ruimtelijke functies van OData in azure Search](search-query-odata-geo-spatial-functions.md)voor meer informatie.
...
- Aanroepen van Booleaanse functies, waaronder:
  - `geo.intersects`, waarmee wordt getest of een bepaald punt zich in een bepaalde veelhoek bevindt. Zie [geo-ruimtelijke functies van OData in azure Search](search-query-odata-geo-spatial-functions.md)voor meer informatie.
  - `search.in`Hiermee wordt een veld-of bereik variabele vergeleken met elke waarde in een lijst met waarden. Zie [OData `search.in` -functie in azure Search](search-query-odata-search-in-function.md)voor meer informatie.
  - `search.ismatch`en `search.ismatchscoring`, waarmee Zoek opdrachten in volledige tekst in een filter context worden uitgevoerd. Zie [OData-functies voor zoeken in volledige tekst in azure Search](search-query-odata-full-text-search-functions.md)voor meer informatie.
- Veld paden of bereik variabelen van het `Edm.Boolean`type. Als uw index bijvoorbeeld een Boole-veld `IsEnabled` bevat met de naam en u wilt alle documenten met dit `true`veld retour neren, kan uw filter expressie alleen de naam `IsEnabled`zijn.
...
Als u een filter expressie zonder haakjes schrijft rond de bijbehorende subexpressies, wordt deze door Azure Search geëvalueerd op basis van een set met operator prioriteits regels. Deze regels zijn gebaseerd op de Opera tors die worden gebruikt voor het combi neren van subexpressies. De volgende tabel geeft een lijst van groepen Opera tors in volg orde van hoogste tot laagste prioriteit:
...
Er zijn limieten voor de grootte en complexiteit van filter expressies die u naar Azure Search kunt verzenden. De limieten zijn ongeveer gebaseerd op het aantal componenten in uw filter expressie. Een goede richt lijn is dat als u honderden componenten hebt, u een risico hebt om de limiet te overschrijden. We raden u aan uw toepassing zodanig te ontwerpen dat er geen filters van een ongebonden grootte worden gegenereerd.
