...
De URL is uitgebreid met de `hotels` index naam.
...
Als u `/stats` toevoegt aan uw URL, worden index gegevens geretourneerd. In Postman moet uw aanvraag er ongeveer als volgt uitzien en bevat de reactie het aantal documenten en de gebruikte ruimte in bytes.
