...
|includeTypelessEntities | Als deze eigenschap is ingesteld op True als de tekst een bekende entiteit bevat, maar niet in een van de ondersteunde categorieën kan worden gecategoriseerd, wordt deze geretourneerd als onderdeel van het `"entities"` complexe uitvoer veld. 
Dit zijn entiteiten die goed bekend zijn, maar niet zijn geclassificeerd als onderdeel van de huidige ondersteunde ' categorieën '. Bijvoorbeeld ' Windows 10 ' is een bekende entiteit (een product), maar ' Products ' is niet in de categorieën die vandaag worden ondersteund. De standaard waarde is `false` |
