...
1. Selecteer **extra** > **NuGet package manager** > **NuGet-pakketten beheren voor oplossing...**. 

1. Klik op **Bladeren** en zoek naar `Microsoft.Extensions.Configuration.Json` het NuGet-pakket. 
...
1. Klik met de rechter muisknop op uw project in de Solution Explorer **Add** > en selecteer**Nieuw item toevoegen...** . 

1. Geef het bestand `appsettings.json` een naam en selecteer **toevoegen**. 

1. Neem dit bestand op in de uitvoermap.
    1. Klik met de rechter `appsettings.json` muisknop op en selecteer **Eigenschappen**. 
...
Voeg `Program.cs`in de volgende naam ruimten toe.
...
Maak een instantie van de `SearchServiceClient` klasse onder `Main`.
...
Voeg `ExitProgram` toe `Main` aan voor het afhandelen van scenario's waarvoor het programma moet worden afgesloten.
...
Maak een nieuw `DataSource` exemplaar door aan `serviceClient.DataSources.CreateOrUpdate(dataSource)`te roepen. `DataSource.AzureBlobStorage`Hiervoor moet u de naam van de gegevens bron, de connection string en de naam van de BLOB-container opgeven.
...
Voeg nu een regel toe `Main` aan om de `CreateOrUpdateDataSource` functie aan te roepen die u zojuist hebt toegevoegd.
...
U ziet dat het veld context is ingesteld op ```"/document/pages/*"``` met een asterisk, wat betekent dat de verrijkings stap voor elke pagina wordt ```"/document/pages"```aangeroepen.
...
Voeg de volgende regels toe `Main`aan.
...
We voegen de model klasse toe aan een nieuw C#-bestand. Klik met de rechter muisknop op uw **Add** > project en selecteer**Nieuw item toevoegen...**, selecteer "klasse" en `DemoIndex.cs`Geef het bestand een naam en selecteer vervolgens **toevoegen**.
...
`CreateSearchIndexClient`Hiermee maakt u `SearchIndexClient` een nieuw met behulp van waarden die zijn opgeslagen in het configuratie bestand van de toepassing (appSettings. json). U ziet dat de query-API-sleutel van de zoek service wordt gebruikt en niet de Administrator-sleutel.
...
Voeg de volgende code toe `Main`aan. De eerste try-catch retourneert de index definitie, met de naam, het type en de kenmerken van elk veld. De tweede is een query met para meters `Select` , waarbij wordt opgegeven welke velden in de resultaten moeten worden `organizations`meegenomen, bijvoorbeeld. Een zoek teken reeks `"*"` voor het retour neren van alle inhoud van één veld.
