...
1. Vervang de waarden **spaceId** `YOUR_SPACE_IDENTIFIER`() en **matchers** (`YOUR_MATCHER_IDENTIFIER`) in het gedeelte **USER_DEFINED_BOUNDARY** .
...
Stel de telemetrie van de sensor zo in dat deze rechtstreeks **Temperature**wordt `sensor.DataType`gelezen voor de sensor met gegevens type temperatuur:
...
1. [Vraag de systeem-API](./security-create-manage-role-assignments.md#retrieve-all-roles) voor alle rollen op om de rol-id op te halen die u wilt toewijzen aan uw door de gebruiker gedefinieerde functie. Doe dit door een geverifieerde HTTP GET-aanvraag in te stellen voor:
    ...
    ```
   Behoud de gewenste rol-ID. Het wordt door gegeven als het JSON Body- **roleId** kenmerk roleId`YOUR_DESIRED_ROLE_IDENTIFIER`() hieronder.
