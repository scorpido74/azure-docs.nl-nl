...
Voor deze zelf studie is de Azure PowerShell module versie 1.0.0 of hoger vereist voor het maken van een certificaat en het installeren van IIS. Voer `Get-Module -ListAvailable Az` uit om de versie te bekijken. Als u PowerShell wilt upgraden, raadpleegt u [De Azure PowerShell-module installeren](/powershell/azure/install-az-ps). Als u de opdrachten in deze zelf studie wilt uitvoeren, moet u `Login-AzAccount` ook uitvoeren om een verbinding te maken met Azure.
