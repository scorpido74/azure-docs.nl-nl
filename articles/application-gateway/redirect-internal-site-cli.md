...
Voeg de omleidings configuratie toe die verkeer verzendt van *www\.consoto.org* naar de listener voor *www\.contoso.com* in de toepassings gateway met [AZ Network Application-Gateway redirect-config Create](/cli/azure/network/application-gateway/redirect-config#az-network-application-gateway-redirect-config-create).
