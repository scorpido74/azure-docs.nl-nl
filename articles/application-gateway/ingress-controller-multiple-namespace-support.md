...
1. Wijzig het bestand [helm-config. yaml](#sample-helm-config-file) op een van de volgende manieren:
   ...
   - `watchNamespace` instellen op een lege teken reeks-AGIC krijgt alle naam ruimten
...
Wanneer de implementatie is geïmplementeerd met de mogelijkheid om meerdere naam ruimten te bekijken, AGIC:
  ...
  - filteren op inkomende resources die aantekeningen hebben met `kubernetes.io/ingress.class: azure/application-gateway`
...
Houd er rekening mee dat de Application Gateway resources die worden gemaakt, de naam ruimte (`production`) bevatten waarvoor ze zijn gemaakt, met uitzonde ring van *listener* en *routerings regel*.

Als de twee ingangs bronnen op verschillende tijdstippen in het AKS-cluster worden geïntroduceerd, is het waarschijnlijk dat AGIC wordt beëindigd in een scenario waarin het Application Gateway opnieuw wordt geconfigureerd en verkeer van `namespace-B` naar `namespace-A`opnieuw wordt gerouteerd.

Als u bijvoorbeeld eerst `staging` hebt toegevoegd, configureert AGIC Application Gateway om verkeer door te sturen naar de back-end-staging-groep. In een later stadium wordt `production` binnenkomend, waardoor AGIC Application Gateway, waardoor het routeren van verkeer naar de `production` back-end-groep wordt gestart.
