...
De firewall bevindt zich in de AzureFirewallSubnet en is geconfigureerd met een toepassings regel verzameling met één regel waarmee toegang `www.microsoft.com`kan worden toegestaan.
...
Als u Power shell lokaal uitvoert, is voor dit script Azure PowerShell vereist. Voer `Get-Module -ListAvailable Az` uit om te zien welke versie is geïnstalleerd. 
