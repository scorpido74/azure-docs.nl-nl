---
title: Zelf studie voor installatie-unpack, Rack, kabel Azure Stack rand fysiek apparaat | Microsoft Docs
description: De tweede zelf studie over het installeren van Azure Stack Edge omvat het uitpakken, het rek en de kabel van het fysieke apparaat.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
---
# <a name="tutorial-install-azure-stack-edge"></a>Zelf studie: Azure Stack rand installeren

In deze zelf studie wordt beschreven hoe u een fysiek Azure Stack edge-apparaat installeert. De installatieprocedure omvat het uitpakken, het in een rek monteren en het bekabelen van het apparaat. 
...
* U hebt alle stappen voor het voorbereiden van de [implementatie van Azure stack Edge](azure-stack-edge-deploy-prep.md)voltooid.
    * U hebt een Azure Stack Edge-resource gemaakt om uw apparaat te implementeren.
    * U hebt de activerings sleutel gegenereerd om uw apparaat te activeren met de Azure Stack Edge-resource.

 
### <a name="for-the-azure-stack-edge-physical-device"></a>Voor de Azure Stack rand van het fysieke apparaat
...
- Bekijk de netwerk vereisten voor de implementatie van Azure Stack Edge en configureer het datacenter netwerk volgens de vereisten. Zie [Azure stack Edge-netwerk vereisten](azure-stack-edge-system-requirements.md#networking-port-requirements)voor meer informatie.
...
3. Pak de doos uit. Controleer of de inhoud van de doos bestaat uit de volgende items:
    - Eén behuizing Azure Stack edge-apparaat
    - Twee netsnoeren
    - Eén Rail Kit-assembly
    - Een boekje met veiligheids-, omgevings-en regelgevings informatie

Als u niet alle hier vermelde items hebt ontvangen, neemt u contact op met de ondersteuning van Azure Stack Edge. De volgende stap is het in het rek monteren van uw apparaat.
...
> [!IMPORTANT]
> Azure Stack edge-apparaten moeten zijn gekoppeld aan een rek voor een goede werking.
...
2. Twee Hook-en loop riemen
    ...
    ![Inhoud van de Rail Kit identificeren](./media/azure-stack-edge-deploy-install/identify-rail-kit-contents.png)
...
4. Als u de rails wilt verwijderen, geeft u de vergren deling van de vergrendelings knop op het eind punt van het gedeelte op en verwijdert u elke rail.
    ...
    ![Hulp programma-minder rails installeren en verwijderen](./media/azure-stack-edge-deploy-install/installing-removing-tool-less-rails.png)
...
4. Schuif de linker-en rechter kant omhoog en klik op de pijl naar rechts op de verticale rack flens en voeg deze toe met behulp van twee combi Naties van schroeven.
    ...
    ![Hulpprogram ma's voor het hulp programma installeren en verwijderen](./media/azure-stack-edge-deploy-install/installing-removing-tooled-rails.png)
...
3. Begrijpt de zijkanten van het systeem stevig af en haal het door, totdat de Rail standoffs vooraan in de J-sleuven staan. Til het systeem omhoog en weg van het rek en plaats het op een niveau vlak.
    ...
    ![Het systeem uit het rek verwijderen](./media/azure-stack-edge-deploy-install/removing-system-rack.png)
...
3. Als u het systeem voor verzen ding in het rek of voor andere onstabiele omgevingen wilt beveiligen, zoekt u de vaste montagekit onder elke hendel en draait u elke schroef met een #2 Phillips draaier.
    ...
    ![Slam-hendel benaderen en vrijgeven](./media/azure-stack-edge-deploy-install/engaging-releasing-slam-latch.png)


## <a name="cable-the-device"></a>Het apparaat bekabelen

De kabels routeren en vervolgens uw apparaat bekabelen. In de volgende procedures wordt uitgelegd hoe u uw Azure Stack edge-apparaat kunt bekabelen voor het energie-en netwerk gebruik.

Voordat u begint met de bekabeling van uw apparaat, zorgt u ervoor dat u over de volgende zaken beschikt:

- Uw Azure Stack rand fysiek apparaat, uitgepakt en rek gekoppeld.
- Twee netsnoeren.
...
> - Het Azure Stack edge-apparaat moet zijn verbonden met het datacenter netwerk, zodat het gegevens uit gegevens bron servers kan opnemen.
...
1. Identificeer de verschillende poorten op het achtergrond vlak van uw apparaat.
    ...
    ![Achtergrond vlak van een aangesloten apparaat](./media/azure-stack-edge-deploy-install/backplane-cabled.png)

2. Zoek de schijf sleuven en de aan/uit-knop aan de voor zijde van het apparaat.

    ![Voor vlak van een apparaat](./media/azure-stack-edge-deploy-install/device-front-plane-labeled-1.png)
...
In deze zelf studie hebt u geleerd over Azure Stack Edge-onderwerpen, zoals het volgende:
...
> [Azure Stack rand verbinden en instellen](./azure-stack-edge-deploy-connect-setup-activate.md)
