...
>[!IMPORTANT]
> Als de identiteit die is toegewezen aan het app-configuratie-exemplaar niet meer wordt geautoriseerd om de versleutelings sleutel van het exemplaar op te slaan, of als de beheerde sleutel permanent wordt verwijderd, is het niet meer mogelijk om gevoelige gegevens te ontsleutelen die zijn opgeslagen in de app. Configuratie-exemplaar. Het gebruik van de functie voor het [verwijderen](../key-vault/key-vault-ovw-soft-delete.md) van Azure Key Vault verkleint de kans dat de versleutelings sleutel per ongeluk wordt verwijderd.
...
2. Ken de machtigingen `GET`, `WRAP`en `UNWRAP` toe aan het toegangs beleid van het doel Key Vault.
...
- [Een .NET Framework-app maken met Azure-app configuratie](quickstart-dotnet-app.md)
...
1. Maak een Azure Key Vault met behulp van de Azure CLI.  Houd er rekening mee dat zowel `vault-name` als `resource-group-name` door de gebruiker worden ingevoerd en uniek moeten zijn.  We gebruiken `contoso-vault` en `contoso-resource-group` in deze voor beelden.
...
1. Maak een Key Vault sleutel. Geef een unieke `key-name` voor deze sleutel op en vervang de namen van de Key Vault (`contoso-vault`) die u in stap 1 hebt gemaakt. Geef op of u liever `RSA` of `RSA-HSM` versleuteling.
