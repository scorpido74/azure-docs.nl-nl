...
Als u een DNS-record wilt maken `az network dns record-set <record-type> add-record` , gebruikt u `<record-type>` de opdracht (waarbij het type record is, dat wil zeggen een, SRV, txt, enz.) Zie `az network dns record-set --help`voor meer informatie.
...
In het volgende voor beeld wordt een lege recordset van het type ' A ' met een TTL van 60-Second gemaakt `--ttl` met behulp van `-l`de para meter (kort formulier):

```azurecli
az network dns record-set a create --resource-group myresourcegroup --zone-name contoso.com --name www --ttl 60
```

In het volgende voor beeld wordt een recordset gemaakt met twee meta gegevens items, afd = Finance en Environment = Production, met behulp van `--metadata` de para meter:

```azurecli
az network dns record-set a create --resource-group myresourcegroup --zone-name contoso.com --name www --metadata "dept=finance" "environment=production"
```

Als u `azure network dns record-set <record-type> add-record` een lege recordset hebt gemaakt, kunnen records worden toegevoegd, zoals beschreven in [een DNS-record maken](#create-a-dns-record).
...
Als u een bestaande recordset wilt ophalen, `az network dns record-set <record-type> show`gebruikt u. Zie `az network dns record-set <record-type> show --help` voor help.
...
U kunt alle records in een DNS-zone weer geven met `az network dns record-set list` behulp van de opdracht. Zie `az network dns record-set list --help` voor help.
...
U kunt beide `az network dns record-set <record-type> add-record` gebruiken om een record in een nieuwe recordset te maken of om een record toe te voegen aan een bestaande recordset.
...
Als u een DNS-record uit een bestaande recordset wilt verwijderen `az network dns record-set <record-type> remove-record`, gebruikt u. Zie `az network dns record-set <record-type> remove-record -h` voor help.

Met deze opdracht wordt een DNS-record uit een recordset verwijderd. Als de laatste record in een recordset wordt verwijderd, wordt de recordset zelf ook verwijderd. Gebruik de `--keep-empty-record-set` optie om in plaats daarvan de lege recordset te blijven instellen.

U moet de record opgeven die moet worden verwijderd en de zone waarvan deze moet worden verwijderd, met dezelfde para meters als bij het maken van `az network dns record-set <record-type> add-record`een record met. Deze para meters worden beschreven in [een DNS-record maken](#create-a-dns-record) en [records van andere typen maken](#create-records-of-other-types) .
...
U kunt geen records toevoegen, verwijderen of wijzigen in de automatisch gemaakte NS-recordset die is ingesteld op de zone`--Name "@"`Apex (, inclusief de aanhalings tekens). De enige toegestane wijzigingen zijn alleen van invloed op de ingestelde TTL en meta gegevens van de recordset.
...
In plaats daarvan kunt u een CNAME-record `az network dns record-set cname set-record`wijzigen met. Meer informatie vindt u in `az network dns record-set cname set-record --help`
...
In plaats daarvan kunt u het SOA-record `az network dns record-set soa update`wijzigen. Zie `az network dns record-set soa update --help` voor help.
...
Als u de TTL van een bestaande recordset wilt wijzigen, `azure network dns record-set <record-type> update`gebruikt u. Zie `azure network dns record-set <record-type> update --help` voor help.
...
Record sets kunnen worden verwijderd met behulp `az network dns record-set <record-type> delete` van de opdracht. Zie `azure network dns record-set <record-type> delete --help` voor help. Als u een recordset verwijdert, worden ook alle records in de recordset verwijderd.

> [!NOTE]
> U kunt de SOA-en NS-record sets niet verwijderen uit de`--name "@"`zone Apex ().  Deze worden automatisch gemaakt wanneer de zone werd gemaakt en worden automatisch verwijderd wanneer de zone wordt verwijderd.
...
U wordt gevraagd om de Verwijder bewerking te bevestigen. Gebruik de `--yes` Schakel optie om deze prompt te onderdrukken.
