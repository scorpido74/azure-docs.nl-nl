...
description: Ga in dit leerpad aan de slag met het beveiligen van DNS-zones en recordsets in Microsoft Azure DNS.
services: dns
author: asudbring
ms.service: dns
ms.topic: article
...
DNS-zones en -records zijn kritieke bronnen. Als u een DNS-zone of één DNS-record verwijderd, kan dit leiden tot een servicestoring. Het is belangrijk dat DNS-zones en -records worden beschermd tegen ongeautoriseerde of toevallige wijzigingen.
...
### <a name="the-dns-zone-contributor-role"></a>De rol DNS-zonebijdrager

De rol DNS-zonebijdrager is een ingebouwde rol voor het beheren van privé-DNS-resources. Met deze rol die wordt toegepast op een gebruiker of groep, kunnen ze DNS-bronnen beheren.

De resourcegroep *myResourceGroup* bevat vijf zones voor Contoso Corporation. Door de DNS-beheerder DNS-zonebijdragermachtigingen toe te kennen aan die brongroep, u volledige controle over deze DNS-zones uitvoeren. Het voorkomt het verlenen van onnodige machtigingen. De DNS-beheerder kan geen virtuele machines maken of stoppen.
...
Open **Access-besturingselement (IAM)** voor de brongroep en selecteer **Vervolgens Toevoegen**en selecteer vervolgens de rol **DNS-zoneinzender.** Selecteer de vereiste gebruikers of groepen om machtigingen te verlenen.

![RBAC op resourcegroepniveau via de Azure-portal](./media/dns-protect-zones-recordsets/rbac1.png)
...
### <a name="zone-level-rbac"></a>Zoneniveau RBAC
...
De resourcegroep *myResourceGroup* bevat bijvoorbeeld de zone *contoso.com* en een subzone *customers.contoso.com*. Voor elk klantaccount worden CNAME-records gemaakt. Aan het beheerdersaccount dat wordt gebruikt voor het beheren van CNAME-records, worden machtigingen toegewezen om records te maken in de *customers.contoso.com* zone. Het account kan alleen *customers.contoso.com* beheren.

RBAC-machtigingen op zoneniveau kunnen worden verleend via de Azure-portal.  Open **Access control (IAM)** voor de zone, selecteer **Toevoegen,** selecteer vervolgens de rol **DNS-zoneinzender en** selecteer de vereiste gebruikers of groepen om machtigingen toe te kennen.

![DNS-zoneniveau RBAC via de Azure-portal](./media/dns-protect-zones-recordsets/rbac2.png)
...
![Record set level RBAC via de Azure portal](./media/dns-protect-zones-recordsets/rbac3.png)
...
De ingebouwde functie DNS-zonebijdrager maakt volledige controle over een DNS-bron mogelijk. Het is mogelijk om uw eigen aangepaste Azure-rollen te bouwen om fijnmazige controle te bieden.
...
* `Microsoft.Network/dnsZones/read`geeft toestemming om DNS-zones te lezen, maar niet om ze te wijzigen, zodat u de zone zien waarin de CNAME wordt gemaakt.

De overige acties worden gekopieerd uit de [ingebouwde rol DNS-zonebijdrager](../role-based-access-control/built-in-roles.md#dns-zone-contributor).
