...
We volgen de regels voor de bestands hoofd indeling van de [RFC 1035](https://www.ietf.org/rfc/rfc1035.txt) -zone voor TXT-records. `\` nu bijvoorbeeld in werkelijkheid alles per RFC vergelijkt. Als u `A\B` opgeeft als waarde voor TXT-record, wordt deze weer gegeven en omgezet als alleen `AB`. Als u wilt dat de TXT-record `A\B` bij resolutie, moet u de `\` opnieuw door lopen. Geef bijvoorbeeld `A\\B`op.
