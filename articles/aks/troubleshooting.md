...
* De logboeken, met `kubectl logs <pod-name>`behulp van.
...
1. De status van `failed` het cluster kan niet worden `upgrade` hersteld `scale` en de bewerkingen kunnen niet worden uitgevoerd. Veelvoorkomende basis problemen en oplossingen zijn:
...
Om het probleem op `az aks show -g myResourceGroup -n myAKSCluster -o table` te lossen en de gedetailleerde status van het cluster op te halen. Op basis van het resultaat:
...
Als een Windows-pod met een Azure Files-koppeling wordt verwijderd en vervolgens opnieuw wordt gemaakt op hetzelfde knoop punt, mislukt de koppeling. Deze fout is opgetreden vanwege `New-SmbGlobalMapping` een mislukte opdracht omdat de Azure files koppeling al aan het knoop punt is gekoppeld.
...
Als u de sleutel van uw opslag account wilt coderen in Base64 `base64`, kunt u gebruiken. Bijvoorbeeld:
