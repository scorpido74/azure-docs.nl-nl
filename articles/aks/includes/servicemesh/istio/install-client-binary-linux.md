...
De `istioctl` binaire client wordt uitgevoerd op uw client computer en u kunt Istio installeren en beheren in uw AKS-cluster. Gebruik de volgende opdrachten om de Istio- `istioctl` client binaire bestanden te installeren in een op bash gebaseerde shell op Linux of [Windows-subsysteem voor Linux][install-wsl]. Met deze opdrachten `istioctl` wordt de binaire client gekopieerd naar de standaard gebruikers programma locatie in uw `PATH` .
