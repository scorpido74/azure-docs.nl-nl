...
description: Meer informatie over het uploaden en openen van gegevens voor Apache Hadoop-taken in HDInsight. Gebruik Azure classic CLI, Azure Storage Explorer, Azure PowerShell, de Hadoop-opdrachtregel of Sqoop.
...
ms.locfileid: "80875007"
---
# <a name="upload-data-for-apache-hadoop-jobs-in-hdinsight"></a>Gegevens uploaden voor Apache Hadoop-taken in HDInsight

HDInsight biedt een Hadoop distributed file system (HDFS) via Azure Storage en Azure Data Lake Storage. Deze opslag omvat Gen1 en Gen2. Azure Storage and Data Lake Storage Gen1 en Gen2 zijn ontworpen als HDFS-extensies. Hiermee kunnen de volledige set componenten in de Hadoop-omgeving rechtstreeks werken op de gegevens die het beheert. Azure Storage, Data Lake Storage Gen1 en Gen2 zijn verschillende bestandssystemen. De systemen zijn geoptimaliseerd voor de opslag van gegevens en berekeningen op die gegevens. Zie [Azure Storage gebruiken met HDInsight](hdinsight-hadoop-use-blob-storage.md)voor informatie over de voordelen van het gebruik van Azure Storage. Zie ook, [Gebruik Data Lake Storage Gen1 met HDInsight](hdinsight-hadoop-use-data-lake-store.md)en Gebruik Data Lake Storage [Gen2 met HDInsight](hdinsight-hadoop-use-data-lake-storage-gen2.md).
...
Als u de opdracht Hadoop wilt gebruiken, moet u eerst verbinding maken met de headnode via [SSH of PuTTY.](hdinsight-hadoop-linux-use-ssh-unix.md)
...
De Azure Data Factory-service is een volledig beheerde service voor het samenstellen van gegevens: opslag-, verwerkings- en bewegingsservices in gestroomlijnde, aanpasbare en betrouwbare pijplijnen voor gegevensproductie.
...
### <a name="storage-exception-for-write-on-blob"></a>Opslaguitzondering voor schrijven op blob
...
**Resolutie:** `fs.azure.write.request.size` gebruik een grotere blokgrootte opgeven. U deze wijziging per gebruik doen `-D` met behulp van de parameter. De volgende opdracht is een voorbeeld `hadoop` met deze parameter met de opdracht:
...
1. Ga in uw browser naar de Gebruikersinterface van Ambari Web voor uw cluster. De URL `https://CLUSTERNAME.azurehdinsight.net`is `CLUSTERNAME` , waar is de naam van uw cluster. Voer de naam en het wachtwoord voor het cluster in wanneer u daarom wordt gevraagd.
2. Selecteer **hdfs**aan de linkerkant van het scherm en selecteer vervolgens het tabblad **Configs.**
3. Voer in het veld `fs.azure.write.request.size` **Filter...** de enter in .
...
Nu u begrijpt hoe u gegevens in HDInsight krijgen, leest u de volgende artikelen om analyse te leren:
