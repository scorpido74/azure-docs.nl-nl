...
1. Maak verbinding met de domein controller met behulp van Extern bureaublad.
    1. Navigeer vanuit het Azure Portal naar **resource groepen** > **OnPremADVRG** > **adVM** > **verbinding maken**.
...
1. Ga in het dash board van de domein controller **Serverbeheer** naar **hulpprogram Ma's** > **Active Directory gebruikers en computers**.
...
1. Maak twee nieuwe gebruikers: **HDIAdmin** en **HDIUser**. Deze twee gebruikers worden aangemeld bij HDInsight-clusters.
    ...
    1. Klik op de pagina **Active Directory gebruikers en computers** met de rechter muisknop op `HDIFabrikam.com`en navigeer vervolgens naar de **nieuwe** > **gebruiker**.
    ...
    1. Voer op de pagina **Nieuw object-gebruiker** `HDIUser` in voor de voor **naam** en **aanmeldings naam**van de gebruiker. De overige velden worden automatisch ingevuld. Selecteer vervolgens **Volgende**.
    ...
    1. Herhaal de bovenstaande stappen om de gebruikers `HDIAdmin`te maken.
...
1. Een globale beveiligings groep maken.
    ...
    1. Klik in **Active Directory gebruikers en computers**met de rechter muisknop op `HDIFabrikam.com`en navigeer vervolgens naar **nieuwe** > **groep**.
...
1. Leden toevoegen aan **HDIUserGroup**.
    ...
    1. Voer in het tekstvak **Geef de object namen op** de tekst in `HDIUserGroup`. Selecteer **OK**en **Klik nogmaals op** de pop-up.
...
1. Selecteer **een resource maken** en typ `directory`. Selecteer **Azure Active Directory** > **maken**.
1. Voer bij **organisatie naam**`HDIFabrikam`in.
...
1. Voer bij **aangepaste domein naam**`HDIFabrikam.com`in en selecteer vervolgens **domein toevoegen**.
...
1. Voer in het tekstvak **groeps naam** `AAD DC Administrators`in.
1. Selecteer **Maken**.
...
1. Navigeren om > **gebruikers** > **nieuwe gebruiker**te **beheren** .
...
1. Voer de volgende gegevens in voor de nieuwe gebruiker:
    ...
    **Identity**
    ...
    |---|---|
    |Gebruikersnaam|Voer `fabrikamazureadmin` in het tekstvak in. Selecteer in de vervolg keuzelijst domein naam `hdifabrikam.com`|
...
1. Voer op de pagina **verbinding maken met Active Directory Domain Services** de gebruikers naam en het wacht woord in voor een ondernemings Administrator-account. Gebruik de gebruikers naam `HDIFabrikam\HDIFabrikamAdmin` en het bijbehorende wacht woord dat u eerder hebt gemaakt. Selecteer vervolgens **Volgende**.
...
1. Selecteer **volgende**op de pagina **aanmeldings configuratie van Azure AD** .
   ![de pagina ' Azure AD-aanmeld configuratie '](./media/apache-domain-joined-create-configure-enterprise-security-cluster/hdinsight-image-0062.png)
...
1. Meld u aan bij Azure Portal.
1. Selecteer **een resource maken** en typ `managed identity`. Selecteer door de **gebruiker toegewezen beheerde identiteit** > **maken**.
...
1. Meld u aan bij Azure Portal.
1. Selecteer **resource maken**, voer `Domain services`in en selecteer **Azure AD Domain Services** > **maken**.
...
> [!NOTE]  
> Elk hulp programma of een toepassing die een geldige open bare sleutel crypto grafie-standaard (PKCS) \#10-aanvraag maakt, kan worden gebruikt om de SSL-certificaat aanvraag te vormen.
...
1. Nu u LDAPS hebt ingeschakeld, moet u ervoor zorgen dat deze bereikbaar is door poort 636 in te scha kelen.
    ...
    1. Selecteer onder **instellingen**de optie **regels voor binnenkomende beveiliging** > **toevoegen**.
...
1. Maak een peer-relatie tussen het virtuele netwerk dat als host fungeert voor Azure AD DS (`HDIFabrikam-AADDSVNET`) en het virtuele netwerk dat wordt gebruikt voor het HDInsight-cluster met ESP (`HDIFabrikam-HDIVNet`). Gebruik de volgende Power shell-code voor de peering van de twee virtuele netwerken.
