...
| Integratie met Azure-services |Spark-clusters in HDInsight worden geleverd met een connector voor Azure Event Hubs. U streamingtoepassingen bouwen met behulp van de Event Hubs. Waaronder Apache Kafka, die al beschikbaar is als onderdeel van Spark. |
...
| Aanpassingsvermogen | Met HDInsight u het aantal clusterknooppunten dynamisch wijzigen met de functie Autoscale. Zie [Azure HDInsight-clusters automatisch schalen](../hdinsight-autoscale-clusters.md). Bovendien kunnen Spark-clusters zonder gegevensverlies worden verwijderd, omdat alle gegevens zijn opgeslagen in Azure Storage of Data Lake Storage. |
...
HDInsight Spark clustert een [ODBC-stuurprogramma](https://go.microsoft.com/fwlink/?LinkId=616229) voor connectiviteit met BI-tools zoals Microsoft Power BI.
...
Spark-toepassingen worden uitgevoerd als onafhankelijke reeksen processen op een cluster. Gecoördineerd door het SparkContext-object in uw hoofdprogramma (het driverprogramma genoemd).

De SparkContext kan verbinding maken met verschillende typen clusterbeheerders, die bronnen voor verschillende toepassingen geven. Deze clusterbeheerders omvatten Apache Mesos, [Apache Hadoop YARN](https://hadoop.apache.org/docs/current/hadoop-yarn/hadoop-yarn-site/YARN.html)of de Spark-clusterbeheerder. In HDInsight wordt Spark uitgevoerd met behulp van de YARN-clustermanager. Als er verbinding is, verkrijgt Spark executors op werkknooppunten in het cluster. Dit zijn processen die berekeningen uitvoeren en gegevens opslaan voor uw toepassing. Vervolgens wordt de code van uw toepassing (gedefinieerd in JAR- of Python-bestanden die worden doorgegeven aan SparkContext) verstuurd naar de executor. Ten slotte verzendt SparkContext taken naar de executors om uit te voeren.
...
De SparkContext maakt verbinding met de Spark-master en is verantwoordelijk voor het converteren van een toepassing naar een gerichte grafiek (DAG) van afzonderlijke taken. Taken die worden uitgevoerd binnen een executorproces op de werkknooppunten. Elke applicatie krijgt zijn eigen executor processen. Die blijven voor de duur van de hele toepassing en taken uitvoeren in meerdere threads.
...
Met Apache Spark in HDInsight worden gegevens opgeslagen in Azure Storage of Azure Data Lake Storage. Bedrijfsexperts en belangrijke besluitvormers kunnen over die gegevens rapporten analyseren en bouwen. En gebruik Microsoft Power BI om interactieve rapporten te maken van de geanalyseerde gegevens. Analisten kunnen beginnen met ongestructureerde/semi-gestructureerde gegevens in clusteropslag, een schema voor de gegevens definiëren met behulp van notebooks, en vervolgens gegevensmodellen bouwen met behulp van Microsoft Power BI. Spark clusters in HDInsight ondersteunen ook een aantal BI-tools van derden. Zoals Tableau, waardoor het eenvoudiger wordt voor data-analisten, bedrijfsexperts en belangrijke besluitvormers.
...
Apache Spark wordt geleverd met [MLlib.](https://spark.apache.org/mllib/) MLlib is een machine learning-bibliotheek die is gebouwd bovenop Spark en die u gebruiken vanaf een Spark-cluster in HDInsight. Spark cluster in HDInsight omvat ook Anaconda, een Python-distributie met verschillende soorten pakketten voor machine learning. En met ingebouwde ondersteuning voor Jupyter- en Zeppelin-notebooks hebt u een omgeving voor het maken van machine learning-toepassingen.
...
Spark-clusters in HDInsight bieden uitgebreide ondersteuning voor het bouwen van realtime analyseoplossingen. Spark heeft al connectors om gegevens uit vele bronnen zoals Kafka, Flume, Twitter, ZeroMQ of TCP-sockets in te nemen. Spark in HDInsight voegt eersteklas ondersteuning toe voor het opnemen van gegevens uit Azure Event Hubs. Event Hubs is de meest gebruikte wachtrijservice in Azure. Met volledige ondersteuning voor Event Hubs is Spark-clusters in HDInsight een ideaal platform voor het bouwen van realtime analysepijplijn.
...
In dit overzicht hebt u algemene informatie gekregen over het gebruik van Apache Spark in Azure HDInsight. Meer informatie over het maken van een HDInsight Spark-cluster en het uitvoeren van enkele Spark SQL-query's:
