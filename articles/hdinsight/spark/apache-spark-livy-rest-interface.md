...
1. Stel omgevings variabelen in om gebruiks gemak te gebruiken. Dit voor beeld is gebaseerd op een Windows-omgeving en wijzigt indien nodig de variabelen voor uw omgeving. Vervang `CLUSTERNAME`en `PASSWORD` met de juiste waarden.
...
HDInsight 3,5-clusters en hoger: Schakel het gebruik van lokale bestands paden standaard uit voor toegang tot bestanden of potten met voorbeeld gegevens. We raden u aan om in plaats daarvan het `wasbs://` pad te gebruiken voor het openen van potten of voorbeeld gegevens bestanden van het cluster.
