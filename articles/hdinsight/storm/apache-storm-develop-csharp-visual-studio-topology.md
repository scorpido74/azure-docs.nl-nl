...
description: Meer informatie over het maken van Storm C#-topologieën in. Maak een topologie voor het tellen van woorden in Visual Studio met behulp van de Hadoop-hulpprogram ma's voor Visual Studio.
...
Als u een C# topologie met een op Linux gebaseerd cluster wilt gebruiken, moet u het `Microsoft.SCP.Net.SDK` NuGet-pakket dat door uw project wordt gebruikt, bijwerken naar versie 0.10.0.6 of hoger. De versie van het pakket moet ook overeenkomen met de primaire versie van Storm die op HDInsight is geïnstalleerd.
...
> [!IMPORTANT]  
> C#-topologieën met op Linux gebaseerde clusters moeten .NET 4.5 en Mono gebruiken om op het HDInsight-cluster te worden uitgevoerd. Zie [mono-compatibiliteit](https://www.mono-project.com/docs/about-mono/compatibility/)voor meer informatie over mogelijke compatibiliteits problemen.

## <a name="install-visual-studio"></a>Visual Studio installeren

U kunt topologieën ontwikkelen C# met SCP.net met behulp van [Visual Studio](https://visualstudio.microsoft.com/downloads/). In deze instructies wordt gebruikgemaakt van Visual Studio 2019, maar u kunt ook eerdere versies van Visual Studio gebruiken.
...
2. Stel de omgevings variabele `JAVA_HOME` in op de map die Java bevat.

3. Stel de `PATH` omgevings variabele in op het toevoegen van de map *% JAVA_HOME% \ bin* .
...
1. Open *Spout.cs*. De belangrijkste onderdelen voor een Spout zijn:
   ...
   * `Ack` (alleen transactionele topologie): verwerkt de bevestigingen die zijn gestart door andere onderdelen in de topologie voor Tuples die vanuit de Spout zijn verzonden. Door een tuple te bevestigen, kan de Spout weten dat deze is verwerkt door downstream-onderdelen.

   * `Fail` (alleen transactionele topologie): verwerkt Tuples die niet worden verwerkt door andere onderdelen in de topologie. Als u een `Fail` methode implementeert, kunt u de tuple opnieuw verzenden zodat deze opnieuw kan worden verwerkt.

2. Vervang de inhoud van de klasse `Spout` door de volgende tekst:
...
3. Open *splitter.cs*. Er is slechts één methode standaard: `Execute`. De `Execute` methode wordt aangeroepen wanneer de schicht een tuple voor verwerking ontvangt. Hier kunt u binnenkomende Tuples lezen en verwerken en uitgaande Tuples verzenden.

4. Vervang de inhoud van de klasse `Splitter` door de volgende code:
...
2. Kies in het dialoog venster **topologie indienen** in de vervolg keuzelijst **Storm-cluster** het Storm-cluster op HDInsight en selecteer vervolgens **verzenden**. U kunt controleren of de verzen ding is geslaagd door het deel venster **uitvoer** weer te geven.
    ...
    > [!NOTE]  
    > U kunt Storm- **topologieën** ook bekijken vanuit **Server Explorer**. Vouw **Azure** > **HDInsight**uit, klik met de rechter muisknop op een storm op HDInsight-cluster en kies vervolgens **Storm-topologieën weer geven**.

    Als u informatie wilt weer geven over de onderdelen in de topologie, selecteert u een onderdeel in het diagram.

3. Selecteer in het gedeelte **topologie samenvatting** de optie **Kill** om de topologie te stoppen.
...
* **ACK**: elke Schicht in de topologie kan `this.ctx.Ack(tuple)` aanroepen om te bevestigen dat een tuple is verwerkt. Wanneer alle schichten de tuple hebben bevestigd, wordt de `Ack` methode van de Spout aangeroepen. Met de methode `Ack` kan de Spout gegevens verwijderen die in de cache zijn opgeslagen voor opnieuw afspelen.

* **Mislukt**: elke schicht kan `this.ctx.Fail(tuple)` aanroepen om aan te geven dat de verwerking voor een tuple is mislukt. De fout wordt door gegeven aan de `Fail` methode van de Spout, waarbij de tuple opnieuw kan worden afgespeeld met behulp van meta gegevens in de cache.
...
* **Java Spout** en  **C# bout**: gedefinieerd in de klasse `HybridTopology_javaSpout_csharpBolt`.

  Er wordt een transactionele versie gedefinieerd in de klasse `HybridTopologyTx_javaSpout_csharpBolt`.

* Spout en **Java-flits**: gedefinieerd in de klasse `HybridTopology_csharpSpout_javaBolt`. **C#**

  Er wordt een transactionele versie gedefinieerd in de klasse `HybridTopologyTx_csharpSpout_javaBolt`.
...
* Gebruik `JavaComponentConstructor` om een exemplaar van de Java-klasse te maken voor een Spout of bout.

* Gebruik `microsoft.scp.storm.multilang.CustomizedInteropJSONSerializer` om gegevens van Java-objecten te serialiseren naar of uit een Java-object naar JSON.
...
Als u een `Get` methode gebruikt om een exemplaar van uw onderdeel te retour neren, moet u ervoor zorgen dat de para meters `Context` en `Dictionary<string, Object>` worden door gegeven aan de constructor. Het volgende voor beeld is een basis `Get` methode waarmee deze waarden correct worden door gegeven:
...
>
> 1. Klik in **Solution Explorer** met de rechtermuisknop op het project en klik op **NuGet-pakketten beheren**.
> 2. Zoek met het **Zoek** veld naar en voeg `Microsoft.SCP.Net.SDK` toe aan het project.
...
Wanneer u een C# topologie gebruikt met een HDInsight-cluster op basis van Linux, kunnen de onderdelen van de Spout die **ConfigurationManager** gebruiken voor het lezen van de configuratie-instellingen tijdens runtime, null-pointer-uitzonde ringen retour neren.
...
Zie de sectie [ConfigurationManager gebruiken](#configurationmanager) van dit document voor meer informatie.
...
1. Open *LocalTest.cs*en voeg bovenaan de volgende `using`-instructie toe:
...
1. Gebruik de volgende code als de inhoud van de klasse `LocalTest`:
    ...
    Neem even de tijd om de opmerkingen van de code te lezen. Deze code maakt gebruik van `LocalContext` om de onderdelen in de ontwikkel omgeving uit te voeren. De gegevens stroom tussen onderdelen wordt persistent gemaakt naar tekst bestanden op het lokale station.
...
Vervang *sshuser* door het SSH-gebruikers account voor het cluster. Vervang *clustername* door de naam van het HDInsight-cluster. Zie [SSH gebruiken met hdinsight](../hdinsight-hadoop-linux-use-ssh-unix.md)voor meer informatie over het gebruik van `scp` en `ssh` met hdinsight.
...
Als het logboek bestand *hdinsight-scpwebapi. out* een `FileNotFoundException`bevat, kan de uitzonde ring worden veroorzaakt door de volgende voor waarden:

* De JDK bevindt zich niet in het pad van de ontwikkel omgeving. Controleer of de JDK in de ontwikkel omgeving is geïnstalleerd en of `%JAVA_HOME%/bin` zich in het pad bevindt.
