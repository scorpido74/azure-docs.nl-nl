...
Spark Structured Streaming is een streamverwerkingsengine gebaseerd op de Spark SQL-engine. Bij het gebruik van Structured streaming kunt u streaming query's op dezelfde manier schrijven als batch query's.
...
Als u Structured Streaming wilt gebruiken met Kafka, moet het project afhankelijk zijn van het pakket `org.apache.spark : spark-sql-kafka-0-10_2.11`. De versie van dit pakket moet overeenkomen met de versie van Spark in HDInsight. Voor Spark 2.2.0 (beschikbaar in HDInsight 3,6) vindt u de afhankelijkheids informatie voor verschillende project typen op [https://search.maven.org/#artifactdetails%7Corg.apache.spark%7Cspark-sql-kafka-0-10_2.11%7C2.2.0%7Cjar](https://search.maven.org/#artifactdetails%7Corg.apache.spark%7Cspark-sql-kafka-0-10_2.11%7C2.2.0%7Cjar).
...
1. Ga in een webbrowser naar `https://CLUSTERNAME.azurehdinsight.net/jupyter`, waarbij `CLUSTERNAME` de naam van het cluster is. Typ desgevraagd de cluster-aanmelding (beheerder) en het cluster-wachtwoord die u hebt gebruikt bij het maken van het cluster.

1. Selecteer **nieuw > Spark** om een notitie blok te maken.

1. Spark streaming heeft batch verwerking, wat betekent dat gegevens worden geleverd als batches en uitvoer uit de batch met gegevens. Als de uitvoerder een time-out voor inactiviteit heeft die korter is dan de tijd die nodig is om de batch te verwerken, worden de uitvoerende partijen voortdurend toegevoegd en verwijderd. Als de time-out voor de uitvoering van de uitvoerender groter is dan de batch duur, wordt de uitvoerder nooit verwijderd. Het **wordt daarom aangeraden om dynamische toewijzing uit te scha kelen door Spark. dynamicAllocation. enabled in te stellen op False bij het uitvoeren van streaming-toepassingen.**
...
1. Maak het onderwerp Kafka. Bewerk de onderstaande opdracht door de `YOUR_ZOOKEEPER_HOSTS` Zookeeper-hostgegevens die in de eerste stap zijn geëxtraheerd, te vervangen. Voer de bewerkte opdracht in uw Jupyter Notebook in `tripdata` om het onderwerp te maken.
...
1. De gegevens verzenden naar Kafka. In de volgende opdracht wordt het `vendorid` veld als de sleutel waarde voor het Kafka-bericht gebruikt. De sleutel wordt door Kafka gebruikt bij het partitioneren van gegevens. Alle velden worden opgeslagen in het Kafka-bericht als een JSON-teken reeks waarde. Voer de volgende opdracht in Jupyter in om de gegevens op te slaan in Kafka met behulp van een batch-query.
...
1. Selecteer gegevens en start de stroom. De volgende opdracht laat zien hoe u gegevens ophaalt uit Kafka met behulp van een batch-query. En schrijf de resultaten vervolgens naar HDFS op het Spark-cluster. In dit voor beeld wordt `select` het bericht (waardeveld) opgehaald uit Kafka en wordt het schema toegepast op het veld. De gegevens worden vervolgens naar HDFS (WASB of ADL) in Parquet-indeling geschreven. Voer de opdracht in de volgende Jupyter-cel in.
...
1. U kunt controleren of de bestanden zijn gemaakt door de opdracht in de volgende Jupyter-cel in te voeren. Hierin worden de bestanden in de `/example/batchtripdata` map weer gegeven.
...
Als u de in deze zelfstudie gemaakte resources wilt opschonen, kunt u de resourcegroep verwijderen. Als u de resource groep verwijdert, wordt ook het bijbehorende HDInsight-cluster verwijderd. En alle andere resources die aan de resource groep zijn gekoppeld.
...
In deze zelf studie hebt u geleerd hoe u Apache Spark Structured streaming kunt gebruiken. Gegevens schrijven en lezen van Apache Kafka op HDInsight. Gebruik de volgende koppeling voor informatie over hoe u Apache Storm gebruikt met Kafka.
