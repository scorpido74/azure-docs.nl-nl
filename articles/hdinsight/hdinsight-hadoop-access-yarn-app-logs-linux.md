...
Er moeten twee configuraties worden toegevoegd aan de aangepaste `mapred-site` in Ambari.

1. Ga in een webbrowser naar `https://CLUSTERNAME.azurehdinsight.net`, waarbij `CLUSTERNAME` de naam van uw cluster is.

1. Ga in de Ambari-gebruikers interface naar **MapReduce2** > **configs** > **Geavanceerd** > **aangepaste mapred-site**.
...
1. Down load de logboeken voor garen-containers voor alle toepassings modellen met de onderstaande opdracht. Hiermee maakt u het logboek bestand met de naam `amlogs.txt` in een tekst indeling.
