...
Voor D14v2 worker-knoop punten is de aanbevolen waarde voor de llap-wachtrij _ * 80 * *.   
...
Als u het aantal uitvoerender wilt aanpassen, kunt u het beste 4 GB geheugen per uitvoerder overwegen, zoals opgegeven door _hive. TEZ. container. size * en ervoor te zorgen dat het totale geheugen dat nodig is voor alle uitvoerers, niet groter is dan het totale beschik bare geheugen voor LLAP daemon-container.  
...
**_Hive. llap. io. thread pool. grootte_*_: deze waarde geeft de grootte van de thread groep voor uitvoerders. Omdat de uitvoeringen van de toepassing zijn hersteld, is deze hetzelfde als het aantal uitvoerender per LLAP-daemon. Voor D14 v2 is de aanbevolen waarde _* 12** .
