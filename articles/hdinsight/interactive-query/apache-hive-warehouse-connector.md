...
1. Navigeer naar de pagina Apache Ambari-Hive van het `https://LLAPCLUSTERNAME.azurehdinsight.net/#/main/services/HIVE/configs` cluster `LLAPCLUSTERNAME` met de naam van uw interactieve query cluster.

1. Navigeer naar **Advanced** > **General** > **component. meta Store. uri's** en noteer de waarde. De waarde kan er ongeveer als volgt `thrift://iqgiro.rekufuk2y2cezcbowjkbwfnyvd.bx.internal.cloudapp.net:9083,thrift://hn1-iqgiro.rekufuk2y2cezcbowjkbwfnyvd.bx.internal.cloudapp.net:9083`uitzien:.

1. Navigeer naar de **Geavanceerde** > **Geavanceerde component-site** > **component. Zookeeper. quorum** en noteer de waarde. De waarde kan er ongeveer als volgt `zk0-iqgiro.rekufuk2y2cezcbowjkbwfnyvd.bx.internal.cloudapp.net:2181,zk1-iqgiro.rekufuk2y2cezcbowjkbwfnyvd.bx.internal.cloudapp.net:2181,zk4-iqgiro.rekufuk2y2cezcbowjkbwfnyvd.bx.internal.cloudapp.net:2181`uitzien:.

#### <a name="from-your-apache-spark-cluster"></a>Vanuit uw Apache Spark-cluster

1. Navigeer naar de pagina Apache Ambari-Hive van het `https://SPARKCLUSTERNAME.azurehdinsight.net/#/main/services/HIVE/configs` cluster `SPARKCLUSTERNAME` met de naam van uw Apache Spark cluster.

1. Ga naar **Geavanceerde** > **Geavanceerde component-Interactive-site** > **Hive. llap. daemon. service. hosts** en noteer de waarde. De waarde kan er ongeveer als volgt `@llap0`uitzien:.
...
|`spark.sql.hive.hiveserver2.jdbc.url`|`jdbc:hive2://LLAPCLUSTERNAME.azurehdinsight.net:443/;user=admin;password=PWD;ssl=true;transportMode=http;httpPath=/hive2`. Ingesteld op de JDBC-connection string, die verbinding maakt met Hiveserver2 op het interactieve query cluster. Vervang `LLAPCLUSTERNAME` door de naam van uw interactieve query cluster. Vervang `PWD` door het werkelijke wacht woord.|
|`spark.datasource.hive.warehouse.load.staging.dir`|`wasbs://STORAGE_CONTAINER_NAME@STORAGE_ACCOUNT_NAME.blob.core.windows.net/tmp`. Stel in op een geschikte, met HDFS compatibele tijdelijke directory. Als u twee verschillende clusters hebt, moet de map voor gefaseerde installatie een map zijn in de map met tijdelijke bestanden van het opslag account van het LLAP-cluster, zodat HiveServer2 er toegang toe heeft.  Vervang `STORAGE_ACCOUNT_NAME` door de naam van het opslag account dat wordt gebruikt door het cluster en `STORAGE_CONTAINER_NAME` met de naam van de opslag container.|
|`spark.datasource.hive.warehouse.metastoreUri`|De waarde die u eerder hebt verkregen uit **Hive. meta Store. uri's**.|
|`spark.security.credentials.hiveserver2.enabled`|`false`voor de client implementatie modus van garen.|
...
1. Filter de tabel `hivesampletable` waarbij de kolom `state` gelijk is `Colorado`aan. Deze query van de Hive-tabel wordt geretourneerd als een Spark-data frame. Vervolgens wordt de data frame opgeslagen in de Hive- `sampletable_colorado` tabel met `write` behulp van de functie.
