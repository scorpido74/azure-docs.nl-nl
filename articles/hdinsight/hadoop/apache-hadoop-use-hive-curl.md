...
* Als u bash gebruikt, hebt u ook JQ nodig, een JSON-processor met opdracht regel.  Zie [https://stedolan.github.io/jq/](https://stedolan.github.io/jq/).
...
**B. Voer** de volgende code uit in Power shell en geef uw referenties op in het pop-upvenster:
...
1. Gebruik het volgende om een tabel met de naam **log4jLogs**te maken:
   ...
   * `STORED AS TEXTFILE LOCATION`-Waar de gegevens worden opgeslagen (het voor beeld/de gegevens Directory) en dat deze als tekst wordt opgeslagen.
...
1. Zodra de status van de taak is gewijzigd in **geslaagd**, kunt u de resultaten van de taak ophalen uit Azure Blob-opslag. De `statusdir` para meter die is door gegeven met de query bevat de locatie van het uitvoer bestand, `/example/rest`in dit geval. Dit adres slaat de uitvoer op in `example/curl` de map in de standaard opslag voor clusters.
