---
title: IP-adressen azure HDInsight-beheer
description: Ontdek van welke IP-adressen u binnenkomend verkeer moet toestaan om netwerkbeveiligingsgroepen en door de gebruiker gedefinieerde routes voor virtuele netwerken met Azure HDInsight correct te configureren.
...
ms.locfileid: "78271815"
---
# <a name="hdinsight-management-ip-addresses"></a>IP-adressen hdinsight-beheer

> [!Important]
> In de meeste gevallen u nu de [functie servicetag](hdinsight-service-tags.md) gebruiken voor netwerkbeveiligingsgroepen, in plaats van handmatig IP-adressen toe te voegen. Nieuwe regio's worden alleen toegevoegd voor servicetags en de statische IP-adressen worden uiteindelijk afgeschaft.

Als u netwerkbeveiligingsgroepen (SMG's) of door gebruikers gedefinieerde routes (UDR's) gebruikt om binnenkomend verkeer naar uw HDInsight-cluster te beheren, moet u ervoor zorgen dat uw cluster kan communiceren met kritieke Azure-status- en beheerservices.  Sommige IP-adressen voor deze services zijn regiospecifiek en sommige zijn van toepassing op alle Azure-regio's. Mogelijk moet u ook verkeer van de Azure DNS-service toestaan als u geen aangepaste DNS gebruikt.
...
Als u de door Azure geleverde DNS-service gebruikt, u toegang toestaan vanaf __168.63.129.16__ op poort 53. Zie het document [Naamoplossing voor VM's en Rolinstanties voor](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md) meer informatie. Als u aangepaste DNS gebruikt, slaat u deze stap over.

## <a name="health-and-management-services-all-regions"></a>Gezondheids- en beheersdiensten: Alle regio's

Verkeer toestaan vanaf de volgende IP-adressen voor Azure HDInsight-status- en beheerservices, die van toepassing zijn op alle Azure-regio's:
...
## <a name="health-and-management-services-specific-regions"></a>Gezondheids- en beheersdiensten: Specifieke regio's

Verkeer toestaan vanaf de IP-adressen die worden vermeld voor de status- en beheerservices van Azure HDInsight in de specifieke Azure-regio waar uw resources zich bevinden:

> [!IMPORTANT]  
> Als de Azure-regio die u gebruikt niet wordt vermeld, gebruikt u de [functie servicetag](hdinsight-service-tags.md) voor netwerkbeveiligingsgroepen.

| Land | Regio | Toegestane IP-adressen van de bron | Toegestane bestemming | Richting |
| ---- | ---- | ---- | ---- | ----- |
...
Zie het document [Azure Government Intelligence + Analytics](https://docs.microsoft.com/azure/azure-government/documentation-government-services-intelligenceandanalytics) voor informatie over de IP-adressen die u voor Azure Government gebruiken.

Zie de sectie [Netwerkverkeer beheren](hdinsight-plan-virtual-network-deployment.md#networktraffic) voor meer informatie.

Als u door de gebruiker gedefinieerde routes (UDR's) gebruikt, moet u een route opgeven en uitgaand verkeer van het virtuele netwerk naar de bovenstaande IP's toestaan met de volgende hopingesteld op 'Internet'.

## <a name="next-steps"></a>Volgende stappen

* [Virtuele netwerken maken voor Azure HDInsight-clusters](hdinsight-create-virtual-network.md)
* [NSG-servicetags (Network Security Group) voor Azure HDInsight](hdinsight-service-tags.md)
