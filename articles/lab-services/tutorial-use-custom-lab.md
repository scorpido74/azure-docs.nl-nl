...
2. Sla het gedownloade **RDP**-bestand van de harde schijf op en gebruik het om verbinding te maken met de virtuele machine. Geef de gebruikersnaam en het wachtwoord op die u bij het maken van de VM in de vorige sectie hebt opgegeven. 
    ...
    > [!NOTE]
    > Er zijn andere manieren om naar de pagina Virtuele machine voor uw VM te gaan. Hier zijn een aantal van hen: 
    > 
    > 1. Zoek naar alle VM's in uw abonnement. Selecteer uw VM in de lijst met VM's om naar de pagina **Virtuele machine** te gaan.
    > 2. Navigeer naar de pagina **Resourcegroep** voor de resourcegroep. Selecteer vervolgens uw VM in de lijst met bronnen in de resourcegroep om naar de pagina **Virtuele machine** te gaan. 
    >
    > Gebruik de knop **Verbinding** maken niet op de werkbalk op de pagina **Virtuele machine** die u bereikt met behulp van de opties. Navigeer in plaats daarvan naar de pagina **Virtuele machine** vanaf de pagina **DevTest Labs** zoals in dit artikel wordt weergegeven en gebruik vervolgens de knop Verbinding **maken** op de werkbalk.
