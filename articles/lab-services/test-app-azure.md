...
1. Gebruik op uw lokale computer het script van het persistent maken van de [referenties voor Azure-bestands shares in](../storage/files/storage-how-to-use-files-windows.md#persisting-azure-file-share-credentials-in-windows) de sectie Windows van [een Azure-bestands share met Windows](../storage/files/storage-how-to-use-files-windows.md) -artikel. 
2. Gebruik `net use` vervolgens de opdracht om de bestands share op uw computer te koppelen. Hier volgt een voor beeld van de opdracht: Geef de naam van uw Azure-opslag en de bestands share op voordat u de opdracht uitvoert. 
...
4. Open in de virtuele machine de **bestanden Verkenner**en selecteer **deze PC** om uw bestands share te vinden.
    ...
    > [!NOTE]
    > Als u de bestands share niet op uw virtuele machine of op uw lokale computer kunt vinden, koppelt u deze opnieuw door de `net use` opdracht uit te voeren. U vindt de `net use` opdracht in de wizard **Connect** van uw **Bestands share** in de Azure Portal.
