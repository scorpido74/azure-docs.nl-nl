...
1. De actie RDP-bestands inhoud ophalen wordt `https://{gateway-hostname}/api/host/{lab-machine-name}/port/{port-number}` aangeroepen om een verificatie token aan te vragen.
...
1. De extern bureau blad-gateway Defert de `https://{gateway-hostname}/api/host/{lab-machine-name}/port/{port-number}` aanroep van naar een Azure-functie om het verificatie token te genereren. De DevTest Labs-service bevat automatisch de functie code in de aanvraag header. De functie sleutel moet worden opgeslagen in de sleutel kluis van het lab. De naam van het geheim dat moet worden weer gegeven als het geheim van de **Gateway token** op de pagina **Lab-instellingen** voor het lab.
...
- De module [toepassings routering aanvragen](/iis/extensions/planning-for-arr/using-the-application-request-routing-module) voor IIS (Internet Information Server) kan worden gebruikt om aanvragen `https://{gateway-hostname}/api/host/{lab-machine-name}/port/{port-number}` om te leiden naar de Azure-functie, die de aanvraag afhandelt om een token voor verificatie op te halen.


## <a name="requirements-for-azure-function"></a>Vereisten voor Azure function
De Azure-functie verwerkt de aanvraag `https://{function-app-uri}/app/host/{lab-machine-name}/port/{port-number}` met de indeling en retourneert het verificatie token op basis van hetzelfde handtekening certificaat dat is geïnstalleerd op de gateway machines. De `{function-app-uri}` is de URI die wordt gebruikt voor toegang tot de functie. De functie sleutel wordt automatisch door gegeven in de koptekst van de aanvraag. Zie [https://github.com/Azure/azure-devtestlab/blob/master/samples/DevTestLabs/GatewaySample/src/RDGatewayAPI/Functions/CreateToken.cs](https://github.com/Azure/azure-devtestlab/blob/master/samples/DevTestLabs/GatewaySample/src/RDGatewayAPI/Functions/CreateToken.cs)voor een voor beeld van een functie. 
...
4. Implementeer **azuredeploy. json** van [https://github.com/Azure/azure-devtestlab/tree/master/samples/DevTestLabs/GatewaySample/arm/gateway](https://github.com/Azure/azure-devtestlab/tree/master/samples/DevTestLabs/GatewaySample/arm/gateway). De sjabloon heeft de volgende para meters:
    ...
    - Het {Storage-account-endpoint} kan worden verkregen door uit `az storage account show --name {storage-acct-name} --query primaryEndpoints.blob`te voeren.  De {Storage-acct-name} is de naam van het opslag account dat bestanden bevat die u hebt geüpload.  
    - De {container-name} is de naam van de container in de {Storage-acct-name} die bestanden bevat die u hebt geüpload.  
    - Het {SAS-token} kan worden verkregen door uit `az storage container generate-sas --name {container-name} --account-name {storage-acct-name} --https-only –permissions drlw –expiry {utc-expiration-date}`te voeren. 
