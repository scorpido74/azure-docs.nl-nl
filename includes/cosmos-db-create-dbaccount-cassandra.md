...
3. Voer op de pagina **Azure Cosmos DB-account** maken de instellingen in voor het nieuwe Azure Cosmos DB-account. 
    ...
    Resourcegroep|Nieuwe maken<br><br>Voer vervolgens dezelfde naam in als accountnaam|Selecteer **Nieuw maken**. Voer vervolgens een nieuwe naam van de resourcegroep in voor uw account. Gebruik voor de eenvoud dezelfde naam als de naam van uw Azure Cosmos-account. 
    ...
    API|Cassandra|De API bepaalt het type te maken account. Azure Cosmos DB biedt vijf API's: Core (SQL) voor documentdatabases, Gremlin voor grafiekdatabases, MongoDB voor documentdatabases, Azure Table en Cassandra. U moet voor elke API een apart account aanmaken. <br><br>Selecteer **Cassandra,** omdat u in deze quickstart een tabel maakt die werkt met de Cassandra API. <br><br>[Meer informatie over de Cassandra-API](../articles/cosmos-db/cassandra-introduction.md).|
...
4. Het duurt enkele minuten om het account te maken. Wacht tot het portaal om de pagina te geven zeggen **Gefeliciteerd! Uw Azure Cosmos DB-account is gemaakt**.
