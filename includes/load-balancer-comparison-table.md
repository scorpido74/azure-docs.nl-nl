...
| Diagnostics | Azure Monitor. Multi-dimensionale metrische gegevens, waaronder byte-en pakket tellers. Status van de status test. Verbindings pogingen (TCP SYN). Uitgaande verbindings status (geslaagde en mislukte stroom SNAT). Afmetingen van actief gegevens vlak. | Alleen Azure-Log Analytics voor open bare Load Balancer. Waarschuwing voor SNAT-uitputting. Aantal statussen van back-end. |
| HA-poorten | Interne load balancer | Niet beschikbaar |
| Standaardbeveiliging | Open bare IP-, open bare Load Balancer-eind punten en interne Load Balancer-eind punten worden gesloten voor binnenkomende stromen, tenzij toegestaan door een netwerk beveiligings groep. Houd er rekening mee dat intern verkeer van het VNET naar de interne load balancer nog steeds is toegestaan. | Standaard geopend. De netwerk beveiligings groep is optioneel. |
...
| [Meerdere front-ends](../articles/load-balancer/load-balancer-multivip-overview.md) | Inkomend en [uitgaand](../articles/load-balancer/load-balancer-outbound-connections.md) | Alleen binnenkomende gegevens |
