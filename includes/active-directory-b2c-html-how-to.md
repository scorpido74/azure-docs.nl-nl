...
De inhoud van uw aangepaste pagina kan HTML-elementen bevatten, waaronder CSS en Java script, maar kan geen onbeveiligde elementen zoals iframes bevatten. Het enige vereiste element is een div-element waarvan `id` is ingesteld op `api`, zoals deze `<div id="api"></div>` in uw HTML-pagina.
...
- Vanwege beveiligings beperkingen biedt Azure AD B2C geen ondersteuning voor `frame`, `iframe`of `form` HTML-elementen.
...
1. Navigeer naar [www.test-cors.org](https://www.test-cors.org/) en plak de URL in het vak **externe URL** .
